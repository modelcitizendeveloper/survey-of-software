...
description: "Comprehensive survey of text chunking strategies for RAG pipelines: fixed-size, recursive, semantic, structure-aware, and hybrid approaches. Covers LangChain…"
keywords: ["haystack splitters", "langchain splitters", "llamaindex parsers", "benchmarking"]
...
|---|---|---|---|
| **LangChain Text Splitters** | General-purpose RAG baseline, plus Markdown and HTML header splitting | Start here. RecursiveCharacterTextSplitter at 512 tokens with 10% overlap is the baseline every other strategy in this survey is measured against, and MarkdownHeaderTextSplitter is the cheapest quality upgrade for structured docs. On LangChain 1.x the splitters ship only in the separate langchain-text-splitters package — `langchain.text_splitter` no longer exists. | 1.1.2 · 2026-04-16 |
| **LlamaIndex Node Parsers** | Quality-critical unstructured text; hierarchical and sentence-window retrieval | The widest strategy range in one install. SemanticSplitterNodeParser, HierarchicalNodeParser, SentenceWindowNodeParser and SentenceSplitter are all in llama-index-core, so semantic and multi-resolution chunking need no extra package. Choose it when chunk quality justifies the embedding cost of semantic splitting. | 0.14.24 · 2026-08-19 |
| **Haystack Document Splitters** | Pipeline-native ingestion where splitting is one component in a graph | No longer the fixed-size-only option. Haystack 3.1.0 ships embedding-based (semantic), hierarchical, recursive, Markdown-header and Python-code splitters beside DocumentSplitter, so strategy range is no longer the reason to rule it out. Pick it for component-based pipelines; its splitters are awkward to use standalone. | 3.1.1 · 2026-09-08 |
| **LangChain Experimental (SemanticChunker)** | Semantic chunking for teams already committed to LangChain | The only semantic chunker in the LangChain stack, and it sits in the experimental package rather than langchain-text-splitters. Weigh that API-stability caveat before putting it in an ingestion path that is expensive to re-run. | 0.4.2 · 2026-05-22 |
...
- Strategy beats library. All three frameworks now expose the same underlying chunking strategies, so the choice between them turns on pipeline fit, not on the retrieval quality any of them can reach.
- Structure-aware splitting is the free upgrade. On documents with consistent headings, splitting on Markdown headers costs the same as recursive splitting; the survey's own S2 benchmark puts it at 0.85 Recall@5 against 0.72 for recursive (internal eval set, not independently reproduced).
- Contextual embeddings give the best quality per dollar. Prepending an LLM-generated document summary to each chunk costs roughly $0.01/doc one-time, against $0.03/doc for embedding-based semantic chunking, and simple title-and-section metadata recovers most of the benefit for free. Pattern documented at https://www.anthropic.com/news/contextual-retrieval
- 512 tokens with 10-15% overlap is the default to beat. Zero overlap drops content at chunk boundaries, and past 15% the added recall flattens while redundancy keeps rising.
- Splitters have moved out of the frameworks' main packages. LangChain 1.x has no `langchain.text_splitter` module — import from `langchain_text_splitters` — and LangChain's only semantic chunker lives in `langchain-experimental`.
...
### RecursiveDocumentSplitter, MarkdownHeaderSplitter, PythonCodeSplitter
- Recursive separator hierarchy, header-aware Markdown splitting, and code splitting
  as first-party components

### EmbeddingBasedDocumentSplitter
- Semantic splitting: embeds sentence groups and splits where the cosine distance
  between neighbors exceeds a percentile threshold
- Haystack's equivalent of LlamaIndex `SemanticSplitterNodeParser`

### HierarchicalDocumentSplitter
- Multi-level chunking for parent/child retrieval

## Pros

✅ **Production-ready**: Enterprise-grade, Fortune 500 adoption
✅ **Sentence-aware**: Clean boundaries by default
✅ **Pipeline integration**: Works seamlessly in Haystack workflows
✅ **Performance**: Lowest token usage among frameworks

## Cons

❌ Smaller ecosystem than LangChain/LlamaIndex
❌ Splitters are pipeline components, so using one outside a Haystack pipeline is
   less natural than calling a LangChain splitter directly
❌ Semantic and hierarchical splitting arrived later than in LlamaIndex, so there is
   less community tuning guidance for them

> **Corrected 2026-08-25 (bead re-4ou).** This page previously listed "No semantic or
> hierarchical chunking" as a limitation. Verified against the `haystack-ai` 3.1.0 wheel
> on PyPI (uploaded 2026-08-24): `haystack.components.preprocessors` ships
> `EmbeddingBasedDocumentSplitter`, `HierarchicalDocumentSplitter`,
> `MarkdownHeaderSplitter`, `RecursiveDocumentSplitter`, `PythonCodeSplitter` and
> `CSVDocumentSplitter` alongside `DocumentSplitter`.
...
✅ **Largest ecosystem**: Most integrations, examples, community support
✅ **Battle-tested**: Used by thousands of production RAG systems
✅ **Easy to use**: Simple API, good defaults
✅ **Framework integration**: Works seamlessly with LangChain ecosystem

## Cons

❌ No semantic chunking (must use external library)
❌ Limited advanced features vs LlamaIndex
❌ No built-in hierarchical chunking
...
✅ **Best quality**: Semantic chunking, advanced RAG techniques
✅ **Hierarchical indexing**: Multi-resolution built-in
✅ **RAG-focused**: Every feature designed for retrieval
✅ **Active research**: Cutting-edge techniques implemented first

## Cons

❌ Steeper learning curve vs LangChain
❌ Semantic chunking is slow and costly
...
✅ **Baseline implemented**: RecursiveCharacterTextSplitter in production
✅ **Quality measured**: Recall@5 on 20+ test queries
✅ **Decision made**: Keep baseline or optimize
✅ **Documentation**: Decision rationale recorded for future team

---

## Next Steps

1. **Measure baseline quality** → S2: Benchmarking
2. **Learn optimization techniques** → S2: Implementation Guide
3. **Find your use case** → [S3: Need-Driven](../S3-need-driven/)
4. **Plan long-term** → S4: Strategic
...
**Conclusion**: Fixed/recursive splitting is near-instant. Semantic splitting is 50× slower but still fast enough (`<1`s per doc). LLM chunking only for critical documents.
...
1. **Implement optimizations** → S2: Approach (Implementation Guide)
2. **Learn from real use cases** → [S3: Need-Driven Use Cases](../S3-need-driven/)
3. **Plan long-term strategy** → S4: Strategic Framework
...
✅ Content has consistent headers/sections
✅ Headers correlate with semantic boundaries
✅ Natural chunking boundaries exist (H2, H3 tags)
...
✅ Unstructured narrative text
✅ No clear structural markers
✅ Variable-length semantic units
✅ Quality > cost
...
4. **Plan strategic roadmap** → S4: Strategic Framework
...
**Break-even**: If custom chunker saves `>$85`k/year (improved quality → less support, faster queries, etc.)
