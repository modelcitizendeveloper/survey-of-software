...
description: "Bot SDKs compared: Discord, Telegram, Slack, Matrix and Nostr — what each platform's SDK provides, and how portable a bot can be between them."
keywords: ["sdk", "Matrix", "Discord", "Telegram", "Nostr", "Slack"]
...
Bot SDKs compared: Discord, Telegram, Slack, Matrix and Nostr — what each platform's SDK provides, and how portable a bot can be between them.
...
|---|---|---|---|
| **matrix-nio** | Anything privacy-critical — healthcare, legal, sensitive data | The only option with end-to-end encryption and full self-hosting. S3 puts privacy first because it is the only GATE in the decision tree; every constraint below it is a trade. | 0.26.0 · 2026-07-23 |
| **discord.py** | Gaming and community bots, high message volume, and the default when nothing else binds | Best UX and the richest feature set. You are a guest on Discord's service. | 2.7.1 · 2026-03-03 |
| **python-telegram-bot** | Mobile-first audiences | Large existing user base and quick to an MVP. Also a guest platform. | 22.8 · 2026-06-12 |
| **slack-bolt** | Enterprise internal tooling — where they already use Slack | "Do they already use Slack?" carries more weight than it appears to: a bot on a platform your users do not have open is a bot nobody runs. | 1.30.0 · 2026-07-15 |
| **nostr-tools** | Censorship resistance | Maximum resistance — relays are pluggable and no single operator controls reach. Simplest protocol in the set, with native Lightning payments. | 1.4.1 · 2025-11-30 |
...
- Discord has best slash command UX but centralized
- Telegram fastest MVP (2-4h) but centralized
- Matrix only option with E2EE + self-hosting
- Matrix lacks native slash commands (use ! prefix)
- Nostr lacks interactive components, has Lightning payments
...
# S2 Recommendation: The Platform Chooses the SDK

S2 examined five SDKs in depth — `discord.py`, `matrix-nio`, `nostr-tools`,
`python-telegram-bot` and `slack-bolt`. The technical comparison produces a
result that looks unhelpful until you accept it:

**These SDKs do not compete.** Each is the client library for one platform, and
each is the best available option for that platform. There is no shootout to run,
because a Discord bot cannot be written with `slack-bolt` at any level of skill.

## What that means for the decision

The question "which bot SDK should I use" is almost always the wrong question. The
real one is **"where are the people I am building for?"** — and the SDK follows
from the answer with no further deliberation.

The exception is a genuinely open choice, where the audience could be assembled
anywhere. That case is rarer than it looks, and S3 handles it.

## What S2 does establish

Since the platform decides the SDK, the useful technical findings are about what
each SDK costs to *operate*, not which is better:

- **The event model differs more than the API surface.** Some SDKs hand you a
  long-lived connection with a callback; others expect you to run a webhook
  endpoint. That determines your deployment shape — a persistent process versus a
  request handler — and it is a hosting decision disguised as a library detail.
- **Rate limits are the operational constraint, not throughput.** Every platform
  imposes them, they differ substantially, and they bind long before Python does.
- **The self-hosting question splits the set cleanly.** Matrix can be run
  entirely on your own infrastructure. Discord, Telegram and Slack cannot — the
  platform is somebody else's service, and your bot is a guest on it. Nostr sits
  apart again: relays are pluggable and no single operator controls reach.

## Where the technical detail actually pays

Read the per-SDK file for the platform you have already chosen. Reading all five
to pick between them is time spent on a choice that was made by your audience
before you opened an editor.
...
# S3 Recommendation: Answer the Constraint, Not the Feature List

S3's decision tree resolves in one pass, because each branch is a hard constraint
rather than a preference. The order matters — the first "yes" is the answer.

| Ask, in order | If yes |
|---|---|
| Is this privacy-critical? Healthcare, legal, sensitive data | **Matrix** (`matrix-nio`) — the only option with end-to-end encryption and full self-hosting |
| Is it an enterprise internal tool? | **Slack** (`slack-bolt`) if they already use Slack; otherwise weigh Matrix self-hosted against buying Slack |
| Is censorship resistance critical? | **Nostr** (`nostr-tools`) for maximum resistance; **Matrix** if federated is sufficient |
| High message volume, above ~1000/hour? | **Discord** (`discord.py`) for gaming and community; **Telegram** otherwise |
| Mobile-first audience? | **Telegram** (`python-telegram-bot`) |
| Developer tooling — CI/CD, source-control integration? | **Slack** for an internal team; **Discord** for an open-source community |
| None of the above | **Discord** (`discord.py`) — best UX and the richest feature set |

## The two branches that carry real weight

**Privacy comes first for a reason.** It is the only constraint in the list that
cannot be satisfied by any other option. Everything below it is a trade; this one
is a gate. If the data is sensitive and you need E2EE plus infrastructure control,
Matrix is not the best choice — it is the only one.

**"Do they already use Slack?" is doing more work than it appears to.** An
enterprise bot lives or dies on whether people see it. Deploying to a platform
your users do not already have open is a bot nobody runs.

## The anti-patterns file is the useful half

`anti-patterns.md` records when *not* to use each SDK, and it is worth more than
the positive recommendations. The recurring mistake in this category is choosing
the platform you like rather than the one your audience is on — and no amount of
SDK quality recovers from that.

## Multi-platform

`multi-platform-strategy.md` covers building for several at once. The short
version: the platform-specific layer is smaller than teams expect and the shared
layer is larger, so structure the bot as domain logic with thin adapters — but do
this because you genuinely have audiences in two places, not to defer the
decision.
