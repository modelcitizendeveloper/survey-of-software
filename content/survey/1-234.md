...
|---|---|---|
| **matrix-nio** | Anything privacy-critical — healthcare, legal, sensitive data | The only option with end-to-end encryption and full self-hosting. S3 puts privacy first because it is the only GATE in the decision tree; every constraint below it is a trade. |
| **discord.py** | Gaming and community bots, high message volume, and the default when nothing else binds | Best UX and the richest feature set. You are a guest on Discord's service. |
| **python-telegram-bot** | Mobile-first audiences | Large existing user base and quick to an MVP. Also a guest platform. |
| **slack-bolt** | Enterprise internal tooling — where they already use Slack | "Do they already use Slack?" carries more weight than it appears to: a bot on a platform your users do not have open is a bot nobody runs. |
| **nostr-tools** | Censorship resistance | Maximum resistance — relays are pluggable and no single operator controls reach. Simplest protocol in the set, with native Lightning payments. |
