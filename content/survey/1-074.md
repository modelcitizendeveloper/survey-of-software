---
title: "1.074 Gradient Boosting Libraries"
description: "Gradient boosting compared: XGBoost, LightGBM and CatBoost — adoption, training speed and categorical handling, and why tabular data still belongs to these."
keywords: ["gradient boosting", "xgboost", "lightgbm", "catboost", "boosted trees", "tabular data", "structured data", "kaggle", "categorical features", "feature importance", "shap", "ensemble methods", "decision trees", "gbdt"]
...
# 1.074 Gradient Boosting Libraries

Gradient boosting compared: XGBoost, LightGBM and CatBoost — adoption, training speed and categorical handling, and why tabular data still belongs to these.
...
|---|---|---|
| **XGBoost** | The clear adoption leader at ~26.1M monthly downloads — the established default, the largest community, and the safest choice when someone else will maintain the model. | 3.4.1 · 2026-08-15 |
| **LightGBM** | Strong second at ~10.8M monthly, and picked for training speed and memory on large datasets via its leaf-wise growth strategy. | 4.7.0 · 2026-07-18 |
| **CatBoost** | The specialist: native categorical handling that removes the encoding step other two require, and strong defaults. Chosen for data shape rather than raw speed. | 1.2.10 · 2026-02-18 |
...
- Only three libraries matter here, and the survey's popularity scan found no credible fourth — an unusually settled category
- The three are close enough on accuracy that the real selection criteria are training time and how much preprocessing you avoid, not predictive power
- CatBoost's argument is preprocessing, not modeling: native categorical support deletes a whole error-prone encoding stage
- XGBoost's ~2.4x adoption lead over LightGBM is worth something concrete — documentation, Stack Overflow answers and hiring all follow adoption
- This category persists because gradient-boosted trees remain the thing to beat on tabular data, which deep learning has repeatedly failed to do
