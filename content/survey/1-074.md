...
- **XGBoost** — The clear adoption leader at ~26.1M monthly downloads — the established default, the largest community, and the safest choice when someone else will maintain the model.
- **LightGBM** — Strong second at ~10.8M monthly, and picked for training speed and memory on large datasets via its leaf-wise growth strategy.
- **CatBoost** — The specialist: native categorical handling that removes the encoding step other two require, and strong defaults. Chosen for data shape rather than raw speed.
...
- CatBoost's argument is preprocessing, not modelling: native categorical support deletes a whole error-prone encoding stage
