---
title: "1.182 Database Diff & Schema Comparison"
...
|---|---|---|
| **Atlas** | Greenfield projects that want infrastructure-as-code discipline for the database and a drift gate that fails a pull request. | The default pick for a new project. Broadest engine coverage in the survey (PostgreSQL, MySQL, MariaDB, SQLite, SQL Server, ClickHouse), a declarative schema-as-code model, and the only best-in-class CI/CD integration here — a GitHub Action that does lint plus drift detection. Budget for the fact that cloud execution, schema monitoring and PR-inline analysis are a paid Atlas Cloud subscription; the diff and migration-generation engine itself is open source. |
| **skeema** | MySQL/MariaDB teams that want the schema reviewed in Git like application code, at high scale. | Uncontested for Git-native MySQL and MariaDB, and unusable for anything else — there is no PostgreSQL support, so the engine decides this one before any feature comparison starts. Schema lives as .sql files in Git, skeema generates the ALTER statements, and its lint rules block dangerous alterations before deploy. No native GitHub Action; CI integration is exit codes and shell. |
| **migra** | Existing deployments already pinned to it. Not a new-project choice, and no longer the drop-in drift supplement the survey body recommends. | Do not adopt. On technical merit migra still has the deepest PostgreSQL introspection in this survey — the only tool covering RLS policies, partitions and function bodies in full, and the only one whose generated SQL is dependency-ordered enough to run as-is — but the author has ended the project and the README now points readers to djrobstep/results. Existing pinned installs keep working against the PostgreSQL versions they were tested on; nothing will be fixed for newer catalog structures. PostgreSQL teams evaluating today should look at Atlas. |
| **Liquibase** | Regulated environments needing changeset history, and enterprises with Oracle or DB2 in the mix. | The choice when the fleet is heterogeneous or the requirement is an audit trail. It is the only tool here with credible Oracle, DB2 and SQL Server coverage under one roof, and its DATABASECHANGELOG table gives compliance work the author/timestamp/hash record no declarative tool produces. Two costs: diff is a secondary feature rather than the point of the product, and Community edition diff misses stored procedures and triggers — the objects most likely to have drifted. Java runtime and verbose XML/YAML changesets come with it. |
| **SchemaSpy** | Documentation, onboarding and per-release schema snapshots, alongside a real diff tool rather than instead of one. | Not a diff tool, and reading it as one is the mistake to avoid. It renders a live database as navigable HTML with ER diagrams, which makes it the best free way to explain a schema to non-DBA stakeholders or to archive schema state as audit evidence. Comparison is manual: render two versions and read them side by side. Output is HTML, not SQL and not machine-readable, and the Java plus GraphViz dependency is awkward to containerize. |
| **pg_dump + diff** | A quick one-off PostgreSQL comparison, or confirming what a more sophisticated tool just told you. | The zero-install baseline every DBA falls back on, and the ground truth others get checked against. It is a text diff, not a semantic one: no understanding of what changed, no dependency ordering, no rollback, and pg_dump output is not reliably executable as a migration. Unreadable on large schemas. |
| **Flyway diff (Teams/Enterprise)** | Teams already standardized on Flyway for migrations, with budget for a paid tier. | Only worth considering if the team is already on Flyway, because the diff feature is paywalled — the free edition has none. For an existing Flyway shop it generates Flyway-compatible versioned migration scripts and drift detection inside a workflow the team already runs. Nobody adopts Flyway for schema diff alone, and the Java runtime is real weight. |
| **mysqldiff / pgdiff (single-purpose CLIs)** | One-off command-line comparisons where installing a framework is not warranted. | Fine for a quick manual comparison, wrong to build a pipeline on. This is a fragmented field of competing Perl, Python and Go implementations with uneven maintenance and shallow handling of stored procedures, triggers and RLS policies. Teams that need this repeatedly in CI move to Atlas, skeema or Liquibase. |
| **Redgate SQL Compare** | SQL Server shops with budget and a correctness bar high enough to justify commercial tooling. | Still the reference standard for SQL Server, and the survey finds no open-source tool that matches its object coverage there. The SSMS plugin, Azure DevOps extension and three-way merge have no open-source equivalent, which is also the lock-in: switching costs are high by design. Per-user licensing is the blocker for smaller shops, and CI/CD use needs additional licensing. |


**What the research found**

- **The first decision is the model, not the tool: declarative desired-state (Atlas, skeema) or recorded migration history (Liquibase, Flyway)** — The two answer different questions — 'does production match what we intended?' versus 'what have we applied, and who approved it?' Teams that start with history eventually want the declarative check, and teams that start declarative eventually want the audit trail. Atlas is the only tool here attempting both.
- **migra is deprecated, and this survey's body has not caught up** — The author posted a deprecation notice on 2025-08-25 and named djrobstep/results as the successor; the last release, 3.0.1663481299, is from 2022-09-18. The survey was written 2026-03-04 and still advises adding migra for PostgreSQL drift investigation regardless of primary tool — that advice is superseded. Its technical strengths were real, so pinned installs are not urgent to rip out, but nothing will be fixed for newer PostgreSQL catalog structures.
- **The database engine eliminates most of the field before any feature comparison begins** — skeema is MySQL/MariaDB only, migra was PostgreSQL only, Redgate is SQL Server first. For Oracle or DB2 the shortlist collapses to Liquibase or a commercial product. Cross-database claims usually mean PostgreSQL and MySQL are first-class and the rest is best-effort.
- **Diff of the objects most likely to drift sits behind a paywall in two of the tools** — Flyway's free edition has no diff at all, and Liquibase Community's diff misses stored procedures and triggers — exactly the objects a production hotfix modifies without touching a migration file. Evaluate against the edition you will actually pay for, not the feature matrix.
- **Every introspection-based tool needs a live database, which is a pipeline cost, not a footnote** — Running drift detection in CI means spinning up a database container, applying migrations, diffing and tearing down on every pull request. It also means a one-off production diff can be gated on VPN access and credential provisioning before any tool question is reached.
...
> ⚠️ **migra is officially deprecated.** Verified 2026-08-25 against its own
> README on master: "This project is officially deprecated... Version
> 3.0.1663481299 (from Sep 18, 2022) was the final release, and there will be
> no further releases." PyPI confirms 2022-09-18 as the last upload. The author
> points users to [results](https://github.com/djrobstep/results). This survey
> was written 2026-03-04, six months after that notice, and recommended migra
> throughout. The recommendations below are corrected in place.
...
1. Use Liquibase diff (multi-database) or Atlas to compare production against the
   best available schema definition. (This step named migra before its
   deprecation was verified — see the banner.)
...
2. Use Atlas to diff staging against the expected schema definition
...
**Do NOT choose migra — it is deprecated.** Final release 2022-09-18; the
author points to `results` as the successor. The profile below describes what
it did, and is kept because the *capability* is what to look for in a
replacement, not because the tool is a live option:
...
3. Do NOT add migra. It is officially deprecated — final release 2022-09-18,
   and its README says there will be no further releases. Its author points to
   `results` as the successor. For PostgreSQL drift investigation alongside a
   primary tool, use Atlas or Liquibase diff, both actively released.
