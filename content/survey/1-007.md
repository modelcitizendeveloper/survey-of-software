...
description: "Comprehensive analysis of pattern matching algorithms and libraries across multiple platforms. Covers KMP, Boyer-Moore, Aho-Corasick, and Rabin-Karp algorithms…"
keywords: ["complexity", "characteristics", "Knuth-Morris-Pratt", "Boyer-Moore", "Aho-Corasick", "Rabin-Karp", "implementation", "performance", "Hyperscan", "pyahocorasick", "Rust memchr", "Rust aho-corasick", "RE2", "ripgrep", "BWA/Bowtie2", "BLAST"]
...
<figure class="sos-fig"><figcaption>single pattern 1gb text <span>lower is faster</span></figcaption><svg viewBox="0 0 720 128" role="img" width="100%" height="128" aria-label="single pattern 1gb text, compared"><title>memchr simd: 0.1 seconds</title><text x="158" y="17.68" class="sos-bl" text-anchor="end">memchr simd</text><rect x="168" y="0" width="2.0" height="26" rx="4" class="sos-bar"/><text x="178.0" y="17.68" class="sos-bv">0.1 seconds</text><title>boyer moore: 0.3 seconds</title><text x="158" y="49.68" class="sos-bl" text-anchor="end">boyer moore</text><rect x="168" y="32" width="2.0" height="26" rx="4" class="sos-bar"/><text x="178.0" y="49.68" class="sos-bv">0.3 seconds</text><title>kmp: 1 second</title><text x="158" y="81.68" class="sos-bl" text-anchor="end">kmp</text><rect x="168" y="64" width="2.0" height="26" rx="4" class="sos-bar"/><text x="178.0" y="81.68" class="sos-bv">1 second</text><title>naive: 17 minutes</title><text x="158" y="113.68" class="sos-bl" text-anchor="end">naive</text><rect x="168" y="96" width="486.0" height="26" rx="4" class="sos-bar"/><text x="662.0" y="113.68" class="sos-bv">17 minutes</text></svg></figure>

<figure class="sos-fig"><figcaption>multi pattern 10k signatures 100mb memory <span>lower is faster</span></figcaption><svg viewBox="0 0 720 96" role="img" width="100%" height="96" aria-label="multi pattern 10k signatures 100mb memory, compared"><title>hyperscan: 1 second</title><text x="158" y="17.68" class="sos-bl" text-anchor="end">hyperscan</text><rect x="168" y="0" width="2.0" height="26" rx="4" class="sos-bar"/><text x="178.0" y="17.68" class="sos-bv">1 second</text><title>aho corasick: 30 seconds</title><text x="158" y="49.68" class="sos-bl" text-anchor="end">aho corasick</text><rect x="168" y="32" width="4.9" height="26" rx="4" class="sos-bar"/><text x="180.9" y="49.68" class="sos-bv">30 seconds</text><title>boyer moore x10k: 50 minutes</title><text x="158" y="81.68" class="sos-bl" text-anchor="end">boyer moore x10k</text><rect x="168" y="64" width="486.0" height="26" rx="4" class="sos-bar"/><text x="662.0" y="81.68" class="sos-bv">50 minutes</text></svg></figure>


| Library | How it works | Best for |
|---|---|---|
| **Standard Library (strstr, std::string::find, str.find)** | Platform-dependent (often Boyer-Moore-Horspool) | General-purpose, 90% of use cases |
| **Hyperscan (Intel)** | Hybrid DFA + SIMD optimization | Network IDS/DPI, ultra-high throughput (1000+ patterns) |
| **pyahocorasick** | Aho-Corasick (C extension) | Python multi-pattern matching |
| **Rust memchr** | SIMD-optimized Boyer-Moore variant | Rust single-byte/multi-byte search |
| **Rust aho-corasick** | Aho-Corasick with DFA/NFA variants | Rust multi-pattern matching |
| **RE2 (Google)** | Linear-time regex (DFA-based) | Regex with guaranteed linear time (no backtracking) |
| **ripgrep** | Rust regex + memchr SIMD | Interactive log search, code search |
| **BWA/Bowtie2** | BWT + FM-index | Bioinformatics NGS read alignment |
| **BLAST** | Seed-and-extend heuristic | Bioinformatics homology search |


**What the research found**

- **Boyer-Moore achieves O(n/m) sublinear time - only algorithm that can skip text characters** — 1-2 GB/s typical, 10x faster than naive for large alphabets
- **Aho-Corasick is O(n) regardless of pattern count - scales to millions of patterns** — Industry standard for IDS (Snort, Suricata) and virus scanning
- **Hyperscan achieves 10-100 GB/s through SIMD + DFA optimization** — 100x faster than naive AC, enables real-time 100 Gbps network inspection
- **Alphabet size is critical factor - DNA (σ=4) vs English (σ=26) vs Unicode (σ=65K)** — BM excels for large alphabets, KMP competitive for small alphabets
- **Standard library implementations are 'good enough' for 90% of use cases** — Use stdlib first, profile before optimizing - lowest maintenance burden
...
| few 2 10 | Run BM for each or consider AC |
| many 10 100 | Aho-Corasick library (pyahocorasick, aho-corasick crate) |
| massive 100plus | Aho-Corasick or Hyperscan for ultra-performance |

| Use case | Pick |
|---|---|
| text editor | stdlib or Rust regex - interactive, patterns change frequently |
| network ids | Hyperscan - 10-100 GB/s, industry standard (Snort, Suricata) |
| virus scanner | Aho-Corasick or Hyperscan - millions of signatures |
| bioinformatics | BWA/Bowtie2 (alignment), BLAST (homology) - domain-specific |
| log analysis | Aho-Corasick (keywords), RE2 (structured), ELK (indexed) |
| streaming | KMP or AC - no text backtracking |

| Alphabet size | Pick |
|---|---|
| small dna | KMP - σ=4, BM less effective |
| medium english | Boyer-Moore - σ=26, excellent skipping |
| large unicode | Boyer-Moore with sparse tables or byte-level matching |

| Constraint | Pick |
|---|---|
| worst case guarantee | KMP (single) or Aho-Corasick (multi) - O(n) guaranteed |
| maximum speed | Hyperscan (multi) or SIMD BM (single) - 10-100 GB/s |
| minimal memory | Naive or Rabin-Karp - O(1) space |
...
| performance first | Hyperscan from start - network security, deep expertise required |
| adaptive | Start simple, profile, evolve - growth startups, data-driven decisions |


**throughput comparison**

- stdlib strstr: 500 MB/s - 2 GB/s
- python str find: 500 MB/s - 2 GB/s
- rust memchr: 5-10 GB/s (SIMD)
