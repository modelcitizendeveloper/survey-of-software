...
description: "Constraint solving in Python: OR-Tools, PuLP, Pyomo, CVXPY and Z3 — scheduling and routing versus convex optimization versus SMT, plus the commercial solvers."
keywords: ["constraint solving", "or-tools", "pulp", "pyomo", "cvxpy", "z3", "z3-solver", "smt solver", "sat solver", "mixed integer programming", "linear programming", "vehicle routing", "scheduling", "gurobi", "cplex", "convex optimization", "cp-sat"]
...
Constraint solving in Python: OR-Tools, PuLP, Pyomo, CVXPY and Z3 — scheduling and routing versus convex optimization versus SMT, plus the commercial solvers.
...
|---|---|---|
| **OR-Tools** | The primary recommendation and the adoption leader (~2.55M monthly, 11.1K stars) — Google-backed, with CP-SAT and routing solvers that cover most scheduling and logistics work outright. | 9.15.6755 · 2026-01-14 |
| **PuLP** | The teaching and prototyping favorite (~1.35M monthly): a readable modeling layer over open-source MIP solvers. Easiest way to state a linear problem correctly. | 3.3.2 · 2026-06-19 |
| **CVXPY** | The convex specialist (~1.05M monthly) — you describe the problem in convex terms and it selects the solver. The right tool when the problem genuinely is convex, and wrong when it is not. | 1.9.2 · 2026-06-22 |
| **Pyomo** | The most expressive modeling language of the open options (~360K monthly), spanning academic and industrial use, with a correspondingly steeper start. | 6.10.1 · 2026-06-04 |
| **Z3** | A different category of solver: SMT/SAT for logical constraints and verification rather than numeric optimization. Microsoft-backed, 10.2K stars. | 5.1.0.0 · 2026-08-16 |
| **Gurobi and CPLEX** | The commercial premium tier — absent from PyPI rankings and dominant in enterprise, chosen for solve time and support on mission-critical models. | 22.2.0.1 · 2026-07-15 |
...
- The first decision is what kind of problem you have — combinatorial scheduling, convex optimization, or logical satisfiability — and getting that wrong makes every library comparison meaningless
- OR-Tools is the defensible default because CP-SAT and the routing solver cover the two problems most teams actually arrive with
- Z3 is filed alongside the optimizers but does not compete with them: SMT answers 'is this satisfiable', not 'what is best'
- Modeling layers (PuLP, Pyomo, CVXPY) are separable from solvers, so the modeling choice can be made without permanently committing to a backend
- PyPI downloads systematically understate commercial solvers — Gurobi and CPLEX are invisible in these rankings and common in enterprise optimization
