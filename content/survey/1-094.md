...
- **OR-Tools** — The primary recommendation and the adoption leader (~2.55M monthly, 11.1K stars) — Google-backed, with CP-SAT and routing solvers that cover most scheduling and logistics work outright.
- **PuLP** — The teaching and prototyping favourite (~1.35M monthly): a readable modelling layer over open-source MIP solvers. Easiest way to state a linear problem correctly.
- **CVXPY** — The convex specialist (~1.05M monthly) — you describe the problem in convex terms and it selects the solver. The right tool when the problem genuinely is convex, and wrong when it is not.
- **Pyomo** — The most expressive modelling language of the open options (~360K monthly), spanning academic and industrial use, with a correspondingly steeper start.
- **Z3** — A different category of solver: SMT/SAT for logical constraints and verification rather than numeric optimization. Microsoft-backed, 10.2K stars.
- **Gurobi and CPLEX** — The commercial premium tier — absent from PyPI rankings and dominant in enterprise, chosen for solve time and support on mission-critical models.
...
- Modelling layers (PuLP, Pyomo, CVXPY) are separable from solvers, so the modelling choice can be made without permanently committing to a backend
