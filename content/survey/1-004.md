...
description: "Comprehensive analysis of binary search tree and B-tree libraries in Python. Covers SortedContainers (production BST alternative), bintrees (deprecated…"
keywords: ["performance", "complexity", "maintenance", "SortedContainers", "bintrees", "BTrees", "characteristics", "B+ tree-like structure", "AVL Tree", "Red-Black Tree", "B-tree"]
...
<figure class="sos-fig"><figcaption>1m insertions sorted <span>lower is faster</span></figcaption><svg viewBox="0 0 720 192" role="img" width="100%" height="192" aria-label="1m insertions sorted, compared"><title>sortedcontainers: 1.2s</title><text x="158" y="17.68" class="sos-bl" text-anchor="end">sortedcontainers</text><rect x="168" y="0" width="9.7" height="26" rx="4" class="sos-bar"/><text x="185.7" y="17.68" class="sos-bv">1.2s</text><title>btrees iobTree: 1.8s</title><text x="158" y="49.68" class="sos-bl" text-anchor="end">btrees iobTree</text><rect x="168" y="32" width="14.6" height="26" rx="4" class="sos-bar"/><text x="190.6" y="49.68" class="sos-bv">1.8s</text><title>bintrees fast avl: 2.5s</title><text x="158" y="81.68" class="sos-bl" text-anchor="end">bintrees fast avl</text><rect x="168" y="64" width="20.2" height="26" rx="4" class="sos-bar"/><text x="196.2" y="81.68" class="sos-bv">2.5s</text><title>btrees oobTree: 3.2s</title><text x="158" y="113.68" class="sos-bl" text-anchor="end">btrees oobTree</text><rect x="168" y="96" width="25.9" height="26" rx="4" class="sos-bar"/><text x="201.9" y="113.68" class="sos-bv">3.2s</text><title>dict plus sort: 8.2s</title><text x="158" y="145.68" class="sos-bl" text-anchor="end">dict plus sort</text><rect x="168" y="128" width="66.4" height="26" rx="4" class="sos-bar"/><text x="242.4" y="145.68" class="sos-bv">8.2s</text><title>bintrees python: 60.0s</title><text x="158" y="177.68" class="sos-bl" text-anchor="end">bintrees python</text><rect x="168" y="160" width="486.0" height="26" rx="4" class="sos-bar"/><text x="662.0" y="177.68" class="sos-bv">60.0s</text></svg></figure>


| Library | Type | Best for | Latest release |
|---|---|---|---|
| **SortedContainers** | B+ tree-like (list of sorted lists) | General-purpose sorted collections | 2.4.0 · 2021-05-16 |
| **bintrees** | AVL, Red-Black, Splay trees | Educational purposes only | 2.2.0 · 2020-12-12 |
| **BTrees** | B-trees (multi-way) | Persistent storage, MVCC, integer keys | 6.5 · 2026-08-20 |
...
- **SortedContainers is 2-10x faster than traditional BSTs despite being pure Python** — Cache locality and exploiting CPython's list optimizations beats pointer-based trees
- **bintrees development was stopped by its author, who points users to sortedcontainers; last release 2.2.0, 2020-12-12** — Migrate to SortedContainers for production code; use only for education
- **BTrees' integer-keyed variants (IIBTree) are 10x more memory-efficient than object-keyed** — For millions of integer keys, BTrees can outperform SortedContainers
- **B-trees have shallow height (log_100 n vs log_2 n for binary trees)** — One million keys: 3 levels (B-tree) vs 20 levels (binary tree)
- **SortedContainers is 182x faster than repeated list.sort() for incremental updates** — Critical for leaderboards, time-series, streaming data
...
| Use case | Pick |
|---|---|
| general purpose | SortedContainers - fastest, maintained, pure Python |
| persistent storage | BTrees - ZODB integration, MVCC |
| integer keys millions | BTrees (IIBTree) - memory-efficient |
| educational | Implement AVL/RB from scratch - learning value |
| legacy bintrees | Migrate to SortedContainers - 2-5x faster, maintained |

| Data size | Pick |
|---|---|
| small 1k | SortedContainers or even list + sort (simple) |
| medium 100k | SortedContainers (default choice) |
| large 1m | SortedContainers or BTrees (IIBTree for integer keys) |
| xlarge 100m | BTrees with ZODB for disk overflow |

| Workload | Pick |
|---|---|
| read heavy | SortedContainers (best cache locality) |
| write heavy | SortedContainers (fewer rotations than AVL) |
...
| **bintrees** | AVL/RB/Splay trees | Development stopped (last release 2020) | ★★☆☆☆ Slow | Educational only |
...
From the bintrees package description:
...
# S1 Recommendations
Default: Use SortedDict/SortedSet from sortedcontainers (pure Python, maintained).
Avoid: Building your own BST unless educational purpose.
For C extension performance: bintrees — but its author stopped development and
points users to SortedContainers; the last release was 2.2.0 in December 2020.
...
Avoid premature optimization - dict/list + bisect.insort sufficient for `<10`K elements.
...
# S3 Recommendations
Range queries: SortedDict with .irange()
Rank queries: SortedList with .bisect_left()
Interval overlaps: Specialized interval tree libraries
...
**Timeline**:
- 2013: bintrees first released on PyPI (AVL, RB, Splay trees for Python)
...
# S4 Recommendations
SortedContainers has strong bus factor (Grant Jenks, active).
Pure Python is strategic advantage (no C compilation, PyPy compatible).
For truly performance-critical: consider Rust extensions (polars, pydantic pattern).
