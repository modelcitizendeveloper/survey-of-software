---
title: "1.102 Document Parsing Libraries"
description: "Document parsing in Python: openpyxl, xlsxwriter, python-docx, python-pptx, pdfplumber and PyPDF2 — reading and writing Excel, Word, PowerPoint and PDF."
keywords: ["format", "capabilities", "performance", "memory", "openpyxl", "xlsxwriter", "pandas", "python-docx", "python-pptx", "pdfplumber", "PyPDF2"]
...
Document parsing in Python: openpyxl, xlsxwriter, python-docx, python-pptx, pdfplumber and PyPDF2 — reading and writing Excel, Word, PowerPoint and PDF.
...
| Library | Recommendation | Performance | Latest release |
|---|---|---|---|
| **openpyxl** | Primary choice for Excel with formatting | 1000 rows/sec read, 500 rows/sec write | 3.1.5 · 2024-06-28 |
| **xlsxwriter** | Best for large report generation | 2000 rows/sec (2x faster than openpyxl) | 3.2.9 · 2025-09-16 |
| **pandas** | Best for data analysis workflows | 10000 rows/sec read (via openpyxl engine) | 3.0.5 · 2026-07-22 |
| **python-docx** | Only production-ready .docx library | 500 paragraphs/sec | 1.2.0 · 2025-06-16 |
| **python-pptx** | Only production-ready .pptx library | 100 slides/sec | 1.0.2 · 2024-08-07 |
| **pdfplumber** | Best for table extraction from PDFs | 20 pages/sec | 0.11.10 · 2026-06-15 |
| **PyPDF2** | Simple PDF operations (not tables) | 50 pages/sec | 3.0.1 · 2022-12-31 |
...
- **top recommendations** — Excel: openpyxl (formatting), xlsxwriter (large reports), pandas (data analysis); Word: python-docx (only production option); PowerPoint: python-pptx (only production option); PDF: pdfplumber (tables), PyPDF2 (simple text/merge)
- **critical insights** — Streaming modes reduce memory 10x (read_only, constant_memory); Security validation critical: formula injection, ZIP bombs, file size limits; Validation first: structure, then data, then business rules; Cloud services (Textract) needed for OCR/scanned documents; Async processing required for >10k documents/day
- **performance benchmarks** — openpyxl: 1000 rows/sec read (standard), 8 sec for 100k rows (read_only); xlsxwriter: 2000 rows/sec write, 6 sec for 100k rows; pandas: 10000 rows/sec, 5 sec for 100k rows; pdfplumber: 20 pages/sec (30 sec for 100 pages)
- **security considerations** — Formula injection: Prefix cell values starting with =+-@ with '; ZIP bombs: Check uncompressed/compressed ratio (<100:1); File size limits: 100 MB Excel, 50 MB Word, 200 MB PDF; MIME validation: Check actual MIME type, not just extension; PDF JavaScript: Use sandboxed environment for untrusted PDFs
...
- ✅ High volume (`>100`k/month) - cloud scales elastically
...
|---------|------------|--------|
| **Text extraction** | ✅ Layout-aware | ✅ Simple |
| **Table extraction** | ✅✅ Excellent | ❌ None |
| **Speed** | ⚠️ Slower (20 pages/sec) | ✅ Fast (50 pages/sec) |
| **Memory** | ⚠️ Higher (5 MB/page) | ✅ Low (1 MB/page) |
| **PDF manipulation** | ❌ Read-only | ✅ Merge, split, rotate |
...
|---------|-----------|------|--------|----------------|
| pandas | Read | 5s | 100 MB | ✅ Fastest read |
| openpyxl | Read (standard) | 15s | 200 MB | ⚠️ Slow for large files |
| openpyxl | Read (read_only) | 8s | 50 MB | ✅ Good compromise |
| xlsxwriter | Write | 6s | 30 MB | ✅ Fastest write |
| openpyxl | Write | 25s | 150 MB | ⚠️ Slow for large files |
| pandas | Write | 20s | 120 MB | ⚠️ Medium speed |
...
| Library | Operation | Time | Memory | Recommendation |
|---------|-----------|------|--------|----------------|
| PyPDF2 | Text only | 10s | 20 MB | ✅ Fast, simple text |
| pdfplumber | Text | 30s | 50 MB | ⚠️ Slower but layout-aware |
| pdfplumber | Tables | 60s | 100 MB | ✅ Best for tables |
...
|---------|----------|------|----------------|
| **openpyxl** | ✅ Mature (10+ years) | Low | Production-ready |
| **xlsxwriter** | ✅ Mature (12+ years) | Low | Production-ready |
| **pandas** | ✅ Mature (13+ years) | Low | Production-ready |
| **python-docx** | ✅ Mature (10+ years) | Low | Production-ready |
| **python-pptx** | ✅ Stable (8+ years) | Low | Production-ready |
| **pdfplumber** | ✅ Stable (7+ years) | Low | Production-ready |
| **PyPDF2** | ✅ Mature (16+ years) | Low | Production-ready |
...
- ✅ When: Scanned documents, handwriting, `>100`k docs/month
- ❌ When: On-premise only, cost-sensitive, `<10`k docs/month

**Commercial libraries** (Aspose, Syncfusion):
- ✅ When: Need 100% Office compatibility (VBA, pivot tables)
- ❌ When: Open-source requirement, budget `<$1`k/year
...
2. ✅ Performance: Profile before optimizing, streaming for `>100`k rows
3. ✅ Scale: Async processing for `>10`k docs/day, consider cloud for `>100`k/month
...
run._element.rPr.rFonts.set(qn('w:eastAsia'), '微软雅黑')  # CJK fallback
...
- Space: O(n × m) - stores all cells in memory
- XML parsing overhead: ~2x slowdown

**openpyxl read-only mode**:
- Time: O(n × m) - same (must parse all XML)
- Space: O(m) - only current row in memory
- Streaming: Yields rows as parsed

**pandas read_excel**:
- Time: O(n × m) + O(s) where s=shared strings table size
- Space: O(n × m) - columnar storage (more efficient than cells)
...
**PyPDF2**:
- Time: O(p × o) where p=pages, o=objects per page
- Space: O(p) - page objects
- Limitation: Simple coordinate-to-text conversion (no layout analysis)

**pdfplumber**:
- Time: O(p × o × l) where l=layout analysis complexity
- Space: O(p × c) where c=characters (stores all character bounding boxes)
- Table extraction: O(p × c²) - checks all character pairs for alignment
...
**Performance Characteristics**:
- Algorithmic complexity (O(n), O(n×m), streaming vs loading)
...
- ✅ Diagnose performance issues (know WHERE bottlenecks are)
- ✅ Optimize code (apply streaming, chunking, format conversion)
- ✅ Secure applications (validate inputs, prevent attacks)
- ✅ Handle edge cases (corrupted files, platform differences)
- ✅ Explain library behavior (understand WHY, not just WHAT)

## Key Insights Expected

1. **OOXML is ZIP + XML** → Can unzip and inspect/modify directly
2. **Shared strings reduce memory** → Repeated text stored once
3. **Streaming modes exist** → 10x memory reduction possible
4. **Security validation critical** → User uploads are attack surface
5. **Edge cases are common** → Test with real user files, not synthetic data
...
├── _rels/
│   └── .rels
├── word/  (or xl/ for Excel, ppt/ for PowerPoint)
│   ├── document.xml
│   ├── styles.xml
│   ├── media/
│   └── _rels/
...
├── [Content_Types].xml     # MIME types for all parts
├── _rels/
│   └── .rels                # Relationships to workbook
├── xl/
...
**Shared strings benefit**: "Apple" appears 1000 times → stored once, referenced 1000 times (saves 99.5% of text memory).
...
**Memory impact**: 1M cells with same format → 1M references to single style object (efficient).
...
**Word documents = paragraphs → runs → text**
...
```
Memory ≈ (rows × cols) × bytes_per_cell
...
**Word memory**:
```
Memory ≈ paragraphs × 1 KB + images × image_size
...
**PDF memory**:
```
Memory ≈ pages × (text_objects + images)
...
Excel 100k rows:
- Unzip: 1 second
- Parse XML: 10 seconds  ← Bottleneck
- Create objects: 4 seconds

Optimization:
- read_only mode: Skip object creation → 8 seconds total (2x speedup)
- data_only mode: Skip formula parsing → 12 seconds (1.25x speedup)
```
...
**Scenario**: Reading 500k row × 50 column Excel file (25M cells)
...
for customer in customers:
    ...
    # Find/replace in all paragraphs (slow: O(n×m))
    for para in doc.paragraphs:
...
```
ROI = (Time_saved × Frequency) / Dev_time

Example 1:
- Current: 30 sec/file × 1000 files/day = 8.3 hours/day wasted
- Optimized: 10 sec/file × 1000 files/day = 2.8 hours/day
- Time saved: 5.5 hours/day
- Dev time: 1 week
- ROI: Break-even in 3 days → DO IT

Example 2:
- Current: 5 sec/file × 10 files/day = 50 sec/day wasted
- Optimized: 2 sec/file × 10 files/day = 20 sec/day
- Time saved: 30 sec/day
- Dev time: 2 weeks
- ROI: Break-even in 4 years → SKIP IT
```
...
3. **Ignoring memory**: Speed ≠ everything (avoid OOM crashes)
...
**Memory hotspots**:
- Excel: 2 KB per cell with formatting → 2 GB for 1M cells
...
**Attack vectors**:
1. **Formula injection**: `=cmd|'/c calc.exe'!A1` → prefix with `'`
2. **ZIP bomb**: 1 MB → 10 GB expansion → check ratio `<100`:1
3. **XXE**: Read local files via XML → disabled by default in Python 3.7+
4. **PDF JavaScript**: Malicious code → use sandboxed environment
...
3. **Scanned PDFs**: No text layer → need OCR
...
```
File size > available memory?
├─ YES → Use streaming (read_only, constant_memory, chunking)
└─ NO → Standard mode OK, optimize only if slow

Repeated access to same file?
├─ YES → Convert to Parquet (5x faster reads)
└─ NO → Read Excel/CSV directly

Need all data at once?
├─ YES → Batch load (pandas, openpyxl standard)
└─ NO → Stream (process row-by-row)

Many independent files?
├─ YES → Parallel processing (ProcessPoolExecutor)
└─ NO → Sequential OK

Expensive operation (OCR, table extraction)?
├─ YES → Cache results (Redis, file cache)
...
| Excel read (1M rows) | OOM | 8s (read_only) | ∞ (vs crash) |
...
**ROI calculation**:
```
Time_saved_per_day = (old_time - new_time) × frequency
Break_even_days = dev_days / time_saved_per_day

If break_even < 30 days → optimize
If break_even > 90 days → skip
```
...
**Memory formula**: rows × cols × 2 KB (standard), × 50 bytes (read_only)
...
**Memory formula**: paragraphs × 1 KB + images
...
**Memory formula**: pages × 5 MB (pdfplumber), × 1 MB (PyPDF2)
...
class BudgetImporter:
    ...
    def get_error_report(self) -> str:
        ...
        for error in self.errors:
            ...
            marker = "❌" if error.severity == 'error' else "⚠️"
...
2. **Three validation layers**: Structure → data types → business rules
...
3. **Fallback patterns**: Multiple patterns increase success rate 60% → 90%
...
- ✅ **Validation**: Structure, data types, business rules
- ✅ **Error handling**: Try/catch with context, user-friendly messages
- ✅ **Security**: File size limits, MIME check, formula sanitization
- ✅ **Logging**: Audit trail (who, what, when), error context
- ✅ **Testing**: Real user files (not just synthetic)
- ✅ **Performance**: Profile with realistic file sizes
- ✅ **Monitoring**: Track success rate, error types, processing time
- ✅ **Documentation**: Error codes, validation rules, supported formats
...
│  │ - Security scanning (virus, formula inject) │         │
│  │ - Audit logging                             │         │
│  │ - Format converters                         │         │
│  │ - Caching layer                             │         │
│  └────────────────────────────────────────────┘         │
...
- ✅ Full control needed (custom validation, transformation)
- ✅ On-premise deployment required (no cloud allowed)
- ✅ Low volume (`<10`k documents/month)
- ✅ Simple formats (modern Office formats)
- ✅ Integration with existing Python stack
- ✅ Cost-sensitive (free libraries vs cloud fees)

**Avoid libraries when:**
- ❌ OCR needed (scanned documents)
- ❌ Complex PDFs (multi-column, forms, annotations)
- ❌ High volume (`>100`k documents/month) - cloud scales better
- ❌ Legacy formats (.doc, .ppt pre-2007) - hard to parse
- ❌ Limited Python expertise on team

### When to Use Cloud Services (AWS Textract, Azure Form Recognizer)

**Choose cloud services when:**
- ✅ OCR required (scanned documents, images)
- ✅ Forms with fixed structure (invoices, receipts, tax forms)
- ✅ High volume (`>100`k/month) - cloud scales elastically
- ✅ Complex PDFs (tables across pages, handwriting)
- ✅ Multiple languages (cloud models pre-trained)
- ✅ Faster time to market (no ML training needed)
...
└──────┬───────┘
       ...
       ↓ (S3 event trigger)
┌──────────────┐
│ SQS Queue    │  (1 message per document)
└──────┬───────┘
       │
       ↓ (poll queue)
┌──────────────────────────────────┐
...
2. **Calculate ROI**: Is optimization worth engineering time?
   ...
   |----------|---------|--------|------------|----------|-----|
   | Excel read | 30s | 10s | 20s × 1000/day = 5.5 hours/day | 1 week | Break-even in 3 days |
   | PDF extract | 5s | 2s | 3s × 100/day = 5 min/day | 2 weeks | Not worth it |
...
│
├─ NO → Migrate all files to .xlsx
│       - One-time conversion with LibreOffice CLI
│       - Update all applications to expect .xlsx
│       - Deprecate .xls support (6 month sunset)
│
└─ YES → Dual-format support
         │
         ├─ High volume? (>10k/month)
         │  └─ Normalize at ingestion (convert .xls → .xlsx on upload)
         │
         └─ Low volume? (<10k/month)
            └─ Support both formats in application
```
...
```
Priority Score = (Impact × Frequency) / Fix Cost
...
Example:
- No error handling: (10 × 1.0) / 1 = 10 (P0)
- No tests: (6 × 0.5) / 5 = 0.6 (P2)
```
...
**Onboarding path** (Junior → Mid proficiency):
...
**Estimated ROI**: 5 weeks investment → 2x productivity on document tasks
...
2. **Invest in training**: 5 weeks → 2x productivity
...
- Scalability (10k → 100k → 1M documents/month)
...
Technical leads can:
- ✅ Choose architecture pattern for their scale
- ✅ Make build-vs-buy decisions confidently
- ✅ Establish governance frameworks
- ✅ Plan for 3-5 year horizon
...
│  │ Shared Components:                          │         │
│  │ - Error handling & retry logic              │         │
│  │ - Security scanning (formula inject, bombs) │         │
...
**Choose open-source libraries when**:
- ✅ Modern formats (.xlsx, .docx, .pptx, .pdf)
- ✅ Digital documents (not scanned)
- ✅ On-premise deployment required
- ✅ Low-medium volume (`<100`k/month)
- ✅ Full control needed (custom validation)
- ✅ Cost-sensitive (free vs $1k-10k/month cloud)
...
- ✅ Scanned documents (require OCR)
- ✅ Handwritten text
- ✅ Complex layouts (tables across pages)
- ✅ High volume (`>100`k/month) - elastic scaling
- ✅ Multiple languages (pre-trained models)
- ✅ Faster time to market (no ML training)
...
```
Is document scanned?
├─ NO → Use Python libraries (free, fast)
└─ YES → Cloud OCR (paid, accurate)
```
...
- ✅ Need 100% Office compatibility (VBA, pivot tables)
- ✅ Budget $1k-10k/year per developer
- ✅ Business-critical (need vendor support)
- ✅ Legacy formats (.doc, .ppt pre-2007)
...
├─ YES → Python libraries (only option)
└─ NO → Continue

Are documents scanned/handwritten?
├─ YES → Cloud services (best OCR)
└─ NO → Continue

Volume > 100k/month?
├─ YES → Cloud services (elastic scaling)
└─ NO → Continue

Need VBA/pivot tables/100% compatibility?
├─ YES → Commercial libraries
└─ NO → Python libraries
...
- ✅ License: MIT/BSD preferred (not GPL for commercial)
- ✅ Maintenance: Updated within 6 months
- ✅ Community: 1k+ GitHub stars, active issues
...
Example:
- Current: 30s/file × 1000 files/day = 8.3 hours/day
- Optimized: 10s/file × 1000 = 2.8 hours/day
- Saved: 5.5 hours/day
- Dev: 1 week (40 hours)
- ROI: Break-even in 7 days → DO IT
```
...
2. **Cloud services dominate OCR**
   - Accuracy: 95% (2025) → 99%+ (2027)
   - Cost: $1.50/1k (2025) → $0.50/1k (2030)
...
2. **Monitor LLMs**: Experimental → production by 2027
...
├─ NO → Migrate all (LibreOffice batch convert)
└─ YES → Dual support or normalize at ingestion
```
...
**Formula**: Priority = (Impact × Frequency) / Fix_Cost
