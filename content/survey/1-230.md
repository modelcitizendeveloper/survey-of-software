...
description: "Open social protocols: ActivityPub, AT Protocol, Matrix and Nostr — how each handles identity, portability and moderation, and what is genuinely standardized."
keywords: ["standardization", "ActivityPub", "AT Protocol / Bluesky", "Matrix", "Nostr"]
...
Open social protocols: ActivityPub, AT Protocol, Matrix and Nostr — how each handles identity, portability and moderation, and what is genuinely standardized.
...
|---|---|---|---|
| **ActivityPub** | Reaching the largest existing decentralized audience | The biggest network in the category at roughly 10M users, and the only one that is a W3C standard. Standardization is the reason to pick it: the specification does not belong to a company. | — |
| **AT Protocol / Bluesky** | Products where sign-up friction and account portability decide adoption | The best user experience in the set and the only one with portable identities, at roughly 15-20M users — larger than ActivityPub's network despite being the newer protocol. Open source rather than standardized. | 0.0.2 · 2023-04-11 |
| **Matrix** | Anything where message confidentiality is a requirement | The only protocol here with built-in end-to-end encryption. That single property decides the choice whenever it is needed, regardless of network size. | 3.0.0 · 2022-04-12 |
| **Nostr** | Simple clients, and payments that ride along with the messages | The simplest protocol in the set, with native Lightning payments. Governed by community NIPs rather than a standards body, which is a tradeoff rather than a defect — the low ceremony is the point. | 0.0.2 · 2023-01-26 |
...
- ActivityPub is largest decentralized network (10M users, W3C standard)
- AT Protocol offers best UX and portable identities (15-20M users)
- Matrix is only protocol with built-in E2E encryption
- Nostr is simplest protocol with native Lightning payments
- No single protocol dominates all use cases
...
Which open social protocols are real — meaning: has a working network, running
implementations and users on it — and what does each one actually guarantee?

This is a survey of **protocols**, not products. The distinction matters because
the thing you commit to is the protocol; the client and the server are
replaceable, and in a working open network several of each exist.

## The Four

- **ActivityPub** (`1.230-activitypub.md`) — the W3C standard, and the largest
  decentralized network
- **AT Protocol / Bluesky** (`1.231-atprotocol-bluesky.md`) — portable identity
  and the best user experience in the set
- **Matrix** (`1.232-matrix-protocol.md`) — the only one with end-to-end
  encryption built in
- **Nostr** (`1.233-nostr.md`) — the simplest, with payments native to it

## What Each Profile Records

For each protocol: how identity works, how content moves between servers, what
the moderation story is, what governs the specification, and the size of the
network actually running it.

Governance is treated as a first-class property rather than trivia. A W3C
standard, an open-source protocol controlled by one company, and a set of
community-authored NIPs are three different kinds of commitment, and the
difference shows up years later rather than on day one.

## Scope Boundary

The comparison is of protocols and their networks. Client libraries for building
bots on top of them are a separate category — see 1.234.


---

# S1 Recommendation: No Protocol Wins Outright

The clearest finding of the rapid pass is a negative one: **no single protocol
dominates all use cases**, and any survey that names one winner here is
compressing away the thing that decides.

## What each one is unambiguously best at

| Protocol | Its distinguishing property | Approximate network |
|---|---|---|
| **ActivityPub** | W3C standard — the specification belongs to nobody | ~10M users, the largest |
| **AT Protocol** | Portable identity and the best user experience | ~15-20M users |
| **Matrix** | Built-in end-to-end encryption — the only one | — |
| **Nostr** | Simplest protocol; native Lightning payments | — |

## The two properties that behave like gates

Most of these differences are trades. Two are not:

**End-to-end encryption.** Matrix has it built in; the others do not. If message
confidentiality is a requirement, the comparison ends there — this is not a
feature you add to a protocol later.

**Standardization.** ActivityPub is a W3C standard, which means the specification
is not controlled by whoever operates the largest server. AT Protocol is open
source but not standardized; Nostr is governed by community NIPs. That difference
does not affect what you can build this month, and it may affect everything about
year three.

## The counterintuitive number

AT Protocol's network is *larger* than ActivityPub's — roughly 15-20M against
~10M — despite ActivityPub being older and the formal standard. Adoption and
standardization are separate axes here, and reading one as evidence of the other
gets this category wrong in both directions.

## What S1 hands forward

Four protocols, no winner, and two hard gates. S2 compares them feature by
feature, S3 works real scenarios, and S4 asks what each commitment costs to
reverse.
...
S1 established that no protocol wins outright. S2 puts the four side by side on
the axes where they actually differ, and scores them, so a reader can see the
shape of the trade rather than being told a conclusion.

## The Files

- `protocol-comparison.md` — the feature matrix proper
- `platform-scores.md` — scored 1-5 across the comparison dimensions
- `use-case-fit.md` — which protocol suits which kind of application
- `recommendation.md` — what the comparison concludes

## What Gets Compared

Identity and portability, content distribution, moderation, encryption,
extensibility, client ecosystem, and governance. Governance is included as a
scored dimension for the reason S1 gives: it is the property whose consequences
arrive latest and are hardest to reverse.

## Method

Protocol capabilities are read from each specification and from the reference
implementations, not from advocacy. Where a protocol supports something through
an extension rather than in its base specification, that distinction is
preserved — an extension is a different guarantee from a core feature, because
implementations may not carry it.
...
Six scenarios, each with a constraint that changes the answer. S1 and S2 establish
that no protocol wins in general; S3's job is to show what "in particular" looks
like.

## The Scenarios

- `scenario-team-bot.md` — a task-management bot for a small team
- `scenario-public-community.md` — a public community platform
- `scenario-privacy-internal.md` — privacy-first internal communication
- `scenario-creator-economy.md` — a creator-economy platform
- `scenario-multi-platform.md` — presence across several networks at once
- `scenario-twitter-alternative.md` — a general microblogging alternative

## Why These Six

Each isolates one binding constraint: confidentiality, reach, monetization,
moderation burden, portability of audience, and the cost of maintaining more than
one integration. A scenario that reaches the same answer as another has been
merged rather than kept for symmetry.

## Method

Each scenario states the constraint first, then the protocol that satisfies it,
then what is given up. Where a scenario has no good answer among the four, it says
so — that is more useful than forcing a recommendation.
...
What does committing to one of these protocols cost to reverse, and what would
have to happen for the choice to go wrong?

In an open-protocol category this question has an unusual shape: the protocol may
outlive every server currently speaking it, or a protocol may be technically
sound and lose its network anyway. Viability and adoption come apart here more
than in most categories.

## The Files

- `protocol-viability.md` — will each protocol still be here, and who decides
- `lock-in-assessment.md` — what actually holds you: identity, audience, or data
- `strategic-guidance.md` — the recommendations that follow
- `evolution-outlook.md` — where the four are heading, 2025-2030
- `build-vs-adopt.md` — whether to build on a protocol or run your own
- `recommendation.md` — the strategic conclusion

## How Lock-in Is Scored Here

Not by data portability alone. In a social protocol the expensive thing to move
is rarely the data — it is the **audience and the identity**, and those move
under different rules than a database export. AT Protocol's portable identity is
therefore a lock-in property in a way a feature list will not show.

## What This Pass Will Not Do

Predict which network wins. The outlook file states what would have to happen for
each outcome and what a reader should watch for, rather than assigning
probabilities to a market that no one can measure.
