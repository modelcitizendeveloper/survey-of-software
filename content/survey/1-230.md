...
| **AT Protocol / Bluesky** | Products where sign-up friction and account portability decide adoption | The best user experience in the set and the only one with portable identities, at roughly 15-20M users — larger than ActivityPub's network despite being the newer protocol. Open source rather than standardized. |
| **Matrix** | Anything where message confidentiality is a requirement | The only protocol here with built-in end-to-end encryption. That single property decides the choice whenever it is needed, regardless of network size. |
| **Nostr** | Simple clients, and payments that ride along with the messages | The simplest protocol in the set, with native Lightning payments. Governed by community NIPs rather than a standards body, which is a tradeoff rather than a defect — the low ceremony is the point. |
