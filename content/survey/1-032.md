...
description: "Libraries and algorithms for calculating string similarity (Levenshtein, Jaro-Winkler, cosine similarity) - the building blocks for fuzzy matching and…"
keywords: ["apache commons text", "rapidfuzz", "string similarity", "strsim", "textdistance"]
...
| Library | How it works | Verdict | Latest release |
|---|---|---|---|
| **rapidfuzz** | C++ (SIMD) | The production default - 400K ops/sec and 12M downloads a month. Also the drop-in replacement for fuzzywuzzy, at 10x the speed. | 3.14.6 · 2026-08-30 |
| **python-Levenshtein** | C | Deprecated at 120K ops/sec - use rapidfuzz, or the maintained Levenshtein fork. | 0.27.4 · 2026-08-08 |
| **textdistance + C backend** | C | 100K ops/sec, but the C backend routes through python-Levenshtein, which is deprecated. | 4.6.3 · 2024-07-16 |
| **strsim (Rust)** | Rust | The Rust pick at 350K ops/sec - zero-cost abstractions, memory safe. | — |
| **Apache Commons Text** | Java | The JVM option at 80K ops/sec; GC overhead is the cost. | — |
| **textdistance (pure Python)** | Python | For algorithm exploration, not production - 8K ops/sec. The survey's pattern is prototype with textdistance, optimize with rapidfuzz. | 4.6.3 · 2024-07-16 |
| **string-similarity (Dice)** | JavaScript | 250K ops/sec | 0.0.2 · 2023-09-24 |
...
- **The community pattern is to use two libraries, not to choose one** — The survey records it as 'prototype with textdistance, optimize with rapidfuzz' — 40+ algorithms while you are exploring, a C++ core once you know which one you need.
- **rapidfuzz is 5-50x faster than the pure-Python alternatives and dominates Python production use** — 12M downloads a month. Where speed matters in Python, the survey records no serious competitor.
- **Bus factor is a universal concern in this category, not a differentiator** — Every major library here is volunteer-driven with a single maintainer except Apache Commons Text, which has Apache Foundation governance and a bus factor above 10.
- **Apache Commons Text is the only institutionally backed option, and that is what it is for** — Twenty-plus years and foundation governance make it the enterprise Java answer; nothing else in the survey offers that kind of continuity.
- **Switching cost is low almost everywhere, which makes the bus-factor risk survivable** — rapidfuzz and textdistance take 2-4 hours to learn and 1-2 weeks to integrate, at low migration risk. Only strsim carries a medium risk, and that is the Rust learning curve rather than the API.
