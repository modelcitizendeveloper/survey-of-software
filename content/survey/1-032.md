...
| Library | How it works | Verdict |
|---|---|---|
| **rapidfuzz** | C++ (SIMD) | The production default - 400K ops/sec and 12M downloads a month. Also the drop-in replacement for fuzzywuzzy, at 10x the speed. |
| **python-Levenshtein** | C | Deprecated at 120K ops/sec - use rapidfuzz, or the maintained Levenshtein fork. |
| **textdistance + C backend** | C | 100K ops/sec, but the C backend routes through python-Levenshtein, which is deprecated. |
...
| **textdistance (pure Python)** | Python | For algorithm exploration, not production - 8K ops/sec. The survey's pattern is prototype with textdistance, optimize with rapidfuzz. |
| **string-similarity (Dice)** | JavaScript | 250K ops/sec |
