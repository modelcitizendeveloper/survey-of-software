...
| **ESLint** | Framework-specific, type-aware or accessibility lint rules; anything leaning on the plugin ecosystem | A linter, not a formatter — run it alongside Prettier, or replace both with Biome. Still the JS/TS linting standard on volume, and the reason to keep it is the plugin ecosystem, which Biome does not match. On the rule count: eslint/eslint main carries 292 rule files in lib/rules, so the 400+ above is not a count of core rules — where that figure came from is not verified. | 10.9.1 · 2026-08-24 |
...
**Comparison:**
- Biome: 15x faster than ESLint (unverified; the harness compares formatters, not linters)
...
- Biome: 200+ rules, 15x faster (unverified), growing
