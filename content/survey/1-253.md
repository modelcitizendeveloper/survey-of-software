---
title: "1.253 Code Formatting Libraries"
description: "Code formatters: Ruff, Black, isort and Flake8 in Python; Prettier, Biome, dprint and ESLint in JavaScript — and Ruff's consolidation of four tools into one."
keywords: ["autopep8", "biome", "black", "blue", "dprint", "eslint", "isort", "prettier", "ruff", "yapf", "performance benchmarks"]
aliases: ["/survey/1-104-2/"]
...
# 1.253 Code Formatting Libraries

Code formatters: Ruff, Black, isort and Flake8 in Python; Prettier, Biome, dprint and ESLint in JavaScript — and Ruff's consolidation of four tools into one.

> [!NOTE]
> **Evidence: [measured, method published](/survey/method/#the-evidence-ladder).** Probes, pinned versions and raw results are at [`harness/1-253-code-formatting/`](https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) in the public repository, with the machine stated.


> [!TIP]
> **Try it: [How fast is Ruff, really?](/workshop/formatters/)** — a working demo of this survey's findings.
> Measure formatter speed claims (Ruff, Black, Biome) on a reproducible corpus at different scales, revealing which speed ratios hold as your code grows.
...
|---|---|---|---|
| **Black** | Conservative teams that want one formatter, no linter, and the canonical style | Actively released and still the reference Python style, but Ruff now emits Black-compatible output far faster while also linting and sorting imports. Choose Black because you want the reference implementation itself, not because it is the lower-risk option. | 26.5.1 · 2026-05-18 |
| **Ruff format** | Every new Python project, and any existing Black + isort + Flake8 stack | The Python default. Black-compatible formatting plus linting and import sorting in one Rust binary, at roughly 81M PyPI downloads a week against Black's 41M — about two to one, not a rout. Still pre-1.0 four years in, so pin the exact version in CI and pre-commit. | 0.5.4 · 2026-08-10 |
| **Blue** | Nothing new; move existing users to Ruff with quote-style single and line-length 79 | Avoid. No PyPI release since 0.9.1 in August 2022 and no repository commit since February 2024 — unmaintained. Its entire differentiator, single quotes at 79 columns, is now two lines of Ruff configuration. | 0.9.1 · 2022-08-01 |
| **autopep8** | Minimal-diff cleanup of a legacy codebase that cannot absorb a full reformat | Repairs PEP 8 violations rather than imposing one style, so it will not settle formatting arguments — which is what the rest of this category is for. Its remaining use is nudging legacy code without a wholesale reformat. | 2.3.2 · 2025-01-14 |
| **YAPF** | A mandated house style that no opinionated formatter can express | Around fifty knobs, which is the opposite of what an opinionated formatter buys a team. Google's repository still takes commits but has not cut a release in over 21 months; treat it as maintenance-only. | 0.43.0 · 2024-11-14 |
| **Prettier** | Polyglot front-end repos; anything where Markdown, YAML or broad language coverage matters | Still the JS/TS default by roughly ten to one on downloads, and the only tool here that formats Markdown and YAML — Biome still does not. A repo containing either keeps Prettier whatever the speed argument says. | 3.9.6 · 2026-07-21 |
| **Biome** | JS/TS/JSON/CSS-only projects where formatter and linter speed is the pain point | The fast unified challenger — formatter and linter in one Rust binary, shipping roughly weekly, but around a tenth of Prettier's download volume. HTML, Vue, Svelte and Astro support landed after this survey was written and is experimental and opt-in; YAML and Markdown remain unsupported. | 2.5.12 · 2026-09-03 |
| **dprint** | Rust-centric toolchains, or a repo where Prettier measurably takes ten seconds or more | Skip unless Prettier is a measured bottleneck. The Wasm plugin architecture is the draw; at roughly 264K weekly downloads against Prettier's 133M, the ecosystem around it is not. | 0.57.4 · 2026-09-05 |
| **ESLint** | Nothing in this category — see 1.250 | A linter, surveyed in 1.250, and listed here only because it is the tool most often mistaken for a formatter. It still ships indent, semi and quotes in lib/rules — a fossil of the years before Prettier, when the linter WAS the formatter. Run eslint-config-prettier so the two stop rewriting the same lines toward different targets. | 10.10.0 · 2026-09-04 |
...
- **Ruff leads Python formatting on volume but has not displaced Black. 81.4M PyPI downloads a week against Black's 41.4M, and 49,319 GitHub stars against 41,814. Black shipped 26.5.1 in May 2026.** — Start new Python projects on `ruff format` + `ruff check`. Do not read an existing Black setup as abandoned tooling — it is maintained and still downloaded at half Ruff's rate.
- **Ruff is still pre-1.0. The current release is 0.16.4, dated 2026-08-20, four years after the project launched.** — Pin the exact Ruff version in CI and pre-commit. A minor bump can restyle a codebase, and a pre-1.0 version contract promises nothing else.
- **Prettier's lead over Biome is roughly ten to one, not close: 132.9M npm downloads a week against Biome's 13.9M, with 52,217 GitHub stars against 25,646.** — Biome is the faster tool, not yet the default one. Budget a Biome migration as a cost you take on, rather than as the safe path.
- **Biome's language gap has narrowed since this survey was written but has not closed. HTML, SVG, Vue, Svelte and Astro are now parsed and linted, with formatting marked experimental and requiring explicit opt-in. YAML and Markdown are still listed as in progress, so Biome formats neither.** — The survey's rule — need HTML, YAML or Markdown, so use Prettier — now holds for YAML and Markdown only. HTML alone no longer rules Biome out, though its HTML formatter is still experimental.
- **Blue is unmaintained. Last PyPI release 0.9.1 on 2022-08-01, last repository commit 2024-02-06, 408 GitHub stars.** — The survey's 'Blue (reluctantly)' row for single-quote teams is a dead end. Ruff covers the same preference with quote-style = 'single' and line-length = 79.
...
- **ruff** (Rust) vs Black (Python): 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- **Biome** (Rust) vs Prettier (JS): 10-31x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- Rust tools (ruff, Biome): 12-32x faster than their predecessors (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus; measured 2026-08-27, @biomejs/biome 1.9.4 vs prettier 3.4.2, 1-12 MB corpus), younger
...
2. **Speed matters**: Rust-based tools (ruff, Biome) are 12-32x faster (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus; measured 2026-08-27, @biomejs/biome 1.9.4 vs prettier 3.4.2, 1-12 MB corpus)
...
- **Python**: Use ruff (format + lint + imports, 16-32x faster; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- Same quality, 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- 100 pages per hour
...
**Answer**: Format-on-save with modern tools adds `<100`ms per save. You won't notice it.
...
- ✅ Want fastest execution (Rust-based tools are 12-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting))
...
3. **Unified tools are the future** — One tool (ruff, Biome) replaces 3+ tools (formatter + linter + import sorter), with 12-32x speed improvement (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting).
...
- **1.252** (Python AST Parsing) — How formatters parse code to understand structure
...
*This document was created as part of research 1.253 (Code Formatting). For tool-specific comparisons and recommendations, see EXPLAINER.md and the 01-discovery/ research.*
...
**Research Domain:** 1.253 Code Formatting
...
- 16-32x faster than Black, depending on corpus size (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus)
...
**Consideration:** Ruff format offers 16-32x faster formatting (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) with `>99.9`% Black compatibility
...
# ESLint — where the boundary falls

**ESLint is a linter, not a formatter. It is surveyed in [1.250 Linters & Static
Analysis](/survey/1-250).**

This file used to carry a full evaluation of ESLint — popularity metrics, plugin ecosystem,
TypeScript support — inside a survey about code formatting. That was a scope error, and it
was made before 1.250 existed to receive the material. What remains here is only the part
that is genuinely about formatting: the boundary between the two tools, which a reader of a
formatting survey does need.

## Why it appears in a formatting survey at all

Because for years the linter *was* the formatter. Before Prettier, the only tool that could
enforce layout in JavaScript was ESLint, through rules like `indent`, `semi` and `quotes`.
Those rules still ship in ESLint 10's `lib/rules`, with users directed to `@stylistic`.

They are a fossil of that era, not a live design position, and running them alongside a
formatter produces two tools rewriting the same lines toward different targets.

## The rule that follows

**Do not lint for formatting.** Run a formatter for layout and a linter for correctness.

In practice that means installing `eslint-config-prettier`, whose entire job is to switch
off every ESLint rule that would fight the formatter. If a project has both tools and no
such config, the fights are already happening.

Stylelint is the clearest precedent: it **removed its stylistic rules outright in version
15** and told users to use a formatter. That is the same boundary, enacted by a project
rather than argued for.

## What is not here

Everything else about ESLint — its plugin architecture, rule coverage, flat config, how it
compares with Biome and oxlint, and the measured answer to "how fast is it" — is in **1.250**,
where the figures are also current. The metrics this file used to carry were from December
2025.
...
2. **Performance matters** - 12-32x speedups (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) are driving adoption
...
**Why Ruff?**
- ~16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) on a large codebase (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus; 16x on a 1 MB one) with `>99.9`% compatibility
...
**blue:** UNMAINTAINED — last release 2022-08-01. Was a niche Black fork for single quotes; use Ruff's `quote-style` instead without meaningful benefit.
...
| Single quote preference | **Ruff** (`quote-style = "single"`) | Blue is unmaintained — last release 0.9.1, 2022-08-01, last commit 2024-02-06 (verified 2026-08-25). Ruff does the same job and is maintained. |
...
- The measured 16-32x speed improvement (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) is compelling
...
Ruff is a unified linter and formatter written in Rust that replaces Black, isort, Flake8, pyupgrade, autoflake, and dozens of plugins - all while running 16-32x faster than Black and 150-1200x faster than YAPF (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting); its linting speedups are unverified here.

## Performance Benchmarks

Astral's own README claims Ruff is "10-100x faster than existing linters (like Flake8) and formatters (like Black)" (vendor: astral-sh/ruff README, accessed 2026-08-27). Measured against Black, formatting lands at the bottom of that range on x86_64 and reaches the low end of the claimed 30x only on aarch64 at a 12 MB corpus. The linter half of the claim is untested here.

- **vs Black: architecture-dependent.** On x86_64 about 20x at 1 MB *falling* to 16x at 12 MB; on aarch64 about 20x at 1 MB *rising* to 32x at 12 MB (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1; x86_64 on a DigitalOcean s5-8vcpu-16gb-30gb, aarch64 on a WSL2 laptop). **The ARM half is not yet evidence** (corrected 2026-08-29): those cells came from a laptop doing other work, and the harness timed each tool in its own window, so a machine that got busier between two tools' windows biases the ratio between them. Whether the ratio genuinely depends on architecture is open until a clean ARM run exists. The x86 figures are the reference and reproduced within 2.7% on a re-run two days later. Faster still with caching, which the measurement disables
- **vs Flake8:** 10-100x faster (vendor: astral-sh/ruff README; unverified here — this survey measured formatters, not linters)
- **vs YAPF:** 150-1200x faster, growing with corpus size (measured 2026-08-27 on x86_64, DigitalOcean s5-8vcpu-16gb-30gb; ruff 0.16.5 vs yapf 0.43.0, 1-12 MB corpus) — the widely repeated "100x" understates it
- **Real-world:** 0.2s for entire codebase vs 20s with Flake8 (vendor: astral-sh/ruff README)
- **dagster (250k LOC):** 0.4s vs pylint's 2.5 minutes (vendor: astral-sh/ruff README)
...
Biome is a performant, all-in-one toolchain for JavaScript, TypeScript, JSX, JSON, and CSS. Designed as a unified replacement for Prettier (formatting) and ESLint (linting), Biome delivers 25x faster formatting and 15x faster linting (unverified) through its Rust-based architecture and multi-threading capabilities.
...
1. **`biome format`** - Code formatting
   ...
   - Multi-threaded (10-31x faster than Prettier; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- 10-31x faster than Prettier, depending on corpus size (measured 2026-08-27, @biomejs/biome 1.9.4 vs prettier 3.4.2, 1-12 MB corpus); multi-threaded
- 100x faster on M1 Max with 10 cores (unverified; vendor figure, not reproduced here)
- 7x faster single-threaded (unverified; the harness does not vary thread count)
- Real-world: 1.3s vs. 28s for 312 files, a 20x speedup (unverified; the harness measured 16-32x on 59-900 files)

**Linter Benchmarks:**
- 15x faster than ESLint (unverified; the harness compares formatters, not linters) (without plugins)
- 4x faster even single-threaded (unverified; thread count not varied here)
...
**Performance:**
- 10-31x faster formatting (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting, multi-threaded)
- 15x faster linting (unverified)
...
**vs. Prettier:**
- Biome: 10-31x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), JS/TS only, 97% compatible, includes linting
- Prettier: Multi-language, mature, slower

**vs. ESLint:**
- Biome: 15x faster (unverified), fewer rules, unified with formatter
...
Biome represents the future of JavaScript tooling: fast, unified, and practical. Its 10-31x formatting speed (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) and 15x linting speed make it ideal for large projects and performance-conscious teams. With 97% Prettier compatibility and growing ESLint rule coverage, Biome is ready for production use.
...
**Performance:**
- 16-32x slower than Ruff formatter (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- Ruff: `>99.9`% compatible, 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), newer
...
# ESLint — the formatter boundary, in detail

**ESLint is a linter. Its architecture, rule coverage, plugin ecosystem, configuration
format and performance are surveyed in [1.250 Linters & Static Analysis](/survey/1-250).**

This file previously ran to 475 lines evaluating ESLint as though it were a member of this
category. It is not one, and the material has moved. What follows is the part a formatting
survey owes its reader: exactly how the two tools overlap, and what to do about it.

## The overlap is real and it is historical

ESLint predates Prettier. In that period the only way to enforce layout in JavaScript was a
lint rule, so ESLint grew a family of them — `indent`, `semi`, `quotes`, `comma-dangle`,
`space-before-function-paren` and their relatives.

Prettier's arrival did not remove those rules; it made them redundant and, worse,
conflicting. Two tools that both rewrite the same line toward different targets do not
converge — they alternate, and which one you see depends on which ran last.

**ESLint 10 still ships them.** They remain in `lib/rules`, with the documentation pointing
at `@stylistic` as their new home. That is a compatibility decision, not a recommendation.

## Three ways teams resolve it, and only one is right

**`eslint-config-prettier` — correct.** A config that turns off every ESLint rule known to
conflict with a formatter. Add it last in the config array so it wins. This is the standard
answer and it is the one to use.

**`eslint-plugin-prettier` — usually wrong.** Runs Prettier *as* an ESLint rule, so
formatting differences surface as lint errors. It works, and it is slow — the formatter runs
inside the linter's rule loop — and it conflates two kinds of output a reader should be able
to tell apart. A formatting difference is not a defect; a lint error implies one.

**Neither — the common failure.** The tools fight, the fight is intermittent, and the team
concludes formatting is a mess rather than that the configuration is missing.

## The precedent worth citing

**Stylelint deleted its stylistic rules in version 15** and directed users to a formatter.
Same boundary, same reasoning, enacted rather than debated — and it is the clearest evidence
that this survey's split between 1.253 and 1.250 reflects where the category itself landed
rather than a taxonomy imposed on it.

## Speed, and what this harness did not measure

The circulating claim that "Biome is 15× faster than ESLint" is a **linting** claim. This
survey's harness compares formatters only, so it could not settle it and said so.

**1.250 took it up**, measured it, and reports it as unsettled for a reason worth knowing:
its first attempt was invalid — ESLint exited with a fatal error and never linted anything —
and the corrected measurement fell below that harness's own repetition threshold. See
...
| **Ruff** | Python | Unified | 16-32x | Moderate | Fast Python format+lint |
...
|------|------|--------------|------------------|----------------|
| **Ruff** | `<100`ms | 2,500,000+ | 16-32x faster | Rust |
...
|------|------------------|---------|----------------|
| **dprint** | `<100`ms | 7-14x faster | Rust |
...
|------|-------|------------|
| **Ruff check** | 10-100x faster (vendor: astral-sh/ruff README) | vs Flake8 |
...
| Tool | Speed | Comparison |
|------|-------|------------|
| **Biome lint** | 15x faster (unverified) | vs ESLint |
...
| **Ruff** | ✓ | Very Fast | `<10`ms | ✓ |
| **Prettier** | ✓ | Moderate | ~100ms | ✓ |
| **Biome** | Community | Very Fast | `<10`ms | ✗ |
| **ESLint** | ✓ | Slow | ~300ms | ✓ |
...
### Ruff vs. Black: The Speedup, Measured
...
- Even Black's best case (cached) is 21x slower (unverified; the harness disables caches)
...
- Ruff time: 180ms
- Speedup: 32x cached, 51x cold (unverified; the harness disables caches and measured 16-32x cold)
...
**Speedup:** 21x faster (unverified; not measured by the harness)
...
- Single-threaded Biome still ~7x faster than Prettier (unverified; not measured by the harness; thread count not varied)
...
**Speedup:** Ruff unified is 53x faster than the traditional stack (unverified; not measured by the harness; formatters only)
...
**Speedup:** Biome unified is 26x faster than separate tools (unverified; not measured by the harness; formatters only)
...
### Python
- **Fastest Formatter:** Ruff (16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting))
- **Fastest Linter:** Ruff (10-100x faster than Flake8 (unverified; the harness compares formatters, not linters))
- **Fastest Import Sort:** Ruff (21x faster than isort; unverified; not measured by the harness)
- **Best Unified:** Ruff (format + lint + import sort)

### JavaScript/TypeScript
- **Fastest Formatter:** dprint (7-14x faster than Prettier (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting))
- **Fastest Practical Formatter:** Biome (10-31x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), better features)
- **Fastest Linter:** Biome (15x faster than ESLint (unverified; the harness compares formatters, not linters))
...
- **16-32x speedups** (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) fundamentally change developer experience
...
**Performance:**
- 10-31x slower than Biome (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- 7-14x slower than dprint (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- Biome: 10-31x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), JS/TS/JSON only, 97% compatible

**vs. dprint:**
- Prettier: More languages, slower, larger ecosystem
- dprint: 7-14x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), plugin-based, less adoption
...
| Project Type | Recommended | Alternative | Rationale |
|-------------|-------------|-------------|-----------|
| **New project** | Biome (format + lint) | Prettier + ESLint | Unified, 10-31x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) |
...
**Note:** This stack is 53x slower than unified Ruff (unverified; not measured by the harness; formatters only). Strongly consider migration.
...
- Performance gain: 16-32x faster formatting (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- Performance gain: 10-31x faster formatting (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), 15x faster linting (unverified)
...
- Rationale: a 10-31x speedup (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) across many developers = major productivity gain
...
**Python:** Ruff strongly recommended (16-32x faster; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
**JavaScript:** Biome strongly recommended (10-31x faster; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
**Python:** Ruff (53x faster than the traditional stack; unverified; not measured by the harness, formatters only)
**JavaScript:** Biome (26x faster than Prettier + ESLint; unverified; not measured by the harness, formatters only)
...
- **Linting:** Ruff
- **Import Sorting:** Ruff
- **Why:** Unified, 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), battle-tested by major projects
...
The code formatting landscape in 2025 is defined by a clear trend: **Rust-based, unified toolchains** (Ruff, Biome) offer transformative performance improvements (16-32x for Ruff against Black, 10-31x for Biome against Prettier; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) while maintaining compatibility with established standards (Black, Prettier).
...
Ruff is an extremely fast Python linter and code formatter written in Rust. It provides a unified toolchain combining linting (Flake8, pylint, pyupgrade, etc.) and formatting (Black-compatible) in a single tool, delivering 16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)s over traditional Python tools.
...
- 16-32x faster than Black (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus)
- 150-1200x faster than YAPF (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- ~250,000 lines: `<100`ms (vs. Black's 1-3 seconds)
- Even without caching, faster than Black with caching

**Linter (`ruff check`):**
- 10-100x faster than Flake8 (unverified; the harness compares formatters, not linters)
...
**Performance:**
- 16-32x faster than Black, 150-1200x than YAPF (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- Pros: Single tool, 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), simpler config
...
Ruff represents the future of Python tooling: fast, unified, and practical. Its Black-compatible formatter achieves `>99.9`% compatibility while delivering 16-32x speed improvements (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting). Combined with its comprehensive linting capabilities, Ruff consolidates 5-10 tools into one.
...
**Rationale:**
- 16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- Unified linting + formatting
...
**Rationale:**
- 7-14x faster than Prettier (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- Unified linting + formatting
...
- Fast local formatting (16-32x faster than Black; measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus)
...
- 7-14x faster than Prettier (measured 2026-08-27, dprint 0.47.6 vs prettier 3.4.2, 1-12 MB corpus)
...
- 10x+ overall speedup (unverified; not measured by the harness)
...
- Modern tools (Ruff, Biome) are 12-32x faster (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus; measured 2026-08-27, @biomejs/biome 1.9.4 vs prettier 3.4.2, 1-12 MB corpus)
...
- Python: Ruff (16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting))
- JavaScript: Biome (10-31x faster than Prettier; measured 2026-08-27, @biomejs/biome 1.9.4 vs prettier 3.4.2, 1-12 MB corpus)
...
- Consider Biome for 10-31x faster formatting (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
**Why Ruff Format:**
- 16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) for CI/CD
...
### Modern Trends
- **Rust implementation**: 10-31x faster than Prettier (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
### Key Success Factors
1. **Technical superiority**: 16-32x performance advantage (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- **Performance**: 16-32x slower than Ruff (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- **Configuration**: "Uncompromising" philosophy aging poorly
- **Platform support**: Python-only, CLI-focused

### Competitive Pressure
- **ruff**: Rust-based, 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting), includes linting
...
### Key Risk Factors
1. **Performance gap**: 16-32x slower than ruff (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
1. **Rust rewrites**: Performance improvements (12-32x measured; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) driving rapid adoption
...
- ruff (Rust): ~100,000 lines/second (unverified; lines/second not measured here)
- Prettier (JavaScript): ~10,000 lines/second
- Biome (Rust): ~300,000 lines/second (unverified; lines/second not measured here)
...
**Counter-trend risk**: JavaScript tooling improved (JIT, better VMs), but unlikely to close the measured 16-32x gap (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting).
...
**Advantages**:
1. **Performance**: 16-32x faster (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) enables new workflows
...
4. **Ecosystem size**: JavaScript ecosystem is 5-10x larger than Python (unverified)

**Biome advantages**:
1. **Performance**: 10-31x faster than Prettier (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- **Performance**: Rust implementations maintain a 12-32x advantage (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- **Consolidation**: Format + lint is standard, format + lint + types emerging

The key strategic insight: **Performance and funding matter more than features.** Tools with the measured 12-32x performance advantages (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) and sustainable funding will win market share from feature-equivalent but slower or underfunded alternatives.
...
### Competition
- **Biome**: Rust-based alternative, 10-31x faster than Prettier (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- **Speed**: 10-31x slower than Biome (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
### Key Risk Factors
1. **Performance gap**: 10-31x slower than Biome (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
**Rationale**: The code formatting ecosystem is consolidating around Rust-based, multi-purpose tools with professional funding. Performance (16-32x measured against Black, 10-31x against Prettier; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) and financial sustainability are decisive strategic factors.
...
1. **Performance**: ~16-32x faster than Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) on a large codebase (measured 2026-08-27, ruff 0.16.5 vs black 26.5.1, 1-12 MB corpus) enables instant formatting
...
**Risks**:
- Performance gap (16-32x slower than ruff; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
**Risks**:
- Performance gap (10-31x slower than Biome; measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- ✅ Performance is acceptable (`<100`K lines)
...
- 16-32x performance advantage over Black (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
- 10-31x performance advantage over Prettier (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
...
The 12-32x performance advantages of Rust implementations (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting) are decisive strategic factors. Combined with professional funding models (VC or foundation), Rust-based tools (ruff, Biome) are better positioned for long-term success than their Python/JavaScript predecessors (Black, Prettier).
...
### Modern Trends
- **Rust implementation**: 16-32x faster than Black, 150-1200x than YAPF (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
- **Multi-tool consolidation**: Format + lint in one tool
...
3. **Technical superiority**: 16-32x performance advantage (measured 2026-08-27, https://github.com/modelcitizendeveloper/survey-of-software/tree/main/harness/1-253-code-formatting)
