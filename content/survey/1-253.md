...
| **Biome** | JS/TS/JSON/CSS-only projects where formatter and linter speed is the pain point | The fast unified challenger — formatter and linter in one Rust binary, shipping roughly weekly, but around a tenth of Prettier's download volume. HTML, Vue, Svelte and Astro support landed after this survey was written and is experimental and opt-in; YAML and Markdown remain unsupported. | 2.5.11 · 2026-08-27 |
| **dprint** | Rust-centric toolchains, or a repo where Prettier measurably takes ten seconds or more | Skip unless Prettier is a measured bottleneck. The Wasm plugin architecture is the draw; at roughly 264K weekly downloads against Prettier's 133M, the ecosystem around it is not. | 0.56.1 · 2026-08-21 |
| **ESLint** | Nothing in this category — see 1.250 | A linter, surveyed in 1.250, and listed here only because it is the tool most often mistaken for a formatter. It still ships indent, semi and quotes in lib/rules — a fossil of the years before Prettier, when the linter WAS the formatter. Run eslint-config-prettier so the two stop rewriting the same lines toward different targets. | 10.9.1 · 2026-08-24 |
