...
|---|---|---|
| **neo4j (official driver)** | Most new Neo4j projects — the default | Tier 1. The official driver, v6.2.0 (2026-05-04), and the best async story in the category; the others need a separate package for it. |
| **gremlinpython** | Keeping the option to change database open | Tier 1, and the portability choice — Apache TinkerPop scores 9/10 on portability against Cypher/GQL's 7 and GSQL's 2. v3.8.1 (2026-07-27). |
| **rdflib** | RDF and SPARQL rather than property graphs | Tier 1 for a different shape of problem. If the data is triples this is the answer, and the property-graph drivers are not. v7.6.0 (2026-02-13). |
| **python-arango** | ArangoDB deployments | Tier 1, official. Note the license change recorded in S4 — ArangoDB moved to BSL 1.1 at 3.12+, which reached existing users. |
| **neomodel** | Django-style models over Neo4j | Tier 2, an OGM rather than a driver — it sits on top of the official driver. v6.2.0 (2026-06-23). |
| **pydgraph** | Dgraph deployments needing horizontal scale | Tier 3, niche. Tied to one database. |
| **pyTigerGraph** | TigerGraph deployments and graph ML workflows | Tier 3, niche — and the heaviest lock-in here: GSQL scores 2/10 on portability, so the query language does not travel. |
| **py2neo** | Nothing new — migrate off it | Tier 4, AVOID. End-of-life; the last release on PyPI is 2021.2.4 from 2023-10-20. Use the official neo4j driver. |
| **pyorient** | Nothing — unmaintained | Tier 4, AVOID. No releases since 2017. |
