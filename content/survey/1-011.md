...
description: "Graph database clients for Python: neo4j, python-arango, gremlinpython and rdflib — query-language lock-in, and raw driver versus object-graph mapper."
keywords: ["tier1 production", "tier2 mature", "tier3 niche", "tier4 avoid", "neo4j", "gremlinpython", "rdflib", "python-arango"]
...
Graph database clients for Python: neo4j, python-arango, gremlinpython and rdflib — query-language lock-in, and raw driver versus object-graph mapper.
...
|---|---|---|---|
| **neo4j (official driver)** | Most new Neo4j projects — the default | Tier 1. The official driver, v6.2.0 (2026-05-04), and the best async story in the category; the others need a separate package for it. | 6.3.0 · 2026-08-31 |
| **gremlinpython** | Keeping the option to change database open | Tier 1, and the portability choice — Apache TinkerPop scores 9/10 on portability against Cypher/GQL's 7 and GSQL's 2. v3.8.1 (2026-07-27). | 3.8.1 · 2026-07-27 |
| **rdflib** | RDF and SPARQL rather than property graphs | Tier 1 for a different shape of problem. If the data is triples this is the answer, and the property-graph drivers are not. v7.6.0 (2026-02-13). | 7.6.0 · 2026-02-13 |
| **python-arango** | ArangoDB deployments | Tier 1, official. Note the license change recorded in S4 — ArangoDB moved to BSL 1.1 at 3.12+, which reached existing users. | 8.3.5 · 2026-08-31 |
| **neomodel** | Django-style models over Neo4j | Tier 2, an OGM rather than a driver — it sits on top of the official driver. v6.2.0 (2026-06-23). | 7.0.0 · 2026-08-26 |
| **pydgraph** | Dgraph deployments needing horizontal scale | Tier 3, niche. Tied to one database. | 25.2.0 · 2026-02-25 |
| **pyTigerGraph** | TigerGraph deployments and graph ML workflows | Tier 3, niche — and the heaviest lock-in here: GSQL scores 2/10 on portability, so the query language does not travel. | 2.0.4 · 2026-05-19 |
| **py2neo** | Nothing new — migrate off it | Tier 4, AVOID. End-of-life; the last release on PyPI is 2021.2.4 from 2023-10-20. Use the official neo4j driver. | 2021.2.4 · 2023-10-20 |
| **pyorient** | Nothing — unmaintained | Tier 4, AVOID. No releases since 2017. | 1.5.5 · 2017-04-29 |
...
- **primary recommendation** — neo4j (official driver) + neomodel (OGM)
- **portability choice** — gremlinpython (Apache TinkerPop)
- **deprecated** — py2neo (EOL April 2025)
- **convergence points** — neo4j driver is clear winner for most new projects; gremlinpython for multi-database portability; py2neo is dead - migrate immediately; Async support varies: neo4j best, others need separate packages
- **lock in analysis** — gremlin: 9/10 portability; cypher gql: 7/10 portability (improving); gsql: 2/10 portability (severe lock-in)
...
- ✅ **Pros**: Most readable query language, largest community
...
├── Neo4j (40-50%) ← Dominant, VC-backed
...
- **Backing Database Trajectory**: Does the library serve a growing or declining database?
...
- Maintenance Health
- Vendor Durability (longevity, funding model, license history)
...
> **Rewritten 2026-08-25 (re-fak).** These sections previously reported vendor
> funding totals, valuations, ARR and market share. That is vendor ranking — the
> arbiter-of-commercial-software role the 3.xxx series exists to keep off the
> published site — and it had reached a live 1.xxx page. The S4 question it was
> answering is real and is kept: *will this still be maintained in three years,
> and what happens to me if the vendor changes course?* It is now answered with
> longevity, funding MODEL, license history and adoption breadth — signals about
> the software's durability and its consequences for a user. Pricing is kept
> throughout: what a tool costs to use is a fact a reader needs.

### Vendor Durability: STRONG

- **Vendor**: Neo4j Inc. (founded 2007) — nearly two decades, which is long for
  this category
- **Funding model**: venture-backed. Worth knowing as a *risk shape* rather than
  as a score: VC-backed vendors are the ones for whom a license change or an
  acquisition is on the table, and ArangoDB below is what that looks like when
  it happens.
- **Business model**: open-source core plus a managed service. The dual
  structure is what puts advanced features behind the Enterprise license —
  see the pricing section for what that costs.
- **Adoption breadth**: very wide, across large enterprises. Read as a
  maintenance signal — a driver this widely deployed does not get abandoned
  quietly — not as an endorsement.
...
### Vendor Durability: COMMUNITY + LABS
...
### Vendor Durability: MODERATE (Changed)

- **Vendor**: ArangoDB GmbH (founded 2014)
- **Funding model**: venture-backed — and this is the entry that shows why the
  funding *model* is worth recording while the funding *total* is not. The
  license changed under existing users.
...
### Vendor Durability: MODERATE (Enterprise Focus)

- **Vendor**: TigerGraph (founded 2012)
- **Funding model**: venture-backed, enterprise sales motion. The same
  license-change risk applies as for any VC-backed vendor here.
- **Focus**: enterprise analytics — fraud detection, supply chain. A narrow
  commercial focus is a durability signal in both directions: the product is
  unlikely to be abandoned, and unlikely to grow toward a smaller user.
- **Adoption**: deployed at large-transaction-volume companies, which is the
  workload the engine is tuned for.
...
### Vendor Durability: FOUNDATION + MULTI-VENDOR
...
1. **Deployment breadth**: the most widely deployed graph database, which is
   why its driver has the most documentation, the most answered questions, and
   the least chance of quiet abandonment
2. **Longevity**: shipping since 2007, with a monthly release cadence today
