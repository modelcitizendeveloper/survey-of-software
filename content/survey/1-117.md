...
|---|---|---|
| **GSAP** | Anything that must outlive a framework, and the hard cases — SVG, timelines, complex scroll-driven work | A+ (9.6/10), the survey's strategic winner. 18 years of continuous development and framework-agnostic, which is the lowest lock-in risk in the category. Now Webflow-owned; check current license terms before relying on the premium plugins |
| **Framer Motion (now published as `motion`)** | React product work — declarative API and layout animations covering 80-90% of a React app's needs | A (8.4/10). Company-backed and actively developed, 35.6M weekly downloads. Renamed: import from motion/react, MIT. React-only, which S4 calls an acceptable lock-in for a React-committed team |
| **React Spring** | Spring-physics motion inside React, where natural dynamics beat duration-based tweens | B (6.6/10) — medium-high abandonment risk. S4 puts it on the avoid list for new React work; if it stalls, migration to Framer Motion (which also does springs) is viable |
| **Anime.js** | Small framework-agnostic animations where bundle size is the binding constraint | C (6.0/10) — very high abandonment risk, which S4 calls its most critical concern. Only with an explicit plan to fork or migrate |
