...
description: "Web animation libraries: GSAP, Framer Motion, React Spring and Anime.js — timeline control against spring physics, and how tightly each binds to a framework."
keywords: ["Framer Motion", "GSAP", "React Spring", "Anime.js"]
...
Web animation libraries: GSAP, Framer Motion, React Spring and Anime.js — timeline control against spring physics, and how tightly each binds to a framework.
...
|---|---|---|---|
| **GSAP** | Anything that must outlive a framework, and the hard cases — SVG, timelines, complex scroll-driven work | A+ (9.6/10), the survey's strategic winner. 18 years of continuous development and framework-agnostic, which is the lowest lock-in risk in the category. Now Webflow-owned; check current license terms before relying on the premium plugins | 3.15.0 · 2026-04-13 |
| **Framer Motion (now published as `motion`)** | React product work — declarative API and layout animations covering 80-90% of a React app's needs | A (8.4/10). Company-backed and actively developed, 35.6M weekly downloads. Renamed: import from motion/react, MIT. React-only, which S4 calls an acceptable lock-in for a React-committed team | 13.2.0 · 2026-09-02 |
| **React Spring** | Spring-physics motion inside React, where natural dynamics beat duration-based tweens | B (6.6/10) — medium-high abandonment risk. S4 puts it on the avoid list for new React work; if it stalls, migration to Framer Motion (which also does springs) is viable | 10.0.4 · 2026-05-22 |
| **Anime.js** | Small framework-agnostic animations where bundle size is the binding constraint | C (6.0/10) — very high abandonment risk, which S4 calls its most critical concern. Only with an explicit plan to fork or migrate | 4.5.0 · 2026-08-17 |
...
- **GSAP is the strategic winner and framework independence is why** — A+ at 9.6/10 on eighteen years of continuous development. Being framework-agnostic is the lowest lock-in risk in the category — it survives the framework churn the others are exposed to.
- **Two of the four are on S4's avoid list for new React work** — React Spring for maintenance risk, Anime.js for abandonment risk, which S4 calls its most critical concern. Neither should start a new project.
- **For React product work the answer is two libraries, not one** — The recommended stack is Framer Motion as primary for 80-90% of needs with GSAP as the specialist for SVG, complex scroll and timeline work.
- **Framer Motion now ships as the `motion` package, and the survey's prose uses the old name throughout** — Import from motion/react, MIT, 35.6M weekly downloads. Searching for the old name finds older material.
- **GSAP is now Webflow-owned, which is a licensing question rather than a technical one** — The survey's license analysis predates the acquisition. Check current terms before relying on the premium plugins.
...
> **Refresh (2026-06, verified 2026-06-26):** Two naming changes material to this survey:
> (1) **"Framer Motion" → `motion`** — the library is now published under the package name `motion`
> (import as `motion/react`), MIT licensed; `framer-motion` remains available for compatibility but
> `motion` is the active release line. Downloads: 35.6M weekly (up substantially from the figures
> below). (2) **GSAP is now Webflow-owned** — strategic viability should be re-read with Webflow as
> the commercial backer rather than independent GreenSock. Neither change alters the category
> placement or primary recommendations, but package name references throughout this survey should be
> noted as using the legacy `framer-motion` name.
