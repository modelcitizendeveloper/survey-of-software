...
- **spaCy** — The production default — fast, opinionated, pre-trained pipelines and an API designed for shipping rather than exploring. The community consensus choice for industrial NLP.
- **Transformers (Hugging Face)** — The route to state-of-the-art: the model hub plus a uniform API over BERT/GPT-class models. The right answer when accuracy beats latency, and the largest community in the category by stars.
- **NLTK** — The academic standard and the best teaching tool — comprehensive classical algorithms and documentation. Widely described as the wrong choice for production.
- **TextBlob** — The smallest possible API for common tasks. Good for prototypes and simple sentiment work; not a foundation to grow on.
- **Gensim** — The specialist for topic modelling, word2vec and document similarity — efficient unsupervised implementations rather than a general pipeline.
