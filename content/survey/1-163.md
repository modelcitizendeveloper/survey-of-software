...
description: "Character encoding detection, transcoding, and CJK text handling. Covers Big5 (Traditional Chinese), GB2312/GBK/GB18030 (Simplified Chinese), Unicode CJK…"
keywords: ["detection", "transcoding", "repair", "cjk conversion", "charset-normalizer", "cchardet", "chardet", "Python codecs", "ftfy", "OpenCC", "zhconv"]
...
|---|---|---|---|
| **charset-normalizer** | detection | active | 3.5.1 · 2026-08-15 |
| **cchardet** | detection | sporadic | 2.1.7 · 2024-06-06 |
| **chardet** | detection | maintenance | 7.6.0 · 2026-08-14 |
| **Python codecs** | transcoding | active | — |
| **ftfy** | repair | active | 6.3.1 · 2024-10-26 |
| **OpenCC** | cjk conversion | active | 1.4.2 · 2026-08-22 |
| **zhconv** | cjk conversion | active | 1.4.3 · 2021-10-31 |
...
- **This is four separate problems, and using the wrong tool for one of them is the category's failure mode** — Transcoding a known encoding, detecting an unknown one, repairing mojibake, and converting Traditional to Simplified. Only the second has genuine competition.
- **If you know the encoding, the answer is the standard library and nothing else** — bytes.decode() and str.encode() are always available, fast and comprehensive. Reaching for a detection library when the encoding is known is wasted work and a source of error.
- **Detection is a speed-against-accuracy trade with a clean decision rule** — charset-normalizer for best accuracy at 95%+, cchardet for 10-100x the speed at 80-95%. Pure Python means charset-normalizer; files above 1MB mean cchardet.
- **chardet is in maintenance mode and charset-normalizer is its successor** — chardet remains the pure-Python fallback but is no longer the default answer. uchardet the survey skips outright in favor of cchardet.
- **Mojibake repair is its own tool, not a detection problem** — ftfy exists because text already decoded through the wrong encoding cannot be fixed by detecting it better.
