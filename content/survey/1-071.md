...
- **scikit-learn** — The universal default and the largest thing in the category by far (~133M monthly downloads, 63.5K stars) — PCA, truncated SVD and a t-SNE implementation, already installed in every scientific Python environment.
- **UMAP (umap-learn)** — The specialised leader and fastest-growing option (~2.4M monthly): faster than t-SNE and better at preserving global structure, which is the specific complaint t-SNE attracts.
- **openTSNE** — An optimized t-SNE for people who want t-SNE specifically and cannot accept the reference implementation's runtime.
- **PaCMAP** — Research-backed challenger claiming better local/global balance than both UMAP and t-SNE. Low adoption is the risk, not the method.
- **TriMAP** — An academic tool rather than a production dependency — worth knowing about, hard to justify as a default.
