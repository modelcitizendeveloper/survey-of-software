---
title: "1.071 Dimensionality Reduction Libraries"
description: "Dimensionality reduction in Python: scikit-learn's PCA and t-SNE, UMAP, PaCMAP, openTSNE and TriMAP — linear versus manifold methods, and what each preserves."
keywords: ["dimensionality reduction", "umap", "umap-learn", "t-sne", "opentsne", "pacmap", "trimap", "pca", "scikit-learn", "manifold learning", "embedding visualization", "feature reduction", "nonlinear dimensionality reduction", "svd", "clustering visualization"]
...
# 1.071 Dimensionality Reduction Libraries

Dimensionality reduction in Python: scikit-learn's PCA and t-SNE, UMAP, PaCMAP, openTSNE and TriMAP — linear versus manifold methods, and what each preserves.
...
|---|---|---|
| **scikit-learn** | The universal default and the largest thing in the category by far (~133M monthly downloads, 63.5K stars) — PCA, truncated SVD and a t-SNE implementation, already installed in every scientific Python environment. | 1.9.0 · 2026-06-02 |
| **UMAP (umap-learn)** | The specialised leader and fastest-growing option (~2.4M monthly): faster than t-SNE and better at preserving global structure, which is the specific complaint t-SNE attracts. | 0.5.12 · 2026-04-08 |
| **openTSNE** | An optimized t-SNE for people who want t-SNE specifically and cannot accept the reference implementation's runtime. | 1.0.4 · 2025-10-27 |
| **PaCMAP** | Research-backed challenger claiming better local/global balance than both UMAP and t-SNE. Low adoption is the risk, not the method. | 0.9.1 · 2026-03-02 |
| **TriMAP** | An academic tool rather than a production dependency — worth knowing about, hard to justify as a default. | 1.2.0 · 2026-08-20 |
...
- The decision is what you need preserved, not which library is best: PCA keeps variance and is interpretable, t-SNE keeps local neighbourhoods, UMAP claims both, and no method keeps everything
- scikit-learn wins by being already present — the adoption gap over UMAP is ~50x, and much of it is that nobody has to install it
- UMAP's rise is mostly about t-SNE's known weakness on global structure, plus runtime; it is the default challenger rather than a different kind of tool
- The emerging methods (PaCMAP, TriMAP) make credible claims with thin adoption — a real risk for anything long-lived, and a reason PCA and UMAP stay the safe answers
- Visualization is where these are used and also where they mislead: apparent clusters in a 2-D projection are an artifact as often as a finding
