...
description: "NFC (13.56 MHz, 0-4cm range) libraries for mobile and web. React Native: react-native-nfc-manager is the only serious option (v3.17.2, ~33K/week). Flutter…"
keywords: ["platform", "stars", "weekly downloads", "react-native-nfc-manager", "React Native", "pub points", "likes", "nfc_manager", "Flutter", "CoreNFC", "iOS", "NfcAdapter", "Android", "note", "flutter_nfc_kit", "Flutter + Web NFC", "NDEFReader", "Android Chrome 89+"]
...
NFC (13.56 MHz, 0-4cm range) libraries for mobile and web. React Native: react-native-nfc-manager is the only serious option (v3.17.2, ~33K/week). Flutter: nfcmanager (preferred) or flutternfckit (adds Web NFC support). Native: CoreNFC (iOS, reader only) and NfcAdapter + HostApduService (Android, full HCE). Web: NDEFReader API (Android Chrome only, NDEF only). Key constraint: NFC is always reader+card — iOS cannot be a card for general apps.
...
|---|---|---|
| **react-native-nfc-manager** | JavaScript/TypeScript | MIT |
| **nfc_manager** | Dart | MIT |
| **CoreNFC** | Swift/Objective-C | Apple SDK |
| **NfcAdapter** | Kotlin/Java | Android SDK |


**What the research found**

- NFC is asymmetric: always reader + card. iOS is reader only. Android can be either. iOS-to-iOS NFC is impossible.
- iOS reads Android HCE: iPhone NFCTagReaderSession treats Android HCE as an NFCISO7816Tag — fully practical for bootstrap protocols.
- HCE cannot be driven from JavaScript: HostApduService.processCommandApdu() is synchronous; JS bridge calls are async. Requires native Android code.
- MIFARE Classic is broken: Crypto-1 was reverse-engineered in 2008. Use DESFire EV2/EV3 or application-layer AES for any new secure deployment.
- Web NFC is Android Chrome only: Not on W3C Standards Track; no Safari or Firefox support announced.
