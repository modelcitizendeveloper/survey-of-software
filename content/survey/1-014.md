...
description: "Network flow libraries: NetworkX, OR-Tools, igraph and graph-tool — maximum flow and minimum-cost flow, and when a solver beats a graph library."
keywords: ["accessed", "github", "stars", "downloads", "NetworkX", "OR-Tools", "igraph", "repository", "graph-tool"]
...
Network flow libraries: NetworkX, OR-Tools, igraph and graph-tool — maximum flow and minimum-cost flow, and when a solver beats a graph library.
...
|---|---|---|---|
| **NetworkX** | Prototyping, research, <100K nodes | BSD (permissive) | 3.6.1 · 2025-12-08 |
| **igraph** | R users, mid-scale (100K-1M nodes) | GPL-2.0 | 1.0.0 · 2025-10-23 |
| **OR-Tools** | Production logistics, optimization | Apache 2.0 | 9.15.6755 · 2026-01-14 |
| **graph-tool** | Research, >1M nodes, max performance | LGPL-3.0 | — |
...
- **There is no winner here, because these libraries occupy non-competing niches** — S4 says so directly and contrasts it with categories where one or two leaders emerge. The choice is which niche matches your work, not which library is best.
- **The axis is scale, and it has hard breakpoints** — NetworkX below ~100K nodes, igraph from 100K to 1M, graph-tool above that. A 10M-node graph is the one case where only graph-tool will finish.
- **Production routing is OR-Tools' niche and the learning curve is the price** — The survey rates it hardest to use and very fast, battle-tested at Google scale — worth the curve for a production logistics system, not for a prototype.
- **NetworkX is the safe default and its ceiling is architectural** — 23 years, 15M downloads a week, NumFOCUS backing, a backward-compatibility culture, scored 54/60. Its performance limit is pure Python and will not be engineered away.
- **License separates these libraries as sharply as performance does** — NetworkX is BSD and OR-Tools Apache 2.0; igraph is GPL-2.0 and graph-tool LGPL-3.0. For a proprietary product that is a first-pass filter, not a footnote.
