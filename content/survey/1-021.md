...
description: "Hypothesis testing and A/B testing libraries - scipy.stats, statsmodels, pingouin (t-tests, ANOVA, chi-square, effect sizes, multiple comparisons)"
keywords: ["pingouin", "scipy stats", "statsmodels"]
...
|---|---|---|
| **scipy.stats** | Foundation, production systems | 0.0.1 · 2024-06-04 |
| **statsmodels** | Statistical modeling | 0.15.0 · 2026-08-27 |
| **pingouin** | Productivity, exploration | 0.6.1 · 2026-03-28 |
...
- **The license, not the features, is what rules pingouin out of a lot of work** — pingouin is GPL while scipy.stats and statsmodels are BSD. For proprietary software the survey routes straight past it, whatever its ergonomics.
- **These three are a stack, not a shortlist** — The survey's own recommendation is scipy.stats as the foundation, pingouin added for productivity where GPL is acceptable, and statsmodels brought in when you need modeling.
- **statsmodels is the only serious Python option for GLM and mixed models** — For statistical modeling the decision matrix lists no second choice at all — the category has one entry.
- **scipy.stats is the safest long-term bet by a clear margin** — 9.5/10 viability against statsmodels at 8.5 and pingouin at 7.0, on a 20-year track record. It is also the answer for regulated work, being validated and accepted by FDA and EMA.
- **pingouin's advantage is what it does automatically, and that is a productivity argument with a shelf life** — Automatic effect sizes and DataFrame output make it best for exploration and learning. S4 calls it acceptable under a two-year horizon and a multi-library strategy beyond that.
