---
title: "1.184 Connection Pool Libraries"
description: "Survey of database connection pooling: pgBouncer and Odyssey (external PostgreSQL proxies), HikariCP (Java), SQLAlchemy QueuePool (Python), asyncpg pool…"
keywords: ["language db", "prepared stmts txn mode", "pgBouncer", "Odyssey", "PgPool-II", "HikariCP", "c3p0", "DBCP2", "SQLAlchemy Pool", "asyncpg", "pg Pool", "Prisma Accelerate"]
...
|---|---|---|
| **pgBouncer** | External proxy | General PostgreSQL pooling |
| **Odyssey** | External proxy | High-traffic, prepared stmts |
| **PgPool-II** | External proxy | Pooling + load balancing |
| **HikariCP** | In-process | All JVM applications |
| **c3p0** | In-process | Legacy Java (avoid new use) |
| **DBCP2** | In-process | Legacy Java/Tomcat |
| **SQLAlchemy Pool** | In-process | Python applications |
| **asyncpg** | In-process | Python async applications |
| **pg Pool** | In-process | Node.js applications |
| **Prisma Accelerate** | External service | Prisma + serverless |


**What the research found**

- **The category is mature and the leaders are not being displaced: pgBouncer for PostgreSQL, HikariCP for the JVM** — Both sit at Adopt on the survey's technology radar. c3p0 and Commons DBCP2 are Hold — maintain what exists, start nothing new on them.
- **pgBouncer's transaction mode is incompatible with server-side prepared statements, and this catches Java teams by default** — PgJDBC promotes to server-side prepared statements at prepareThreshold=5 on its own. The fix is prepareThreshold=0, trading per-statement performance for compatibility — or Odyssey, which supports both.
- **In-process pools are actively harmful under serverless, not merely suboptimal** — Each ephemeral instance opens its own pool. The answer is a managed pooler: RDS Proxy on Lambda, Hyperdrive on Cloudflare Workers, Prisma Accelerate for Prisma.
- **The first question is in-process or external proxy, and it decides everything after it** — An external proxy pools across every application instance and is transparent to application code; an in-process pool cannot see the other instances competing for the same server connections.
- **Odyssey is the credible challenger, and the reason is architectural rather than incremental** — pgBouncer is single-threaded by design; Odyssey runs a thread per route and supports prepared statements in transaction mode. The survey positions it at Trial — strong technical case, smaller ecosystem.
