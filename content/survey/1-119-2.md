...
description: "Push notification libraries: @react-native-firebase/messaging + @notifee/react-native (React Native, 370K+125K/week), firebasemessaging +…"
keywords: ["platform", "weekly downloads", "@react-native-firebase/messaging", "React Native", "@notifee/react-native", "pub points", "likes", "firebase_messaging", "Flutter", "flutter_local_notifications", "firebase-admin", "Server", "note", "@expo/expo-notifications", "awesome_notifications", "web-push", "apns2", "OneSignal", "Novu"]
...
Push notification libraries: @react-native-firebase/messaging + @notifee/react-native (React Native, 370K+125K/week), firebasemessaging + flutterlocalnotifications (Flutter, 160 pts), firebase-admin (server). Web Push via VAPID. Third-party services: OneSignal (free), Novu (open-source). Key: FCM legacy API removed June 2024, Android 13+ requires permission, delivery is best-effort not guaranteed.
...
| Library | Language | License | Latest release |
|---|---|---|---|
| **@react-native-firebase/messaging** | JavaScript/TypeScript | Apache 2.0 | 26.4.0 · 2026-09-05 |
| **@notifee/react-native** | JavaScript/TypeScript | Apache 2.0 | 9.1.8 · 2024-12-20 |
| **firebase_messaging** | Dart | BSD | — |
| **flutter_local_notifications** | Dart | MIT | — |
| **firebase-admin** | Node.js/Python/Java/Go | Apache 2.0 | 7.5.0 · 2026-07-02 |
...
- FCM legacy API removed June 2024: Server code using https://fcm.googleapis.com/fcm/send is broken. Migrate to v1 API with OAuth 2.0 service account.
- Delivery ≠ Display: Delivery library (firebase/messaging) gets message to device; display library (notifee / flutter_local_notifications) shows it. Both needed for proper behavior.
- Android 13+ requires permission: POST_NOTIFICATIONS is now required; opt-in rates dropping toward iOS levels (~50-70%).
- react-native-push-notification is dead: Unmaintained since 2022. Use @react-native-firebase/messaging + @notifee/react-native.
- iOS Web Push requires PWA install: Safari iOS 16.4+ supports Web Push only for Add-to-Home-Screen PWAs.
