...
description: "Character decomposition and radical analysis for CJK characters. Tools for identifying semantic components, radicals, and character structure - essential for…"
keywords: ["ecosystem scan", "library"]
...
|---|---|---|---|
| **Unihan (Unicode Han Database)** | Radical and stroke-count metadata that has to stay correct as Unicode keeps adding characters. | The authoritative radical-stroke source and the only entry here with institutional backing. kRSUnicode gives a Kangxi radical plus residual stroke count for every encoded CJK ideograph. Actively maintained — unicode-org/unihan-database last committed 2026-07-24. | 0.0.2 · 2016-04-13 |
| **BabelStone IDS** | Current IDS coverage in a single UTF-8 text file, including characters encoded after 2019. | Better maintained than the survey's primary pick. Its own page states the current version tracks Unicode 16.0 (September 2024) with 97,680 entries, against CJKVI-IDS's last commit in 2019. The survey listed its maintenance as Unknown; it is not. | — |
| **CJKVI-IDS** | Structural decomposition of characters encoded up to about 2019, plus the Waseikanji and CDP side datasets nothing else carries. | The most comprehensive open IDS collection, but frozen — last commit 2019-02-06, last release February 2018. Still usable; pin a commit and expect no coverage of characters encoded since, including Ext I and Ext J. | — |
| **hanzipy** | Learner-facing tools wanting decomposition plus CC-CEDICT definitions from one dependency. | The only pip-installable decomposer here that runs on Python 3 and is still being touched — MIT, repo commits through 2025-08-12, though PyPI has sat at 1.0.4 since 2023-09-17. Its decomposition data is the Gavin Grover cjk-decomp set, not IDS, so results differ from an IDS-based pipeline. | 1.0.4 · 2023-09-17 |
| **makemeahanzi** | Mnemonics, etymology-type classification and stroke graphics for common characters. | Active (last commit 2026-03-08) and the best etymology source, but not permissively licensed as the survey claimed: dictionary.txt is LGPL-3.0-or-later and graphics.txt is under the Arphic Public License. Verified content is 9,574 entries, 9,033 carrying an etymology type, 6,966 of them pictophonetic. | — |
| **cjk-decomp** | Permissively licensed decomposition data where GPL or LGPL cannot be accepted. | Archived on GitHub since 2018-06-09 and never coming back, but Apache-2.0 and still the substrate under hanzipy. Safe to vendor when copyleft is a blocker. | — |
| **cjklib** | Nothing new. Extract its SQLite database if you want the data, and skip the library. | Avoid. Last release 0.3.2 on 2012-05-22, and Python 2 only — cjklib/util.py does not parse under Python 3. Licensed LGPL, not the BSD-like the S2 table states. Its data lives on inside makemeahanzi. | 0.3.2 · 2012-05-22 |
| **cjklib3** | not recommended | Not the rescue the survey hoped for. The Python 3 fork stopped at 0.0.3 on 2021-03-31, has one GitHub star, and has had no commit since. S4's suggestion to revisit it 'if the community adopts it' is settled: it did not. | 0.0.3 · 2021-03-31 |
| **pypinyin** | Pinyin, Zhuyin and heteronym handling alongside a decomposition data source. | Healthy and by far the most used package here — 2,001,941 PyPI downloads in the last month, repo active into 2026, latest release 0.55.0 (2025-07-20). It converts readings and does no structural decomposition, so pair it rather than substitute it. | 0.55.0 · 2025-07-20 |
| **dragonmapper** | Converting between romanization systems when decomposition is handled elsewhere. | Maintained but adjacent: transcription conversion between characters, Pinyin, Zhuyin and IPA, plus traditional/simplified identification. Latest release 0.3.0 (2024-11-19), Python 3.9+. No decomposition of any kind. | 0.3.0 · 2024-11-19 |
...
- **No maintained Python library does comprehensive decomposition, so the survey's data-first recommendation holds** — cjklib is Python 2 and last shipped in 2012; its cjklib3 fork died in 2021 at one star. hanzipy is the only maintained decomposer on PyPI and it wraps a single fixed dataset. Parsing IDS and Unihan text directly remains the lower-risk path.
- **IDS is no longer a 12-operator set — Unicode 15.1 added four more, and the survey missed it** — U+2FFC..U+2FFF (SURROUND FROM RIGHT, SURROUND FROM LOWER RIGHT, HORIZONTAL REFLECTION, ROTATION) joined the original U+2FF0..U+2FFB in 2023. A parser hardcoding the 12 operators from Unicode 3.0 will mis-read any data using the new four. S4's claim of no operator change in 25+ years was already false when written.
- **The survey's primary IDS recommendation has been frozen since 2019, and its dismissed alternative has not** — cjkvi-ids last committed 2019-02-06. BabelStone IDS states it tracks Unicode 16.0 with 97,680 entries. Unicode has since added Ext I (622 characters, 15.1) and Ext J (4,298 characters, 17.0), so a 2019 dataset carries structural coverage gaps rather than fixable bugs.
- **Licensing, not capability, is the binding constraint on the good data** — cjkvi-ids is GPLv2 except ids-ext-cdef.txt; makemeahanzi's dictionary.txt is LGPL-3.0-or-later with Arphic-licensed graphics; cjklib is LGPL. cjk-decomp (Apache-2.0) is the only permissive decomposition set in the survey, and it has been archived since 2018.
- **Radical data and structural decomposition are different products and most projects need both** — Unihan's kRSUnicode gives one radical plus a residual stroke count and never answers which components build a character. IDS answers that and carries no radical index. Neither source substitutes for the other, which is why the survey's stack is layered rather than a single pick.
...
### 1. CJKVI-IDS (widely used; no longer maintained)

> **Correction, 2026-08-25 (re-4ou).** Headed "Primary Recommendation" with
> "ongoing development". Verified: last push **2023-02-24**. It is frozen, and it
> predates Ext-I (Unicode 15.1) and Ext-J (17.0). Still the most widely used IDS
> dataset and fine for characters it covers — but choose it knowing it has
> stopped tracking Unicode, and see BabelStone IDS below, which tracks 16.0.

**Repository**: [cjkvi/cjkvi-ids on GitHub](https://github.com/cjkvi/cjkvi-ids)
**Last Release**: February 20, 2018 (v18.02.20)
**Last push**: 2023-02-24 (verified 2026-08-25)
...
### CJKVI-IDS: Frozen, Not Merely Release-Shy

> **Correction, 2026-08-25 (re-4ou).** This section read "Active Development,
> Stale Releases" and explained the gap as rolling updates. Verified against the
> GitHub API: `cjkvi/cjkvi-ids` was **last pushed 2023-02-24** — three and a half
> years ago — with 527 stars and not archived. The repository is not receiving
> ongoing activity, so the "rolling updates" explanation does not hold. (Direct
> commit-history calls were rate-limited at the time of checking; the push date
> comes from the repository search API and is the verified figure.) `>`
> **BabelStone IDS is the better-maintained source** and this survey rated its
> maintenance "Unknown". babelstone.co.uk states it tracks Unicode 16.0
> (September 2024) with 97,680 entries. Its license terms are not stated on the
> page, which is a real reason for caution — but not the reason given here.

**Last Release**: February 2018 (v18.02.20)
**Last push**: 2023-02-24 (verified)
**What the gap actually means**: the data has stopped tracking new Unicode
versions. Ext-I shipped in Unicode 15.1 and Ext-J in 17.0; a source frozen in
2023 does not cover them.
