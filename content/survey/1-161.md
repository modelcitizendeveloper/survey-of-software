...
| **cjklib** | Nothing new. Extract its SQLite database if you want the data, and skip the library. | Avoid. Last release 0.3.2 on 2012-05-22, and Python 2 only — cjklib/util.py does not parse under Python 3. Licensed LGPL, not the BSD-like the S2 table states. Its data lives on inside makemeahanzi. |
| **cjklib3** | not recommended | Not the rescue the survey hoped for. The Python 3 fork stopped at 0.0.3 on 2021-03-31, has one GitHub star, and has had no commit since. S4's suggestion to revisit it 'if the community adopts it' is settled: it did not. |
| **pypinyin** | Pinyin, Zhuyin and heteronym handling alongside a decomposition data source. | Healthy and by far the most used package here — 2,001,941 PyPI downloads in the last month, repo active into 2026, latest release 0.55.0 (2025-07-20). It converts readings and does no structural decomposition, so pair it rather than substitute it. |
| **dragonmapper** | Converting between romanization systems when decomposition is handled elsewhere. | Maintained but adjacent: transcription conversion between characters, Pinyin, Zhuyin and IPA, plus traditional/simplified identification. Latest release 0.3.0 (2024-11-19), Python 3.9+. No decomposition of any kind. |
