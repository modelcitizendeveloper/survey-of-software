---
title: "1.185 Query Builder Libraries"
description: "Survey of SQL query builder libraries: Knex.js, Kysely, Drizzle (TypeScript/Node.js), jOOQ (Java), SQLAlchemy Core (Python), and Squirrel (Go). Covers type…"
keywords: ["type safety", "code gen", "migrations", "edge ready", "Knex.js", "Kysely", "Drizzle", "jOOQ", "SQLAlchemy Core", "Squirrel", "QueryDSL", "Arel"]
...
- **Query builders are a middle tier, not an ORM alternative: above raw SQL strings, below object-relational mapping** — You keep thinking in tables, rows and columns while gaining composition and dialect portability. If you want objects and class hierarchies, this is the wrong category.
- **In TypeScript the choice is between two new libraries, and Knex is not one of them for new work** — S4 is direct: 'Do not choose Knex.js unless you have an existing codebase to maintain.' Drizzle for schema management and edge runtimes, Kysely for a leaner dependency with schema managed separately.
- **Go is the one ecosystem where the answer is usually both** — sqlc for the roughly 80% of queries that are static and want maximum type safety, Squirrel for the 20% that are genuinely dynamic. Most production services have both.
- **Python has no meaningful alternative to SQLAlchemy Core** — S4 calls it the default 'without meaningful alternatives' and advises learning it properly, since the investment carries across all Python database work.
- **Staying on Knex is defensible; the trigger to move is active schema development** — For a stable schema with good test coverage, migration is not worth it. For complex queries under active schema change, the one-time cost to Kysely buys back runtime errors.
