...
|---|---|---|
| **Vite** | Native ESM dev server; UNIFIED Rolldown (Rust) bundler as of Vite 8 (was esbuild-dev/Rollup-prod) | Modern web apps (SPA/MPA), React/Vue/Svelte projects; also the engine under Slidev (1.110.6), Vitest, Storybook |
| **esbuild** | Go-based bundler, extreme compilation speed | Library builds and CI speed — but prefer it VIA Vite; S4 scores direct use 4/10 on bus factor |
| **Rollup** | ES module-native bundler, library-focused | JavaScript libraries, multiple output formats (ESM/CJS/UMD) |
| **Parcel** | Zero-config bundler, convention over configuration | Throwaway prototypes and teaching — NOT for anything you will maintain (S4 scores its viability 2/10) |
| **Turbopack** | Rust-based bundler, Next.js-specific (currently) | Next.js 15+ applications only |
| **Rolldown** | Rust bundler, drop-in Rollup replacement; now the unified bundler inside Vite 8 | Vite 8 apps (default); Rollup migrations wanting Rust speed |
| **Webpack** | JavaScript bundler; loader + plugin architecture, maximum configurability | Existing large codebases and edge cases needing the mature plugin ecosystem; not greenfield projects |
