...
description: "Chinese text has no word boundaries - word segmentation is the fundamental problem. Survey of Jieba (most popular), CKIP (traditional Chinese), pkuseg…"
keywords: ["ckip", "jieba", "ltp", "pkuseg"]
...
|---|---|---|---|
| **Jieba** | Rapid prototyping, real-time | Fast (400KB/s) | 0.42.1 · 2020-01-20 |
| **CKIP** | Taiwan/HK text, research | Moderate | — |
| **PKUSeg** | Medicine, social media, batch | Slow | 0.0.25 · 2020-06-27 |
| **LTP** | Multi-task NLP pipelines | Variable (39-21K sent/s) | 4.2.14 · 2024-06-08 |
...
- **Four libraries, four distinct jobs — the choice is made by script and domain, not by ranking** — Jieba for speed and prototyping, CKIP for Traditional Chinese, PKUSeg for domain-specific text, LTP when you need a full NLP pipeline rather than segmentation alone.
- **Traditional Chinese is a hard fork in this category, and CKIP is the answer to it** — It reaches the highest accuracy in the survey at 97.33% F1. A Simplified-Chinese segmenter is not a partial answer for Traditional text; it is the wrong tool.
- **License is where CKIP costs you, and it is the lowest score on the board** — CKIP scores 2/5 on license against 5/5 for Jieba and PKUSeg. Highest accuracy and most restrictive terms sit on the same library.
- **LTP has the strongest institutional position and the highest bus factor** — 4.45/5 overall, top marks on maintenance, institution and bus factor. Where the concern is a system still working in five years rather than this quarter's accuracy, it is the safest entry.
- **The upgrade path is prototype then specialize, and it is cheap** — The survey's own route: start on Jieba, move to PKUSeg when accuracy starts to matter, and only then consider whether the domain or the script forces a different tool.
...
- You're processing `>50`M segments/month (cost per query drops with vendor volume discounts)
