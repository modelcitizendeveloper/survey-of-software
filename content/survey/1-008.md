...
|---|---|---|
| **tslearn** | DTW + Shapelets + ML | MONITOR — capable across DTW, shapelets and ML, but rests on a smaller maintainer base |
| **STUMPY** | Matrix Profile for pattern discovery | SAFE for production — the only matrix-profile implementation here, on a UC Riverside academic foundation with no commercial competitor |
| **sktime** | Unified ML framework | SAFE for production — Turing Institute + NumFOCUS backing, the widest institutional base in the set |
| **tsfresh** | Automatic feature extraction | SAFE for production — the only broad feature-extraction option, commercially backed by Blue Yonder |
| **dtaidistance** | Fast DTW distances | MONITOR — rated excellent at DTW distance and the only one so rated; smaller maintainer base |
| **pyts** | Imaging + Transformations | AVOID for production — the weakest maintenance in the set |
