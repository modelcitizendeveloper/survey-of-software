...
description: "Time series search: STUMPY, tslearn, sktime, dtaidistance, tsfresh and pyts — DTW, shapelets and matrix profile for finding past patterns, not forecasting."
keywords: ["focus", "tslearn", "STUMPY", "sktime", "tsfresh", "dtaidistance", "pyts"]
...
Time series search: STUMPY, tslearn, sktime, dtaidistance, tsfresh and pyts — DTW, shapelets and matrix profile for finding past patterns, not forecasting.
...
|---|---|---|---|
| **tslearn** | DTW + Shapelets + ML | MONITOR — capable across DTW, shapelets and ML, but rests on a smaller maintainer base | 0.9.0 · 2026-07-02 |
| **STUMPY** | Matrix Profile for pattern discovery | SAFE for production — the only matrix-profile implementation here, on a UC Riverside academic foundation with no commercial competitor | 1.14.1 · 2026-02-08 |
| **sktime** | Unified ML framework | SAFE for production — Turing Institute + NumFOCUS backing, the widest institutional base in the set | 1.1.0 · 2026-07-28 |
| **tsfresh** | Automatic feature extraction | SAFE for production — the only broad feature-extraction option, commercially backed by Blue Yonder | 0.21.2 · 2026-05-31 |
| **dtaidistance** | Fast DTW distances | MONITOR — rated excellent at DTW distance and the only one so rated; smaller maintainer base | 2.5.0 · 2026-09-08 |
| **pyts** | Imaging + Transformations | AVOID for production — the weakest maintenance in the set | 0.13.0 · 2023-06-18 |
...
- **These libraries do not compete — the capability matrix is mostly empty cells** — STUMPY is the only one doing matrix profile, tsfresh the only one doing broad feature extraction, dtaidistance the only one rated excellent at DTW distance. Most pairs have no overlap to compare.
- **S4 sorts the six into three safe, two to monitor and one to avoid in production** — Safe: sktime, STUMPY, tsfresh. Monitor: tslearn, dtaidistance. Avoid for production: pyts, whose maintenance is the weakest in the set.
- **Institutional backing is what separates the safe three** — sktime has the Turing Institute and NumFOCUS, STUMPY an academic foundation at UC Riverside with no commercial competitor, tsfresh commercial backing from Blue Yonder. The monitored two rest on smaller maintainer bases.
- **Only STUMPY does streaming and GPU** — If the requirement is real-time anomaly detection rather than batch analysis, the field narrows to one library before any other criterion applies.
- **The strategic move is an abstraction layer around the tactical picks, not a single-library standard** — S4's guidance for tslearn is explicit — use it, but keep the migration path to sktime open.
...
The key differentiator is not "which library is best" but **"which library fits your search pattern and scale requirements."**
