...
description: "RAG pipelines: LlamaParse, Unstructured, Docling, Reducto and PyPDF for parsing, with recursive, structure-aware and semantic chunking strategies compared."
keywords: ["LlamaParse", "PyPDF / PyPDFLoader", "Unstructured", "Docling", "Reducto", "RecursiveCharacterTextSplitter", "LangChain", "Structure-Aware Chunking", "Semantic Chunking", "Parent-Child", "Page-Level Chunking", "Fixed-Size Chunking", "Hybrid Search", "Cross-Encoder Reranking", "Dense Retrieval Only", "BM25 Only", "llamaindex", "complementary usage", "rag focused", "orchestration", "both needs"]
...
RAG pipelines: LlamaParse, Unstructured, Docling, Reducto and PyPDF for parsing, with recursive, structure-aware and semantic chunking strategies compared.
...
|---|---|---|---|
| **LlamaIndex** | Document-heavy RAG where retrieval quality is the product | Purpose-built for RAG rather than adapted to it — 160+ data connectors via LlamaHub and NodeParsers that do the chunking work the survey rates as the single biggest lever. The 2025 benchmarks it cites (35% retrieval accuracy, 40% speed) are the survey's own figures and were not re-measured here. v0.14.24 (2026-08-19). | 0.12.1 · 2025-12-02 |
| **LangChain** | Multi-step orchestration around the retrieval, not the retrieval itself | General-purpose LLM orchestration. Its RecursiveCharacterTextSplitter is the baseline this survey recommends starting from, but the framework's strength is workflow rather than ingestion. v1.3.17 (2026-08-25). | 1.4.0 · 2026-09-03 |
| **Haystack** | Pipeline-shaped RAG with explicit components | The third option the S1 pass profiles. v3.1.0 (2026-08-25) — actively released alongside the other two. | 3.1.1 · 2026-09-08 |
| **LlamaIndex + LangChain together** | Teams that need both good retrieval and real orchestration | The survey's own recommended pattern rather than a fourth product: LlamaIndex for ingestion and retrieval, LangChain for orchestration and agents. They are not competitors in the way the framework question implies. | 0.12.1 · 2025-12-02 |
...
- chunking impact: ~60% of RAG accuracy (research finding)
- hybrid vs dense: 40-50% precision improvement
- reranking improvement: Up to 48% quality, 25% cost reduction
- semantic vs recursive: +2-3% recall
- llamaparse speed: ~6 seconds per document
- llamaindex vs langchain retrieval: 35% better accuracy, 40% faster
...
> **🔄 2026 Refresh (verified 2026-05-31):** **LlamaParse v2** launched **2025-12-18** with simplified **tier-based** config — **Fast / Cost-Effective / Agentic / Agentic Plus** (≈1 / 3 / 10 / 45 credits/page), automatic model routing, and up to ~50% lower cost vs. the old multi-mode setup. **Cost-Effective** is the default for most PDFs; **Fast** does not emit markdown. Verify exact credit pricing against the live pricing page before quoting figures. [LlamaParse v2 blog](https://www.llamaindex.ai/blog/introducing-llamaparse-v2-simpler-better-cheaper)

> **🔄 Refresh (2026-07, verified 2026-07-13):** Two additions since the May pass: (1) **LiteParse** — LlamaIndex's **open-source, local-first** document parser (CLI + TypeScript-native, released March 2026), a zero-cost "fast-mode" counterpart to the managed LlamaParse for agent pipelines where speed, privacy, or local execution matter; it preserves spatial layout and supports local OCR. (2) **LlamaParse Agentic Plus** now emits **bounding-box citations** — precise visual grounding that tracks the exact source location of extracted content (LaTeX, handwriting, infographics) for audit-ready workflows. Managed LlamaParse and open-source LiteParse sit at opposite ends of the same spectrum rather than competing. [LiteParse blog](https://www.llamaindex.ai/blog/liteparse-local-document-parsing-for-ai-agents)
...
> **🔄 Refresh (2026-07, verified 2026-07-13):** LlamaIndex has repositioned from a "data/RAG framework" toward an **agentic document-workflow stack**. **Workflows 1.0** (2026, Python + TypeScript) is a lightweight **event-driven agentic framework** — multi-step orchestration with agent + tool support, deployable as production microservices, with retrieval now one capability inside the agent loop rather than the headline. The project's own framing is now "AI Agents for Document OCR + Workflows." This does not change the S1 verdict (still a top-tier RAG option) but reframes what the library *is*; the pure-"index" characterization below predates the shift.
...
- A ~$1.25B private valuation is a durability signal in one direction and a risk
  in the other: it buys runway, and it sets a monetisation expectation that
  usually arrives as license or pricing changes.
...
**Strengths:**
1. ✅ **Longest runway:** $260M raised. This makes the project unlikely to
   disappear for lack of money. It says nothing about whether the API is good.
...
> **🔄 Refresh (2026-07, verified 2026-07-13):** *Category-level signal.* Through 2026 the "heavyweight RAG framework" category is under pressure — **agent SDKs, the Model Context Protocol (MCP), and coding agents** that generate bespoke pipelines on demand are absorbing use cases that previously justified a full framework (a shift LlamaIndex's own founder has acknowledged). This raises the strategic-viability question for *every* tool in this survey, not just LlamaIndex. LlamaIndex's response — **Workflows 1.0** (event-driven agent framework) plus **LiteParse** (OSS local parser) — is a credible pivot toward agentic document workflows, so its own outlook stays **Low-Medium** risk; but the broader recommendation should increasingly weigh "framework vs. agent-SDK + MCP tools" as a live build-vs-adopt decision. See 1.201 (agent frameworks), 2.074 (MCP).
