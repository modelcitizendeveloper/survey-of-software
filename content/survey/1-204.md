...
|---|---|---|
| **LlamaIndex** | Document-heavy RAG where retrieval quality is the product | Purpose-built for RAG rather than adapted to it — 160+ data connectors via LlamaHub and NodeParsers that do the chunking work the survey rates as the single biggest lever. The 2025 benchmarks it cites (35% retrieval accuracy, 40% speed) are the survey's own figures and were not re-measured here. v0.14.24 (2026-08-19). |
| **LangChain** | Multi-step orchestration around the retrieval, not the retrieval itself | General-purpose LLM orchestration. Its RecursiveCharacterTextSplitter is the baseline this survey recommends starting from, but the framework's strength is workflow rather than ingestion. v1.3.17 (2026-08-25). |
| **Haystack** | Pipeline-shaped RAG with explicit components | The third option the S1 pass profiles. v3.1.0 (2026-08-25) — actively released alongside the other two. |
| **LlamaIndex + LangChain together** | Teams that need both good retrieval and real orchestration | The survey's own recommended pattern rather than a fourth product: LlamaIndex for ingestion and retrieval, LangChain for orchestration and agents. They are not competitors in the way the framework question implies. |
