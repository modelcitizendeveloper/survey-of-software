...
| **RQ (Redis Queue)** | Minimal, Redis-only, and the best developer experience in the category. The community's answer for straightforward queuing at moderate scale. | 2.11.0 · 2026-08-17 |
| **Dramatiq** | The modern reply to Celery — actor-based, strongly typed, with error handling treated as a first-class design concern rather than configuration. | 2.2.0 · 2026-06-17 |
| **Huey** | The lightweight end: SQLite or Redis backing and near-zero operational surface. Right when you want background jobs without adding infrastructure. | 3.3.4 · 2026-08-05 |
| **TaskiQ** | The async-native newcomer, built for asyncio codebases where Celery's threading model is the friction. | 0.12.5 · 2026-08-21 |
