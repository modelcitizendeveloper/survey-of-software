...
- **Celery** — The industry standard and the default for serious distributed processing — the richest ecosystem, chains and chords, multiple brokers. Its complexity is the reason every other entry here exists.
- **RQ (Redis Queue)** — Minimal, Redis-only, and the best developer experience in the category. The community's answer for straightforward queuing at moderate scale.
- **Dramatiq** — The modern reply to Celery — actor-based, strongly typed, with error handling treated as a first-class design concern rather than configuration.
- **Huey** — The lightweight end: SQLite or Redis backing and near-zero operational surface. Right when you want background jobs without adding infrastructure.
- **TaskiQ** — The async-native newcomer, built for asyncio codebases where Celery's threading model is the friction.
