---
title: "1.252 Python Code Parsing & AST Libraries"
description: "Python code parsing and rewriting: ast, LibCST, parso, RedBaron, rope and Bowler — which of them preserve formatting and comments through a transform."
keywords: ["stars", "ast", "maintainer", "libcst", "redbaron", "rope", "parso", "bowler", "Find insertion point", "Add field", "Update signature", "Preserve style", "Validate syntax"]
aliases: ["/survey/1-104-1/"]
...
# 1.252 Python Code Parsing & AST Libraries

Python code parsing and rewriting: ast, LibCST, parso, RedBaron, rope and Bowler — which of them preserve formatting and comments through a transform.
...
|---|---|---|---|
| **LibCST** | Changing code and handing it back to the humans who wrote it | The survey's pick, and the reason is formatting: a concrete syntax tree keeps comments, whitespace and quote style, so a rewrite produces a diff a reviewer will accept. Maintained by Meta. v1.9.0 (2026-07-29), Python 3.9+. | 1.9.0 · 2026-07-29 |
| **ast (standard library)** | Reading code — linting, analysis, anything that does not write back | Zero dependencies and always present, which makes it the right default for analysis. It discards formatting and comments, so any tool that regenerates source from it destroys the original layout. That single property decides when to reach past it. | — |
| **rope** | Project-wide refactorings — rename, extract, move across files | Operates on a project rather than a file, which is what refactoring actually requires. LGPL-3.0-or-later, which is a real constraint for some commercial use. v1.14.0 (2025-07-12). | 1.14.0 · 2025-07-12 |
| **RedBaron** | Nothing new — it is unmaintained | AVOID. Last release 0.9.2 on 2019-03-17 — over seven years, verified on PyPI. Its simple modification API was the appeal; LibCST covers that ground and is maintained. | 0.9.2 · 2019-03-17 |
| **parso** | Parsing code that does not fully parse — editors and tooling | Error-recovering parser used by Jedi. Reach for it when the input is half-written rather than valid, which is the case an AST-based tool handles worst. v0.8.7 (2026-05-01), MIT. | 0.8.7 · 2026-05-01 |
...
- LibCST (9.5/10) - Instagram/Meta, 1.8k stars, 3.1M weekly downloads
- ast (6.5/10) - Python stdlib, zero dependencies, formatting loss
- rope (7.5/10) - 2.1k stars, comprehensive refactoring, LGPL
...
**Research ID**: 1.252 - Python AST/Code Parsing Libraries
