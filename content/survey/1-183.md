...
description: "Survey of database testing libraries and frameworks: testcontainers, pytest-postgresql, factory_boy, faker, DBUnit, sqlmock, and pgTAP. Covers test isolation…"
keywords: ["per test speed", "sqlmock", "factory_boy build", "startup overhead", "infrastructure required", "factory_boy create", "pytest-postgresql", "pgTAP", "Testcontainers", "DBUnit"]
...
| Library | Per Test Speed | Startup Overhead | Latest release |
|---|---|---|---|
| **sqlmock (Go)** | < 1ms | — | — |
| **factory_boy build()** | < 1ms | — | 3.3.3 · 2025-02-03 |
| **DatabaseCleaner :transaction** | 1-5ms | DB startup | — |
| **factory_boy create()** | 5-50ms | None + DB | 3.3.3 · 2025-02-03 |
| **pytest-postgresql** | 1-3s startup then < 5ms | 1-3s | 9.1.0 · 2026-09-04 |
| **pgTAP** | 10-100ms | DB startup | — |
| **Testcontainers (warm)** | 5-15s startup then 1-50ms | 5-15s | 4.15.0 · 2026-07-24 |
| **DatabaseCleaner :truncation** | 50-500ms | DB startup | — |
| **DBUnit** | 100ms-1s | DB startup | 0.2 · 2020-04-15 |
...
- **Testcontainers is the category's settled answer for integration tests, and Docker in CI is what made it possible** — The survey records the consensus as 'the gold standard for database integration testing' and describes it as the turning point when database tests stopped being flaky.
- **The real axis is not which library but real database versus mock, and the cost is measured per test** — sqlmock and factory_boy build() answer in under a millisecond and touch no database; Testcontainers costs 5-15 seconds of startup and then 1-50ms a test. Pick per test layer, not per project.
- **Every language ecosystem has a settled stack, and they are not interchangeable** — Python: factory_boy + Faker + Testcontainers or pytest-postgresql. Go: sqlmock for unit, Testcontainers-go for integration. Rails: DatabaseCleaner + FactoryBot + Faker.
- **Connect the migration tool to the container setup, or the tests drift from production schema** — S4 makes this step three of greenfield setup: migrations must run before integration tests, every time.
- **Do not defer test infrastructure until the project is bigger** — The survey is explicit: 'by the time the project is bigger, test debt is embedded'. The greenfield configuration cost is 2-4 hours; the legacy remediation it describes runs 9 months in phases.
