...
description: "Matrix decomposition across languages: NumPy/SciPy, Eigen, Julia LinearAlgebra and Rust — SVD, eigenvalue and QR, and the speed-versus-productivity trade."
keywords: ["python", "cpp", "julia", "rust", "low level", "python data science", "python gpu", "cpp systems", "cpp matlab transition", "julia scientific", "rust memory safety", "maximum performance", "sparse matrices", "distributed hpc", "NumPy GitHub Repository", "SciPy GitHub Repository", "CuPy GitHub Repository", "Eigen GitLab Repository", "Armadillo GitLab Repository", "nalgebra GitHub Repository", "NumPy Linear Algebra Documentation", "SciPy Linear Algebra Documentation", "CuPy Linear Algebra Documentation", "Eigen Dense Matrix Documentation", "Armadillo Documentation"]
...
Matrix decomposition across languages: NumPy/SciPy, Eigen, Julia LinearAlgebra and Rust — SVD, eigenvalue and QR, and the speed-versus-productivity trade.
...
<figure class="sos-fig"><figcaption>svd 5000x5000 matrix <span>lower is faster</span></figcaption><svg viewBox="0 0 720 192" role="img" width="100%" height="192" aria-label="svd 5000x5000 matrix, compared"><title>cusolver gpu: 0.05 seconds</title><text x="158" y="17.68" class="sos-bl" text-anchor="end">cusolver gpu</text><rect x="168" y="0" width="9.7" height="26" rx="4" class="sos-bar"/><text x="185.7" y="17.68" class="sos-bv">0.05 seconds</text><title>cupy gpu rtx3090: 0.08 seconds</title><text x="158" y="49.68" class="sos-bl" text-anchor="end">cupy gpu rtx3090</text><rect x="168" y="32" width="15.6" height="26" rx="4" class="sos-bar"/><text x="191.6" y="49.68" class="sos-bv">0.08 seconds</text><title>numpy mkl: 1.2 seconds</title><text x="158" y="81.68" class="sos-bl" text-anchor="end">numpy mkl</text><rect x="168" y="64" width="233.3" height="26" rx="4" class="sos-bar"/><text x="409.3" y="81.68" class="sos-bv">1.2 seconds</text><title>julia mkl: 1.3 seconds</title><text x="158" y="113.68" class="sos-bl" text-anchor="end">julia mkl</text><rect x="168" y="96" width="252.7" height="26" rx="4" class="sos-bar"/><text x="428.7" y="113.68" class="sos-bv">1.3 seconds</text><title>eigen compiled o3: 2.0 seconds</title><text x="158" y="145.68" class="sos-bl" text-anchor="end">eigen compiled o3</text><rect x="168" y="128" width="388.8" height="26" rx="4" class="sos-bar"/><text x="564.8" y="145.68" class="sos-bv">2.0 seconds</text><title>numpy openblas: 2.5 seconds</title><text x="158" y="177.68" class="sos-bl" text-anchor="end">numpy openblas</text><rect x="168" y="160" width="486.0" height="26" rx="4" class="sos-bar"/><text x="662.0" y="177.68" class="sos-bv">2.5 seconds</text></svg></figure>


| Library | Group | Best for |
|---|---|---|
| **NumPy** | python | numpy.linalg - standard Python LA, 38.7K stars, BLAS/LAPACK backend |
| **SciPy** | python | scipy.linalg + sparse - comprehensive, 13.2K stars, includes advanced decompositions |
| **CuPy** | python | GPU-accelerated NumPy replacement, 8.6K stars, cuBLAS/cuSOLVER backend |
| **Eigen** | cpp | header-only C++ template library, 4.2K stars, hand-optimized + optional BLAS |
| **Armadillo** | cpp | MATLAB-like C++ API, 758 stars, LAPACK backend |
| **LinearAlgebra** | julia | stdlib, built-in, OpenBLAS/MKL backend, excellent performance |
| **nalgebra** | rust | pure Rust with optional BLAS, 3.9K stars, memory-safe |
| **LAPACK** | low level | Fortran gold standard, 40+ years, industry foundation |
| **Intel MKL** | low level | proprietary optimized BLAS/LAPACK, fastest on Intel CPUs |
| **cuSOLVER** | low level | NVIDIA GPU solver library, 10-100× speedup for large matrices |


**What the research found**

- NumPy/SciPy are default for Python ecosystem (seamless ML framework integration)
- Eigen preferred for C++ (header-only, no dependencies, expression templates)
- Julia LinearAlgebra best performance-to-productivity ratio (near-C speed, Python-like syntax)
- GPU acceleration (CuPy/cuSOLVER) critical for matrices >10K×10K (10-100× speedup)
- LAPACK remains foundation (all major libraries wrap or reimplement it)
...
| When | Pick |
|---|---|
| python data science | NumPy/SciPy (ecosystem integration, 38.7K+13.2K stars) |
| python gpu | CuPy (10-100× speedup, NVIDIA GPUs, 8.6K stars) |
| cpp systems | Eigen (header-only, no dependencies, 4.2K stars) |
| cpp matlab transition | Armadillo (MATLAB-like syntax, 758 stars) |
| julia scientific | LinearAlgebra (stdlib, best performance + productivity) |
| rust memory safety | nalgebra (compile-time safety, 3.9K stars) |
| maximum performance | Intel MKL (CPU) or cuSOLVER (GPU) |
| sparse matrices | SciPy sparse.linalg or Eigen SparseCore |
| distributed hpc | ScaLAPACK or Elemental |


**performance benchmarks**

- note: Performance varies by CPU/GPU model, matrix structure, decomposition variant
