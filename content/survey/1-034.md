...
- **yagmail** — The two-or-three-line option, Gmail-oriented. Right for scripts, automation and prototypes; not a foundation for a product's transactional mail.
- **redmail** — The modern API in the category, with first-class Jinja2 templating. Reach for it when the email bodies are the complicated part.
- **django-anymail** — The anti-lock-in choice — one interface across 15+ providers, so switching ESP is a config change. The default for Django apps using a managed provider.
- **Flask-Mail** — Flask-native convenience wired into app config. Simpler than raw smtplib, less flexible than framework-agnostic options, and unusable outside Flask.
- **envelopes** — A middle ground with no framework dependency — less raw than smtplib, less opinionated than yagmail.
- **mailer (marrow.mailer)** — Selected for transport flexibility rather than API comfort: mail queues, local maildir for testing, custom delivery routing.
