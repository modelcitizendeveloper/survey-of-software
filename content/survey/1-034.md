---
title: "1.034 Email Sending Libraries"
description: "Python email sending: smtplib, yagmail, redmail, django-anymail and Flask-Mail — plus the DIY-versus-provider cost question that decides it first."
keywords: ["email libraries", "smtplib", "yagmail", "redmail", "red-mail", "django-anymail", "flask-mail", "envelopes", "marrow.mailer", "transactional email", "smtp", "sendgrid", "mailgun", "postmark", "esp", "email deliverability", "mime"]
...
# 1.034 Email Sending Libraries

Python email sending: smtplib, yagmail, redmail, django-anymail and Flask-Mail — plus the DIY-versus-provider cost question that decides it first.
...
|---|---|---|
| **smtplib + email.mime (standard library)** | Maximum control, zero dependencies, works anywhere Python does. The cost is verbosity: connection management, TLS and MIME construction are all yours. | — |
| **yagmail** | The two-or-three-line option, Gmail-oriented. Right for scripts, automation and prototypes; not a foundation for a product's transactional mail. | 0.16.0 · 2026-05-26 |
| **redmail** | The modern API in the category, with first-class Jinja2 templating. Reach for it when the email bodies are the complicated part. | 0.6.0 · 2023-02-25 |
| **django-anymail** | The anti-lock-in choice — one interface across 15+ providers, so switching ESP is a config change. The default for Django apps using a managed provider. | 15.2 · 2026-09-05 |
| **Flask-Mail** | Flask-native convenience wired into app config. Simpler than raw smtplib, less flexible than framework-agnostic options, and unusable outside Flask. | 0.10.0 · 2024-05-23 |
| **envelopes** | A middle ground with no framework dependency — less raw than smtplib, less opinionated than yagmail. | 0.4 · 2013-11-13 |
| **mailer (marrow.mailer)** | Selected for transport flexibility rather than API comfort: mail queues, local maildir for testing, custom delivery routing. | 4.0.3 · 2019-09-16 |
...
- The library choice is downstream of a bigger one: the survey puts managed providers at roughly 8-10x cheaper than self-hosted SMTP until about 100K emails/day, so most projects should pick an ESP first and a library second
- django-anymail's value is not features but portability — it makes the ESP decision reversible, which matters precisely because the ESP decision is the expensive one
- The framework extensions (Flask-Mail, django-anymail) are not competing with the general libraries; the framework you are already in usually settles it
- yagmail and redmail solve opposite ends of the same problem: minimum boilerplate versus serious templating
- Deliverability, not sending, is where high-volume email actually fails — no library choice compensates for missing infrastructure
