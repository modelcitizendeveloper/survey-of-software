---
title: "1.001 Advanced Sorting Libraries"
description: "Comprehensive analysis of advanced sorting algorithms and libraries in Python. Covers Timsort, NumPy sorting, radix/counting sorts, parallel sorting…"
keywords: ["timsort builtin", "numpy sorting", "radix counting sort", "parallel sorting", "external sorting", "sortedcontainers", "specialized algorithms", "memory efficient sorting", "algorithm complexity", "implementation patterns", "library", "performance benchmarks"]
...
# 1.001 Advanced Sorting Libraries
...
> **Quick guide**
>
> **Reach for `sorted()` first.** Under a million elements, mixed types, sorted once —
> it is fast enough and costs nothing to maintain. The exceptions are worth knowing
> because each one is large.
>
> - **All integers, held in memory** → `np.sort(kind='stable')`. Radix sort runs in
>   O(n) and skips the comparison floor entirely. ~8x faster (18ms vs 152ms at 1M).
> - **Items arrive over time and order must hold throughout** → `SortedContainers`.
>   It never re-sorts. ~180x faster past roughly 100 inserts (0.045s vs 8.2s at 10k).
> - **Only the top handful out of a large set** → `heapq.nlargest`. Keeps N and
>   discards the rest. ~43x faster (42ms vs 1,820ms for top 100 of 10M).
> - **Numeric work above ~1M rows, or DataFrames** → NumPy, or polars for multi-column
>   (9.3ms at 1M integers; ~11.7x faster than pandas on strings).
> - **Larger than memory** → external merge sort. Not slower — otherwise impossible.
>
> The pattern under all of it: how much you keep, how much you discard, and whether
> order has to hold while data is still arriving.

> [!TIP]
> **Try it: [Sortie](/workshop/sortie/)** — a working demo of this survey's findings.
> It measures, in your browser, the case where re-sorting on every insert loses to keeping a list ordered, then runs the real Python libraries via WebAssembly.

## At a glance

*Findings checked against this survey's current text on 2026-08-24.*

<div class="sos-tiles"><div class="sos-tile"><div class="sos-tile-h">10k incremental inserts</div><div class="sos-tile-n">182&times;</div><div class="sos-tile-s"><strong>sortedlist</strong> 0.045s &middot; repeated sort 8.234s</div></div><div class="sos-tile"><div class="sos-tile-h">top 100 from 10m</div><div class="sos-tile-n">43&times;</div><div class="sos-tile-s"><strong>heapq nlargest</strong> 42ms &middot; full sort 1,820ms</div></div><div class="sos-tile"><div class="sos-tile-h">100gb log file</div><div class="sos-tile-n">3&times;</div><div class="sos-tile-s"><strong>external merge ssd</strong> 60 minutes &middot; external merge hdd 180 minutes</div></div></div>

<figure class="sos-fig"><figcaption>1m integers <span>lower is faster</span></figcaption><svg viewBox="0 0 720 160" role="img" width="100%" height="160" aria-label="1m integers, compared"><title>polars: 9.3ms</title><text x="158" y="17.68" class="sos-bl" text-anchor="end">polars</text><rect x="168" y="0" width="29.7" height="26" rx="4" class="sos-bar"/><text x="205.7" y="17.68" class="sos-bv">9.3ms</text><title>numpy stable radix: 18ms</title><text x="158" y="49.68" class="sos-bl" text-anchor="end">numpy stable radix</text><rect x="168" y="32" width="57.6" height="26" rx="4" class="sos-bar"/><text x="233.6" y="49.68" class="sos-bv">18ms</text><title>numpy quicksort: 28ms</title><text x="158" y="81.68" class="sos-bl" text-anchor="end">numpy quicksort</text><rect x="168" y="64" width="89.5" height="26" rx="4" class="sos-bar"/><text x="265.5" y="81.68" class="sos-bv">28ms</text><title>pandas: 52ms</title><text x="158" y="113.68" class="sos-bl" text-anchor="end">pandas</text><rect x="168" y="96" width="166.3" height="26" rx="4" class="sos-bar"/><text x="342.3" y="113.68" class="sos-bv">52ms</text><title>builtin: 152ms</title><text x="158" y="145.68" class="sos-bl" text-anchor="end">builtin</text><rect x="168" y="128" width="486.0" height="26" rx="4" class="sos-bar"/><text x="662.0" y="145.68" class="sos-bv">152ms</text></svg></figure>


| Library | How it works | Best for | Latest release |
|---|---|---|---|
| **Built-in (sorted/list.sort)** | Timsort | General-purpose <1M elements | — |
| **NumPy** | Quicksort + radix sort (stable) | Numerical arrays, integers (O(n) radix) | 2.5.3 · 2026-09-06 |
| **SortedContainers** | B-tree variant | Incremental updates (182x speedup) | 2.4.0 · 2021-05-16 |
| **Pandas** | Timsort wrapper | DataFrame sorting (being replaced by Polars) | 3.0.5 · 2026-07-22 |
| **Polars** | Rust parallel sort | Large DataFrames (11.7x faster than Pandas) | 1.44.1 · 2026-09-02 |
| **heapq** | Binary heap | Top-K selection (O(n + k log k)) | — |
...
- **A single NaN silently corrupts heapq, bisect.insort and SortedList** — Every comparison against NaN is False, so no structure raises and all three are permanently wrong afterwards. Pushing [5,3,NaN,9,7,1] makes heapq report its minimum as 3 with 1.0 still in the array; bisect and SortedList both yield [3,5,nan,1,7,9]. Filter or reject NaN at ingest wherever floats come from division, parsing or sensors. Inf is safe and orders correctly.
- **Top-k with small k is a different problem from streaming inserts** — The 182x SortedContainers result is measured against re-sorting a list that retains everything, and assumes you want the whole ordering. If you only need the top handful, a bounded heap or bounded bisect list costs O(k) memory instead of O(n) and roughly 30x less time per insert. SortedList wins again the moment you need anything else — a median, rank queries, removals, expiry.
- **The 43x heapq.nlargest figure is batch-only** — It re-derives from a complete collection (42ms vs 1,820ms, top 100 of 10M). Under a correct-after-every-insert constraint the batch path does not exist, and the figure does not apply.
- **NumPy stable sort uses O(n) radix sort for integers - 8.4x faster than built-in** — Always use np.sort(kind='stable') for integer arrays
- **SortedContainers 182x faster than repeated list.sort() for incremental updates** — Crossover point at ~100 insertions; essential for leaderboards, streaming data
...
| Data size | Pick |
|---|---|
| small 100k | Built-in sort() - simple, fast enough (~10ms) |
| medium 1m | NumPy for numerical, built-in for objects |
| large 10m | NumPy + Polars for DataFrames |
| xlarge 100m | Polars with parallelization |
| data gt ram | External merge sort with optimal I/O |
...
| Access pattern | Pick |
|---|---|
| incremental updates | SortedContainers - 182x faster than re-sorting |
| batch processing | NumPy or Polars depending on data type |
| top k only | heapq.nlargest() or np.partition() - 18x faster |
| nearly sorted | Timsort - 10x adaptive speedup |

| Requirement | Pick |
|---|---|
| stability required | Timsort, NumPy stable, Polars |
| in place required | list.sort(), NumPy arr.sort() |
| minimal memory | Timsort (in-place), heapsort |
| fastest integers | NumPy stable sort (O(n) radix) |
| fastest general | Polars for DataFrames, NumPy for arrays |
...
## Default Choice
Use Python's built-in `sorted()` or `list.sort()` for general-purpose sorting (`<1`M elements).
...
- **Parallel sorting**: Only for `>5`M elements (diminishing returns)
...
- Only parallelize for `>5`M elements (severe diminishing returns on 8+ cores)
...
Default to built-in sort() for `<1`M elements.

## Future Outlook
- SIMD vectorization (AVX-512) already provides 10-17x speedup (NumPy 2023)
- GPU sorting viable for `>100`M elements but high complexity
