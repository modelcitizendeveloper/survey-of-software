...
description: "Threshold signatures and secret sharing: FROST implementations, tss-lib, cggmp21, shamir-mnemonic and PyCryptodome — splitting a key nobody holds whole."
keywords: ["PyCryptodome", "shamir-mnemonic", "ZF FROST", "PyFrost", "dfns/cggmp21", "bnb-chain/tss-lib", "givre"]
...
Threshold signatures and secret sharing: FROST implementations, tss-lib, cggmp21, shamir-mnemonic and PyCryptodome — splitting a key nobody holds whole.
...
|---|---|---|
| **PyCryptodome** | Shamir secret sharing in Python — splitting a key at rest, never signing with it | High viability (5yr): 50M+ PyPI downloads a month and a CVE response record, though solo-maintained (Legrandin). Its shares are UNVERIFIABLE — a corrupted share yields silently wrong output rather than an error — so adversarial setups need Feldman VSS on top, and no clean Python library provides it |
| **shamir-mnemonic** | SLIP-39 mnemonic shares — human-transcribable key backup | High viability (5yr). Trezor-sponsored as a core wallet product and inside their security review programme. Explicitly NOT for server-side use |
| **ZF FROST (frost-ed25519, frost-p256, frost-ristretto255)** | Threshold Schnorr in Rust, against the RFC 9591 standard | High viability (5yr). Zcash Foundation with multi-year commitment, NCC Group audit 2023 covering ed25519, p256, ristretto255 and ed448 — but the secp256k1-tr Taproot crate is OUTSIDE that audit scope |
| **dfns/cggmp21** | Threshold ECDSA in Rust — the best audited option in the survey | High viability (3-5yr). Kudelski Security audit with vulnerabilities disclosed and patched, MIT + Apache 2.0 so anyone can fork. Supports periodic key-share refresh where GG20 does not, which is the differentiator for keys held over years |
| **bnb-chain/tss-lib** | Threshold ECDSA in Go — battle-tested in Thorchain and bridge deployments | Moderate viability (3-5yr). NO public audit, and GG20 is aging with no proactive security. Single corporate sponsor: if Binance's priorities shift, maintenance slows |
| **givre (LFDT-Lockness)** | Threshold Schnorr in Rust where speed matters more than an audit trail | Faster than ZF FROST but NO public audit yet. S2's conclusion is that neither FROST implementation is universally dominant |
| **PyFrost (zellular-xyz)** | FROST from Python for non-adversarial research — the only Python option, and it ships the networking layer too | LOW-MEDIUM viability (1-3yr). NO AUDIT, which S4 calls the fundamental blocker, on a small single-project team, and it implements an ICE-FROST variant rather than vanilla RFC 9591. Real production use exists, so the risk is unknown rather than absent. A funded public audit would move it to 3-5yr |


**What the research found**

- Python has no production-grade threshold signing library. The Python gap is structural: threshold ECDSA and audited FROST are Rust/Go only. Python developers must use PyCryptodome for SSS (key splitting at rest), PyFrost for non-adversarial FROST research, or a managed service for production threshold signing.
