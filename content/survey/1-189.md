---
title: "1.189 Database Observability & Profiling"
description: "Survey of database observability and profiling tools: pg_stat_statements and pgBadger (PostgreSQL), pt-query-digest and PMM (MySQL/Percona), EXPLAIN ANALYZE…"
keywords: ["aggregated stats", "per query plan", "log based", "real time", "mysql", "postgresql", "self hosted", "pg_stat_statements", "pgBadger", "pt-query-digest", "EXPLAIN ANALYZE", "auto_explain", "pghero", "pganalyze", "PMM", "sqlfluff"]
...
| Library | Per Query Plan | Real Time |
|---|---|---|
| **pg_stat_statements** | No | Yes |
| **pgBadger** | No | No |
| **pt-query-digest** | No | Partial |
| **EXPLAIN ANALYZE** | Yes | Ad-hoc |
| **auto_explain** | Yes (logged) | No |
| **pghero** | No | Yes |
| **pganalyze** | Yes | Yes |
| **PMM** | Yes (QAN) | Yes |
| **sqlfluff** | No | — |
...
- **Load pg_stat_statements first — the survey calls it the single most impactful thing you can do for a PostgreSQL database** — Everything else in the category builds on it. A tool comparison is premature until it is on.
- **The complete PostgreSQL lifecycle is covered by four free tools, and they cover it between them** — pg_stat_statements for statistics, auto_explain and EXPLAIN ANALYZE for plans, pgBadger for batch reporting over slow logs. Minimal configuration, no license.
- **MySQL has an equivalent stack, not a lesser one** — pt-query-digest plus PMM is the MySQL answer to pgBadger plus pghero, and PMM brings a full open-source monitoring stack with it.
- **Paid tools buy a dashboard or index advice, not query visibility** — pghero wraps pg_stat_statements with a UI; pganalyze adds a SaaS collector and index recommendations. Both sit on top of data the free tools already expose.
- **The cheapest place to catch a bad query is before it ships** — sqlfluff lints SQL at write time, which is the only tool here that acts before a query reaches production.
