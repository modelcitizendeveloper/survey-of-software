...
|---|---|---|
| **Black** | Conservative teams that want one formatter, no linter, and the canonical style | Actively released and still the reference Python style, but Ruff now emits Black-compatible output far faster while also linting and sorting imports. Choose Black because you want the reference implementation itself, not because it is the lower-risk option. |
| **Ruff format** | Every new Python project, and any existing Black + isort + Flake8 stack | The Python default. Black-compatible formatting plus linting and import sorting in one Rust binary, at roughly 81M PyPI downloads a week against Black's 41M — about two to one, not a rout. Still pre-1.0 four years in, so pin the exact version in CI and pre-commit. |
| **Blue** | Nothing new; move existing users to Ruff with quote-style single and line-length 79 | Avoid. No PyPI release since 0.9.1 in August 2022 and no repository commit since February 2024 — unmaintained. Its entire differentiator, single quotes at 79 columns, is now two lines of Ruff configuration. |
| **autopep8** | Minimal-diff cleanup of a legacy codebase that cannot absorb a full reformat | Repairs PEP 8 violations rather than imposing one style, so it will not settle formatting arguments — which is what the rest of this category is for. Its remaining use is nudging legacy code without a wholesale reformat. |
| **YAPF** | A mandated house style that no opinionated formatter can express | Around fifty knobs, which is the opposite of what an opinionated formatter buys a team. Google's repository still takes commits but has not cut a release in over 21 months; treat it as maintenance-only. |
| **Prettier** | Polyglot front-end repos; anything where Markdown, YAML or broad language coverage matters | Still the JS/TS default by roughly ten to one on downloads, and the only tool here that formats Markdown and YAML — Biome still does not. A repo containing either keeps Prettier whatever the speed argument says. |
| **Biome** | JS/TS/JSON/CSS-only projects where formatter and linter speed is the pain point | The fast unified challenger — formatter and linter in one Rust binary, shipping roughly weekly, but around a tenth of Prettier's download volume. HTML, Vue, Svelte and Astro support landed after this survey was written and is experimental and opt-in; YAML and Markdown remain unsupported. |
| **dprint** | Rust-centric toolchains, or a repo where Prettier measurably takes ten seconds or more | Skip unless Prettier is a measured bottleneck. The Wasm plugin architecture is the draw; at roughly 264K weekly downloads against Prettier's 133M, the ecosystem around it is not. |
| **ESLint** | Framework-specific, type-aware or accessibility lint rules; anything leaning on the plugin ecosystem | A linter, not a formatter — run it alongside Prettier, or replace both with Biome. Still the JS/TS linting standard on volume, and the reason to keep it is the plugin ecosystem, which Biome does not match. On the rule count: eslint/eslint main carries 292 rule files in lib/rules, so the 400+ above is not a count of core rules — where that figure came from is not verified. |
