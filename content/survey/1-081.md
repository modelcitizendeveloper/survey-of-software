...
description: "Comprehensive research on convex hull algorithms and libraries across multiple programming languages. Covers algorithm theory (Graham Scan, QuickHull, Jarvis…"
keywords: ["primary", "secondary", "scipy.spatial.ConvexHull", "CGAL", "geo-rust", "quickhull3d", "Qhull"]
...
|---|---|---|
| **scipy.spatial.ConvexHull** | Python, and the default for a data-science or ML pipeline — NumPy, Pandas, Matplotlib and scikit-learn integration | LOW risk. 14.4k stars, 1,393+ contributors, 1.4M dependents; Qhull underneath and ~50% faster than pyhull |
| **CGAL** | C++ where correctness beats speed — exact computation, comprehensive geometry beyond hulls | LOW-MEDIUM risk. 30-year track record, 5.7k stars, released Jan 2026, 1M points in 1.63s (3D). GPL/commercial dual license, so a proprietary build means negotiating |
| **geo-rust** | Rust — near-native performance and the ecosystem's geometry crate | LOW-MEDIUM risk. 12M+ downloads, 90 versions, active development |
| **JTS Topology Suite** | Java, and comprehensive 2D geometry rather than hulls alone | Excellent maturity — Eclipse Foundation, 2.2k stars, v1.20.0 (Sep 2024). Graham Scan O(n log n): comprehensive, not fastest |
| **Boost.Geometry** | C++ projects already on Boost — template-based, strategy-pattern API | Proven and extensively versioned as part of Boost. Comprehensive at a measurable performance cost |
| **quickhull3d** | JavaScript 3D — the option Three.js incorporated | MEDIUM risk. 142 stars, 6,212 ops/sec at 100 points. Single-maintainer concern; for graphics work prefer the Three.js built-in |
| **convex-hull (mikolalysenko)** | JavaScript n-dimensional hulls | Concerning: eleven years old with no updates, though still 48k weekly downloads. The best JavaScript 2D option by default rather than by activity |
| **Qhull** | C/C++ where raw speed wins — 3M points in 4.7s in 2D — and the backend most other entries here wrap | MEDIUM-HIGH risk AS A DIRECT DEPENDENCY, and the survey's only avoid: dormant development, single-maintainer risk. Entirely fine reached through a wrapper such as scipy or CGAL |
| **VQhull** | Tracking where planar Quickhull performance is going | NOT A SHIPPING LIBRARY — a 2025 arXiv paper (2510.09417) on a parallel Quickhull variant, measured at 1.6-16x sequential and 1.5-11x parallel over Qhull. Watch it; do not depend on it |
| **convexhull-js** | Nothing new — superseded for JavaScript 2D | INACTIVE. v1.0.0 published ten years ago, 2,507 ops/sec at 100 points. Use convex-hull (mikolalysenko) instead |
| **hull.js** | Nothing in this category — it computes CONCAVE hulls, a different problem | Out of category and deprecated. Named here so a reader searching npm for "hull" does not mistake it for a convex-hull library |


**What the research found**

- **Language decides this, and every language has exactly one obvious answer** — Python scipy.spatial.ConvexHull, Java JTS, Rust geo, JavaScript 3D quickhull3d. The survey calls quickhull3d the only viable JS 3D option, so there is nothing to compare it against.
- **In C++ the choice is correctness against speed, and the survey refuses to collapse it** — CGAL gives exact computation and 1M points in 1.63s in 3D; Qhull gives 3M points in 4.7s in 2D. Degenerate and near-degenerate inputs are where the difference shows up.
- **Qhull is the engine under most of these and a poor direct dependency** — scipy and others wrap it. Depended on directly it is the survey's one avoid — dormant development and single-maintainer risk — which is a statement about the project, not the algorithm.
- **The best-used JavaScript option is eleven years old with no updates** — convex-hull (mikolalysenko) still draws 48k weekly downloads. It is the default by adoption rather than by activity, and the survey says so.
- **The fastest thing measured here is not a library you can install** — VQhull is a 2025 paper on a parallel Quickhull variant, 1.6-16x over Qhull. Worth tracking; not something to depend on.
...
| Library | Reason | Risk |
|---|---|---|
| scipy.spatial.ConvexHull | Python standard, 14.4k stars, institutional backing, 1.4M dependents | LOW |
| CGAL | Most capable C++ option, exact computation guarantees, 30-year track record | LOW-MEDIUM |
| geo-rust | Rust ecosystem leader, memory safety, 12M+ downloads, active development | LOW-MEDIUM |

**Tier 2 acceptable**

| Library | Reason | Risk | Caveat |
|---|---|---|---|
| quickhull3d | Best JavaScript 3D option, ThreeJS integration | MEDIUM | Single-maintainer concern, prefer Three.js built-in for graphics projects |

**Tier 3 avoid**

| Library | Reason | Risk | Note |
|---|---|---|---|
| Qhull (direct dependency) | Dormant development, single-maintainer risk | MEDIUM-HIGH | Acceptable via wrappers (scipy, CGAL) |
