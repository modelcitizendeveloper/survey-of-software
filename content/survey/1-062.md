...
description: "Python password hashing — OWASP 2025 ranks Argon2id first and demotes bcrypt to legacy; argon2-cffi ships compliant defaults, zero CVEs."
keywords: ["argon2 cffi", "bcrypt", "passlib", "hashlib scrypt"]
...
Python password hashing — OWASP 2025 ranks Argon2id first and demotes bcrypt to legacy; argon2-cffi ships compliant defaults, zero CVEs.
...
|---|---|---|
| **argon2_cffi** | Production (since 2015) | 25.1.0 · 2025-06-03 |
| **bcrypt** | Production (10+ years) | 5.0.0 · 2025-09-25 |
| **passlib** | UNMAINTAINED since October 2020 | 1.7.4 · 2020-10-08 |
| **hashlib_scrypt** | — | — |
...
- OWASP's bcrypt section restricts it: 'The bcrypt password hashing function should only be used for password storage in legacy systems where Argon2 and scrypt are not available.' The same document's summary is looser, listing bcrypt among 'strong, slow hashing algorithms such as Argon2id, bcrypt, or PBKDF2' — the two sentences disagree (read 2026-08-26)
- argon2-cffi: 74,095,778 downloads/month (pypistats, 2026-08-26), zero CVEs, OWASP-compliant defaults out of box
- bcrypt (PyCA): 215,267,956 downloads/month (pypistats, 2026-08-26), ~2.9x argon2-cffi. OWASP restricts bcrypt to legacy systems, but the LIBRARY is not stagnant — PyCA shipped bcrypt 5.0.0 on 2025-09-25
- passlib: UNMAINTAINED since October 2020, FastAPI/Ansible migrating away, use libpass fork
- hashlib.scrypt: Stdlib availability is convenient but parameter coupling makes tuning difficult
...
| When | Pick |
|---|---|
| new project | argon2-cffi with default RFC 9106 configuration |
| high security | argon2-cffi with 128 MiB memory, 3 iterations |
| fips compliance | hashlib.pbkdf2_hmac with 600,000+ iterations |
| legacy migration | Dual verification pattern (argon2-cffi + existing library) |
| passlib replacement | libpass fork OR direct argon2-cffi/bcrypt |
...
- **Recommendation**: bcrypt "should only be used for password storage in legacy systems where Argon2 and scrypt are not available" (OWASP Password Storage Cheat Sheet, §bcrypt, read 2026-08-26)
...
**bcrypt**:
- OWASP: bcrypt "should only be used for password storage in legacy systems where Argon2 and scrypt are not available"
- Not memory-hard (GPU vulnerable)
- 72-byte password limit
...
| Body | Status | Document |
|------|--------|----------|
| OWASP | bcrypt "should only be used for password storage in legacy systems where Argon2 and scrypt are not available" (§bcrypt) | Password Storage Cheat Sheet |
...
| 2026 | Argon2id primary | bcrypt "should only be used ... in legacy systems where Argon2 and scrypt are not available" (§bcrypt, read 2026-08-26) — though the same document's summary line still lists bcrypt among acceptable algorithms |
