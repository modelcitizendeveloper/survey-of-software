...
description: "Survey of database schema migration tools and libraries: Alembic, Django migrations, Flyway, Liquibase, golang-migrate, Prisma Migrate, and Sqitch. Covers…"
keywords: ["github stars", "notable adopters", "Alembic", "Django Migrations", "Flyway", "Liquibase", "golang-migrate", "Prisma Migrate", "Sqitch"]
...
| Library | Status | Latest release |
|---|---|---|
| **Alembic** | Very Active | 1.19.2 · 2026-09-04 |
| **Django Migrations** | Very Active | 6.1.1 · 2026-09-02 |
| **Flyway** | Active (Redgate) | 0.2.0 · 2025-12-24 |
| **Liquibase** | Active | — |
| **golang-migrate** | Active | — |
| **Prisma Migrate** | Very Active | 8.0.0-rc.13 · 2026-09-08 |
| **Sqitch** | Stable | — |
...
- **Ecosystem decides, not features: Alembic for SQLAlchemy, Django migrations for Django, Flyway for JVM and polyglot, golang-migrate for Go** — The survey records the Python case as settled — 'if you use SQLAlchemy, you use Alembic. Not a question.'
- **Switching migration tools is painful and rarely done — this is a decision you keep** — Every documented path works the same way, by declaring a baseline at the current schema and starting forward: alembic stamp head, Flyway baseline, migrate force.
- **Rollback is the axis the JVM choice turns on, and it is a licensing question** — Flyway scores lowest on rollback in its free tier and Liquibase highest. If rollback is a hard requirement on the JVM, that alone picks Liquibase.
- **Autogenerate splits the category in two, permanently** — Alembic, Django and Prisma generate migrations by diffing models. Flyway and golang-migrate do not and are not meant to — they are SQL-first, which is also why they score highest on DBA-friendly output.
- **The risk to watch is Flyway feature creep into paid tiers** — S4 names it explicitly: if dry-run or another essential feature moves behind the paywall, the fallbacks are golang-migrate or Liquibase.
...
### Future Trends
**State-based / declarative migrations** are gaining momentum. Atlas and skeema (covered in 1.045) take the approach of "describe desired state, generate the diff" rather than "describe the change." This is the direction Prisma Migrate and Alembic autogenerate already lean.
