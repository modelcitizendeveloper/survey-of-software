---
title: "1.187 Embedded Databases"
description: "Survey of embedded databases — in-process databases without a separate server. Covers SQL embedded (SQLite, DuckDB), key-value stores (RocksDB, LevelDB…"
keywords: ["random read", "sequential write", "random write", "best use", "SQLite", "DuckDB", "RocksDB", "LevelDB", "LMDB", "BadgerDB", "bbolt"]
...
| Library | Random Read | Best Use | Latest release |
|---|---|---|---|
| **SQLite (WAL)** | Good | OLTP, SQL | 5.1.1 · 2023-11-01 |
| **DuckDB** | Excellent (columnar) | OLAP, analytics | 1.5.5 · 2026-09-07 |
| **RocksDB** | Good (with bloom) | Write-heavy KV | 0.7.0 · 2020-12-29 |
| **LevelDB** | Good | Simpler KV | 1.5.1 · 2024-01-15 |
| **LMDB** | Excellent | Read-heavy KV | 2.3.0 · 2026-07-12 |
| **BadgerDB** | Good | Go KV, large values | 1.0.0 · 2025-02-02 |
| **bbolt** | Good | Go transactional KV | — |
...
- **SQLite handles about 90% of embedded needs, and the rule is to deviate only against a specific requirement it cannot meet** — Reaching for anything else without naming what SQLite fails at is how this category gets over-engineered.
- **SQLite is the only entry with a credible 30-year horizon** — The survey's test is explicit: data that must still be readable in 2050 goes in SQLite. Format stability and zero organizational risk are the reason, not performance.
- **The split is workload-shaped: OLTP versus OLAP versus key-value, and each has a settled answer** — SQLite for relational, DuckDB for in-process analytics, RocksDB for write-heavy key-value at scale, LMDB for read-heavy maximum throughput.
- **In Go the choice is a risk trade, not a performance one** — bbolt for transactional read-heavy state with zero-risk maintenance; BadgerDB for write-heavy work with higher organizational risk. Both beat CGO bindings where build simplicity matters.
- **SQLite is turning into a platform, which extends the category rather than replacing it** — libSQL/Turso add network extensions, Litestream adds streaming replication, and SQLite WASM puts it in the browser — use cases that were previously outside embedded databases entirely.
