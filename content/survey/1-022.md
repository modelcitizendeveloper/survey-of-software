...
description: "Python mathematical optimization — scipy.optimize, Google OR-Tools and peers, and why scipy's ubiquity stops short of MILP."
keywords: ["scipy optimize", "or tools", "pyomo", "cvxpy", "pulp", "gekko", "pymoo", "github stars", "problem types", "maturity"]
...
Python mathematical optimization — scipy.optimize, Google OR-Tools and peers, and why scipy's ubiquity stops short of MILP.
...
| Library | Maturity | Latest release |
|---|---|---|
| **scipy_optimize** | Mature (20+ years) | 1.18.1 · 2026-08-21 |
| **or_tools** | Very active (Google-maintained, Python 3.13 support) | 9.15.6755 · 2026-01-14 |
| **pyomo** | Active (v6.9.4, academic standard) | 6.10.1 · 2026-06-04 |
| **cvxpy** | Mature (Stanford research project, JMLR publication) | 1.9.2 · 2026-06-22 |
| **pulp** | Mature (Python >= 3.9) | 3.3.2 · 2026-06-19 |
| **gekko** | Active (BYU PRISM lab) | 1.3.2 · 2025-12-31 |
| **pymoo** | Active (IEEE Access publication, MSU COIN lab) | 0.6.2 · 2026-06-28 |
...
- scipy.optimize is ubiquitous (part of 100M+ monthly scipy downloads) but limited to smaller-scale problems; it gained MILP in 1.9.0 (2022) via scipy.optimize.milp, a HiGHS wrapper, and still has no NLP-with-integers or constraint-programming path
- Google OR-Tools (12.6k GitHub stars) emerged as the most popular open-source solver with broad problem type coverage
- HiGHS is the rising star of open-source solvers: opt-in in SciPy 1.6.0, the linprog default from SciPy 1.9.0, and the default in MATLAB linprog/intlinprog from R2024a
- Pyomo (2.1k stars, 123k weekly downloads) leads algebraic modeling with 20+ solver backends
- CVXPY specializes in convex optimization with disciplined convex programming (DCP) analysis
...
| When | Pick |
|---|---|
| rapid prototyping | PuLP or scipy.optimize |
| production lp milp | OR-Tools or HiGHS (via Pyomo/PuLP) |
| nonlinear | scipy.optimize (small), Pyomo+IPOPT (large), GEKKO (dynamic) |
| convex | CVXPY (with DCP verification) |
| multi objective | pymoo |
| multi solver flexibility | Pyomo (20+ backends) |
| commercial justified | Gurobi/CPLEX for large-scale MILP with strict time limits |
...
CVXPY is a Python-embedded modeling language for **convex optimization** problems. It checks problems against the Disciplined Convex Programming (DCP) ruleset, so a problem CVXPY accepts is convex and can be solved globally and efficiently.

**Key finding**: CVXPY is the specialist for convex optimization, with Stanford academic origins and JMLR publication (2016). Unlike general-purpose tools, CVXPY **checks DCP compliance** at modeling time, preventing common errors. The check is one-directional: passing proves convexity, failing does not disprove it.
...
### ❌ Not suitable when:
- **Large or solver-heavy MILP**: CVXPY *does* take integer and boolean variables (`cp.Variable(integer=True)`, `boolean=True`) and dispatches them to a MIP-capable backend — GLPK_MI, CBC, SCIP, HiGHS, Gurobi, MOSEK, XPRESS or COPT. But you must have one installed, and for MILP-first work PuLP, Pyomo or OR-Tools model it more directly
...
1. **DCP verification is the killer feature**: it checks DCP compliance at modeling time and refuses to solve a non-compliant problem. Note the limit — CVXPY's own tutorial shows `sqrt(1 + square(x))`, a convex function that fails the rules and is accepted only when rewritten as `norm(hstack(1, x), 2)`.
...
**Rationale**:
- The most widely used of the multi-objective libraries (Platypus and jMetalPy are the maintained alternatives)
...
- Mixed-integer *nonlinear* problems (MILP is covered by `scipy.optimize.milp` since 1.9.0; MINLP is not)
...
1. **HiGHS adoption**: HiGHS is the `linprog` default in SciPy from 1.9.0 and the engine behind `scipy.optimize.milp`; in MATLAB it is the `linprog` default from R2024a and the only algorithm `intlinprog` still ships. Strong signal of quality.

2. **SCIP now Apache 2.0**: Previously academic-only, now fully open source. Major development for open-source MILP.

3. **Commercial solvers left the Mittelmann benchmarks**: CPLEX and XPRESS results were removed in 2018 at IBM's and FICO's demand, Gurobi withdrew in August 2024, MindOpt in December 2024. The suites are still maintained, but open-source-versus-commercial comparison is no longer available there.

4. **OR-Tools CP-SAT dominance**: Google's CP-SAT wins MiniZinc Challenge medals, competitive with commercial CP solvers.

5. **CVXPY's DCP check is a ruleset, not a convexity detector**: it verifies that an expression is *written* in DCP form. Convex problems that are not in DCP form are rejected until rewritten. Still the strongest safety net available for convex modeling in Python.
...
**Key insight**: scipy.optimize is ubiquitous but limited to smaller-scale problems. It gained mixed-integer linear programming in SciPy 1.9.0 (July 2022) through `scipy.optimize.milp`; it still has no path to mixed-integer *nonlinear* problems or constraint programming.
...
| Linear programming | ✅ Yes | `linprog` (HiGHS available since SciPy 1.6.0, the default since 1.9.0) |
| Integer variables | ✅ Yes (linear only) | `milp`, added SciPy 1.9.0, a HiGHS wrapper; also `linprog(integrality=...)` with `method='highs'` |
...
# Solve with HiGHS (available since SciPy 1.6.0, the default since 1.9.0)
...
### ❌ Not suitable when:
- **Integer variables in a nonlinear model (MINLP)**: no capability → use Pyomo, GEKKO. For plain MILP, `scipy.optimize.milp` covers it, though PuLP and Pyomo give you a modeling layer over it.
...
1. **HiGHS adoption**: SciPy 1.6.0 (Dec 2020) added HiGHS methods to `linprog` as an opt-in; SciPy 1.9.0 (July 2022) made HiGHS the default and deprecated the legacy methods. Competitive LP performance from an MIT-licensed solver.

2. **MILP arrived in 1.9.0**: `scipy.optimize.milp` wraps HiGHS. The remaining gap is mixed-integer *nonlinear* programming and constraint programming, which still need other libraries.
...
| MILP support | ✅ Yes (`milp`, 1.9.0+) | ✅ Yes | ❌ No | ✅ Yes |
...
- HiGHS integration: SciPy 1.6.0 release notes (Dec 2020) and SciPy 1.9.0 release notes (July 2022, HiGHS made default, `milp` added)
...
- **scipy.optimize.milp**: HiGHS wrapper, added SciPy 1.9.0; no modeling layer, matrix input only
...
- Need capabilities current tool lacks (e.g., MINLP or constraint programming when using scipy.optimize)
...
|---------|-------------------|
| **scipy.optimize** | Need MINLP or constraint programming, large-scale LP, or a modeling layer over MILP |
