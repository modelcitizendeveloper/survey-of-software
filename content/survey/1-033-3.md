...
keywords: ["huggingface tokenizers", "sentencepiece", "tiktoken", "byte level bpe", "sentencepiece cjk"]
...
| Library | Load Time | Memory | Latest release |
|---|---|---|---|
| **SentencePiece** | 50ms | 8MB | 0.2.2 · 2026-07-12 |
| **tiktoken (ported)** | 30ms | 5MB | 0.14.0 · 2026-08-17 |
| **HF Tokenizers** | 80ms | 15MB | 0.23.2 · 2026-09-03 |
...
- **S1 and S4 pick different winners, and that is the finding** — S1 recommends SentencePiece on CJK merit — character_coverage and no whitespace splitting are deliberate CJK design. S4 recommends HuggingFace Tokenizers on risk: 95% five-year viability against SentencePiece's 85%.
- **tiktoken is the only entry carrying HIGH vendor lock-in** — SentencePiece has none and HuggingFace low. If the tokenizer choice must outlive a model-provider decision, that single row rules tiktoken out.
- **Token efficiency on CJK is an API cost and a context-window question, not an accuracy one** — It is why SentencePiece is the pick when training a new model with significant CJK data — the same text costs fewer tokens.
- **tiktoken's CJK improvement path is the one rated uncertain** — SentencePiece is stable and HuggingFace growing. For a CJK-heavy system that is a trajectory bet, not a current-capability comparison.
- **Speed and ecosystem pull toward tiktoken, everything else pulls away** — It is the choice for real-time inference inside the OpenAI ecosystem on English-dominant text — a narrow, well-defined slot rather than a default.
...
**Rationale:**
- ✅ **Well-capitalised** — venture-backed with a large private valuation, so
  near-term abandonment is unlikely. Read as runway, not as endorsement: a high
  private valuation also raises the monetisation pressure that eventually reaches
  the license.
