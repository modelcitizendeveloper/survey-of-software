---
title: "1.110.4 Browser Python Execution Libraries"
description: "Running Python in the browser: Pyodide, PyScript, JupyterLite, Brython and Skulpt — WebAssembly runtimes against transpilers, and what each costs to load."
keywords: ["Rapid Library Search", "Comprehensive Solution Analysis", "Need-Driven Discovery", "Strategic Solution Selection", "tier 1 wasm based", "tier 2 transpilers", "tier 3 legacy", "Pyodide", "JupyterLite", "PyScript", "Brython", "Skulpt"]
...
Running Python in the browser: Pyodide, PyScript, JupyterLite, Brython and Skulpt — WebAssembly runtimes against transpilers, and what each costs to load.
...
| Library | Group | Architecture | Latest release |
|---|---|---|---|
| **Pyodide** | tier 1 wasm based | CPython compiled to WebAssembly | 0.0.2 · 2021-12-22 |
| **JupyterLite** | tier 1 wasm based | Serverless Jupyter notebooks (Pyodide-powered) | 0.8.3 · 2026-08-20 |
| **PyScript** | tier 1 wasm based | HTML-first framework (Pyodide/MicroPython dual runtime) | 0.3.3 · 2024-09-09 |
| **Brython** | tier 2 transpilers | Python-to-JavaScript transpiler (runtime) | 3.14.3 · 2026-06-20 |
| **Skulpt** | tier 3 legacy | Python 2.x implementation in JavaScript | 1.2.0 · 2021-03-25 |
...
- Most computational widgets DON'T need Python - JavaScript is 3000x smaller and 200x faster
- WebAssembly-based solutions (Pyodide, JupyterLite, PyScript) are strategic winners for 5+ year horizon
- JavaScript transpilers (Brython, Skulpt) face declining strategic relevance
- JupyterLite has highest viability confidence (9.5/10) due to Linux Foundation backing
- Skulpt is obsolete (Python 2 EOL) - immediate migration required for existing deployments
...
| When | Pick |
|---|---|
| data science notebooks | JupyterLite (Linux Foundation backing, full Jupyter UX, proven at 500k-student scale) |
| scientific computing | Pyodide or PyScript (NumPy/Pandas/Matplotlib support) |
| interactive tutorials | PyScript <py-repl> (zero-config, beginner-friendly, 4s startup) |
| embedded repls | PyScript or raw Pyodide (professional REPL UI vs minimal bundle) |
| computational widgets | JavaScript FIRST (3000x smaller), Pyodide only if NumPy/SciPy required |
| secure code execution | Pyodide + multi-layer defense (Web Workers, timeouts, memory limits) |
| mobile first apps | PyScript (MicroPython) or Brython (<1MB bundles, <1s startup) |
| rapid prototyping | PyScript (HTML-first, dual runtime flexibility) |
| custom integration | Pyodide (maximum control, 6.4MB core) |
| legacy python 2 | Skulpt (migration to Python 3 required - DO NOT adopt for new projects) |
