---
title: "1.016 Social Network Analysis Libraries"
description: "Social network analysis: NetworkX, igraph, graph-tool, SNAP.py, NetworKit and CDlib — centrality and community detection, from teaching scale to billion-node."
...
Social network analysis: NetworkX, igraph, graph-tool, SNAP.py, NetworKit and CDlib — centrality and community detection, from teaching scale to billion-node.
...
|---|---|---|---|
| **NetworkX** | Learning, prototyping, Jupyter exploration, graphs under ~100K nodes | The starting point, and still the right answer below ~100K nodes — the largest community in the category by a wide margin (~276M PyPI downloads/month, 17.2K GitHub stars, verified 2026-08-25) and the best documentation. Also the slowest by 10-100x and the heaviest on memory. A graph passing ~100K nodes is the signal to migrate, not to optimize. BSD-3-Clause, so no license friction anywhere. | 3.6.1 · 2025-12-08 |
| **igraph** | Production analysis on graphs from ~100K to ~10M nodes | The pragmatic production default: a C core with Python and R bindings, roughly 10-50x faster than NetworkX, reached 1.0.0 on 2025-10-23 and drawing ~3.2M downloads/month. The catch is GPL-2.0 — the one library here that needs a legal conversation before it ships inside proprietary software. | 1.0.0 · 2025-10-23 |
| **graph-tool** | Research needing SBM or hierarchical community detection; 10M-100M nodes on a single machine | The capability pick rather than the speed pick: it is the only library in this set offering stochastic block model community detection, which can force the choice outright. The costs are steep — LGPL-3.0, conda or distribution packages only (its PyPI entry is a stale 2.11 and is not the shipping channel), and a bus factor of one: 1,924 repository commits are Tiago Peixoto's, across 20 contributors. Actively developed: release 3.7 on 2026-08-16. | — |
| **snap.py** | Reproducing older Stanford SNAP results inside a pinned Python 3.9 environment; nothing new | Avoid for new work — this reverses the survey's original advice. The last PyPI release is 6.0.0 from 2020-12-31 and its newest wheels are cp39, so it will not pip-install on any Python version still supported; the snap-python repository has had no push since 2022-06-21, and downloads have fallen to ~1,400/month. Its billion-node reputation is real but no longer reachable through a maintained package. For that scale use NetworKit on 32+ cores, or a distributed engine. | 3.3.2 · 2026-03-06 |
| **NetworKit** | Multi-core servers and HPC batch analysis on graphs of 10M+ nodes | The parallel specialist, and the cleanest license here (MIT). Its 5-25x advantage is conditional on hardware: on 16+ cores it is the fastest option in this set, on a laptop it buys complexity for no gain. Now the default answer above 10M nodes, since snap.py no longer installs. Active — 11.2.1 on PyPI 2026-01-09, repository pushed 2026-08-23. | 11.2.1 · 2026-01-09 |
| **CDlib** | Comparing community-detection algorithms systematically; overlapping and temporal communities | Not a general-purpose library and not a competitor to the others — a community-detection meta-package wrapping 40+ algorithms over a NetworkX, igraph or graph-tool backend, with evaluation tooling for comparing them. Add it to a backend, never in place of one. BSD-2-Clause, 0.4.1 released 2026-07-25, ~21K downloads/month. | 0.4.1 · 2026-07-25 |
...
- snap.py has fallen out of the category: its last PyPI release is 6.0.0 (2020-12-31) and its newest wheels are cp39, so it cannot be installed on any supported Python. The survey's original 'above 1B nodes, use SNAP' advice no longer holds — NetworKit on 32+ cores, or a distributed engine, is the replacement.
- The decision here is a 10-100x performance cliff, not a feature gap: NetworkX and the compiled libraries implement broadly the same algorithms, and the question is which side of the cliff your graph sits on. Graph size, not analysis type, picks the library.
- License splits the field before performance does. NetworKit (MIT), NetworkX (BSD-3-Clause) and CDlib (BSD-2-Clause) are unrestricted; igraph is GPL-2.0 and graph-tool LGPL-3.0, so the two strongest general-purpose performers are the two that need legal review for proprietary use.
- graph-tool's stochastic block model support is the only capability in this category with no substitute — everywhere else the libraries differ in speed and ergonomics rather than in what they can compute. That single feature is what justifies its conda-only install and its single-maintainer bus factor, and nothing else about it does.
- NetworKit's speed advantage is a hardware claim, not a library claim: the 5-25x figure assumes 16+ cores. On a laptop it costs setup complexity and returns nothing over igraph, which is why the category default stays NetworkX for exploration and igraph for production.
