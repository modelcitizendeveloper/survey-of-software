...
- **Pillow** — The primary recommendation for general work and the dominant library by a wide margin (~102M monthly downloads, roughly 4x scikit-image daily) — a Python-community 'key ecosystem project'. Covers resizing, conversion, filters and overlay without pulling in a vision stack.
- **OpenCV (opencv-python)** — The specialised complement rather than a competitor: computer vision and advanced processing. Enormous capability, and a heavy dependency for work that Pillow already does.
- **scikit-image** — The scientific option (~20.5M monthly) — segmentation, morphology and measurement in the NumPy/SciPy idiom. Chosen when images are data rather than pictures.
- **ImageIO** — Format breadth and I/O, including video and scientific formats. The right reach when the difficulty is reading the file, not editing it.
