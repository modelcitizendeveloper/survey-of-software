---
title: "1.080 Image Processing Libraries"
description: "Python image processing: Pillow, OpenCV, scikit-image and ImageIO — everyday manipulation, computer vision and scientific analysis, and where each stops."
keywords: ["image processing", "pillow", "pil", "opencv", "opencv-python", "scikit-image", "imageio", "thumbnail generation", "image resizing", "format conversion", "webp", "watermarking", "computer vision", "image filters", "color space conversion", "batch image processing"]
...
Python image processing: Pillow, OpenCV, scikit-image and ImageIO — everyday manipulation, computer vision and scientific analysis, and where each stops.
...
|---|---|---|
| **Pillow** | The primary recommendation for general work and the dominant library by a wide margin (~102M monthly downloads, roughly 4x scikit-image daily) — a Python-community 'key ecosystem project'. Covers resizing, conversion, filters and overlay without pulling in a vision stack. | 12.3.0 · 2026-07-01 |
| **OpenCV (opencv-python)** | The specialised complement rather than a competitor: computer vision and advanced processing. Enormous capability, and a heavy dependency for work that Pillow already does. | 5.0.0.93 · 2026-07-29 |
| **scikit-image** | The scientific option (~20.5M monthly) — segmentation, morphology and measurement in the NumPy/SciPy idiom. Chosen when images are data rather than pictures. | 0.26.0 · 2025-12-20 |
| **ImageIO** | Format breadth and I/O, including video and scientific formats. The right reach when the difficulty is reading the file, not editing it. | 2.37.4 · 2026-07-20 |
...
- Pillow's dominance is a genuine signal rather than inertia: most image work is resize, convert and overlay, and Pillow does exactly that without a vision dependency
- The category is complementary, not competitive — Pillow plus OpenCV or Pillow plus scikit-image are more common in production than any one library alone
- Reaching for OpenCV to resize a thumbnail is the characteristic over-engineering here, and the dependency weight is the cost
- scikit-image and Pillow differ in mental model, not just features: NumPy arrays as measurable data versus images as pictures to edit
...
*The weighted score in this paragraph is this survey's own judgment on the criteria it names, made when it was written. It is not a measurement; see the Evidence line at the top of the page.*
...
### Secondary Recommendation: **OpenCV**
...
### Specialized Recommendation: **PIL-SIMD**
