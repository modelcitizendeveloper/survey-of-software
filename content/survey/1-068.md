...
description: "Python API interception: mitmproxy (38K stars) is the canonical tool with a Python addon API (request/response hooks, flow modification, recording/replay).…"
keywords: ["verdict", "pypi", "mitmproxy", "proxy.py", "Envoy", "Traefik", "iptables/TPROXY"]
...
Python API interception: mitmproxy (38K stars) is the canonical tool with a Python addon API (request/response hooks, flow modification, recording/replay). Transparent interception requires iptables/TPROXY + mitmproxy in transparent mode. TLS interception uses split TLS with a custom CA; certificate pinning requires Frida for bypass. Production proxies: Envoy (C++ CNCF standard, <1ms overhead, Lua/WASM/extproc extensions) and Traefik (Go, Docker/K8s auto-discovery). Python logic in Envoy via extproc gRPC filter. proxy.py is a lighter-weight Python alternative. eBPF/Cilium is emerging for kernel-level L7 proxying (2027+ horizon).
...
|---|---|---|
| **mitmproxy** | Python | Python-scriptable MITM proxy for dev/test interception |
| **proxy.py** | Python | Lightweight plugin-based HTTP proxy |
| **Envoy** | C++ | Production L7 service proxy (CNCF standard) |
| **Traefik** | Go | Production reverse proxy with Docker/K8s auto-discovery |
| **iptables/TPROXY** | Linux kernel | Transparent OS-level traffic redirection |


**What the research found**

- **mitmproxy and Envoy are not competitors and comparing them is the category's main confusion** — One is a developer tool for interception and scripted testing; the other is production L7 infrastructure. The survey states plainly that both will remain relevant.
- **The two permanent slots are the ones with no vendor in them** — Envoy at CNCF-standard confidence, and iptables/TPROXY, which is the Linux kernel. Everything else in the survey is a project that can change hands.
- **Traefik carries a post-acquisition watch** — The survey flags it explicitly. A good reverse proxy whose governance question is open is a different risk from a technical one.
- **mitmproxy is the dominant Python MITM tool and is maintained by a small team** — Active development, high confidence on capability, and a narrow maintainer base — the standard shape for a developer tool that became a de facto standard.
- **eBPF is the five-year trend that changes this category's shape** — Handling L7 in the kernel reduces the need for Envoy sidecars in Kubernetes. It is a reason to keep the proxy layer replaceable rather than to wait.
...
- `mitmdump` — non-interactive, for scripts
...
5. Client verifies mitmproxy's cert against the installed CA — passes if CA is trusted
...
**Bypass approaches**:
1. **android-unpinner** (official mitmproxy tool): Automatically repackages an APK to disable certificate pinning. `pip install android-unpinner && android-unpinner app.apk`. Handles common pinning implementations without manual decompilation.
2. **Root the device** + use Frida to hook the SSL verification function: `frida -U -l ssl_bypass.js com.target.app`
3. **Repack the APK manually**: Decompile, remove pinning code, recompile
...
# HTTP/3 is auto-detected when the client negotiates h3
```

**Limitations**: HTTP/2 server push is not supported. HTTP/2 to HTTP/1.1 downgrade is the default behavior. **gRPC cleartext (h2c, HTTP/2 prior knowledge without TLS) is not supported** — encrypted gRPC (h2 over TLS) works fine.
...
**Python logic in Envoy → ext_proc filter**
...
**Why this beats other approaches**: No test doubles in application code. The app runs unmodified — only the network is intercepted.
...
**Integration with Burp Suite**: mitmproxy can chain to Burp — send intercepted flows to Burp for its active scanner. Use `--upstream http://burp-host:8080`.
...
4. Browse/use the app — all traffic is intercepted
...
**Not mitmproxy**: Rate limiting in production traffic paths requires Envoy or Traefik — not Python-based proxies.
