...
description: "Not trivial - many-to-many mappings, regional variants (Taiwan, Hong Kong, Mainland). OpenCC (gold standard with locale-aware configs), HanziConv…"
keywords: ["hanziconv", "opencc", "zhconv", "zhconv rs"]
...
|---|---|---|
| **OpenCC** | SAFE BET | 1.4.2 · 2026-08-22 |
| **zhconv-rs** | GROWTH BET | 0.4.2 · 2026-09-08 |
| **HanziConv** | AVOID | 0.3.2 · 2016-09-01 |
...
- **OpenCC handles about 95% of Traditional-Simplified conversion needs** — 9,400 GitHub stars against 563 for zhconv and 189 for HanziConv, 50+ contributors against 2, and a Wikipedia dependency. The popularity gap here is not close.
- **HanziConv is technical debt — the survey rates its ten-year abandonment as certain** — Very high risk at five years, declining technology, an explicit AVOID. It is in the inventory as something to migrate off, not something to choose.
- **The real choice is risk tolerance, not quality** — S4's framing: OpenCC is the safe bet, zhconv-rs is the growth bet. Neither is better; they are different bets on where the category goes.
- **Wikipedia's dependency is what makes OpenCC's low risk credible** — Ten-plus years of production use, billions of conversions, and an institutional user with a reason not to let it die — bus factor above 5.
- **Conversion is not a character-by-character mapping, which is why the simple tools stay simple** — Context-aware conversion is what separates OpenCC from lookup-table approaches; the same simplified character can map to different traditional forms depending on meaning.
