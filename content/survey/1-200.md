...
keywords: ["langchain", "llamaindex", "haystack", "semantic kernel", "dspy", "Multi-step workflows", "RAG systems with retrieval", "Production deployment", "Team collaboration required", "langsmith", "langfuse", "phoenix"]
weight: 1200000
date: 2026-07-13
lastmod: 2026-07-13
---
...
> **🔄 2026 Refresh (verified 2026-05-31):** LangChain reached **1.0 on 2025-10-22** — its first semver-stable release, committing to no breaking public-API changes until 2.0. The 1.0 line is agent-centric (a `create_agent` abstraction built on the LangGraph runtime) with a new middleware system (built-in human-in-the-loop, summarization, PII redaction). The "breaking changes every 2–3 months" characterization below predates 1.0 and **no longer applies** to the 1.0 line. LangGraph also reached **1.0 (2025-10-22)** — durable execution, built-in persistence, first-class human-in-the-loop — and is now a dominant production orchestration primitive.

> **🔄 Refresh (2026-06, verified 2026-06-26 via PyPI):** The LangGraph **1.2 line** (1.2.0 shipped 2026-05-12; current patch **1.2.6**, 2026-06-18) adds three production-oriented runtime features:
> - **DeltaChannel** — a channel type that stores only the incremental delta at each step instead of re-serializing the full accumulated value, cutting checkpoint/storage overhead for large growing state (e.g. long message lists); supports periodic full snapshots to bound read latency.
> - **Per-node timeouts via `TimeoutPolicy`** — a hard wall-clock `run_timeout`, a progress-resetting `idle_timeout`, or both, applied per node; on expiry LangGraph raises `NodeTimeoutError`, discards that attempt's writes, and defers to the retry policy.
> - **Streaming API v3** — a content-block-centric protocol replacing dict-shaped `StreamEvent` events with typed, per-channel projections (`GraphRunStream` / `AsyncGraphRunStream`) the caller drives by iteration. `>`
> On the **LangChain** side, the 1.3 line is current (**1.3.11**, 2026-06-22) and continues to build its agent runtime on the LangGraph 1.2 line. *Corrections to the freshness flag:* the flagged "1.2.2 / LangChain 1.3.2" snapshot is already superseded — LangGraph 1.2.2 did ship (2026-05-26) but the three features are 1.2-line additions (introduced at 1.2.0), and the current releases are LangGraph **1.2.6** and LangChain **1.3.11**, not 1.3.2. (Note: LangGraph 1.2.3 was yanked.)

> **🔄 Refresh (2026-07, verified 2026-07-13 via PyPI):** Patch-level movement only — **LangGraph 1.2.8** (2026-07-06; fixes a DeltaChannel `updateState`-on-fresh-thread bug that forced a stub checkpoint) and **LangChain 1.3.13**. *Flag correction:* the 2026-07-02 sweep flagged 1.2.7 as current, but 1.2.8 shipped four days later — 1.2.7 was already superseded. No verdict, architecture, or 1.2-line feature change.
...
- **Production**: Helicone and LiteLLM provide rate limiting as middleware
...
**When NOT to Use**:
- Simple prototypes (overkill for a small throwaway script)
...
> **🔄 2026 Refresh (verified 2026-05-31):** This S4 analysis predates **LangChain 1.0 and LangGraph 1.0 (both 2025-10-22)**, which materially change the stability comparison below. LangChain 1.0 is **semver-stable** (no breaking changes until 2.0), invalidating the "breaking changes every 2–3 months" risk cited throughout. LangGraph 1.0 (durable execution, built-in persistence, first-class human-in-the-loop) plus its 2026 follow-up releases made LangGraph a dominant production orchestration primitive — it overtook CrewAI in GitHub stars in early 2026, and LangChain 1.0's `create_agent` is built on the LangGraph runtime. Separately, **Microsoft moved AutoGen to maintenance mode** in favor of the Microsoft Agent Framework. Rankings below that treat Semantic Kernel as uniquely "stable (v1.0+)" should now also credit LangChain/LangGraph. *(No verifiable "LangGraph 1.1" — refer to "1.0 + 2026 follow-ups".)*
