---
title: "1.120 Discrete Event Simulation Libraries"
description: "Comprehensive analysis of five discrete event simulation libraries for Python: SimPy (process-based, hosted on GitLab, most mature), Mesa (agent-based…"
keywords: ["repository", "pypi", "github mirrors", "first release", "paradigm", "python support", "SimPy", "github stars", "joss paper", "Mesa", "salabim", "Ciw", "corporate backing", "desmod"]
...
Comprehensive analysis of five discrete event simulation libraries for Python: SimPy (process-based, hosted on GitLab, most mature), Mesa (agent-based modeling, 3.1k GitHub stars), salabim (yieldless API, built-in animation, ~100-200 stars), Ciw (queueing networks specialist, 128 stars), and desmod (component-based, Western Digital). Research covers performance characteristics, modeling paradigms (process-based vs event-based vs agent-based), real-time vs batch simulation, statistics collection, visualization integration, and simulation-optimization coupling patterns. Key discovery: No comprehensive benchmarks exist across libraries; SimPy is industry standard but salabim offers lower learning curve with yieldless API.
...
| Library | Paradigm | License | Latest release |
|---|---|---|---|
| **SimPy** | Process-based DES | MIT | 4.1.2 · 2026-05-24 |
| **Mesa** | Agent-based modeling | Apache-2.0 | 3.5.1 · 2026-03-15 |
| **salabim** | Process-based DES with animation | MIT | 26.0.8 · 2026-06-24 |
| **Ciw** | Queueing network simulation | MIT | 3.2.7 · 2025-12-05 |
| **desmod** | Component-based DES | MIT | 0.6.1 · 2020-04-16 |
...
- **repository hosting** — GitHub stars not primary metric; GitLab mirrors exist
- **community size** — simpy: Industry standard, largest ecosystem, hosted on GitLab; mesa: 3,100 GitHub stars, academic backing, JOSS published; ciw: 128 GitHub stars, queueing specialist; salabim: ~100-200 GitHub stars, active development; desmod: Corporate-backed (Western Digital), smaller community
- **performance gaps** — benchmarks available: NO comprehensive cross-library benchmarks found; simpy claim: Documented as fast, process-based efficiency; salabim tech: Uses greenlet coroutines for performance; mesa frames: Developed 2024 to address Mesa performance limits; python vs compiled: ~2.2x slower than R-simmer (only comparison found); assessment: Performance adequate for business modeling, not real-time embedded
...
- Advance clock in fixed steps (e.g., Δt = 0.1 seconds)
...
| **Utilization (ρ)** | Fraction of time resource is busy | ρ = λ / (c × μ), where λ=arrival rate, μ=service rate, c=# servers |
...
| **Little's Law** | L = λ × W | Average entities in system (L) = arrival rate (λ) × avg time in system (W) |
...
**Components**:
- **Entities**: Arrivals (inter-arrival time ~ Exponential(λ=0.2 entities/time unit))
- **Resource**: Server (capacity=1)
- **Queue**: FIFO, unlimited capacity
- **Service time**: ~ Exponential(μ=0.25 time units)
...
- ρ = λ/μ = 0.2/0.25 = 0.8 (stable, < 1)
- Expected wait time (W_q) = ρ / (μ - λ) = 0.8 / (0.25 - 0.2) = 16 time units
- Expected queue length (L_q) = λ × W_q = 0.2 × 16 = 3.2 entities
...
Deep technical analysis of five Python DES libraries: SimPy, Mesa, salabim, Ciw, and desmod.
Covers modeling paradigms, performance characteristics, API ergonomics, and integration patterns.
...
No comprehensive cross-library benchmarks exist. Performance claims are largely
anecdotal. For most business simulation workloads, all five libraries are adequate.
Choose on API ergonomics and paradigm fit, not perceived performance.
...
Map library selection to concrete use cases and developer personas.
Provide decision frameworks and real-world integration patterns.
...
**Start with SimPy** — 10-minute quickstart, best documentation, generator API is explicit.
If `yield` is confusing: try **salabim** (yieldless option).
...
**SimPy** + pandas + matplotlib — established stack; no additional dependencies.
All five libraries integrate with pandas DataFrames for results.

## Decision Rules

| If you need... | Use |
|----------------|-----|
| Autonomous agents, spatial environment, emergence | Mesa |
| Queueing networks (M/M/c, open networks) | Ciw |
| Large modular hierarchy (component DAG) | desmod |
| Built-in 2D/3D animation | salabim |
| Everything else | SimPy |
...
Assess long-term viability, maintenance risks, and strategic library selection
...
| Library | Viability | Risk | Confidence |
|---------|-----------|------|------------|
| SimPy | HIGH | Low | 20+ year track record, MIT license, active |
| Mesa | HIGH | Medium | Active dev, JOSS, academic funding dependency |
| salabim | MEDIUM | Medium | Single maintainer, smaller community |
| Ciw | MEDIUM | Medium | Academic niche, slow development pace |
| desmod | LOW-MEDIUM | High | Corporate-backed, small community, stagnant |

## Strategic Recommendation

**Default choice for production systems: SimPy.**
20+ years, GitLab-hosted (stable), MIT license, no corporate ownership risk.
Community large enough to survive any individual contributor departure.

**Agent-based modeling: Mesa is safe.**
Active academic and commercial adoption, JOSS publication, responsive maintainers.
mesa-frames (2024) shows the ecosystem is still innovating.

**Avoid desmod for new projects** unless you are Western Digital or a similar large
industrial organization. Community too small for external dependence.
...
Python DES is consolidating around SimPy for process-based and Mesa for agent-based.
Commercial tools (AnyLogic, Arena, Simul8) remain dominant for non-programmer analysts.
Python's advantage: integration with optimization (OR-Tools, scipy.optimize) and ML.
