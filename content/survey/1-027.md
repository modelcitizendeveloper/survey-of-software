...
| Library | Best for |
|---|---|
| **scipy.integrate.quad** | 1D integrals with continuous integrands |
| **scipy.integrate.romberg** | Smooth integrands when function evaluations are cheap |
| **Gaussian Quadrature** | Integrands approximated well by polynomials |
| **Monte Carlo Integration** | High-dimensional integrals (>3D) |
| **Adaptive Simpson's Rule** | Piecewise smooth functions |
...
**Default** — Use scipy.integrate.quad for general 1D integrals

| Dimension | Pick |
|---|---|
| 1d | scipy.integrate.quad (adaptive Gauss-Kronrod) |
| 2d 3d | scipy.integrate.dblquad/tplquad or nquad |
| 4d 6d | scipy.integrate.nquad with careful tolerance tuning |
| 7d plus | Monte Carlo methods (scipy or custom) |

| Integrand type | Pick |
|---|---|
| smooth | Romberg or Gaussian quadrature (fastest) |
| oscillatory | scipy.integrate.quad with oscillatory parameter |
| singular | scipy.integrate.quad with points parameter |
| discontinuous | Adaptive methods, split at discontinuities |
| polynomial | Gaussian quadrature (optimal) |

| Accuracy requirement | Pick |
|---|---|
| low 3 digits | Trapezoid rule (numpy.trapezoid) - fastest |
| medium 6 digits | Simpson's rule or quad with higher tolerance |
| high 12 digits | quad with default tolerance (1.49e-8) |
| extreme 15 plus | quad with lowered tolerance + extended precision |

| Computational budget | Pick |
|---|---|
| few evaluations | Gaussian quadrature or Romberg |
| moderate | scipy.integrate.quad (adaptive) |
| unlimited | High-order fixed methods or Monte Carlo refinement |
