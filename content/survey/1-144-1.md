...
keywords: ["dragonmapper", "pypinyin", "xpinyin"]
...
|---|---|---|
| **pypinyin** | Active (v0.55.0) | 0.55.0 · 2025-07-20 |
| **dragonmapper** | Stable (v0.2.6) | 0.3.0 · 2024-11-19 |
| **xpinyin** | Active (v0.7.7) | 0.7.7 · 2025-06-02 |
...
- **pypinyin is the strategic choice and dragonmapper is the risk** — Sustainability 4.6/5 against 2.0/5, active maintenance against inactive, 2-3 maintainers against 1, and 80%+ three-to-five-year confidence against 60%.
- **python-pinyin and pypinyin are the same library, and the naming trips people up** — The repository is python-pinyin; the package installs and imports as pypinyin. A separate package called 'pinyin' exists and is not this.
- **Heteronym handling is the capability that separates the field** — A Chinese character can have different readings by context. pypinyin's context awareness is what makes it more than a lookup table, and it is why the survey routes there by default.
- **Only two of the three do Zhuyin at all** — xpinyin is Pinyin-only. If Bopomofo output is a requirement the field is pypinyin or dragonmapper before any other criterion applies.
- **dragonmapper's remaining reason to exist is bidirectional conversion** — Converting between transcription systems rather than from characters. It is a niche tool by the survey's own positioning, against pypinyin as critical infrastructure.
