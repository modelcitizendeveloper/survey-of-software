...
description: "Python QR code generation: qrcode, segno, PyQRCode and qrcodegen — styling and branding versus strict ISO compliance, Micro QR support and dependency weight."
keywords: ["qr code", "qr code generation", "python-qrcode", "qrcode", "segno", "pyqrcode", "qrcodegen", "nayuki", "micro qr", "error correction level", "svg qr code", "styled qr codes", "iso 18004", "barcode"]
weight: 1080100
date: 2026-06-17
lastmod: 2026-06-17
---
...
Python QR code generation: qrcode, segno, PyQRCode and qrcodegen — styling and branding versus strict ISO compliance, Micro QR support and dependency weight.

## At a glance


- **qrcode (python-qrcode)** — The default by adoption — ~6.1M monthly downloads against segno's ~561K, with styling, embedded images, and PNG/SVG/console output. The choice when the code has to look like something.
- **segno** — The standards-compliance choice: strict ISO conformance, Micro QR support, no dependencies, and better performance. Chosen on correctness and footprint rather than popularity.
- **PyQRCode** — Minimal and simple, for SVG or terminal output with almost no configuration. Low adoption and little movement.
- **qrcodegen (Nayuki)** — Correctness-first and available across many languages — the pick when several runtimes must produce identical codes. Tiny Python adoption (~10K monthly).


**What the research found**

- Adoption and correctness point at different libraries: qrcode wins ~11:1 on downloads while segno is the one with strict ISO compliance, Micro QR, and no dependencies
- Micro QR support is the clearest functional divide in the category, and it is only worth caring about when the printed code is physically small
- Dependency weight is a live criterion here — segno's zero dependencies is a selection reason in constrained or security-reviewed environments
- qrcodegen is the cross-language answer, and reaching for it is a system-architecture decision rather than a Python one
...
**For a QR/document-generation SaaS**: A persona currently using **segno** ✅ has a compliance safety margin if customers later require ISO certification.
...
**Effort**: 4-8 hours for a small codebase (like a typical QR/document-generation SaaS)
...
- **Persona estimate (QR/document-generation SaaS)**: ~4 hours (few QR generation points)
...
**Persona context (QR/document-generation SaaS)**: Currently using **segno** (zero dependencies) = maximum deployment simplicity.
...
2. **Internal Fork Preparation** (Low Cost):
   - Document segno usage patterns in the codebase
