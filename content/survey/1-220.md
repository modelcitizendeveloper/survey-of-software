...
description: "Python libraries for calendar formats and protocols. icalendar (4.7M/month) for RFC 5545 parsing. caldav (107K/month) for CalDAV client. corrected 2026-08-25…"
keywords: ["CalDAV", "iCalendar", "ICS", "caldav", "icalendar", "vobject", "calendar sync", "calendar integration", "Outlook calendar", "Google Calendar", "Microsoft Graph", "RFC 4791", "RFC 5545", "ical"]
...
Python libraries for calendar formats and protocols. icalendar (4.7M/month) for RFC 5545 parsing. caldav (107K/month) for CalDAV client. corrected 2026-08-25 (re-8yk): this said "legacy", which the survey's own 2026-05-31 correction banner overturns. Verified on PyPI: vobject 0.9.9, 2024-12-16, maintained under the py-vobject org. vobject (783K/month) revived Dec 2024; reach for it when you need vCard.
...
| Library | Use For | Latest release |
|---|---|---|
| **icalendar** | Parse/generate ICS files | 7.3.0 · 2026-08-19 |
| **caldav** | CalDAV server communication | 3.3.0 · 2026-09-03 |
| **vobject** | vCard support (only library covering vCard + iCalendar) | 0.9.9 · 2024-12-16 |
...
- icalendar is de facto standard for RFC 5545 (4.7M downloads/month)
- caldav 3.2.1 (May 2026) - RFC 6764 auto-discovery since v2.2, full async (AsyncDAVClient) since v3.0
- vobject was revived under the py-vobject org (0.9.9, Dec 2024) - icalendar remains the default unless you need vCard
- Google Calendar: CalDAV v2 is read/write - only ICS feed subscriptions are read-only
- Microsoft 365/Outlook has no CalDAV at all - use Microsoft Graph (EWS retires Oct 2026)
...
> **🔄 2026 Refresh (verified 2026-05-31) — important corrections:**
> - **caldav is now v3.2.1 (May 2026)** (was 2.2.3); requires **Python ≥3.10**; **full async support (AsyncDAVClient) since v3.0** — the "synchronous only" limitation below is no longer true. Experimental JMAP client (RFC 8620/8984) added; release cadence now ~monthly.
> - **Google Calendar CalDAV is READ/WRITE** (v2 API), not read-only — the "Google read-only" claims below are wrong; only *ICS feed subscriptions* are read-only.
> - **Microsoft 365 / Outlook does NOT support CalDAV at all** (never has) — it uses EWS (retiring **Oct 1 2026**) and the **Microsoft Graph API** (REST). The matrix below incorrectly shows M365 with "full" CalDAV support.
> - Companions also bumped: **icalendar 7.2.0** (Jun 2026; Python 3.10–3.14), **tsdav 2.3.0**. **vobject was revived** under the py-vobject org (v0.9.9, Dec 2024) — it is no longer abandoned.
...
| Latest Version | **3.2.1 (May 2026)** | Recent, active (was 2.2.3) |
| Release Frequency | Monthly | ⭐⭐⭐⭐☆ Active pace |
| Python Support | 3.10+ | ⭐⭐⭐⭐☆ Modern Python |
...
- ~~Synchronous only (no asyncio support)~~ → **RESOLVED**: full async (AsyncDAVClient) since v3.0 (2026)
- ~~Google Calendar write limitation~~ → **CORRECTION**: Google CalDAV v2 is read/write; only ICS feed subscriptions are read-only
- ❌ **No Microsoft 365/Outlook CalDAV** — Outlook requires Microsoft Graph API (EWS retires Oct 2026)
...
| Google Calendar | ✅ | ✅ | ❌ | CalDAV v2 read/write (OAuth2/app password); ICS feeds read-only |
| Microsoft 365 | ❌ | ❌ | ❌ | **No CalDAV** — use Microsoft Graph API (EWS retires Oct 2026) |
| Nextcloud | ✅ | ✅ | ✅ | Best overall support |
| Radicale | ✅ | ✅ | ❌ | Self-hosted, lightweight |
| Vikunja | ✅ | ✅ | ✅ | Task-focused with calendar support |

**Key takeaway**: Works read/write with Google (CalDAV v2), Apple, Fastmail, Nextcloud, Vikunja. **Outlook/M365 is the gap** — no CalDAV, use Microsoft Graph.
...
- ⚠️ Google CalDAV v2 is read/write; the API is only needed for Google-specific features (the old "CalDAV read-only" belief is wrong — only ICS feeds are read-only)
...
| Latest Version | 7.2.0 (Jun 2026) | Recent, actively maintained |
...
**Limited support**:
- ✅ Google Calendar: CalDAV v2 **read/write** (only ICS feed subscriptions are read-only)
- ❌ **Microsoft 365 / Outlook: no CalDAV** — use Microsoft Graph API (EWS retires Oct 2026)
- ❌ Proton Calendar: No CalDAV support

**Key takeaway**: caldav covers Google + Apple + self-hosted read/write; **Outlook is the real gap** (Graph API only).
...
- ⚠️ Outlook/M365 needs Microsoft Graph (no CalDAV); Google CalDAV is read/write
...
**Version**: 3.2.1 (May 2026; Python ≥3.10; async AsyncDAVClient since v3.0) — *was 2.2.3 at original survey*
...
**Google Calendar (2026 correction)**: Google CalDAV v2 supports read AND write (create/update/delete events). The old "read-only" note conflated CalDAV with ICS feed subscriptions (those are read-only). **Outlook is the real gap** — it has never supported CalDAV; use Microsoft Graph.

## Async Support

**Current status (2026)**: **Full async since caldav v3.0** (`AsyncDAVClient`). (The old "synchronous only / use the unofficial `caldav-async`" note no longer applies.)
...
| Google Calendar | ✅ Read/write | CalDAV v2 (only ICS feed subscriptions are read-only) |
| Microsoft 365 / Outlook | ❌ | No CalDAV — Microsoft Graph API only (EWS retires Oct 2026) |
| Proton Calendar | ❌ | No CalDAV support |
...
**Version**: 7.2.0 (Jun 2026; Python 3.10–3.14) — *was 6.3.2 at original survey*
...
> **🔄 2026 Corrections (verified 2026-05-31) — these override any stale statements below:**
> - **`caldav` is now v3.2.1** (May 2026), **Python ≥3.10**, with **full async (AsyncDAVClient) since v3.0** — "synchronous only" is no longer true. **`icalendar` is 7.2.0** (Py 3.10–3.14).
> - **Google Calendar CalDAV v2 is READ/WRITE**, not read-only. Only *ICS feed subscriptions* are read-only. Any "Google read-only" statement below is incorrect.
> - **Microsoft 365 / Outlook has NO CalDAV support** (never did) — use the **Microsoft Graph API** (legacy EWS retires Oct 1 2026). Any matrix row showing M365 with CalDAV support is incorrect.
> - **`vobject` was revived** under the py-vobject org (v0.9.9, Dec 2024) — it is maintained, not "stagnant/abandoned." (icalendar is still the default recommendation for calendar-only work.)
...
> **🔄 2026 Corrections (verified 2026-05-31):** Where this doc says "Google Calendar CalDAV is
> read-only," that is incorrect — Google CalDAV v2 is **read/write** (only ICS feed subscriptions are
> read-only). **Outlook/M365 has no CalDAV** (use Microsoft Graph API; EWS retires Oct 2026). Current
> versions: caldav 3.2.1 (async), icalendar 7.2.0, vobject revived (py-vobject 0.9.9).
...
> **🔄 2026 Corrections (verified 2026-05-31):** caldav is now **v3.2.1** with **full async since
> v3.0** — statements below about "synchronous only / no async" are outdated. **Google CalDAV is
> read/write** (the "Google read-only / deprecation risk" notes are incorrect; only ICS feeds are
> read-only). **Outlook/M365 has no CalDAV** (Microsoft Graph only; EWS retires Oct 2026).
...
- **Backend**: icalendar (default), vobject (alternate; revived Dec 2024)
...
> **🔄 2026 Correction (verified 2026-07-02):** Current icalendar is **7.2.0** (Py 3.10–3.14). The
> note below that "Google CalDAV may become read-only/deprecated" is moot — Google CalDAV v2 is
> read/write today; only ICS feed subscriptions are read-only.
...
> **🔄 2026 Corrections (verified 2026-05-31) — override stale statements below:**
> **vobject was REVIVED** (py-vobject org, v0.9.9 Dec 2024) — the "Stagnant/Declining/5-year LOW"
> rating below is outdated; it is maintained again (Production/Stable). **caldav** is now v3.2.1
> (async since v3.0, Py ≥3.10); **icalendar** 7.2.0. **Google CalDAV is read/write** (not read-only;
> only ICS feeds are read-only); **Outlook/M365 has no CalDAV** (use Microsoft Graph; EWS retires Oct 2026).
...
| **caldav** | ★★★★☆ Strong | ★★★★☆ Strong | ★★★★★ Excellent | ★★★☆☆ Medium | 5-year: MEDIUM |
| **vobject** | ★★★☆☆ Revived (2024) | ★★★☆☆ Niche | ★★★★☆ Good | ★★★☆☆ Medium | 5-year: MEDIUM |
...
1. **caldav goes quiet**: no release in 18+ months.
   *Not the case today* — caldav 3.2.1 shipped 2026-05-28. This is a condition
   to watch for, not a description of the library's current state.
...
vobject ONLY for vCard (revived; see the correction banner)
```
...
**Package**: [vobject](https://pypi.org/project/vobject/)
**Maintainer**: py-vobject org (David Arnold) — **revived 2023-2024**
**First Release**: 2005 (~20 years old)
**Latest Stable**: **0.9.9 (Dec 2024)** — actively maintained again (0.9.7 Feb 2024, 0.9.8 Oct 2024)

> **🔄 2026 Correction:** The "stagnant / last release 2019 / no maintainer / avoid" thesis in this
> file is **outdated**. vobject was revived under the **py-vobject** org and is Production/Stable again.
> (icalendar remains the default pick for calendar-only work, but vobject is no longer abandoned.)
