...
description: "Text differencing algorithms and libraries for computing changes between text sequences. Covers line-based diff (Myers, patience, histogram)…"
keywords: ["line based", "semantic", "specialized", "difflib", "diff-match-patch", "libdiff", "tree-sitter", "gumtree", "difftastic", "jsondiff", "xmldiff"]
...
| Library | Group | Status | Latest release |
|---|---|---|---|
| **difflib** | line based | active | — |
| **diff-match-patch** | line based | active | 20241021 · 2024-10-21 |
| **libdiff** | line based | Not identifiable as a maintained project — the closest match (kristapsdz/libdiff, C, 15 stars) has not been touched since 2018-06-11. Treat as unavailable. | 0.1.1 · 2020-03-27 |
| **tree-sitter** | semantic | active | 0.26.0 · 2026-06-30 |
| **gumtree** | semantic | active | — |
| **difftastic** | semantic | active | — |
| **jsondiff** | specialized | active | 2.2.1 · 2024-08-29 |
| **xmldiff** | specialized | active | 3.0 · 2026-06-11 |
...
- **Nine libraries across five categories, and they are not substitutes for one another** — Line diff, structural AST diff, format-specific diff for JSON and XML, git-diff parsing and edit distance are different problems. The first question is which of the five you have.
- **difflib is the baseline and the rule is to upgrade only when proven necessary** — Stdlib means zero abandonment risk, zero dependency risk and universal familiarity. Its limits are pure Python and basic algorithms.
- **Parsing a git diff is its own problem with its own library** — unidiff exists because reading unified-diff output in CI and code review is not the same task as producing a diff. Reaching for difflib there is a category error.
- **Semantic diff means an AST, and that means tree-sitter** — Detecting a refactor rather than a text change is the one requirement none of the line-based tools can meet.
- **DeepDiff carries a bus factor of one against 15M downloads a month** — S4 records the risk and its mitigation — an active community around a single primary maintainer. Worth knowing before it becomes load-bearing.
