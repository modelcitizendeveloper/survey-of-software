...
description: "Comprehensive analysis of Voronoi diagram and Delaunay triangulation libraries for Python, covering general-purpose (scipy.spatial), constrained 2D (triangle)…"
keywords: ["scipy.spatial", "triangle", "PyVista", "shapely", "pyvoro", "freud", "MeshLib", "libigl", "scikit-geometry", "PyMesh"]
...
Comprehensive analysis of Voronoi diagram and Delaunay triangulation libraries for Python, covering general-purpose (scipy.spatial), constrained 2D (triangle), 3D visualization (PyVista), 2D geospatial (shapely), particle systems (freud, pyvoro), and advanced 3D mesh processing (MeshLib, libigl).
...
|---|---|---|---|
| **scipy.spatial** | General unconstrained Voronoi/Delaunay in 2D-8D — where 90% of projects start and stay | Very low risk. Qhull backend, 20+ years inside SciPy, 50M+ monthly downloads, NumPy its only dependency | 1.18.1 · 2026-08-21 |
| **shapely** | 2D geospatial operations on the output — unions, intersections, buffers, which scipy does not do | Low risk. GEOS backend, 3.8K stars, 25M+ monthly downloads, at home in the GeoPandas ecosystem | 2.1.2 · 2025-09-24 |
| **PyVista** | 3D visualization of what scipy computed — VTK rendering, Jupyter support | Low risk, volunteer-driven with 1,400+ downstream projects. Pairs with scipy rather than replacing it | 0.49.0 · 2026-09-08 |
| **libigl** | Academic geometry processing and exploration in 3D | Low-medium risk. 3.5K stars, 100+ contributors; one of the two migration targets S4 names for PyMesh users | 2.6.3 · 2026-09-04 |
| **MeshLib** | Production 3D boolean operations — mesh union and intersection, ~10x faster than the alternatives | Growing rather than established (~500 stars, 20+ contributors), and the other PyMesh migration target | 3.1.3.429 · 2026-08-03 |
| **freud** | Periodic boundary conditions — molecular dynamics and materials science, where scipy fundamentally cannot help and no workaround exists | Medium risk: single-lab (Glotzer Lab, NIH-funded), niche but healthy. Preferred over pyvoro for particle systems | 0.1.2 · 2018-08-09 |
| **triangle** | Constrained 2D Delaunay — enforcing edges, boundaries and holes. The only option in the category | Medium maintenance risk (individual maintainer over a stable C library), but the LICENSE is the real constraint: Shewchuk's Triangle is non-free for commercial use and this Python wrapper inherits it | 20250106 · 2025-01-07 |
| **pyvoro** | 3D particle Voronoi tessellation without periodic boundaries | Medium-high risk — unclear maintainer, under 5K monthly downloads, community health rated poor. Use freud for particle work instead | 1.3.2 · 2014-09-02 |
| **scikit-geometry** | Experimental access to CGAL from Python | High risk, experimental. Still alpha quality after 5+ years with many CGAL features missing, and it inherits CGAL's GPL/commercial licensing. Not recommended for production; scipy.spatial covers most of what it offers | — |
| **PyMesh** | Nothing new — legacy 3D mesh processing only | High risk, ABANDONED. Last release 2019, community rated dead, superseded by MeshLib and libigl. S4's 20-year outlook: will not be maintained; existing users should migrate | 0.1.14 · 2017-03-25 |
...
- scipy.spatial is default choice (95% of use cases, Qhull backend)
- triangle library is only option for 2D constrained Delaunay (licensing considerations)
- freud critical for periodic boundary conditions (materials science, MD)
- PyVista for 3D visualization (VTK wrapper, 1400+ dependent projects)
- PyMesh is inactive, migrate to MeshLib or libigl
...
| When | Pick |
|---|---|
| default stack | scipy.spatial (general purpose); shapely (2D geospatial); PyVista (3D visualization) |
| specialized needs | Constrained 2D: triangle (check license) or CDT; Periodic boundaries: freud; 3D booleans: MeshLib; Particle systems: freud > pyvoro |
| avoid | PyMesh (inactive maintenance); Custom implementations (use libraries) |
