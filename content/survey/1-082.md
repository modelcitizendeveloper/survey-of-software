...
|---|---|---|
| **scipy.spatial** | General unconstrained Voronoi/Delaunay in 2D-8D — where 90% of projects start and stay | Very low risk. Qhull backend, 20+ years inside SciPy, 50M+ monthly downloads, NumPy its only dependency |
| **shapely** | 2D geospatial operations on the output — unions, intersections, buffers, which scipy does not do | Low risk. GEOS backend, 3.8K stars, 25M+ monthly downloads, at home in the GeoPandas ecosystem |
| **PyVista** | 3D visualization of what scipy computed — VTK rendering, Jupyter support | Low risk, volunteer-driven with 1,400+ downstream projects. Pairs with scipy rather than replacing it |
| **libigl** | Academic geometry processing and exploration in 3D | Low-medium risk. 3.5K stars, 100+ contributors; one of the two migration targets S4 names for PyMesh users |
| **MeshLib** | Production 3D boolean operations — mesh union and intersection, ~10x faster than the alternatives | Growing rather than established (~500 stars, 20+ contributors), and the other PyMesh migration target |
| **freud** | Periodic boundary conditions — molecular dynamics and materials science, where scipy fundamentally cannot help and no workaround exists | Medium risk: single-lab (Glotzer Lab, NIH-funded), niche but healthy. Preferred over pyvoro for particle systems |
| **triangle** | Constrained 2D Delaunay — enforcing edges, boundaries and holes. The only option in the category | Medium maintenance risk (individual maintainer over a stable C library), but the LICENSE is the real constraint: Shewchuk's Triangle is non-free for commercial use and this Python wrapper inherits it |
| **pyvoro** | 3D particle Voronoi tessellation without periodic boundaries | Medium-high risk — unclear maintainer, under 5K monthly downloads, community health rated poor. Use freud for particle work instead |
...
| **PyMesh** | Nothing new — legacy 3D mesh processing only | High risk, ABANDONED. Last release 2019, community rated dead, superseded by MeshLib and libigl. S4's 20-year outlook: will not be maintained; existing users should migrate |
