...
|---|---|---|
| **scipy.optimize.linear_sum_assignment** | Hungarian algorithm (O(n³)), C++ backend | Default choice - ecosystem integration, stability, good performance |
| **NetworkX bipartite matching** | Hopcroft-Karp (O(E√V)), pure Python | Graph-centric workflows, pure Python requirement, visualization |
| **lapjv (package: lap)** | Jonker-Volgenant variant (O(n³)), C++ | Performance-critical applications, real-time matching, sparse matrices |
| **munkres** | Hungarian algorithm, pure Python | Legacy environments, Python 2 compatibility (avoid for new projects) |
| **lapsolver** | Multiple algorithms (Hungarian, Jonker-Volgenant), C++ | Research, algorithm experimentation |
