...
description: "Comprehensive analysis of Python libraries for solving bipartite matching and linear assignment problems. Covers Hungarian algorithm implementations…"
keywords: ["implementation", "performance", "scipy.optimize.linear_sum_assignment", "NetworkX bipartite matching", "lapjv", "munkres", "lapsolver"]
...
|---|---|---|---|
| **scipy.optimize.linear_sum_assignment** | Hungarian algorithm (O(n³)), C++ backend | Default choice - ecosystem integration, stability, good performance | 1.18.1 · 2026-08-21 |
| **NetworkX bipartite matching** | Hopcroft-Karp (O(E√V)), pure Python | Graph-centric workflows, pure Python requirement, visualization | 3.6.1 · 2025-12-08 |
| **lapjv (package: lap)** | Jonker-Volgenant variant (O(n³)), C++ | Performance-critical applications, real-time matching, sparse matrices | 1.3.27 · 2024-07-19 |
| **munkres** | Hungarian algorithm, pure Python | Legacy environments, Python 2 compatibility (avoid for new projects) | 1.1.4 · 2020-09-15 |
| **lapsolver** | Multiple algorithms (Hungarian, Jonker-Volgenant), C++ | Research, algorithm experimentation | 1.1.0 · 2020-02-04 |
...
- **scipy.optimize is the safe default for 80% of use cases** — Best ecosystem integration, maintenance, and sufficient performance for problems < 5000 items
- **lapjv offers 2-5x performance advantage but with high maintenance risk** — Last release 2019, likely abandoned by 2030. Use only for proven bottlenecks with migration plan.
- **NetworkX is only viable pure Python option but 10-100x slower** — Choose only when compilation impossible or graph features needed
- **Sparse matrix support is critical for many real-world problems** — Computer vision, logistics, and scheduling often have < 10% dense connectivity. lapjv is only library with good sparse support.
- **Scale drives library choice: < 500 items any library works, 500-5000 scipy vs lapjv trade-off, > 5000 need hierarchical approach** — Don't optimize prematurely - most problems are small enough for scipy


**What the research recommends**

**Default** — Use scipy.optimize.linear_sum_assignment for new projects

| Use case | Pick |
|---|---|
| production systems | scipy.optimize - lowest risk, best maintenance |
| real time tracking | lapjv - fastest, but plan migration path |
| task scheduling | scipy.optimize - ecosystem fit, good enough performance |
| logistics delivery | scipy.optimize + hierarchical decomposition for large scale |
| pure python required | NetworkX - only viable pure Python option |

| Scale | Pick |
|---|---|
| small problems under 500 | scipy.optimize or NetworkX - any library works |
| medium 500 to 5000 | scipy.optimize default, lapjv if bottleneck |
| large over 5000 | Hierarchical decomposition + scipy or lapjv per shard |

| Strategic 5 year outlook | Pick |
|---|---|
| conservative path | scipy.optimize - 95% confidence will be maintained through 2030+ |
| performance path | lapjv with scipy fallback - 60% confidence, plan migration by 2027-2030 |
| pure python path | NetworkX - 80% confidence, stable niche role |
