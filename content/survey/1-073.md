...
- **Prophet** — The starting point the community consistently names — handles seasonality and holidays with little tuning, and is designed for business forecasting by people who are not statisticians.
- **statsmodels** — The academic and statistical standard: ARIMA/SARIMAX, exponential smoothing, proper diagnostics. Chosen when the forecast has to be defended, not just produced.
- **Darts** — The modern unified framework — classical and neural models behind one API, so model comparison stops being a rewrite. The most comprehensive option, and the newest.
- **pandas** — Not a forecaster but the universal substrate: resampling, rolling windows and alignment. Every workflow here passes through it.
- **TensorFlow / Keras (deep learning)** — The survey's consistent community finding is that this is overkill for most business forecasting — justified by data volume, not by ambition.
