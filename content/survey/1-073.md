---
title: "1.073 Time Series Forecasting Libraries"
description: "Time series forecasting in Python: Prophet, statsmodels ARIMA, Darts, pandas and deep-learning approaches — business forecasting versus statistical rigour."
keywords: ["time series", "forecasting", "prophet", "facebook prophet", "arima", "statsmodels", "darts", "pandas", "seasonality", "trend decomposition", "sarimax", "exponential smoothing", "demand forecasting", "anomaly detection", "deep learning forecasting"]
...
# 1.073 Time Series Forecasting Libraries

Time series forecasting in Python: Prophet, statsmodels ARIMA, Darts, pandas and deep-learning approaches — business forecasting versus statistical rigour.
...
|---|---|---|
| **Prophet** | The starting point the community consistently names — handles seasonality and holidays with little tuning, and is designed for business forecasting by people who are not statisticians. | 1.4.0 · 2026-08-15 |
| **statsmodels** | The academic and statistical standard: ARIMA/SARIMAX, exponential smoothing, proper diagnostics. Chosen when the forecast has to be defended, not just produced. | 0.15.0 · 2026-08-27 |
| **Darts** | The modern unified framework — classical and neural models behind one API, so model comparison stops being a rewrite. The most comprehensive option, and the newest. | 0.47.0 · 2026-09-04 |
| **pandas** | Not a forecaster but the universal substrate: resampling, rolling windows and alignment. Every workflow here passes through it. | 3.0.5 · 2026-07-22 |
| **TensorFlow / Keras (deep learning)** | The survey's consistent community finding is that this is overkill for most business forecasting — justified by data volume, not by ambition. | 0.1 · 2025-01-19 |
...
- The community advice the survey collected is remarkably uniform: start with Prophet, add complexity only when it fails — and treat deep learning as the last resort rather than the goal
- Prophet and statsmodels are not competing on accuracy so much as on audience: one is for shipping a forecast, the other for defending one
- Darts changes the shape of the work rather than the models — one API over many families makes honest comparison cheap, which is where most forecasting projects actually go wrong
- pandas belongs in the list even though it forecasts nothing; the data-preparation step is where most time series errors are introduced
...
**Investment recommendation**: **Aggressive but staged investment** in forecasting capabilities, with 60% in proven methods, 25% in advanced techniques, and 15% in emerging technologies.
