...
description: "LLM fine-tuning frameworks — Axolotl, LLaMA Factory and Unsloth, whose custom Triton kernels buy 2.7x speed at 74% less VRAM."
keywords: ["axolotl", "llama factory", "unsloth", "peft"]
...
LLM fine-tuning frameworks — Axolotl, LLaMA Factory and Unsloth, whose custom Triton kernels buy 2.7x speed at 74% less VRAM.
...
|---|---|---|
| **axolotl** | Very active (2025-2026 updates: multimodal, LoRA optimizations, GRPO) | 0.18.0 · 2026-07-17 |
| **llama_factory** | Active (ACL 2024 publication, enterprise adoption) | 0.9.5 · 2026-05-30 |
| **unsloth** | 2.7x faster than HF Transformers, 74% less memory (tested vs latest Transformers) | 2026.9.3 · 2026-09-08 |
| **peft** | Stable (official Hugging Face library) | 0.20.0 · 2026-07-28 |
...
- Unsloth achieves 2.7x faster LoRA training with 74% less VRAM through custom Triton kernels (0% accuracy loss)
- LLaMA Factory supports 100+ models with no-code web UI (LlamaBoard), unified API across LLaMA/Mistral/Qwen/ChatGLM
- Axolotl provides full RLHF pipeline (SFT → reward modeling → PPO/DPO/GRPO) via YAML configuration
- PEFT is official Hugging Face library, enables multi-task deployment (one 13GB model + ten 50MB adapters)
- Hardware democratization: Unsloth + QLoRA enables fine-tuning 7B models on consumer GPUs (RTX 3090, free Colab)


**lora training speed**

- unsloth: 2.7x faster than Transformers baseline
- llama factory: 1.5x faster (Unsloth integration)
- axolotl: 1.2x faster (optimizations)
- peft: 1.0x (baseline)


**vram usage**

- unsloth: -74% vs baseline
- llama factory: -50% (QLoRA + optimizations)
- axolotl: -40% (LoRA optimizations, Feb 2025)
- peft: Baseline (parameter reduction but no kernel optimization)


**parameter reduction**

- lora: 90%+ fewer trainable parameters
- qlora: 90%+ trainable + 75% storage (4-bit quantization)
- full ft: 100% parameters (baseline)
