...
- **NetworkX** — The one to learn on and prototype with — pure Python, trivial install, the best documentation and by far the largest community (2.3M+ daily downloads). Also the slowest, by 40-250x. Stop when graphs pass ~10K nodes.
- **igraph** — The balanced default for production — a C++ core with Python bindings, a measured 6-11x faster than NetworkX on whole tasks (2026-08-26), and usable from R and C++ too. The choice when you need speed without adopting a research toolchain.
- **NetworKit** — Built for billion-edge networks with OpenMP parallelism; on specific algorithms it is dramatically faster (PageRank ~0.2s where graph-tool takes ~1.7s). Reach for it when the graph is the scaling problem.
- **graph-tool** — Maximum single-node performance via a C++/Boost core, at the cost of the hardest installation in the category.
- **rustworkx** — Rust-backed, NetworkX-shaped API — the migration path when you want speed without relearning the vocabulary.
