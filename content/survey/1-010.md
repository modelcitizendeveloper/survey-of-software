---
title: "1.010 Graph Analysis Libraries"
description: "Python graph analysis: NetworkX, igraph, NetworKit, graph-tool and rustworkx — a measured 6-11x gap on whole tasks, and which side your graph sits on."
keywords: ["graph analysis", "networkx", "igraph", "networkit", "graph-tool", "rustworkx", "graph algorithms", "pagerank", "centrality", "community detection", "shortest path", "network analysis", "billion-edge graphs", "k-core"]
...
# 1.010 Graph Analysis Libraries

Python graph analysis: NetworkX, igraph, NetworKit, graph-tool and rustworkx — a measured 6-11x gap on whole tasks, and which side your graph sits on.
...
|---|---|---|
| **NetworkX** | The one to learn on and prototype with — pure Python, trivial install, the best documentation and by far the largest community (2.3M+ daily downloads). Also the slowest, by 40-250x. Stop when graphs pass ~10K nodes. | 3.6.1 · 2025-12-08 |
| **igraph** | The balanced default for production — a C++ core with Python bindings, a measured 6-11x faster than NetworkX on whole tasks (2026-08-26), and usable from R and C++ too. The choice when you need speed without adopting a research toolchain. | 1.0.0 · 2025-10-23 |
| **NetworKit** | Built for billion-edge networks with OpenMP parallelism; on specific algorithms it is dramatically faster (PageRank ~0.2s where graph-tool takes ~1.7s). Reach for it when the graph is the scaling problem. | 11.2.1 · 2026-01-09 |
| **graph-tool** | Maximum single-node performance via a C++/Boost core, at the cost of the hardest installation in the category. | — |
| **rustworkx** | Rust-backed, NetworkX-shaped API — the migration path when you want speed without relearning the vocabulary. | 0.18.1 · 2026-07-30 |
...
- This category's decision is not features but a performance gap that is real and routinely overstated. Measured 2026-08-26 on Python 3.14 (networkx 3.6.1 vs igraph 1.0.0, Erdos-Renyi graphs at mean degree 8, best of 5 runs after warmup): igraph is 1.9x-17.5x faster depending on operation and size, and 6x-11x on whole tasks once graph construction is counted. Construction itself is 6x-10x and often dominates. The gap widens with size — at 1,000 nodes it is 1.9x-12.7x, at 100,000 nodes 9x-16.6x.
- igraph is the balanced answer for most production work — most of the speed, a fraction of the installation pain of graph-tool
- Installation difficulty, not API difficulty, is what keeps teams on NetworkX past the point where it hurts
- For the largest graphs the winner is algorithm-specific rather than library-specific: NetworKit beats graph-tool on PageRank and k-core, which is not a general ranking
...
**Result**: measurably slower on large graphs — 6x-11x (measured 2026-08-26, networkx 3.6.1 vs igraph 1.0.0) against igraph. Figures of 40-250x circulate widely; this survey repeated them without a benchmark, and they are not reproduced by measurement.
...
- **Code optimization is marginal**: 10-20% improvement, against roughly an order of magnitude from changing library
...
1. **Performance gaps are real but routinely overstated** — measured 6x-11x on whole tasks, not the 40-250x in circulation, and not 2x either
...
- **Performance**: Slowest of the five. Measured against igraph: 6x-11x (measured 2026-08-26, networkx 3.6.1 vs igraph 1.0.0)
...
- **Performance**: 6x-11x (measured 2026-08-26, networkx 3.6.1 vs igraph 1.0.0)
...
- **Performance**: Reported fastest overall; the "up to 250x" figure is an unsourced figure this survey never benchmarked
...
| igraph | ✅ 6-11x faster | ⚠️ 59.6s | ✅ Good | ✅ Moderate | ⚠️ Compilation |
| NetworKit | ✅ 10x faster | ✅ 0.2s | ✅ Excellent | ✅ Efficient | ⚠️ C++ deps |
| graph-tool | ⚠️ unbenchmarked here | ✅ 1.7s | ✅ Excellent | ⚠️ High | ❌ Complex |
...
- Fastest available; the 40-250x speedup is unsourced and was never benchmarked here
...
- **Order-of-magnitude speed differences** between NetworkX and the C/C++ backends — measured at 6x-11x against igraph on whole tasks
...
**Performance vs Usability Trade-off**: NetworkX remains popular despite being several times slower because it's trivial to install and has excellent documentation. Choose performance libraries when speed matters more than convenience.
...
- **Performance**: Baseline. Measured against igraph: 6x-11x (measured 2026-08-26, networkx 3.6.1 vs igraph 1.0.0)
...
- **Performance**: reported fastest; 40-250x is unsourced and was never benchmarked here
...
**Performance Gain**: roughly an order of magnitude for community detection
...
**Market Leaders:**
- **Neo4j**: The most widely deployed option, with the largest body of
  documentation and answered questions behind it, and active GenAI integration
