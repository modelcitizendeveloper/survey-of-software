...
keywords: ["quick reference", "data source catalog"]
...
|---|---|---|
| **Hanzi Writer** | Rendering animated stroke order in a browser | MIT, a few kilobytes, and the reason rendering is not the hard part of this category. Whatever data you choose, something will draw it. |
| **Make Me a Hanzi** | Chinese, when you need the most complete set | The most complete Chinese dataset — and its license is MIXED: glyph and dictionary data carry different terms. Read the repository before shipping. This is exactly the split a one-word license column hides. |
| **KanjiVG** | Japanese kanji | CC BY-SA 3.0 — usable commercially with attribution, but SHARE-ALIKE propagates to derivatives. That constrains any product mixing it with proprietary data, and projects tend to discover it late. |
| **animCJK** | Multi-language coverage in one place | Broadest coverage in the set. Verify its license terms directly before relying on it. |
| **Unihan kTotalStrokes** | Stroke COUNT, which is a different question from stroke order | Authoritative, maintained by the Unicode Consortium, and far smaller than vector data. If you only need to sort or filter by complexity, this answers it — reaching for an SVG dataset to obtain a number is the most common over-build here. |
| **cjklib** | Radical and component decomposition | LGPL. Component structure rather than stroke sequence — again, a different question. |
...
Stroke-order software is unusual in this library: **the code is the easy part and
the data is the product.** Rendering an animated stroke sequence is a solved
problem — Hanzi Writer does it in a few kilobytes of JavaScript. What is hard is
obtaining, for tens of thousands of characters, a correct sequence of strokes in
a form a program can draw.

So S2 catalogs **data sources**, not libraries, and asks of each: which writing
system does it cover, how complete is it, what format is it in, and — the question
that decides most projects — **what are you allowed to do with it?**

## The Four Groups

- `svg-stroke-order-sources.md` — the vector data itself: Make Me a Hanzi,
  KanjiVG, HanziVG, animCJK, and Hanzi Writer as the renderer that consumes it
- `animated-dictionaries.md` — online services that display stroke order rather
  than giving you data to embed
- `stroke-count-databases.md` — Unihan's `kTotalStrokes`, cjklib,
  KRADFILE/RADKFILE for radical decomposition
- `reference-data.md` — frequency and stroke-count tables for sequencing what a
  learner sees first

## Why Licensing Is Treated as a Primary Axis

Because it eliminates options faster than coverage or quality does, and because
the terms here are unusually varied for what looks like a single category:

| Resource | License | Commercial use |
|---|---|---|
| Hanzi Writer | MIT | Yes |
| KanjiVG | CC BY-SA 3.0 | Yes, with attribution — and share-alike |
| Make Me a Hanzi | Mixed | Check the repository |
| animCJK | Open source | Verify before relying on it |
| cjklib | LGPL | Yes |

Two of those rows carry obligations a permissive license does not. **CC BY-SA is
share-alike**: derivative data must carry the same terms, which is a real
constraint on a product that mixes sources. And "mixed" is the answer for the
most complete Chinese dataset, because its glyph data and its dictionary data are
licensed separately.

Any comparison here that leads with coverage is answering the second question
first.

## Method

License terms are read from each project's own `COPYING`/`LICENSE` file or
repository statement rather than from summaries, since this is exactly the field
where a summary flattens a split license into one word.


---

# S2 Recommendation: Pick the Data First, the Renderer Follows

## The shape of the decision

Rendering is commodity. **Hanzi Writer** is MIT-licensed and draws animated
stroke order from a JSON format; whatever data you choose, something will draw it.
So the real decision is which dataset you can use, and that is settled by writing
system and by license — in that order.

## By writing system

| You need | Use | Note |
|---|---|---|
| Chinese (Hanzi) | **Make Me a Hanzi** | The most complete Chinese set. Its license is **mixed** — glyph and dictionary data carry different terms. Read the repository before shipping. |
| Japanese (Kanji) | **KanjiVG** | CC BY-SA 3.0: usable commercially, but requires attribution **and share-alike on derivatives** |
| Multi-language | **animCJK** | Broadest coverage in one place; verify its license terms directly |
| Stroke counts only | **Unihan `kTotalStrokes`** | Authoritative, maintained by the Unicode Consortium, and far smaller than vector data |
| Radical decomposition | **KRADFILE/RADKFILE**, or **cjklib** (LGPL) | Different question from stroke order — component structure, not stroke sequence |

## The distinction that saves the most work

**Stroke count is not stroke order.** If your application only needs to sort,
index or filter by complexity, Unihan's `kTotalStrokes` answers it with a small
authoritative table and no vector data at all. Reaching for an SVG dataset to
obtain a number is the most common over-build in this category.

Ask which you need before choosing a source. They have different sizes, different
licenses and different maintainers.

## What to check before committing

1. **Read the actual license file**, not a summary. The most complete Chinese
   dataset has split terms, and that is invisible in any table that gives one
   answer per project.
2. **If you are mixing sources, check share-alike.** CC BY-SA propagates to
   derivatives; combining it with proprietary data is where projects discover the
   constraint late.
3. **Separate the renderer from the data** in your own code. The renderer is
   replaceable and the data is not, so a hard dependency on one dataset's format
   is the lock-in worth avoiding.
...
### 3.2 Completion Certificate Integration

For a language-learning app that issues completion certificates (e.g. via a QR-code
credential service), stroke-order mastery data can populate the certificate payload.
...
2. **Credential-focused**
   - Issue verifiable completion certificates (e.g. via a QR-code credential service)
