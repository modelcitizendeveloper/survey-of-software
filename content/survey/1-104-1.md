...
|---|---|---|
| **LibCST** | Changing code and handing it back to the humans who wrote it | The survey's pick, and the reason is formatting: a concrete syntax tree keeps comments, whitespace and quote style, so a rewrite produces a diff a reviewer will accept. Maintained by Meta. v1.9.0 (2026-07-29), Python 3.9+. |
...
| **rope** | Project-wide refactorings — rename, extract, move across files | Operates on a project rather than a file, which is what refactoring actually requires. LGPL-3.0-or-later, which is a real constraint for some commercial use. v1.14.0 (2025-07-12). |
| **RedBaron** | Nothing new — it is unmaintained | AVOID. Last release 0.9.2 on 2019-03-17 — over seven years, verified on PyPI. Its simple modification API was the appeal; LibCST covers that ground and is maintained. |
| **parso** | Parsing code that does not fully parse — editors and tooling | Error-recovering parser used by Jedi. Reach for it when the input is half-written rather than valid, which is the case an AST-based tool handles worst. v0.8.7 (2026-05-01), MIT. |
