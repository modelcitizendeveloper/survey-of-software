...
description: "P2P and mesh networking: libp2p in Go, Rust, JS and Python, plus ZeroMQ, nng, aiortc and Kademlia — NAT traversal, peer discovery and relay strategies."
keywords: ["go-libp2p", "rust-libp2p", "js-libp2p", "py-libp2p", "pyzmq", "pynng", "aiortc", "libdatachannel", "python-kademlia", "btdht", "Hyperswarm", "noiseprotocol"]
...
P2P and mesh networking: libp2p in Go, Rust, JS and Python, plus ZeroMQ, nng, aiortc and Kademlia — NAT traversal, peer discovery and relay strategies.
...
|---|---|---|---|
| **go-libp2p** | A full P2P stack — peer discovery, NAT traversal, routing and pubsub — and the sidecar a Python application should shell out to | High viability, 5+ year horizon. Protocol Labs plus Ethereum, production validated. The 2023 Protocol Labs layoffs raised concern, but it is too embedded in Ethereum infrastructure to lose maintenance — client teams contribute independently | 1.7.0 · 2026-03-10 |
| **rust-libp2p** | A full P2P stack in Rust — the Polkadot-side implementation of the same protocol | High viability, 5+ year horizon. Parity/Polkadot plus community, production validated | — |
| **js-libp2p** | A full P2P stack in Node.js or the browser | High viability, 5+ year horizon, Protocol Labs | 3.3.11 · 2026-09-02 |
| **pyzmq** | Messaging between peers whose addresses you already know — push/pull, pub/sub, req/rep over LAN or VPN | High viability, 5+ years. S4 says it cannot meaningfully be abandoned: stable protocol, simple wrapper, and the Jupyter community would fork it. NOT P2P in the discovery or NAT-traversal sense, and the survey flags that mischaracterization as the category's main trap | 27.2.0 · 2026-08-20 |
| **libdatachannel** | WebRTC DataChannel from C++, Rust or Node.js | High viability, 3-5 years, community plus Streamr. No security audit | — |
| **aiortc** | Browser-compatible P2P from Python — WebRTC with ICE/STUN/TURN built in, though it still needs external signalling | Moderate viability, 2-3 years, and S4 calls it the survey's main risk: a talented SOLO maintainer, no security audit, and no clear successor for Python WebRTC if he becomes unavailable. Watch datachannel-python (PyO3 bindings to libdatachannel) | 1.15.0 · 2026-07-13 |
| **pynng** | Messaging topologies from Python over nng rather than ZeroMQ | Moderate viability, 3 years, community-maintained. No audit | 0.9.0 · 2026-02-04 |
| **Hyperswarm** | Node.js P2P on Holepunch's custom protocol rather than libp2p | Moderate viability, 3 years, Holepunch-sponsored. No audit, and a custom protocol rather than a standard one | 4.17.1 · 2026-09-08 |
| **noiseprotocol** | Building a secure channel in Python — all Noise patterns (XX, IK, NK), the handshake framework libp2p, WireGuard and Signal use | Small community. A handshake framework, NOT a transport — it secures a connection you have already made | 0.3.1 · 2020-03-03 |
| **python-kademlia** | An application-layer DHT for custom routing or lookup in Python | LOW viability, 1-2 year horizon. Irregular maintenance, ~750 stars, no audit, and not BitTorrent-DHT compatible | 2.2.3 · 2025-03-31 |
| **py-libp2p** | Nothing in production — experimental libp2p in Python | LOW viability, horizon INDEFINITE EXPERIMENTAL. Five-plus years in that state, deprioritized by Protocol Labs, missing QUIC, NAT traversal, Gossipsub and Kademlia. S4 does not count it as a viable option; use a go-libp2p sidecar instead | 0.7.0 · 2026-07-09 |
| **btdht** | BitTorrent mainline DHT (BEP5) interop only | INACTIVE — ~85 stars, no meaningful commit in several years. Explicitly not recommended for new projects; use libtorrent's Python bindings if you need real BitTorrent DHT | 0.3.3 · 2019-09-14 |
...
- Python has no production-grade full P2P stack. py-libp2p is experimental and missing critical modules (QUIC, NAT traversal, Gossipsub, Kademlia). Python options: pyzmq for known-address messaging, aiortc for WebRTC P2P, or go-libp2p sidecar for full P2P. ZeroMQ is mischaracterized as P2P — it is a messaging topology library requiring known peer addresses.
