...
description: "Python compression compared: Zstandard, LZ4, Brotli, Snappy and the zlib-ng/isal drop-ins — the speed-versus-ratio curve and where each algorithm sits on it."
keywords: ["compression", "zstandard", "zstd", "lz4", "brotli", "snappy", "python-snappy", "zlib-ng", "isal", "gzip", "zlib", "pep 784", "compression ratio", "decompression speed", "archival storage", "web assets"]
weight: 1050000
date: 2026-03-06
lastmod: 2026-03-06
---
...
Python compression compared: Zstandard, LZ4, Brotli, Snappy and the zlib-ng/isal drop-ins — the speed-versus-ratio curve and where each algorithm sits on it.

## At a glance


- **Zstandard (zstandard)** — The default for almost everything — best overall balance of ratio and speed, ~80M monthly PyPI downloads, and a standard-library inclusion under PEP 784 that the survey records as pending.
- **LZ4** — The speed champion at ~660 MiB/s, trading ratio away deliberately. Chosen when milliseconds are the budget — real-time streaming, hot caches.
- **Brotli** — The ratio champion and by far the slowest to compress. Right for write-once/read-many: web assets and archival storage. Shipped with Python 3.7+.
- **Snappy (python-snappy)** — Very fast, low ratio, and mostly selected for ecosystem reasons — Hadoop and Google-adjacent data platforms that already speak it. Needs system dependencies.
- **zlib-ng / isal** — Not new algorithms but drop-in replacements: identical gzip/zlib output at 2-3x the speed. The cheapest win in the category when existing code cannot change.


**What the research found**

- Zstandard has effectively ended the default question — it beats gzip on both axes at once, which is why it is heading for the standard library
- The remaining choices are the ends of the curve, not the middle: LZ4 when speed is the constraint, Brotli when size is, and nothing much in between worth switching for
- zlib-ng and isal are the overlooked option — a 2-3x speedup with no format change and no call-site edits, for code that cannot migrate
- Brotli's compression cost is severe enough to be an architectural fact rather than a tuning parameter, which confines it to write-once workloads
- Snappy is usually inherited rather than chosen; the surrounding data platform decides it
