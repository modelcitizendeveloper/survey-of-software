---
title: "1.188 Version-Controlled & Branchable Databases"
description: "Survey of version-controlled and branchable database tools implementing git semantics for data: Dolt (MySQL-compatible SQL with branches/commits), LakeFS…"
keywords: ["primary", "secondary", "Dolt", "LakeFS", "Nessie", "Splitgraph"]
...
Primary focus on Dolt, the only entry offering git semantics through an ordinary
SQL interface.
...
| Library | Group | Language | Verdict |
|---|---|---|---|
| **Dolt** | primary | Go | Safe to build on. Enough funding, community and technical depth to be a reliable 3-5 year bet. |
| **LakeFS** | secondary | Go | Reasonably safe to build on for data lake use cases. The primary risk is the open-core model's feature boundary drift. |
| **Nessie** | secondary | Java | Safe to build on within the Iceberg ecosystem. The risk is concentration in Dremio. |
| **Splitgraph** | secondary | Python | Not recommended for new projects. Splitgraph is in maintenance mode; community activity has declined sharply. |


**What the research found**

- **Dolt is the only tool offering git-style branching on relational table data through a SQL interface** — MySQL-compatible, branch/commit/merge/diff on tables, and a single binary. The survey records no serious alternative at that tier.
- **The category splits by what you are versioning, and the tools do not overlap** — Relational tables go to Dolt; files in S3 or GCS go to LakeFS; Iceberg or Delta Lake table metadata goes to Nessie. Choosing wrongly means the tool cannot see your data at all.
- **There is no PostgreSQL-native option yet** — Doltgres is the thing to wait for. Splitgraph is the alternative, and the survey is direct that adopting it may mean maintaining it.
- **Splitgraph is technically interesting and has stalled in community adoption** — Technical merit and viability part company here; the survey separates them explicitly.
- **The strongest fit is moderate-write internal tooling with audit requirements, not high-throughput production data** — Version control costs write performance, which is irrelevant at low volume and decisive at high volume. Dolt also lacks native primary/replica clustering, so high availability is a real gap.
...
This stage dives deep into the implementation architecture of the four primary tools in the version-controlled database space. The heaviest coverage is on **Dolt**, because it is the only tool here that presents a versioned store through an ordinary SQL interface — which makes its internals the ones an application team actually has to operate and debug, rather than a layer they sit behind.
...
At low write volumes this overhead is irrelevant. A configuration or work-tracking store handling tens to hundreds of operations a day is nowhere near the performance ceiling, and the version-control benefits outweigh the cost by a wide margin. The trade-off only bites once writes are sustained and hot.
...
Server mode — connecting over the MySQL wire protocol — rather than the Go library is the more common pattern in practice, and the only workable one in a polyglot environment where the clients are not all Go.
...
An infrastructure engineer manages a complex system where configuration and operational state need to be versioned, auditable, and branchable. The canonical shape is a task or issue tracking database that records every change, gives each worker its own branch-level view of work state, and merges those views back to a shared main branch.
...
### Worked pattern: branch-per-worker agent coordination

The pattern in its concrete form: an agent-coordination system uses Dolt as the backing store for its work-tracking database. Each worker agent runs in its own checkout with its own Dolt branch, and merges that branch back to the shared state when it finishes a unit of work. This gives:

- **Isolation**: One worker's in-progress changes do not interfere with another's.
- **Merge coordination**: Merging a worker's branch back to the central state is a single operation, with the database performing the three-way merge rather than the application.
- **Audit trail**: Every change is a Dolt commit with a timestamp and message, giving a complete history of what records were created, updated and closed.
- **Rollback**: A worker that writes incorrect records can have them rolled back without losing other state.
...
- **High availability**: Dolt does not yet have native primary/replica clustering comparable to MySQL Group Replication. For high-availability requirements, this is a meaningful gap. For internal tooling, single-node reliability is typically sufficient; for anything customer-facing it is not.
...
**For multi-agent coordination systems specifically**: the shape fits Dolt unusually well — low write volume, SQL-queryable state, one branch per worker, and a merge back to shared state on completion. The open question for anyone adopting it there is PostgreSQL compatibility, which means tracking Doltgres.
