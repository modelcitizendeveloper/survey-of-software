...
|---|---|---|
| **Zustand** | Hook-based, no providers, single store | Modern React apps, default choice for 90% of projects |
| **Redux Toolkit** | Flux pattern, unidirectional flow, RTK Query included | Enterprise apps needing strict patterns, time-travel debugging |
| **Jotai** | Atomic state model, bottom-up composition | Complex derived state, Recoil migration, surgical updates |
| **MobX** | Reactive/observable pattern, automatic dependency tracking | Existing MobX codebases only - avoid for new projects (maintenance mode) |
| **Pinia** | Vue 3 official state management, replaces Vuex | All Vue 3 projects, no alternatives |
| **Valtio** | Proxy-based, mutable-looking API with immutable updates | Developers preferring mutable syntax, niche use cases |
| **TanStack Store** | Signal-based reactivity, framework-agnostic | TanStack ecosystem integration, cross-framework libraries |
| **Recoil** | Atomic state model (Facebook) | ARCHIVED - Migrate to Jotai immediately |
| **Preact Signals** | Signal-based reactivity, Google-backed | Performance-critical apps, paradigm shift acceptance |
| **Nanostores** | Tiny atomic stores, framework-agnostic | Small projects, framework-agnostic libraries |
