---
title: "1.186 Time-Series Databases"
description: "Survey of time-series databases and extensions: TimescaleDB (PostgreSQL extension), InfluxDB, Prometheus, QuestDB, VictoriaMetrics, and TDengine. Covers…"
keywords: ["query language", "write throughput", "cardinality", "retention control", "postgresql compatible", "TimescaleDB", "InfluxDB v3", "Prometheus", "QuestDB", "VictoriaMetrics", "TDengine", "Graphite"]
...
| Library | Query Language | Retention Control |
|---|---|---|
| **TimescaleDB** | SQL (full) | Chunk-drop |
| **InfluxDB v3** | SQL / InfluxQL | Object storage |
| **Prometheus** | PromQL | Drop blocks |
| **QuestDB** | SQL + extensions | Partition drop |
| **VictoriaMetrics** | MetricQL / PromQL | Configurable |
| **TDengine** | SQL + extensions | Retention policies |
| **Graphite** | Functional pipeline | Round-robin auto |


**What the research found**

- **The first question is whether time-series is your only workload or one of several** — TimescaleDB keeps everything in one PostgreSQL tier with SQL, joins, foreign keys and PostGIS. A purpose-built TSDB wins only once time-series stops sharing the database with relational data.
- **Prometheus is not a general time-series database and should not be evaluated as one** — It is built for the scrape-alert-graph pattern. The survey's guidance for cloud-native teams is to add VictoriaMetrics as remote storage when disk runs out and never to remove Prometheus, which stays the scraping and alerting layer.
- **Apache Arrow and Parquet are becoming the interoperability layer, and that changes what lock-in means** — A TSDB storing Parquet on object storage can be read by DuckDB, Polars and pandas with no export step. InfluxDB v3 is built on it.
- **OpenTelemetry adoption is blurring the line between metrics database and observability backend** — VictoriaMetrics, InfluxDB and TimescaleDB all accept OTLP. Systems that take it natively inherit adoption from the OTel ecosystem.
- **Graphite is a maintain-until-justified position, not a choice** — The survey's advice to legacy operators is to keep it running until a migration project earns its cost, then move to Prometheus or VictoriaMetrics.
