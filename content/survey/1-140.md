...
description: "Python libraries for classical Latin morphology — CLTK, pyLatinam, PyWORDS — and why the model package chosen inside CLTK moves accuracy by 25 points."
keywords: ["CLTK", "pyLatinam", "PyWORDS", "Stanza", "PROIEL", "ITTB", "Perseus treebank", "latin", "classical latin", "ancient greek", "morphology", "declension", "conjugation", "lemmatization", "XPOS", "treebank", "inflection"]
...
Python libraries for classical Latin morphology — CLTK, pyLatinam, PyWORDS — and why the model package chosen inside CLTK moves accuracy by 25 points.
...
|---|---|---|---|
| **CLTK** | Classical Latin in production — the only such solution the survey found | The winner, v1.5.0, 75-80% accuracy on a Stanford-backed Stanza pipeline. Package choice inside it matters as much as the library: PROIEL reaches 70% where ITTB reaches 45% | 2.5.1 · 2026-02-12 |
| **pyLatinam** | Nothing — it cannot be installed | Not installable: unavailable via pip and abandoned | 0.1.9 |
| **PyWORDS** | Dictionary lookup, which is a different problem | Wrong tool for this category — it looks words up and does not generate morphology | 1.0.0 · 2017-03-31 |
...
- CLTK only production-ready library for classical Latin
- Package selection critical: PROIEL (70%) vs ITTB (45%) = 25% accuracy difference — the choice INSIDE the library matters as much as the choice of library
- Known-word database (500 words) leverages Zipf's Law for 50% coverage, lifting 70% to 75-80%
- Translation validation provides path to 97-98% accuracy
- XPOS codes contain morphological features (declension, case, tense) — this is what makes programmatic morphology possible at all
...
# Domain Explainer: Classical Language Libraries

*For someone who has never worked with an inflected language or an NLP pipeline.
...
Most of the software world assumes that a word is a word. You look it up, you
match it, you count it. That assumption is built into search engines, into
spell-checkers, into every `word in text` in every program ever written.

Latin breaks it.

In Latin, a single noun appears in about twelve different written forms
depending on what it is doing in the sentence, and a single verb in well over a
hundred. `puella`, `puellae`, `puellam`, `puellā`, `puellās` are all the same
word — "girl" — wearing different endings to say *whose* girl, *to* the girl,
*by* the girl.

This category is the shelf of tools that turn those endings back into meaning:
given `puellam`, say "that is *puella*, feminine, singular, accusative — the
direct object". And in the other direction: given *puella*, produce all twelve
forms.
...
**Inflection** — changing a word's form to signal its grammatical role. English
does a little (*dog* / *dogs*, *walk* / *walked*). Latin does a lot.

**Lemma** — the dictionary form. The lemma of `puellam` is `puella`. This is what
you would look up.

**Declension** — the inflection pattern of a noun. Latin has five, and which one
a noun follows determines what its endings look like. `puella` is first
declension.

**Conjugation** — the same idea for verbs. Latin has four.

**Case** — the grammatical role an ending signals: subject (nominative),
direct object (accusative), possession (genitive), and so on. Six of them.

**Morphology** — the study of word forms; here, the set of facts about a form:
its lemma, its part of speech, its case, number, gender, tense.

**Parsing** — reading a form and recovering its morphology. `puellam` →
*puella*, noun, feminine, singular, accusative.

**Generation** — the reverse. *puella* → all twelve forms.

**Part-of-speech tag (POS)** — a coarse label: NOUN, VERB, ADJ. Universal across
languages.

**XPOS** — the language-specific tag, which for Latin packs the detailed
morphology into a code. This is where declension, case and tense actually live,
and recovering them from XPOS is what makes any of this programmatically useful.

**Treebank** — a body of text where every word has been hand-annotated with its
morphology and its syntactic role. This is the training data.

**Model** — a statistical parser trained on a treebank. It has read a lot of
annotated Latin and guesses at new Latin.

**Pipeline** — the chain that takes raw text through tokenization, POS tagging,
...
People arrive at this category wanting one of two things, and confusing them is
the most common mistake here.

### Job 1: Generation — "give me all the forms"

You have a lemma. You want the paradigm: every inflected form, labeled.

This is a **rule-following** problem. Latin's declension patterns are regular
enough to be written down, with a known list of irregularities. In principle a
table and a few hundred lines of code do it.

### Job 2: Parsing — "what is this form doing here?"

You have running text. You want to know, for each word, what it is.

This is a **guessing** problem, and it is much harder, because inflected forms
are ambiguous. `puellae` is simultaneously genitive singular, dative singular,
and nominative plural. Which one it is depends on the rest of the sentence.
There is no table that resolves it; you need a model that has read a lot of
Latin.

**Almost every library in this category is better at one job than the other**,
and the survey's finding is that the parsing job has a good answer while the
generation job does not.

---

## What Is Actually Available

Three candidates, and the shape of the category is unusual: it is not a field of
competitors, it is one option and two things that are not options.

### CLTK — the Classical Language Toolkit

An academic project covering Latin, Ancient Greek and other historical
languages. For Latin parsing it wraps **Stanza**, a neural NLP pipeline from
Stanford, and this is the production-ready path.

### pyLatinam

A dedicated Latin morphology generator, and exactly the tool Job 1 calls for.
**It cannot be installed** — not available via pip, and abandoned. This is worth
stating plainly rather than as a caveat: the right-shaped tool for the
generation job exists conceptually and does not exist practically.

### PyWORDS

A Python interface to Whitaker's Words, a long-standing Latin dictionary
program. It **looks words up**. It does not generate paradigms and it does not
parse sentences in context. Not a worse tool for the job — a tool for a
different job, which is why the survey rates it as "wrong tool" rather than
"third place".
...
The survey's central result is not about choosing a library at all.

CLTK drives Stanza, and Stanza offers several Latin **models**, each trained on
a different treebank. Which one you select is a single string in a configuration
call. And it moves accuracy by **twenty-five points**:

| Model | Trained on | Latin of | Measured accuracy |
|---|---|---|---|
| `ittb` (**the default**) | Index Thomisticus Treebank | Thomas Aquinas, 13th century | **45%** (9/20) |
| `proiel` | PROIEL Treebank | Biblical and general Latin | **70%** (14/20) |
| `perseus` | Perseus Treebank | Caesar, Cicero, Virgil | correct on the spot checks, but has multi-word tokenization problems |

The default is trained on **medieval scholastic philosophy**. If your text is
Cicero, the default model has spent its whole life reading a different dialect
of your language, seven centuries removed.

This is the thing to take away from the survey even if you never touch Latin:

> **In a wrapped NLP pipeline, the choice inside the library can matter more
> than the choice of library** — and the default is chosen for the maintainer's
> convenience, not for your text.

Two caveats. The accuracy figures come from a **20-item hand-checked
sample**, which is enough to establish that the gap is real and large and not
enough to pin it to a point. And "accuracy" here means correct lemma and part of
speech on classical nouns, not a general parsing score.

---

## Getting From 70% to Useful

70% is not good enough to show a learner. The survey's route to 97-98% is worth
understanding because it is a general pattern, not a Latin trick.

**Step 1 — a known-word database (70% → 75-80%).** Hand-annotate a few hundred
of the most frequent words, and check them before asking the model. This works
because of **Zipf's law**: word frequency in natural language falls off so
steeply that a few hundred words cover roughly half of running text. You are
not fixing the model; you are routing the common cases around it.

**Step 2 — translation validation (75-80% → 97-98%).** Parse, then check the
parse against an independent signal — a known translation, or a second parser
that disagrees. Disagreement flags the item for review rather than shipping a
confident error.

The shape of both steps: **the model is one input, not the answer.** A cheap
deterministic layer in front and a cheap disagreement check behind turns a 70%
parser into something usable.

Costs, roughly: hours to change the model package, ~42 hours to curate a
500-word database at a sustainable pace, ~40 hours for the validation layer,
~140 hours all-in.
...
- **Offline and free.** Models run on-device. No API cost, no per-call price,
  nothing that scales with usage.
- **Not small.** Roughly 46 MB to download, around 224 MB of models resident.
- **Fast enough.** Under 100 ms per parse.
- **Extends to Ancient Greek** on the same infrastructure, which is unusual and
  valuable — the Greek work is mostly configuration rather than a new stack.
- **No verb conjugation generator in CLTK.** Job 1 remains unserved. If you need
  full verb paradigms, you are writing tables.

---

## Choosing

**Parsing running Latin?** CLTK over Stanza, with the `proiel` package —
explicitly, never the default. Then a known-word layer for the frequent
vocabulary.

**Generating noun paradigms?** CLTK can do it. Expect to check the output.

**Generating verb paradigms?** Nothing here does it. Write the tables, or accept
lemmatization-based validation instead of generation.

**Looking words up?** PyWORDS, and none of the above applies.

**Medieval Latin?** The default `ittb` package is finally the right one — it was
never bad, it was trained on Aquinas and evaluated on Cicero.

---

## Related Surveys

- **1.033 Natural Language Processing** — the general-purpose NLP category this
  one specializes
- **1.148 Morphological Analysis Libraries** — the same problem across modern
  inflected languages
- **1.141 Spaced Repetition Algorithms** — scheduling, for anything that turns
  this output into practice
...
**Performance**:
- Parsing time: `<100`ms per sentence (acceptable for interactive use)
...
- ✅ **Production viability**: integration effort of roughly 25-30 hours takes a
  working parser from 45% to 75-80% accuracy
...
**Phase 1: MVP** (70% accuracy)
- Stanza PROIEL baseline
- A basic command-line parser over it
- **Timeline**: days, not weeks — this is the out-of-the-box tier
...
### Adoption Verdict: ✅ **VIABLE**

**Rationale**:
1. **Technical Feasibility**: ~25-30 hours of integration reaches 75-80% accuracy
