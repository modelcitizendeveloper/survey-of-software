...
|---|---|---|
| **CLTK** | Classical Latin in production — the only such solution the survey found | The winner, v1.5.0, 75-80% accuracy on a Stanford-backed Stanza pipeline. Package choice inside it matters as much as the library: PROIEL reaches 70% where ITTB reaches 45% |
| **pyLatinam** | Nothing — it cannot be installed | Not installable: unavailable via pip and abandoned |
| **PyWORDS** | Dictionary lookup, which is a different problem | Wrong tool for this category — it looks words up and does not generate morphology |
...
You have a lemma. You want the paradigm: every inflected form, labelled.
