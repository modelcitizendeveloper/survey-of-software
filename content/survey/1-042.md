...
- **pygtrie** — Google's pure-Python trie, maintained since 2014 — no C dependency, readable, mutable. The default below roughly 100K keys.
- **datrie** — Double-array trie for large, static-ish dictionaries (100K-10M keys) with fast lookups. Requires a known alphabet, which is the constraint that rules it in or out.
- **marisa-trie** — The memory-constrained choice — compressed, static, build-once/query-many, with RecordTrie for multi-value associations. Not mutable.
- **hat-trie** — C++-backed and cache-friendly, selected when the trie mutates frequently — dynamic routing tables rather than fixed dictionaries.
- **pyahocorasick** — A different problem wearing trie clothing: Aho-Corasick matches thousands of patterns against a text in one pass, rather than answering prefix queries.
