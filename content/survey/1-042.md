...
description: "Python tries: pygtrie, datrie, marisa-trie, hat-trie and pyahocorasick — prefix trees for autocomplete and routing, chosen by mutability and memory."
keywords: ["trie", "prefix tree", "pygtrie", "datrie", "marisa-trie", "hat-trie", "pyahocorasick", "aho-corasick", "patricia trie", "radix tree", "suffix tree", "ternary search tree", "autocomplete", "longest prefix match", "typeahead", "spell checking"]
...
Python tries: pygtrie, datrie, marisa-trie, hat-trie and pyahocorasick — prefix trees for autocomplete and routing, chosen by mutability and memory.
...
|---|---|---|
| **pygtrie** | Google's pure-Python trie, maintained since 2014 — no C dependency, readable, mutable. The default below roughly 100K keys. | 2.6.1 · 2026-09-01 |
| **datrie** | Double-array trie for large, static-ish dictionaries (100K-10M keys) with fast lookups. Requires a known alphabet, which is the constraint that rules it in or out. | 0.8.3 · 2025-08-28 |
| **marisa-trie** | The memory-constrained choice — compressed, static, build-once/query-many, with RecordTrie for multi-value associations. Not mutable. | 1.4.1 · 2026-04-08 |
| **hat-trie** | C++-backed and cache-friendly, selected when the trie mutates frequently — dynamic routing tables rather than fixed dictionaries. | 0.3 · 2016-02-08 |
| **pyahocorasick** | A different problem wearing trie clothing: Aho-Corasick matches thousands of patterns against a text in one pass, rather than answering prefix queries. | 2.3.1 · 2026-04-27 |
| **Redis sorted sets / custom trie** | The DIY route, chosen for distribution and real-time updates rather than for the data structure itself. | — |
...
- The selection axis is mutability and memory, not speed: marisa-trie and datrie are fast because they are static, and giving up mutation is the price of both
- pygtrie being pure Python is a feature — the deployment cost of a C extension often outweighs the lookup gap below ~100K keys
- Aho-Corasick is filed here but answers a different question (many patterns against one text, not one prefix against many strings); conflating the two is a common category error
- Several distinct structures live under one name — Patricia, radix, suffix and ternary search trees make different space/time trades, and the library you pick has usually already chosen for you
...
- **Latency**: `<100`μs (routing is critical path)
