...
- **Protocol Buffers** — The default and the enterprise standard — 5-10x faster than JSON, roughly 60% smaller, 20+ languages, and the strongest schema evolution story. The cost is a schema language and a compilation step.
- **FlatBuffers** — Zero-copy deserialization, reported 10-100x protobuf on large reads with random access into the buffer. Pays for it in larger messages, slower writes, and a harder schema definition. The gaming and real-time choice.
- **MessagePack** — The simple cross-language option — schemaless, JSON-shaped, minimal adoption cost. The pick when you want JSON's ergonomics with a smaller wire.
- **Apache Arrow** — Not a message format but a columnar in-memory standard; it wins when the data is analytical and the goal is moving between tools without a conversion.
- **Apache Avro** — Built for streaming, with the schema carried alongside the data — the reason it dominates in Kafka-shaped pipelines.
- **Cap'n Proto** — The most compact messages and zero-copy reads, from protobuf's original designer. Smaller ecosystem is the trade.
- **CBOR** — The standards-track choice (IETF), schemaless, and the one to pick when web-standards compliance or constrained-device interop is the requirement.
