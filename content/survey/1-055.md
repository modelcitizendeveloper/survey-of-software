...
description: "Binary formats: Protocol Buffers, FlatBuffers, MessagePack, Arrow, Avro, Cap'n Proto and CBOR — schema evolution, zero-copy reads, and adoption cost."
keywords: ["binary serialization", "protocol buffers", "protobuf", "flatbuffers", "messagepack", "msgpack", "apache arrow", "apache avro", "cap'n proto", "capnproto", "cbor", "zero-copy", "schema evolution", "grpc", "cross-language serialization", "wire format"]
...
Binary formats: Protocol Buffers, FlatBuffers, MessagePack, Arrow, Avro, Cap'n Proto and CBOR — schema evolution, zero-copy reads, and adoption cost.
...
|---|---|---|
| **Protocol Buffers** | The default and the enterprise standard — 5-10x faster than JSON, roughly 60% smaller, 20+ languages, and the strongest schema evolution story. The cost is a schema language and a compilation step. | 5.0.0 · 2022-03-23 |
| **FlatBuffers** | Zero-copy deserialization, reported 10-100x protobuf on large reads with random access into the buffer. Pays for it in larger messages, slower writes, and a harder schema definition. The gaming and real-time choice. | 25.12.19 · 2025-12-19 |
| **MessagePack** | The simple cross-language option — schemaless, JSON-shaped, minimal adoption cost. The pick when you want JSON's ergonomics with a smaller wire. | 1.1.12 · 2020-08-17 |
| **Apache Arrow** | Not a message format but a columnar in-memory standard; it wins when the data is analytical and the goal is moving between tools without a conversion. | 21.2.0 · 2026-07-21 |
| **Apache Avro** | Built for streaming, with the schema carried alongside the data — the reason it dominates in Kafka-shaped pipelines. | 1.12.2 · 2026-08-17 |
| **Cap'n Proto** | The most compact messages and zero-copy reads, from protobuf's original designer. Smaller ecosystem is the trade. | 2.2.4 · 2026-07-03 |
| **CBOR** | The standards-track choice (IETF), schemaless, and the one to pick when web-standards compliance or constrained-device interop is the requirement. | 10.0.12 · 2026-03-04 |
...
- There is no ranking here, only a fork: schema-driven formats (protobuf, Avro, Cap'n Proto) buy evolution and validation, schemaless ones (MessagePack, CBOR) buy adoption speed, and choosing wrong is expensive to reverse
- Protocol Buffers is the defensible default not because it wins any single benchmark but because schema evolution is the requirement teams discover late
- FlatBuffers' zero-copy advantage is asymmetric — enormous on read, negative on write — so it fits read-many workloads and little else
- Arrow and Avro are often mis-compared with the rest: Arrow is an in-memory layout and Avro is a streaming format, and both are usually chosen by the surrounding platform
