...
- **scikit-learn** — The universal foundation (~134M monthly downloads) — TF-IDF, vectorizers and classical classifiers. Battle-tested, well documented, and the right baseline before anything heavier.
- **Hugging Face Transformers** — The accuracy leader and the industry default for modern NLP. The cost is compute — worth it when accuracy is the binding constraint and resources exist.
- **spaCy** — The production workhorse for the pipeline stages around classification — tokenization, tagging and entity recognition at industrial throughput.
- **FastText** — The speed champion for resource-constrained production, and the survey's clearest risk flag: the project was archived in March 2024. Fast, but no longer maintained.
- **NLTK** — Retained for foundational and educational tasks rather than production throughput.
