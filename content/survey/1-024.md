...
keywords: ["numpy random", "pcg random", "random module", "scipy stats", "secrets", "xoshiro", "performance benchmarks", "statistical quality"]
...
> **Coverage: S1/S2/S3 of the four-pass method.** S4 has not been completed, so this is a partial survey rather than a finished comparison.
...
|---|---|---|
| **numpy.random** | Default choice | ✅✅✅ Fast |
| **secrets** | Security only | ⚠️ Slow |
| **random** | Simple scripts | ⚠️ Medium |
| **scipy.stats** | Statistical analysis | ✅✅✅ Fast |
| **Xoshiro/SFC64** | Extreme performance | ✅✅✅✅ Fastest |


**What the research found**

- **The decision is security versus performance, not which PRNG algorithm** — The survey is explicit that arguing about generators is the wrong question. Simulation and ML go to numpy.random, security goes to secrets, simple scripts go to random.
- **numpy.random.default_rng() is the answer for about 95% of use cases** — PCG64, vectorized, 3-4 GB/s. Reaching past it needs a stated reason.
- **secrets is 10-100x slower than the alternatives, and that cost is the point** — Tokens, passwords and anything cryptographic draw on OS entropy and are unpredictable by design. Using a fast PRNG there is the failure this category exists to prevent.
- **This is a solved problem for 99% of use cases** — The ecosystem has unambiguous boundaries between its three options. Time spent evaluating beyond the decision tree is time spent on infrastructure, not on the product.
- **Random number generation is infrastructure, not differentiation** — The survey's own framing: the choice matters operationally, in the way choosing an operating system does, and rarely creates any competitive advantage.
