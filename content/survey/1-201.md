...
description: "Multi-agent orchestration frameworks for building collaborative AI systems. 2026 REFRESH: the field expanded from a 3-way race to a 6+ framework field…"
keywords: ["autogen", "crewai", "metagpt", "langgraph", "openai agents sdk", "microsoft agent framework", "google adk", "smolagents", "aws strands agents", "strands", "claude agent sdk", "agentcore", "new frameworks 2026", "TECHNICAL_ARCHITECTURE"]
...
Multi-agent orchestration frameworks for building collaborative AI systems. 2026 REFRESH: the field expanded from a 3-way race to a 6+ framework field — LangGraph (production/stateful default), OpenAI Agents SDK (lightweight/OpenAI-native), Microsoft Agent Framework (Azure/.NET; merged AutoGen+Semantic Kernel, v1.0 GA Apr 2026), Google ADK (A2A-native/GCP), smolagents (minimal/code-first) join CrewAI (role-based crews), with AutoGen now maintenance-mode and MetaGPT superseded for software-dev by purpose-built coding agents (OpenHands). There is no single default; choose by complexity, model/cloud commitment, and production needs. The original 'CrewAI for 80% of teams' recommendation is superseded.
...
|---|---|---|---|
| **LangGraph** | Complex, stateful, long-running, audited, human-in-the-loop production agents | The production/stateful default (1.0, 2025-10-22). 3-5yr risk LOW. Weak for simple LLM-to-tool-to-return agents. | 1.2.11 · 2026-08-11 |
| **OpenAI Agents SDK** | Fastest setup; teams on OpenAI; code-executing / tool-heavy agents | Lightweight and OpenAI-native; sandboxed code execution and first-class MCP shipped April 2026. 3-5yr risk LOW. Weak for deterministic graph control and audit trails. | 0.22.1 · 2026-09-08 |
| **Microsoft Agent Framework** | Microsoft/Azure enterprises, .NET shops, governed production | v1.0 GA 2026-04-03; merges AutoGen + Semantic Kernel; the only first-class enterprise .NET agent framework. 3-5yr risk LOW. Young; single-process today. | 1.17.0 · 2026-09-03 |
| **Google ADK** | Enterprises wanting Google managed deployment plus cross-framework A2A interop; teams on Gemini/GCP | A2A-native; now on the 2.x line with a graph Workflow Runtime (breaking 1.x-to-2.x changes). 3-5yr risk LOW. Deepest value is GCP-bound. | 2.8.0 · 2026-08-27 |
| **CrewAI** | Role-based crews, fastest prototyping (non-graph) | 1.0 GA 2025-10-20, financially sound, risk LOW - but the role-based pick, not the universal 80% default; LangGraph overtook it in enterprise adoption in early 2026. | 1.15.20 · 2026-09-08 |
| **smolagents** | Prototyping; workflows where Python code execution is the task | Minimal (~1,000 LOC), lock-in LOW, 3-5yr risk LOW-MEDIUM. LocalPythonExecutor is not a security boundary - a sandbox is mandatory. | 1.26.0 · 2026-05-29 |
| **AWS Strands Agents** | Teams on AWS/Bedrock wanting an open SDK with a first-party managed deployment path | Open SDK pairing with Bedrock AgentCore (GA since Oct 2025). 3-5yr risk LOW; lock-in MEDIUM once AgentCore is adopted. | 1.55.0 · 2026-09-08 |
| **Claude Agent SDK** | OS-access, tool-heavy and MCP-heavy agentic work on Claude models | 3-5yr risk LOW, model-tied. Since 2026-06-15 it bills from a separate capped credit pool at API rates, then per token - NOT from Claude Max-plan limits. | 0.2.152 · 2026-09-02 |
| **AutoGen** | Existing AutoGen codebases; cross-language agents (Python and .NET) | MAINTENANCE MODE - bug and security fixes only. Superseded by the Microsoft Agent Framework; do not pick it for new projects, migrate instead. | 0.14.1 · 2026-06-30 |
| **MetaGPT** | Influential reference design for an AI software company (PM to Architect to Engineer to QA); academic research | Superseded for shipping software-dev agents by purpose-built coding agents (OpenHands et al.). Formal OSS releases stalled; 3-5yr risk MEDIUM-HIGH. | 0.8.2 · 2025-03-09 |
...
- No single default: the 2026 field is a 6+ framework race, and the original "CrewAI for 80% of teams" recommendation is superseded. Choose by workflow complexity, model/cloud commitment, and production needs.
- LangGraph is the new production/stateful default (complex, audited, human-in-the-loop agents). CrewAI stays the role-based-crew and fast-prototyping pick, and "CrewAI to LangGraph" is the most common documented migration - teams prototype in CrewAI, hit the control-flow ceiling, and port out.
- AutoGen is in MAINTENANCE MODE and should not be chosen for new projects: Microsoft merged AutoGen + Semantic Kernel into the Microsoft Agent Framework (v1.0 GA 2026-04-03), which is the Microsoft/Azure and .NET pick.
- MetaGPT is superseded for shipping software-dev agents by purpose-built coding agents (OpenHands, ~70k stars, ~72% SWE-bench Verified). Its formal OSS releases have stalled and 3-5yr risk is MEDIUM-HIGH.
- Every major cloud now ships a first-party SDK with a managed runtime: OpenAI Agents SDK, Google ADK plus Vertex Agent Engine, AWS Strands plus Bedrock AgentCore, Microsoft Agent Framework plus Azure Foundry. Lock-in is LOW at the OSS layer and MEDIUM once the managed platform is adopted.
...
> **🔄 2026 Refresh (verified 2026-05-31): AutoGen is now in MAINTENANCE MODE — superseded by the
> Microsoft Agent Framework.** Microsoft consolidated AutoGen + Semantic Kernel into the
> **Microsoft Agent Framework** (v1.0 GA 2026-04-03; convergence announced Oct 2025). The
> microsoft/autogen README now carries a maintenance-mode banner: no new features, community-managed,
> bug/security fixes only; new users are directed to MAF, existing users to the official migration
> guide. Star count is ~58.6k (not 50.4k). **Do not recommend AutoGen for new projects** — see
> `new-frameworks-2026.md` (Microsoft Agent Framework). The profile below describes the pre-maintenance state.

**Repository:** github.com/microsoft/autogen
**GitHub Stars:** ~58.6k (May 2026) — *was 50.4k at original survey*
**Contributors:** 559
...
> **🔄 2026 Refresh (verified 2026-05-31):** CrewAI hit **1.0 GA on 2025-10-20** (~52k stars, May 2026,
> growing) and added **MCP, A2A, and runtime state checkpointing** — closing its historical
> "no checkpointing" gap. Backed by CrewAI Inc. (~$44.5M raised); commercial tier rebranded
> **CrewAI Enterprise → CrewAI AMP**. **But its "default for 80% of teams" status no longer holds:**
> LangGraph overtook it in enterprise adoption in early 2026, and "CrewAI → LangGraph" is the most
> common documented migration (teams prototype in CrewAI, hit the control-flow ceiling, port out).
> CrewAI remains the best pick for **role-based crews and fast prototyping** — see the tiered
> recommendation in `recommendation.md`. (Vendor metrics like "60% of F500" are self-reported.)

**Repository:** github.com/crewAIInc/crewAI
**GitHub Stars:** ~52k (May 2026) — 1.0 GA 2025-10-20
...
> **🔄 2026 Refresh (verified 2026-05-31):** MetaGPT (~68k stars, May 2026) is now best framed as an
> **influential reference design** ("AI software company": PM→Architect→Engineer→QA), not the practical
> go-to for shipping software-dev agents. **Formal OSS releases have stalled** (latest tag in the
> v0.8.x range; team focus shifted to the commercial **MGX, rebranded "Atoms" / atoms.dev in Jan 2026**).
> For *production* software-dev agents the field moved to **purpose-built coding agents** —
> **OpenHands** (~70k stars, ~72% SWE-bench Verified), SWE-agent, Devin. 3–5yr risk for the OSS
> framework: **MEDIUM–HIGH** (risks becoming a research artifact as investment flows to Atoms).

**Repository:** github.com/FoundationAgents/MetaGPT
**GitHub Stars:** ~68k (May 2026) — *formal OSS releases stalled; team focus on Atoms/MGX SaaS*
...
# 2026 Entrants — Agent Frameworks Added in the May 2026 Re-survey

> **🔄 Refresh (2026-07-16, mini-sweep):** Two updates since June 26:
> - **LangGraph 1.2.8** (July 6, 2026) — patch-level from 1.2.6; no architecture or verdict change.
> - **MCP 2026-07-28 final spec** is landing this month (RC locked May 2026; final release July 28):
>   stateless core (runs behind ordinary load balancers without sticky sessions), MCP Apps
>   (server-rendered UIs), Tasks extension (long-running work), tighter OAuth/OIDC alignment, and
>   formal deprecation policy. Every framework below that integrates MCP (LangGraph, CrewAI, OpenAI
>   Agents SDK, Strands, Claude Agent SDK) should be re-evaluated against the 2026-07-28 spec on
>   the next M-tier pass.

> **🔄 Added 2026-05-31.** The original 1.201 (AutoGen / CrewAI / MetaGPT) predated the
> frameworks below, which now dominate the landscape. Profiles are shopping-level (S1) with a
> viability read; all facts web-verified May 2026. Star counts are approximate and move daily.

The field is no longer a 3-way race. As of May 2026 there is **no single "best" general-purpose
agent framework** — choice is driven by *workflow complexity*, *model/cloud commitment*, and
*production requirements*. The five entrants below, plus the re-rated incumbents, form a 6–8
framework field.

---

## LangGraph — the production / stateful default

- **What:** low-level **graph** orchestration (nodes = steps/agents, edges = control flow, a
  persisted **state object** threads through). By LangChain Inc.; standalone (no LangChain needed).
- **Release:** **1.0 on 2025-10-22** (a stability stamp, not a rewrite). Actively maintained
  through 2026. *(No verifiable "1.1" — treat as "1.0 line.")* **~33.5k stars.**
- **Differentiator:** production primitives — durable/resumable execution, built-in persistence,
  checkpointing at every node, first-class human-in-the-loop.
- **Backing:** LangChain Inc. ($125M Series B, Oct 2025, ~$1.25B valuation). Commercial:
  LangGraph Platform (now folded under "LangSmith Deployment").
- **Best for:** complex, stateful, long-running, audited, human-in-the-loop production agents.
  **Weak for** simple "LLM→tool→return" agents (boilerplate, steep curve).
- **Adoption (vendor-cited):** Uber, LinkedIn, Klarna.
- **Viability:** momentum HIGH, lock-in MEDIUM (managed platform pull), **3–5yr risk LOW.**

## OpenAI Agents SDK — the lightweight / OpenAI-native pick

- **What:** OpenAI's lightweight framework; production successor to Swarm (2024). Primitives:
  **agents, handoffs, guardrails, sessions**, built-in tracing. Python + TypeScript.
- **Release:** launched **March 2025**; **major April 2026 upgrade**. **~26.8k stars** (v0.17.4, May 2026).
- **April 2026 — shipped vs. announced (verified):** **SHIPPED/GA** — native **sandboxed code
  execution** (Blaxel/Cloudflare/Daytona/E2B/Modal/Runloop/Vercel), **first-class MCP**, model-native
  harness. **IN DEVELOPMENT (not GA):** isolated **subagents** and "code mode." Classic
  agent-to-agent **handoffs** have been GA since v1 — don't conflate with the new subagents primitive.
- **Model-agnostic?** Partially — non-OpenAI models work via LiteLLM/Any-LLM, but officially
  **"best-effort, beta."** The claim that it's "OpenAI-only" is outdated/wrong; the claim that it's
  fully neutral is overstated.
- **Best for:** fastest setup; teams on OpenAI; **code-executing / tool-heavy agents** (best-in-class
  after April 2026 sandbox+MCP). **Weak for** deterministic graph control / audit trails.
- **Viability:** momentum VERY HIGH (first-party), lock-in MEDIUM (OpenAI-first), **3–5yr risk LOW.**

## Microsoft Agent Framework (MAF) — the Microsoft/Azure & .NET answer

- **What:** unifies **AutoGen + Semantic Kernel** into one SDK (built by both teams, now merged).
  **.NET + Python** parity (the only first-class enterprise **.NET** agent framework). MIT.
- **Release:** **v1.0 GA 2026-04-03** (verified — official devblog; "stable APIs + LTS commitment").
  RC Feb 2026; preview Oct 2025. **~10.9k stars** (net-new repo; understates adoption as it absorbs
  two predecessor communities).
- **Features:** graph workflow engine; sequential/concurrent/handoff/group-chat/Magentic-One
  patterns; MCP; built-in OpenTelemetry; declarative YAML agents; DevUI; deep Azure integration.
  A2A "1.0 support coming soon" (announced, not shipped at GA).
- **Best for:** Microsoft/Azure enterprises, **.NET shops**, governed production. **Caveats:** young
  (v1.0 ~2mo old); single-process today (distributed runtime "planned"); some Python connectors maturing.
- **Viability:** momentum HIGH, lock-in MEDIUM (Azure gravity; framework itself open), **3–5yr risk LOW.**

## Google ADK (Agent Development Kit) — the A2A-native / Google-Cloud pick

- **What:** Google's **code-first** agent framework (Python primary; Java, Go). Multi-agent native,
  model-agnostic (Gemini native + 100+ via LiteLLM), **A2A-native**, "paved road" to Vertex AI
  **Agent Engine**.
- **Release:** **v1.0 GA 2025-05-22** (Google I/O); now on the **2.x line (v2.1.0, May 2026)** with a
  graph Workflow Runtime + Task API (**breaking changes** 1.x→2.x — signals API churn). **~19.9k stars.**
- **Best for:** enterprises wanting Google's managed deployment + **cross-framework A2A interop**;
  teams on Gemini/GCP. **Caveats:** younger ecosystem; deepest value is GCP-bound (Agent Engine).
- **Viability:** momentum HIGH, lock-in MEDIUM (open framework, GCP-pull for managed), **3–5yr risk LOW.**

## Hugging Face smolagents — the minimal, code-first lane

- **What:** HF's ~1,000-LOC **"agents that think in code"** library — agents emit **Python code** as
  actions (claimed ~30% fewer steps than JSON tool-calls). Model- and modality-agnostic; MCP/LangChain
  tool reuse.
- **Release:** late 2024/early 2025; **~27.6k stars** (v1.26.0, May 2026), strong star velocity.
- **Best for:** prototyping; workflows where **Python/code execution is the task**; tiny readable
  codebase. **Caveats:** `LocalPythonExecutor` is **not a security boundary** — sandbox is mandatory;
  thinner enterprise tooling; lower complex-task success in one benchmark (49% vs LangGraph 62%).
- **Viability:** momentum HIGH, lock-in LOW (easiest exit), **3–5yr risk LOW–MEDIUM** (may stay a
  prototyping/niche tool by design).

## AWS Strands Agents — the AWS-native / Bedrock managed lane

> **🔄 Added 2026-06-26** (freshness sweep). Web-verified 2026-06-26.

- **What:** AWS's open-source, **model-driven** agent SDK (Python + TypeScript) — define an agent
  from a model + tools + prompt, the model drives the loop. Apache-2.0. Provider-agnostic (Bedrock,
  Anthropic, OpenAI, Ollama, LiteLLM), but the "paved road" is AWS.
- **Release / traction:** open-sourced **May 2025**; **14M+ downloads** (AWS-cited). On the 1.x line,
  actively maintained.
- **2026 developments (verified):** **Strands Labs** launched **2026-03-12** — a separate GitHub org
  for *experimental* projects (Robots, Robots Sim, AI Functions); explicitly a "testing ground"
  feeding ideas into the core SDK, **not** production services. Pairs with **Amazon Bedrock
  AgentCore** — a managed, serverless agent runtime (memory, identity, gateway, observability) that
  has been **GA since October 2025**; Strands is the recommended SDK for deploying onto it.
  *(Correction to the sweep flag: AgentCore is GA since Oct 2025, not an April-2026 preview; and
  Strands Labs is experimental robotics/dev tooling, not a managed harness.)*
- **Positioning:** AWS's answer to Google ADK + Vertex Agent Engine and OpenAI Agents SDK — write
  agents with an open SDK, deploy to a first-party managed runtime (AgentCore) on AWS.
- **Best for:** teams on AWS/Bedrock wanting an open SDK with a first-party managed deployment path.
  **Weak for** teams avoiding AWS gravity — deepest value (AgentCore) is AWS-bound.
- **Viability:** momentum HIGH (AWS-backed, strong download velocity), lock-in LOW at the SDK layer /
  MEDIUM once AgentCore is adopted, **3–5yr risk LOW.**

## Claude Agent SDK (Anthropic) — the OS-access / MCP-native lane

> **🔄 Added 2026-06-26** (freshness sweep). Web-verified 2026-06-26.

- **What:** Anthropic's agent SDK (Python + TypeScript) — the harness behind Claude Code, exposed
  as a general agent-building toolkit. Built-in **file, shell, and code-execution tools**, deep OS
  access, subagents, and a first-class **MCP** ecosystem. Model-tied (Claude).
- **Rename (verified):** shipped **September 2025** as the *Claude Code SDK*, then renamed to
  **Claude Agent SDK** the same month to signal broader (non-coding) agent use — npm
  `@anthropic-ai/claude-code` → `@anthropic-ai/claude-agent-sdk`, Python `claude-code-sdk` →
  `claude-agent-sdk`. *(Correction to the sweep flag: the rename was Sept 2025, not 2026.)*
- **Billing split (verified, effective 2026-06-15):** Agent SDK usage, headless `claude -p`, Claude
  Code GitHub Actions, and **third-party apps authenticating through the Agent SDK** no longer draw
  from Claude subscription limits. They consume a **separate monthly credit pool** ($20 Pro / $100
  Max 5x / $200 Max 20x) billed at standard **API rates**, then **per-token** beyond the credit.
  **Interactive** Claude Code in a terminal/IDE is unchanged (still subscription limits).
  *Practical caveat:* third-party Agent-SDK **harnesses do NOT draw from Max-plan limits** — they hit
  the capped credit pool, then pay per token. Confirms the sweep caveat.
- **Positioning:** cited in comparisons vs OpenAI Agents SDK and Google ADK on **deep OS access,
  built-in file/shell tools, and the strongest MCP story** — strongest where the agent must read,
  run, and edit a real environment.
- **Best for:** OS-access / tool-heavy / MCP-heavy agentic work on Claude models. **Weak for**
  model-agnostic or cost-capped fleets after the June 15 billing change.
- **Viability:** momentum VERY HIGH (first-party, fast-moving), lock-in MEDIUM (Claude-tied; MCP
  eases tool portability), **3–5yr risk LOW** — *watch the credit-pool economics* for heavy automation.

---

## Also note (adjacent, software-dev niche)

**OpenHands** (~70k stars, ~72% SWE-bench Verified) and peers (SWE-agent, Devin) are **purpose-built
coding agents** that have largely superseded MetaGPT for *shipping* software-dev work. Relevant to the
"software development" recommendation (see `metagpt.md` refresh note).

## Sources
LangGraph 1.0 ([changelog](https://changelog.langchain.com/announcements/langgraph-1-0-is-now-generally-available)) ·
OpenAI Agents SDK April 2026 ([HelpNetSecurity](https://www.helpnetsecurity.com/2026/04/16/openai-agents-sdk-harness-and-sandbox-update/)) ·
MAF 1.0 ([devblog](https://devblogs.microsoft.com/agent-framework/microsoft-agent-framework-version-1-0/)) ·
Google ADK ([repo](https://github.com/google/adk-python)) · smolagents ([repo](https://github.com/huggingface/smolagents)) ·
comparisons ([codebridge](https://www.codebridge.tech/articles/choosing-a-multi-agent-framework-langgraph-crewai-microsoft-agent-framework-or-openai-agents-sdk), [gurusup](https://gurusup.com/blog/best-multi-agent-frameworks-2026)) ·
Strands Agents ([AWS Open Source blog](https://aws.amazon.com/blogs/opensource/introducing-strands-agents-an-open-source-ai-agents-sdk/), [Strands Labs / InfoQ 2026-03-12](https://www.infoq.com/news/2026/03/aws-strands-agents/), [Bedrock AgentCore docs](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/strands-sdk-memory.html)) ·
Claude Agent SDK ([Claude Help Center — use with your plan](https://support.claude.com/en/articles/15036540-use-the-claude-agent-sdk-with-your-claude-plan), [June 15 2026 billing change](https://www.techtimes.com/articles/317625/20260602/anthropic-ends-subscription-subsidy-agents-june-15-credit-pool-replaces-flat-rate-access.htm))
...
> **🔄 2026 Refresh (verified 2026-05-31) — the "CrewAI for 80%" default no longer holds.**
> When this survey was written, CrewAI's main rivals were LangChain/AutoGen, so a single default
> made sense. By May 2026 the field fragmented into a 6+ framework race (LangGraph 1.0, OpenAI
> Agents SDK, Google ADK 2.x, Microsoft Agent Framework GA, smolagents), and the consensus is
> **there is no single 80% default** — pick by complexity, model/cloud commitment, and production
> needs. See `new-frameworks-2026.md`. Updated tiered recommendation: `>`
> | Need | Pick |
> |------|------|
> | Complex, stateful, audited, human-in-the-loop **production** | **LangGraph** (the new "serious production" default) |
> | Simple / few agents, fastest start, OpenAI-native, code-exec | **OpenAI Agents SDK** |
> | Role-based **crews**, fastest prototyping (non-graph) | **CrewAI** (still strong — but role-based default, not universal) |
> | **Microsoft/Azure** enterprise or **.NET** | **Microsoft Agent Framework** (AutoGen is now maintenance-mode) |
> | **Google Cloud / Gemini** + cross-framework A2A interop | **Google ADK** |
> | Minimal, code-first, code-execution-is-the-task | **smolagents** |
> | Shipping **software-dev** agents | **purpose-built coding agents** (OpenHands et al.) — MetaGPT is superseded |

## Quick Answer (original 2025 — superseded, see refresh note above)
...
**Provider Integration (changed at 1.0):** CrewAI 1.x calls **native SDKs directly for the
five major providers** (OpenAI, Anthropic, Google Gemini, Azure OpenAI, AWS Bedrock) and
falls back to **LiteLLM for 100+ other providers** only when the optional `crewai[litellm]`
extra is installed. In the 0.x era LiteLLM was the central, required abstraction; 1.0 removed
it from the default install (LiteLLM was quarantined on PyPI), so the model-agnostic reach now
depends on native SDKs plus an opt-in LiteLLM fallback rather than a single universal layer.
...
**Optional Extras (pip install):**
- LLM providers: anthropic, aws, azure-ai-inference, bedrock, google-genai, litellm (`crewai[litellm]`, the opt-in fallback for 100+ non-native providers)
...
| **Anthropic Claude** | ✅ Via extras | ✅ Native SDK | ✅ Supported |
| **Google Gemini** | ✅ Via extras | ✅ Native SDK | ✅ Supported |
| **Local Models (Ollama)** | ✅ Via extras | ✅ Supported | ✅ Supported |
| **Model Mixing** | ✅ Different LLMs per agent (unique) | ❌ Single model per crew | ❌ Not documented |
| **Provider Count** | 75+ (via Together.AI) | 5 native SDKs + 100+ via optional LiteLLM | Limited documentation |
...
*Scores in this table are this survey's own judgment on the criteria named, made when it was written. They are not measurements; see the Evidence line at the top of the page.*
...
> **🔄 SUPERSEDED IN PART (2026 refresh, verified 2026-05-31).** This pass was
> written when the survey covered only AutoGen / CrewAI / MetaGPT. Its framing —
> and especially "CrewAI = 80% of multi-agent use cases" below — is **no longer
> the survey's position**: the field is now a 6+ framework race with no single
> default. The per-use-case fit scores here still describe those three
> frameworks accurately; what does not survive is the claim that one of them
> covers most of the category. See the refreshed
> S1 recommendation and
> new-frameworks-2026.md. `>`
> CrewAI itself is healthy — v1.15.17, released 2026-08-25, 420 releases on PyPI.
> The retired claim is about its share of the category, not its viability.
...
1. **CrewAI suits role-based multi-agent workflows** — where the work decomposes
   into named roles with defined handoffs. *(This originally read "CrewAI = 80%
   of multi-agent use cases"; that share claim is superseded — see the banner.)*
...
> **🔄 2026 Refresh (verified 2026-05-31):** The viability picture below predates the 2026 field.
> Updated read (all 3–5yr risk **LOW** unless noted): **LangGraph** (production/stateful default,
> $125M-backed), **OpenAI Agents SDK** (first-party, lightweight), **Microsoft Agent Framework**
> (v1.0 GA Apr 2026; **AutoGen is now maintenance-mode** — migrate), **Google ADK** (A2A-native,
> GCP), **CrewAI** (role-based; share eroding vs LangGraph but financially sound), **smolagents**
> (minimal; risk LOW–MEDIUM), **MetaGPT** (OSS stalled; risk **MEDIUM–HIGH**, superseded by OpenHands
> for software-dev). Lock-in is generally LOW at the OSS layer; MEDIUM where a managed platform
> (LangGraph Platform, Vertex Agent Engine, Azure Foundry) or single-vendor model (OpenAI) is adopted.
> See `../S1-rapid/new-frameworks-2026.md`. The rankings below are the original 2025 assessment. `>`
> **🔄 2026-06-26 addendum (verified):** Two first-party SDKs round out the cloud-vendor field as
> peers. **AWS Strands Agents** (open SDK, model-driven; pairs with Bedrock **AgentCore**, GA since
> Oct 2025) for **Bedrock / AWS-native managed deployments** — AWS's analogue to Google ADK+Vertex.
> **Claude Agent SDK** (Anthropic; renamed from Claude Code SDK, Sept 2025) for **OS-access /
> file-shell / MCP-heavy** agentic work on Claude models. Both 3–5yr risk **LOW**; lock-in MEDIUM
> (Strands once AgentCore is adopted; Claude Agent SDK is model-tied). **Note:** effective
> 2026-06-15 the Claude Agent SDK (incl. third-party harnesses and headless `claude -p`) bills from
> a **separate capped credit pool** at API rates, then per-token — **not** from Claude Max-plan
> limits; weigh this for high-volume automation.
