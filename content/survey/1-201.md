...
| **Microsoft Agent Framework** | Microsoft/Azure enterprises, .NET shops, governed production | v1.0 GA 2026-04-03; merges AutoGen + Semantic Kernel; the only first-class enterprise .NET agent framework. 3-5yr risk LOW. Young; single-process today. | 1.15.0 · 2026-08-21 |
| **Google ADK** | Enterprises wanting Google managed deployment plus cross-framework A2A interop; teams on Gemini/GCP | A2A-native; now on the 2.x line with a graph Workflow Runtime (breaking 1.x-to-2.x changes). 3-5yr risk LOW. Deepest value is GCP-bound. | 2.7.1 · 2026-08-17 |
| **CrewAI** | Role-based crews, fastest prototyping (non-graph) | 1.0 GA 2025-10-20, financially sound, risk LOW - but the role-based pick, not the universal 80% default; LangGraph overtook it in enterprise adoption in early 2026. | 1.15.17 · 2026-08-26 |
...
| **AWS Strands Agents** | Teams on AWS/Bedrock wanting an open SDK with a first-party managed deployment path | Open SDK pairing with Bedrock AgentCore (GA since Oct 2025). 3-5yr risk LOW; lock-in MEDIUM once AgentCore is adopted. | 1.53.0 · 2026-08-21 |
| **Claude Agent SDK** | OS-access, tool-heavy and MCP-heavy agentic work on Claude models | 3-5yr risk LOW, model-tied. Since 2026-06-15 it bills from a separate capped credit pool at API rates, then per token - NOT from Claude Max-plan limits. | 0.2.144 · 2026-08-21 |
