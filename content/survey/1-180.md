---
title: "1.180 Python ORMs"
description: "Survey of Python Object-Relational Mapper libraries: SQLAlchemy 2.x, Django ORM, SQLModel, Tortoise ORM, Peewee, PonyORM, and piccolo. Covers sync vs async…"
keywords: ["github stars", "last release", "async", "SQLAlchemy", "Django ORM", "SQLModel", "Tortoise ORM", "Peewee", "PonyORM", "piccolo"]
...
|---|---|---|
| **SQLAlchemy** | Very Active | 2.0.52 · 2026-08-31 |
| **Django ORM** | Very Active | 6.1.1 · 2026-09-02 |
| **SQLModel** | Active | 0.0.42 · 2026-08-28 |
| **Tortoise ORM** | Active | 1.1.8 · 2026-08-10 |
| **Peewee** | Slow | 4.5.1 · 2026-09-08 |
| **PonyORM** | Slow | 0.7.20 · 2026-08-09 |
| **piccolo** | Active | 1.36.0 · 2026-07-27 |
...
- **The framework picks the ORM, not the other way round** — A Django app uses the Django ORM — S4 calls it 'no debate'. FastAPI on Pydantic models uses SQLModel. Only a framework-free project genuinely chooses.
- **SQLAlchemy is the default when nothing else decides for you** — The survey records the community consensus as 'the industry standard Python ORM - use it unless you have a specific reason not to'. It pays for that with the steepest learning curve of the five compared.
- **Exit cost varies more than any feature: SQLModel is trivial to leave, Django ORM is not** — SQLModel is SQLAlchemy underneath, so leaving means removing a layer. Django ORM is framework-coupled and leaving it 'usually means full framework migration'.
- **Async and type safety are where the newer options beat the incumbents** — Tortoise leads on async and SQLModel on type safety, both above SQLAlchemy; Peewee scores lowest on each. If either is the requirement, the incumbents are not the answer.
- **One red flag says you chose wrong: routinely writing raw SQL to work around the ORM** — S4's remedy is to drop to SQLAlchemy Core or to raw SQL with result mapping rather than keep fighting the abstraction.
