...
|---|---|---|---|
| **sortedcontainers** | Pure Python; SortedList is a list of short lists with a positional index | Order that must hold between inserts — ranges, neighbours, rank | 2.4.0 · 2021-05-16 |
| **polars** | Rust, columnar, multi-threaded, lazy | Frames above pandas' comfortable size, or where memory is the constraint | 1.44.1 · 2026-09-02 |
| **pyrsistent** | Persistent vectors/maps/sets with structural sharing; C extension (pvectorc) optional and absent on 3.14 | Shared state where old versions must stay valid — undo, snapshots, concurrency | 0.20.0 · 2023-10-25 |
| **cytoolz / toolz** | Cython compilation of toolz's functional utilities | Functional pipelines at no cost over hand-written loops | 1.1.0 · 2025-10-19 |
| **more-itertools** | Pure Python; the itertools recipes and far more, maintained | Iteration vocabulary the standard library describes but does not ship | 11.1.0 · 2026-05-22 |
| **bintrees** | Binary/AVL/red-black trees in Python with optional Cython trees | Nothing new — migrate to sortedcontainers, as its own README says | 2.2.0 · 2020-12-12 |
...
comparison usually called "pure Python beats C". Measured today it is pure Python beating
...
filter-and-sum 9×, sort 2.6×. The "5–100×" in circulation was measured against an older pandas; the range
...
*Every figure measured 2026-08-19 for the sorted-how floor model
...
above each insert and a list of short lists shifts at most one of them. The compiled tree it is
usually said to beat (bintrees) only ever shipped compiled for Python 3.6–3.9 on Mac
...
- **bintrees** - **DEVELOPMENT STOPPED** (last release 2020-12-12), officially recommends sortedcontainers
...
**Deprecated/Legacy:**
- **bintrees**: Development stopped by its author; last release 2.2.0, 2020-12-12
...
> **Measured (2026-08-19):** SortedList 61 ms vs `bisect.insort` 553 ms at 100K keys; polars 2–9× pandas 3; cytoolz 1.7× toolz; pyrsistent pure Python on 3.14. S2 §0 has the tables. One persona to read alongside these: the engineer who asked for 'sorted' and needed `heapq` or `list.sort()` — S2 §0.2.
...
- Insert: O(log n) - 61 ms for 100K insertions (against 553 ms for `bisect.insort`)
...
1. **Replace bintrees with sortedcontainers** - 8× faster, and bintrees is retired
2. **Evaluate polars for datasets `>1GB`** - 2–9× over pandas 3
