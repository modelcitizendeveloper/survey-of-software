...
description: "Python collections beyond the built-ins: sortedcontainers, polars, pyrsistent, cytoolz and more-itertools — sorted, columnar, persistent and functional."
keywords: ["python collections", "sortedcontainers", "sortedlist", "sorteddict", "polars", "pandas alternative", "pyrsistent", "persistent data structures", "immutable collections", "cytoolz", "toolz", "more-itertools", "bintrees", "data structures", "functional programming"]
weight: 1040000
date: 2026-03-06
lastmod: 2026-03-06
---
...
Python collections beyond the built-ins: sortedcontainers, polars, pyrsistent, cytoolz and more-itertools — sorted, columnar, persistent and functional.

## At a glance


- **sortedcontainers** — The sorted-collection answer, and the survey's most striking result: pure Python that outruns C extensions — roughly 10x faster than bintrees on 100K insertions (332ms vs 3746ms). Drop-in SortedList/SortedDict/SortedSet, Apache-2.0.
- **polars** — The dataframe alternative — Rust-backed, multi-threaded, columnar, with lazy evaluation, reported at 5-100x pandas on common operations. The pick above roughly 10GB or where memory is the constraint.
- **pyrsistent** — Persistent, immutable structures for functional and concurrent code — the choice when shared mutable state is the actual bug you are removing.
- **cytoolz** — Cythonised toolz, 2-5x faster than the pure-Python original, for functional composition over iterables.
- **more-itertools** — The extended iteration vocabulary the standard library stops short of, at 129M+ monthly downloads — the least exotic thing in this list and the most widely installed.


**What the research found**

- This is not one category but four — sorted structures, dataframes, immutability and functional utilities — and the only thing the libraries share is that the built-ins ran out
- sortedcontainers is the counter-intuitive headline: pure Python beating C extensions by ~10x, because algorithmic choice beat implementation language
- polars is the only entry that displaces an incumbent rather than filling a gap, and the crossover is dataset size rather than preference
- more-itertools is effectively standard-library-adjacent by adoption; reaching for it is closer to a default than a dependency decision
