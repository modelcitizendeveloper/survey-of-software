---
title: "1.040 Advanced Collections Libraries"
description: "Python collections beyond the built-ins, measured: sortedcontainers (pure Python, 9x the stdlib's C at keeping order), polars, pyrsistent, cytoolz…"
keywords: ["python collections", "sortedcontainers", "sortedlist", "sorteddict", "bisect insort", "heapq", "polars", "pandas alternative", "pyrsistent", "persistent data structures", "immutable collections", "cytoolz", "toolz", "more-itertools", "bintrees", "data structures", "functional programming"]
weight: 1040000
date: 2025-10-01
lastmod: 2026-08-19
---

# 1.040 Advanced Collections Libraries

Python collections beyond the built-ins, measured: sortedcontainers (pure Python, 9x the stdlib's C at keeping order), polars, pyrsistent, cytoolz, more-itertools — and when a sorted structure is the wrong answer.

> [!NOTE]
> **Quick guide**
>
> **Need it sorted between inserts** (ranges, neighbours, rank)? sortedcontainers — pure Python, 9× the stdlib's `bisect.insort` at 100K keys, 40× at 1M.
> **Only sorted at the end?** `list.append` then `.sort()` — 2.5× faster. **Only the minimum?** `heapq` — 10× faster. **Never?** `dict`.
> **Frames, immutability, functional:** polars (2–9× pandas 3), pyrsistent (updates without copying, 50×), cytoolz (1.7× toolz), more-itertools (vocabulary, no speed claim).

> [!TIP]
> **Try it: [in order](/workshop/in-order/)** — a working demo of this survey's findings.
> Start the race in your browser: a hundred thousand keys kept in order, one insert at a time — pure-Python sortedcontainers against the standard library's C, bars arriving in the order the structures finish — then the operations that decide whether you needed it sorted at all.

## At a glance

<div class="sos-tiles"><div class="sos-tile"><div class="sos-tile-h">pure Python SortedList vs the stdlib C at 1M keys</div><div class="sos-tile-n">40&times;</div><div class="sos-tile-s"><strong>SortedList</strong> 1.4 s &middot; bisect.insort 56 s</div></div><div class="sos-tile"><div class="sos-tile-h">pyrsistent 1000 updates of a 100K vector</div><div class="sos-tile-n">64&times;</div><div class="sos-tile-s"><strong>pvector.set</strong> 6 ms &middot; copy a tuple each time 381 ms</div></div></div>

<figure class="sos-fig"><figcaption>insert 100K keys one at a time keeping order <span>lower is faster</span></figcaption><svg viewBox="0 0 720 192" role="img" width="100%" height="192" aria-label="insert 100K keys one at a time keeping order, compared"><title>heapq, min only: 6 ms</title><text x="158" y="17.68" class="sos-bl" text-anchor="end">heapq, min only</text><rect x="168" y="0" width="5.3" height="26" rx="4" class="sos-bar"/><text x="181.3" y="17.68" class="sos-bv">6 ms</text><title>dict, no order: 10 ms</title><text x="158" y="49.68" class="sos-bl" text-anchor="end">dict, no order</text><rect x="168" y="32" width="8.8" height="26" rx="4" class="sos-bar"/><text x="184.8" y="49.68" class="sos-bv">10 ms</text><title>append then sort once: 24 ms</title><text x="158" y="81.68" class="sos-bl" text-anchor="end">append then sort once</text><rect x="168" y="64" width="21.1" height="26" rx="4" class="sos-bar"/><text x="197.1" y="81.68" class="sos-bv">24 ms</text><title>SortedList, pure Python: 61 ms</title><text x="158" y="113.68" class="sos-bl" text-anchor="end">SortedList, pure Python</text><rect x="168" y="96" width="53.6" height="26" rx="4" class="sos-bar"/><text x="229.6" y="113.68" class="sos-bv">61 ms</text><title>bintrees RBTree, pure Python today: 506 ms</title><text x="158" y="145.68" class="sos-bl" text-anchor="end">bintrees RBTree, pure Python today</text><rect x="168" y="128" width="444.7" height="26" rx="4" class="sos-bar"/><text x="620.7" y="145.68" class="sos-bv">506 ms</text><title>list + bisect.insort, stdlib C: 553 ms</title><text x="158" y="177.68" class="sos-bl" text-anchor="end">list + bisect.insort, stdlib C</text><rect x="168" y="160" width="486.0" height="26" rx="4" class="sos-bar"/><text x="662.0" y="177.68" class="sos-bv">553 ms</text></svg></figure>


| Library | How it works | Best for |
|---|---|---|
| **sortedcontainers** | Pure Python; SortedList is a list of short lists with a positional index | Order that must hold between inserts — ranges, neighbours, rank |
| **polars** | Rust, columnar, multi-threaded, lazy | Frames above pandas' comfortable size, or where memory is the constraint |
| **pyrsistent** | Persistent vectors/maps/sets with structural sharing; C extension (pvectorc) optional and absent on 3.14 | Shared state where old versions must stay valid — undo, snapshots, concurrency |
| **cytoolz / toolz** | Cython compilation of toolz's functional utilities | Functional pipelines at no cost over hand-written loops |
| **more-itertools** | Pure Python; the itertools recipes and far more, maintained | Iteration vocabulary the standard library describes but does not ship |
| **bintrees** | Binary/AVL/red-black trees in Python with optional Cython trees | Nothing new — migrate to sortedcontainers, as its own README says |


**What the research found**

- **Pure Python beats the standard library's C at keeping things sorted — 9x at 100K keys, 40x at 1M** — SortedList inserts 100K random keys in 61 ms; bisect.insort on a flat list takes 553 ms, because each insert moves half the list. At 1M it is 1.4 s against 56 s. Below about 5K keys the flat list wins; above, it loses by more every decade. The C extension sortedcontainers famously beat (bintrees) no longer ships compiled for any current Python — what it still beats is the standard library's own.
- **Most people who ask for 'sorted' need something cheaper** — Order only at the end: append then sort, 24 ms — 2.5x faster than keeping order as you go. Only the minimum: heapq, 6 ms — 10x faster, and 5x faster to pop. Membership: a set, 12x faster than any sorted structure. A sorted structure pays exactly when order must hold between inserts — ranges, neighbours, rank — and that is the question to ask first.
- **polars is 2-9x pandas 3, not 5-100x** — 2M rows, 8 threads, pandas 3.0.5 with Arrow strings: group-by with three aggregates 2.3-4x (two runs), filter+sum 9x, sort 2.6x. pandas got faster; polars is still the pick above roughly 10 GB or where memory is the constraint, for a smaller and more honest reason.
- **pyrsistent's case holds without its C extension, which 3.14 does not have** — pvectorc is not built for CPython 3.14; the library runs pure Python there. 1,000 single-element updates of a 100K vector still cost 6 ms against 381 ms copying — structural sharing is the point, and it is algorithmic. The price is construction: 20x slower than a list to build by append.
- **cytoolz is 1.7x toolz, and the standard library by hand sits between them** — One pipeline — groupby, frequencies(pluck), reduceby — over 200K records: cytoolz 30 ms, defaultdict/Counter by hand 46 ms, toolz 52 ms. The case for cytoolz is that the functional style stops costing anything; the case for toolz alone is thin.
...
# Domain Explainer: Advanced Collections Libraries
...
"I need this collection sorted" means one of four things, and each has a different right answer.
Measured for the survey on one machine, 100,000 random keys inserted one at a time:

| You mean… | Use | 100K keys |
|---|---|---|
| sorted **at every moment** — ranges, neighbours, rank between inserts | **sortedcontainers** `SortedList` (pure Python) | **61 ms** — the stdlib's C, `bisect.insort`, takes 553 ms |
| sorted **at the end** | `list.append`, then `.sort()` once | 24 ms — 2.5× faster than keeping order as you go |
| **smallest-first** only | `heapq` | 6 ms — and 5× faster to pop |
| **never** sorted | `dict` / `set` | 10 ms — and membership 12× faster than anything sorted |

The other three corners of this survey are separate questions with separate answers:
**polars** when a dataframe is too big for pandas (2–9× pandas 3 on 2M rows); **pyrsistent**
when old versions of a value must stay valid (1,000 updates of a 100K vector: 6 ms against 381 ms
copying); **cytoolz** when you write functionally and want it to cost nothing (1.7× toolz, 1.5× the
by-hand version); **more-itertools** for iteration vocabulary, no speed claim made.

That is the finding. The rest of this page is why, and the race itself runs in your browser at
[/workshop/in-order/](/workshop/in-order/).

---

## The one-paragraph version

This is four categories wearing one label. The only thing sortedcontainers, polars, pyrsistent and
the toolz family share is that Python's built-in list, dict and set ran out — for four unrelated
reasons: order that must hold between inserts; columns too wide for pandas; values that must not
change under someone else's feet; and a functional style that reads well and ran slowly. The
headline — pure Python outrunning compiled code at keeping a collection sorted — is real and
measured, and the build found it needs restating: the compiled tree it famously beat no longer
ships for any current Python, and what it beats today, by ten times at a hundred thousand keys and
forty at a million, is the standard library's own C.

---

## What a sorted structure actually does, and why pure Python wins

**The flat list.** `bisect.insort` keeps a Python list sorted by finding the spot (binary search,
fast) and then moving every pointer above it one slot to the right (a `memmove`, also fast — but
proportional to how much list sits above the insert). Average cost per insert grows with the list;
total cost for N inserts grows with N². At 1,000 keys that memmove is trivial and the flat list
wins the race. At 100,000 it is 553 ms; at 1,000,000 it is 56 seconds.

**The list of lists.** `SortedList` keeps many short lists — about a thousand items each — and an
index of their lengths. An insert is a bisect to find the right sublist, a bisect within it, and
a shift of at most a thousand pointers. That is why it scales: 61 ms at 100K, 1.4 s at 1M — and why
it can be pure Python and still win. The algorithm beat the implementation language, which is the
survey's founding observation.

**The tree.** bintrees' red-black tree does one pointer-chase per level, in Python, for every
insert and every comparison — 506 ms for the 100K race, 8× behind SortedList — and this is the
comparison the 2025 edition called "pure Python beats C". Measured today it is pure Python beating
pure Python: the compiled trees only ever shipped as wheels for Python 3.6–3.9 on Mac and Windows.
On any current interpreter `FastRBTree` *is* `RBTree`. Development stopped in 2017; the README
says to use sortedcontainers.

**When you did not need it sorted.** Every other structure in the race is faster than SortedList,
because each of them refuses part of the job. Append-then-sort refuses order *between* inserts
(24 ms). A heap refuses everything but the minimum (6 ms, and pops 5× faster than SortedList).
A dict refuses order altogether (10 ms, membership 12× faster). The question to ask first is
which part of the job you can refuse; sortedcontainers is the answer only when the answer is
"none of it".

---

## The other three corners, briefly

**polars (dataframes).** Rust, columnar, multi-threaded, lazy. Measured on 2M rows with 8 threads
against pandas 3.0.5 (which now has Arrow-backed strings): group-by with three aggregates 2.3–4.0× (two runs),
filter-and-sum 9×, sort 2.6×. The 2025 figure of "5–100×" was against an older pandas; the range
is real and its top is gone. The crossover is still dataset size and memory, not taste.

**pyrsistent (persistence).** A persistent vector gives you a new value on every change without
copying the old one — structural sharing, the trick from Clojure. 1,000 single-element updates of a
100K-element vector: 6 ms, against 381 ms copying a tuple each time. The price is construction
(100K appends: 20× slower than a list) and, on CPython 3.14, the absence of the C extension the
survey once credited with 2–20×: `pvectorc` is not built for it, the library runs pure Python,
and its case holds anyway because the win is algorithmic.

**cytoolz / toolz (functional).** One pipeline — group by region, count users, sum bytes per user —
over 200K records: cytoolz 30 ms, hand-written `defaultdict`/`Counter` 46 ms, toolz 52 ms. cytoolz
is 1.7× toolz, not the 2–5× recorded, and the interesting number is the middle one: plain toolz
costs you against the standard library, cytoolz makes the style free.

**more-itertools (vocabulary).** 348M downloads a month — more than anything else here — for
`chunked`, `windowed`, `partition`, `unique_everseen` and two hundred relatives: the standard
library's itertools recipes, done once, maintained. No performance claim is made and none was
tested.
...
1. **Which of the four "sorted" meanings is yours?** If order must hold between inserts, S1 sends
   you to sortedcontainers and the floor model shows you `irange`. If not, the standard library
   has a faster answer and no dependency.
2. **How many keys?** Below a few thousand, a flat list and `bisect` is simplest and fastest.
3. **Is the data a frame, a shared value, or a pipeline?** Those are the other three corners, and
   they do not compete with each other or with the first.
4. **Is this already in your requirements file?** bintrees and blist are inherited, not chosen;
   both have successors.

S1 gives the pick in a page. S2 §0 has the measured race from 1K to 1M, the operations, and the
other three corners. S3 walks personas; S4 is the viability case.

---

## Where to go next

- **Run the race on your machine.** [/workshop/in-order/](/workshop/in-order/) — pick a size, start
  it, watch the bars arrive in finishing order. Then the operations panel asks whether you needed
  order at all.
- **S1** — the pick, and the corrected numbers.
- **S2 §0** — the measured tables and the verification record.
- **S3** — personas, from the leaderboard keeper to the undo-stack author.
- **S4** — strategic viability.

## Related surveys

- **1.001 Sorting** — sorting once; this survey is about staying sorted. Its floor model, sortie,
  is this one's elder sibling.
- **1.004 Binary Search Trees** — the structure bintrees implemented and sortedcontainers replaced.
- **1.042 Tries**, **1.047 Caching** — the neighbouring data-structure questions.

</details><details>
<summary>S1: Rapid Discovery</summary>

# S1 - Rapid Discovery: Advanced Collections Libraries

*Rewritten 2026-08-19 after every figure was measured for the in-order floor model
(`/workshop/in-order/`). Download counts are pypistats.org, last 30 days, read 2026-08-19.
Timings are what a Python caller sees on one machine (CPython 3.14.7, aarch64, single thread
unless stated); the full sweep and the method are in S2 §0, and the bench imports the page's
own `core.py`.*
...
| The built-in ran out because… | Use | Measured |
|---|---|---|
| order must hold **between inserts** | **sortedcontainers** `SortedList` / `SortedDict` / `SortedSet` | 100K random inserts: **61 ms**, against 553 ms for `bisect.insort` and 506 ms for the tree it replaced; 1M: 1.4 s against 56 s |
| order is only needed **at the end** | `list.append` + `.sort()` (stdlib) | 24 ms — 2.5× faster than SortedList |
| only the **minimum** matters | `heapq` (stdlib) | 6 ms; 10K pops 5× faster than SortedList |
| a frame is **too big for pandas** | **polars** | 2M rows, 8 threads: 2.3–4× group-by, 9× filter, 2.6× sort over pandas 3.0.5 |
| old **versions must stay valid** | **pyrsistent** | 1,000 updates of a 100K vector: 6 ms against 381 ms copying |
| functional style should **cost nothing** | **cytoolz** | 1.7× toolz; 1.5× a hand-written `defaultdict`/`Counter` |
| you keep rewriting itertools recipes | **more-itertools** | 348M downloads/mo; no speed claim |

```python
from sortedcontainers import SortedList
sl = SortedList()
for k in stream: sl.add(k)                    # stays sorted; 61 ms per 100K
sl.irange(lo, hi)                             # the operation a set cannot do
sl.bisect_left(k); sl[-1]; sl.pop(0)          # rank, max, min

# and the three cheaper answers when you can refuse part of the job:
l.append(k); l.sort()                         # order at the end only
heapq.heappush(h, k); heapq.heappop(h)        # minimum only
d[k] = v                                      # no order
```

## The race, measured

Random 40-bit keys inserted one at a time; the flat list wins early and loses by more every
decade:

| keys | SortedList (pure Python) | `bisect.insort` (stdlib C) | append + sort | heapq | dict | bintrees RBTree (pure Python today) |
|---|---|---|---|---|---|---|
| 1,000 | 0.3 ms | **0.2 ms** | 0.09 ms | 0.05 ms | 0.03 ms | 2.7 ms |
| 10,000 | **4.1 ms** | 5.1 ms | 1.3 ms | 0.5 ms | 0.4 ms | 38 ms |
| 100,000 | **61 ms** | 553 ms | 24 ms | 6 ms | 10 ms | 506 ms |
| 1,000,000 | **1.4 s** | 56 s | 416 ms | 65 ms | 120 ms | — |

Downloads a month: more-itertools 348M, sortedcontainers 275M, polars 79M, toolz 64M,
pyrsistent 40M, cytoolz 10M, bintrees 20K, blist 7K.

## Three things that decide most choices

**"Pure Python beats C" is true — of the standard library's C.** SortedList beats
`bisect.insort` by 9× at 100K keys and 40× at 1M, because a flat list must shift everything
above each insert and a list of short lists shifts at most one of them. The compiled tree the
2025 edition compared against (bintrees) only ever shipped compiled for Python 3.6–3.9 on Mac
and Windows; on any current interpreter it is pure Python, and loses by 8×.

**Most "sorted" requirements are cheaper than sorted.** If you can refuse part of the job — order
between inserts — the standard library is faster and dependency-free. The floor model's second
panel is that question.

**The other three corners are not in competition.** polars, pyrsistent and cytoolz answer
different questions than sortedcontainers and than each other; the only thing to get wrong is
reaching for one of them for the other's problem.

## When each one stops being a reason

| You chose | It stops paying when |
|---|---|
| sortedcontainers | you only read the order once, or only ever take the minimum |
| a flat list + bisect | you pass a few thousand keys — then it loses by more every decade |
| polars | the frame fits comfortably in pandas and the team knows pandas |
| pyrsistent | nothing holds old versions — then you pay 20× on construction for nothing |
| toolz (without cy) | it is slower than the standard library by hand; use cytoolz or loops |
| bintrees / blist | always — both are retired; migrate to sortedcontainers |

## Install

```bash
pip install sortedcontainers     # pure Python, Apache-2.0
pip install polars               # Rust wheels
pip install pyrsistent           # pure Python on 3.14 (pvectorc not built); fine
pip install cytoolz              # brings toolz
pip install more-itertools
```

## What changed since the 2025 edition

The headline ratio held (~8–9× at 100K) and its framing did not: the comparison is against
the standard library's C now, because bintrees' C trees do not exist for any current Python.
"polars 5–100×" is 2–9× against pandas 3. "cytoolz 2–5×" is 1.7×. pyrsistent's "C extension
2–20×" is not built for 3.14, and its case holds regardless. The record is S2 §0.

---

**Date compiled**: 2025-10 · **Measured and rewritten**: 2026-08-19
...
(Qualcomm Snapdragon X under WSL2), single thread unless stated, best-of wall time — with the
bench importing the floor model's own `core.py` (`01-discovery/bench/`), and the sorted race run
again in the browser at [/workshop/in-order/](/workshop/in-order/) (Pyodide 314.0.5, Python
3.14.2) at about half native speed, same shape, same finishing order. Versions: sortedcontainers
2.4.0, bintrees 2.2.0, pyrsistent 0.20.0, toolz 1.1.0, cytoolz 1.1.0, more-itertools 11.1.0,
polars 1.43.2, pandas 3.0.5.

### 0.1 The race — N random 40-bit keys, inserted one at a time

| keys | SortedList (pure Python) | `bisect.insort` on a list (stdlib C) | `append` then `sort` | `heapq.heappush` | `dict` | bintrees `RBTree` (pure Python on 3.14) |
|---|---|---|---|---|---|---|
| 1,000 | 0.3 ms | **0.2 ms** | 0.09 ms | 0.05 ms | 0.03 ms | 2.7 ms |
| 10,000 | **4.1 ms** | 5.1 ms | 1.3 ms | 0.5 ms | 0.4 ms | 38 ms |
| 100,000 | **61 ms** | 553 ms | 24 ms | 6 ms | 10 ms | 506 ms |
...
In the browser (Pyodide, one tab, Chromium): 100K keys — SortedList 136 ms, insort 1.43 s,
append+sort 41 ms, heapq 11 ms, dict 6 ms; 10K — 9.0 ms vs 16 ms; 1K — 0.7 ms vs 0.3 ms. Same
crossover, same order.

### 0.2 Did you need it sorted? — operations at 100,000 keys

| operation | SortedList | flat list + bisect | set / heapq |
|---|---|---|---|
| 10,000 membership tests | 3.6–4.6 ms | 3.4–3.8 ms | **set: 0.35 ms** |
| 100 range queries | 0.5 ms (`irange`) | 0.1 ms (two bisects + slice) | — (not possible) |
| 10,000 pop-the-minimum | 26–33 ms | 246 ms (`list.pop(0)`) | **heapq: 5.5–10.6 ms** |

### 0.3 The other three corners

| measurement | result |
|---|---|
| polars vs pandas 3.0.5, 2M rows, 8 threads — group-by with three aggregates | pandas 130 ms, polars 32 ms — **4.0×** |
| same — filter + sum | pandas 113 ms, polars 12 ms — **9.3×** |
| same — sort by two columns | pandas 333 ms, polars 131 ms — **2.6×** |
| pyrsistent: 1,000 single-element updates of a 100K-element vector | `pvector.set` 6 ms; copying a tuple each time 381 ms — **50–65×**; implementation on 3.14: pure Python (`pvectorc` not built) |
| pyrsistent: build 100K by append | `pvector.append` 38–40 ms; `list.append` 1.8–2.1 ms — 20× |
| one functional pipeline (groupby + frequencies(pluck) + reduceby), 200K records | toolz 52 ms; hand-written `defaultdict`/`Counter` 46 ms; cytoolz 30 ms — cytoolz **1.7×** toolz, 1.5× by hand |

### 0.4 What the numbers say

1. **Pure Python beats the standard library's C at keeping things sorted — 9× at 100K, 40× at
   1M.** A flat list moves half of itself per insert; a list of ~1,000-item lists moves at most
   one of them. Below ~5K keys the flat list wins.
2. **The compiled opponent is gone.** bintrees' C trees shipped as wheels only for Python 3.6–3.9
   on Mac/Windows (PyPI file list, 2020-12-12); on any current interpreter `FastRBTree is RBTree`
   and it finishes 8× behind. The survey's headline ratio held; its framing is restated.
3. **Most "sorted" is cheaper than sorted.** Order at the end: 2.5× faster. Minimum only: 10×
   faster to build, 5× to pop. Membership: 12× faster with a set. A sorted structure pays exactly
   when order must hold between inserts.
4. **polars is 2–9× pandas 3**, not 5–100×; pandas got faster. Still the pick on size and memory.
5. **pyrsistent's case is algorithmic** and survives losing its C extension on 3.14.
6. **cytoolz is 1.7× toolz**, and plain toolz is slower than the standard library by hand.
7. **more-itertools** makes no performance claim and is the most installed library in the survey.

### 0.5 What did not survive verification

| Recorded | Actual |
|---|---|
| "pure Python beating C extensions by ~10x (332 ms vs 3746 ms vs bintrees, 100K)" | ratio holds (61 vs 506 ms, 8.3×) — but the opponent is pure Python on every current interpreter; the C that loses by 9× is `bisect.insort` |
| "polars 5–100× pandas" | 2–9× against pandas 3.0.5, 8 threads |
| "cytoolz 2–5× toolz" | 1.7× |
| "pyrsistent C-extension available (2–20× speedup)" | `pvectorc` not built for CPython 3.14; pure Python there; 50× claim on updates holds regardless |
| nothing on when NOT to use a sorted structure | added — S1's first table, the floor model's second panel |
...
> **Note (2026-08-19):** figures quoted below are the 2025 edition's. Measured for the floor model: SortedList 61 ms vs `bisect.insort` 553 ms at 100K keys (the C opponent is now the standard library's, not bintrees'); polars 2–9× pandas 3; cytoolz 1.7× toolz; pyrsistent pure Python on 3.14. Add one persona the edition lacked: the engineer who asked for 'sorted' and needed `heapq` or `list.sort()` — S2 §0.2.
...
> **Note (2026-08-19):** bintrees' compiled trees do not exist for any current Python, which makes its retirement complete rather than advisory; sortedcontainers 2.4.0 (2021) has had no release because it has needed none; pyrsistent's C extension is absent on 3.14 and its value is algorithmic. Download counts 2026-08-19: more-itertools 348M/mo, sortedcontainers 275M, polars 79M, toolz 64M, pyrsistent 40M, cytoolz 10M. Measured figures: S2 §0.
