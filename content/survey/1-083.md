...
description: "Point cloud processing: Open3D, PCL, PDAL, laspy, trimesh and CloudCompare — registration, segmentation and LiDAR formats for 3-D scan data."
keywords: ["4PS", "S1-rapid", "S2-comprehensive", "S3-need-driven", "S4-strategic", "primary", "supplementary", "Open3D", "PCL", "PDAL", "Potree", "pyntcloud", "cilantro", "pclpy", "laspy"]
...
Point cloud processing: Open3D, PCL, PDAL, laspy, trimesh and CloudCompare — registration, segmentation and LiDAR formats for 3-D scan data.
...
| Library | Group | Language | Recommendation |
|---|---|---|---|
| **Open3D** | primary | Python/C++ | Primary choice for most use cases |
| **PCL (Point Cloud Library)** | primary | C++ | Mandatory for ROS, conditional otherwise |
| **PDAL** | primary | C/C++ | Mandatory for geospatial workflows |
| **Potree** | primary | JavaScript | Standard for web visualization |
| **pyntcloud** | supplementary | Python | Learning/education only |
| **cilantro** | supplementary | C++ | Performance optimization supplement |
| **pclpy** | supplementary | Python/C++ | When PCL algorithms needed from Python |
| **laspy** | supplementary | Python | Python LAS/LAZ I/O |


**What the research found**

- Open3D emerges as default choice for Python-first teams (modern API, ML integration, active development)
- PCL mandatory for ROS but declining outside robotics (maintenance mode, complex API)
- PDAL has geospatial monopoly (30+ formats, CRS handling, no viable alternative)
- Potree standard for web visualization (billion-point scale, browser-based, no competition)
- Multi-library stacks are normal (PDAL for I/O, Open3D for processing, Potree for viz)


**What the research recommends**

| Persona | Primary | Supplementary | Avoid |
|---|---|---|---|
| Robotics Engineer (ROS/SLAM) | PCL | Open3D (offline); cilantro (optimization) | pyntcloud (too slow); PDAL (no ROS) |
| Geospatial Analyst (LiDAR) | PDAL | laspy (Python scripts); Open3D (custom analysis); Potree (web delivery) | PCL (no geospatial); pyntcloud (scale) |
| ML Researcher (3D CV) | Open3D | PyTorch Geometric (datasets/GNN); trimesh (mesh hybrid) | PCL (C++ friction); PDAL (geospatial focus) |
| Startup (New Product) | Open3D | Potree (if web needed) | PCL (too complex); pyntcloud (performance ceiling) |
| Enterprise (Multi-Domain) | Open3D (default) | PDAL (GIS division); PCL (robotics division); Commercial (if certification) | — |
...
Not centroid-based—random sampling within voxel.
...
Point cloud size: 1M points × 12 bytes = 12 MB
