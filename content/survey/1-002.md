---
title: "1.002 Fuzzy String Search Libraries"
description: "Python fuzzy string matching: RapidFuzz, TheFuzz, python-Levenshtein, Jellyfish and difflib — throughput, license, and when phonetics beat edit distance."
keywords: ["fuzzy search", "fuzzy string matching", "rapidfuzz", "fuzzywuzzy", "thefuzz", "python-levenshtein", "jellyfish", "difflib", "levenshtein distance", "jaro-winkler", "soundex", "metaphone", "record linkage", "deduplication", "approximate string matching"]
...
# 1.002 Fuzzy String Search Libraries

Python fuzzy string matching: RapidFuzz, TheFuzz, python-Levenshtein, Jellyfish and difflib — throughput, license, and when phonetics beat edit distance.
...
|---|---|---|
| **RapidFuzz** | Default choice — measured ~2,500 pairs/sec against FuzzyWuzzy's ~1,200, MIT licensed, and a drop-in API replacement. Migration is an import change. | 3.14.6 · 2026-08-30 |
| **TheFuzz (formerly FuzzyWuzzy)** | The legacy incumbent, renamed in 2021. Battle-tested and well documented, but GPL-2.0 and the slowest of the dedicated libraries. Reach for it only where a codebase already depends on it. | 0.22.1 · 2024-01-19 |
| **python-Levenshtein** | Narrow and fast (~1,800 pairs/sec) when pure Levenshtein distance is all you need, including over non-Latin scripts. | 0.27.4 · 2026-08-08 |
| **Jellyfish** | The phonetic specialist — Soundex, Metaphone, NYSIIS. The right tool for name matching; struggles on long text. | 1.2.1 · 2025-10-11 |
| **difflib (standard library)** | Zero dependencies and slowest (~1,000 pairs/sec), using Ratcliff-Obershelp. Fine for small datasets where adding a dependency is the larger cost. | — |
...
- RapidFuzz supersedes FuzzyWuzzy on both axes that matter — roughly twice the throughput and MIT instead of GPL-2.0 — while keeping the same API, so there is rarely a reason to start elsewhere
- The license is the real fork: FuzzyWuzzy's GPL-2.0 is what pushes commercial users off the incumbent, not its speed
- Phonetic matching is a different problem from edit distance; Jellyfish exists because Soundex-class algorithms answer 'sounds like' questions that Levenshtein cannot
- difflib is not a fallback for the others — it is the choice when a new dependency costs more than the throughput gap
