...
- **RapidFuzz** — Default choice — measured ~2,500 pairs/sec against FuzzyWuzzy's ~1,200, MIT licensed, and a drop-in API replacement. Migration is an import change.
- **TheFuzz (formerly FuzzyWuzzy)** — The legacy incumbent, renamed in 2021. Battle-tested and well documented, but GPL-2.0 and the slowest of the dedicated libraries. Reach for it only where a codebase already depends on it.
- **python-Levenshtein** — Narrow and fast (~1,800 pairs/sec) when pure Levenshtein distance is all you need, including over non-Latin scripts.
- **Jellyfish** — The phonetic specialist — Soundex, Metaphone, NYSIIS. The right tool for name matching; struggles on long text.
