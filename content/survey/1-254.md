...
| **Testing Library** | Component tests in React, Vue, Svelte, Angular and React Native | An addition to your runner choice, never a replacement for it — Testing Library is not a test runner and needs Jest, Vitest or Mocha underneath. Budget for both. It remains the de facto component standard: 68.2M weekly downloads for @testing-library/dom and 55.7M for @testing-library/react, verified 2026-08-25. Commit activity is quiet for a package this widely used (dom-testing-library last pushed 2025-11-12, react-testing-library 2026-04-02), which reads as a mature API rather than a stalled one, but is worth re-checking on your own schedule. MIT. | 16.3.2 · 2026-01-19 |
...
- Playwright has won end-to-end outright — 85.0M weekly downloads and 95,122 GitHub stars against Cypress's 7.4M and 51,000 (2026-08-25). For new E2E work this is no longer a selection question; the only live question is whether to migrate an existing Cypress suite.
- Jest is not dying and does not need replacing on a deadline: 47.2M weekly downloads and same-day repo commits. But its stable cadence has slowed (30.2.0 Sept 2025, 30.3.0 March 2026, 30.4.2 May 2026), and it stays the required runner for React Native, where the survey finds no viable alternative.
- pytest is uncontested in Python — 1.09 billion PyPI downloads in the last month, 9.1.1 released 2026-06-19. The survey evaluates no runner-up worth comparing, so the Python decision is made before you start.
- Testing Library is a companion, not a runner: it layers on Jest, Vitest or Mocha, so a component-testing plan costs you two dependencies, not one. Teams that budget for it as a runner replacement pick twice.
