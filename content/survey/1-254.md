---
title: "1.254 Testing Libraries"
description: "Testing libraries across the stack: pytest, Jest, Vitest, Mocha, Testing Library, Playwright and Cypress — unit, component and end-to-end boundaries."
keywords: ["cypress", "jest", "playwright", "pytest", "testing", "vitest", "benchmark"]
aliases: ["/survey/1-118/"]
...
# 1.254 Testing Libraries

Testing libraries across the stack: pytest, Jest, Vitest, Mocha, Testing Library, Playwright and Cypress — unit, component and end-to-end boundaries.
...
|---|---|---|---|
| **Vitest** | New JavaScript/TypeScript unit and integration suites; Vite and TypeScript-first codebases | The default choice for new JS/TS test suites. Verified 2026-08-25: 95.2M weekly npm downloads, which is roughly twice Jest's 47.2M — the survey's 2025 framing of Vitest "eating Jest's lunch" has already resolved. Stable line is 4.1.11 (2026-08-18) with 5.0 at release candidate, so expect a major-version migration soon. MIT. | 2.0 · 2023-05-08 |
| **Jest** | React Native (no viable alternative); large existing Jest suites not worth migrating | Stable, not abandoned, but no longer the default. Verified 2026-08-25: 47.2M weekly downloads and repo commits the same day, though stable releases have slowed markedly — 30.2.0 (2025-09-28), 30.3.0 (2026-03-10), 30.4.2 (2026-05-09). Keep it for React Native; plan a Vitest migration for strategic web codebases. MIT. | 30.5.1 · 2026-09-01 |
| **pytest** | Any Python codebase — web, data science, ML, CLI, infrastructure | Uncontested in Python, and the survey evaluates no runner-up worth the comparison. Verified 2026-08-25: 1.09 billion PyPI downloads in the last month, version 9.1.1 released 2026-06-19, repo active. Choose it by default; reach for stdlib unittest only on trivial projects. MIT. | 9.1.1 · 2026-06-19 |
| **Playwright** | New end-to-end suites; cross-browser validation and polyglot teams | The E2E winner outright, not a co-leader. Verified 2026-08-25: 85.0M weekly downloads and 95,122 GitHub stars against Cypress's 7.4M and 51,000 — an 11x distribution gap. Version 1.62.1 (2026-07-30), Microsoft-maintained, with bindings beyond JavaScript. Apache-2.0, the only non-MIT tool in this inventory, which matters if your license policy allowlists MIT only. | 1.62.0 · 2026-07-31 |
| **Cypress** | Maintaining an existing Cypress suite; Chromium-focused JavaScript-only teams | Still actively shipping — 15.21.1 published 2026-08-25, the day this was checked — so the survey's 2025 "viability concerns" framing overstates the risk of the code going dark. The cost is distribution: Playwright now has ~11x its weekly downloads (7.4M vs 85.0M), which decides hiring, examples and plugin support. Reasonable to maintain, hard to justify starting. Parallel execution needs paid Cypress Cloud. MIT. | 16.0.0 · 2026-09-01 |
| **Testing Library** | Component tests in React, Vue, Svelte, Angular and React Native | An addition to your runner choice, never a replacement for it — Testing Library is not a test runner and needs Jest, Vitest or Mocha underneath. Budget for both. It remains the de facto component standard: 68.2M weekly downloads for @testing-library/dom and 55.7M for @testing-library/react, verified 2026-08-25. Commit activity is quiet for a package this widely used (dom-testing-library last pushed 2025-11-12, react-testing-library 2026-04-02), which reads as a mature API rather than a stalled one, but is worth re-checking on your own schedule. MIT. | 16.3.3 · 2026-08-27 |
...
- **pytest-asyncio is the most-installed pytest plugin at 51.9M weekly downloads -- 27% of pytest's own 192.1M -- and it is the only one in the layer that supplies CAPABILITY rather than ergonomics. Without it pytest collects an async test, calls it, gets a coroutine that never runs, and reports a pass.** — A green test that executed nothing is the worst failure mode in a test suite. Take it if you write async tests; there is no alternative. Note it reached 1.0 only in 2026 after years at 0.x, with fixture-scoping changes between minors -- pin it, and set asyncio_mode = strict so an undriven coroutine is an error rather than a pass.
- **Several widely-installed pytest plugins are ergonomics over something that already works, and the wrapped thing is MORE installed than the wrapper: coverage 62.8M against pytest-cov 45.5M, and pytest-mock 19.2M wraps unittest.mock from the standard library.** — Know what you are buying. coverage.py runs fine as `coverage run -m pytest`, and unittest.mock costs nothing. The plugins buy fixture-scoped teardown and not needing a wrapper command -- convenience, not capability. Their absence is an inconvenience rather than a gap, which matters for a project minimizing dependencies.
- **Licensing does not differentiate this layer. Everything is MIT, Apache-2.0 or BSD except hypothesis at MPL-2.0, which is file-level weak copyleft on a dependency that is not distributed with your product.** — Completes a pattern across the range rather than repeating it: 1.250 found license decisive for a vendor EMBEDDING a linter (Pylint is GPL), 1.251 found a different shape again (pyright open, Pylance proprietary above it), and here it binds nobody.
- **pytest's extension point is not a moat around the product -- it IS the product. Hooks, fixtures and a pytest11 entry point mean installing a plugin activates it, and almost everything teams rely on (async, coverage, parallelism, mocking ergonomics) is written by somebody else.** — Sharpens the pattern this range keeps finding. In linting (1.250) and type checking (1.251) the plugin layer defends an incumbent core that does the real work against a faster rewrite. In testing the core is deliberately small and the LAYER does the real work -- which is why 'pytest versus X' comparisons that ignore plugins are comparing the smaller half.
- Vitest has passed Jest, not merely caught up: 95.2M weekly npm downloads vs Jest's 47.2M (npm registry, 2026-08-25). For a new JavaScript or TypeScript suite Vitest is the default and Jest is the thing you migrate away from.
...
# The pytest plugin layer

**Added 2026-08-29 as an extension, not a new survey.** pytest is covered elsewhere in this
survey; its plugin layer was not covered at all, and a stack trace over 16 real application
manifests found three plugins nobody had written about. Every figure below was read from the
PyPI JSON API and pypistats.org on **2026-08-29**.

## Why the layer deserves its own file

The other surveys in the 1.250-259 range keep arriving at the same finding from different
directions: **the incumbent's durable advantage is an extension point the faster rewrite
cannot reproduce.** Flake8 and ESLint hold off Rust linters that way (1.250); mypy holds off
pyright and ty that way (1.251).

pytest is the case where the extension point is not a defensive moat but **the product
itself.** Its core is a runner and a fixture system; almost everything a team actually relies
on — async support, coverage, parallelism, mocking ergonomics — is a plugin somebody else
wrote. A survey that describes the runner and stops has described the smaller half.

## The layer, measured

| Plugin | Version (date) | Weekly downloads | Share of pytest's own | License |
|---|---|---|---|---|
| **pytest** | 9.1.1 (2026-06-19) | 192,082,201 | — | MIT |
| **pytest-asyncio** | 1.4.0 (2026-05-26) | **51,895,598** | 27% | Apache-2.0 |
| **pytest-cov** | 7.1.0 (2026-03-21) | 45,520,974 | 24% | MIT |
| **pytest-xdist** | 3.8.0 (2025-07-01) | 29,018,329 | 15% | MIT |
| **pytest-mock** | 3.15.1 (2025-09-16) | 19,234,207 | 10% | MIT |
| hypothesis | 6.167.0 (2026-08-30) | 9,955,378 | 5% | **MPL-2.0** |
| pytest-django | 4.14.0 (2026-08-10) | 4,536,811 | 2% | BSD |
| pytest-benchmark | 5.3.0 (2026-08-23) | 2,269,555 | 1% | BSD-2-Clause |

Related packages that are not pytest plugins but do the work several of them wrap:
...
| `coverage` | **62,785,441** |
| `faker` | 15,322,983 |
| `responses` | 12,307,180 |
| `freezegun` | 11,681,200 |
| `respx` | 5,040,003 |
| `time-machine` | 4,118,323 |

## What the numbers say

**Several "plugins" are ergonomics over something that already works, and the wrapped thing is
more installed than the wrapper.** `coverage` at 62.8M against `pytest-cov` at 45.5M is the
clearest case: pytest-cov is a thin integration layer over coverage.py, and coverage.py is
perfectly usable without it. `pytest-mock` at 19.2M wraps `unittest.mock`, which is in the
standard library and costs nothing.

That is worth knowing before adding either. The plugin buys you fixture-scoped setup and
teardown — `mocker` undoes its patches automatically, `--cov` wires the run without a wrapper
command — and it does not buy capability. **If a plugin's job is ergonomics, its absence is an
inconvenience rather than a gap.**

**pytest-asyncio is the exception and the most-installed plugin here.** 51.9M weekly, 27% of
pytest's own volume. It is not ergonomics: pytest cannot run an `async def` test at all
without something to drive the event loop, so this is capability. Note it reached **1.0 only
recently** — 1.4.0 in May 2026, after years in `0.x` — and its earlier releases changed
fixture-scoping behavior between minors. Pin it.

**Licensing is not a differentiator, and this time that is the finding.** Everything here is
MIT, Apache-2.0 or BSD except hypothesis at MPL-2.0. MPL is file-level weak copyleft, and a
test dependency is not distributed with your product anyway. 1.250 found licensing *did*
matter for linters — Pylint's GPL binds a vendor embedding it — and 1.251 found a different
shape again, an open core under a proprietary editor layer. Here it binds nobody, which
completes the pattern rather than repeating it.

**pytest-xdist at 29M is the parallelism story**, and it is the plugin whose value scales with
the thing 1.250 measured: a test suite's cost grows with the codebase, and `-n auto` is the
cheapest response.

## The three the stack trace actually found

Demand measured over 16 application manifests: **pytest-asyncio (4), pytest-cov (3),
pytest-mock (2)**. All three were invisible to earlier passes because they live in
`[dependency-groups]` rather than in the main dependency list — a manifest-reading limitation,
not a signal about the packages.

That ordering matches the download figures exactly, which is a small piece of evidence that
16 manifests is enough to see the shape of this layer even though it is a small sample.

## First read

**Take pytest-asyncio if you write async tests** — there is no alternative and no way around
it. Pin the version.

**Take pytest-cov and pytest-mock for the ergonomics, knowing what they wrap.** Both are thin,
both are optional, and a project that wants fewer dependencies loses convenience rather than
capability.

**Take pytest-xdist when the suite is slow enough to notice**, which is later than people
think and arrives suddenly.

**Reach for hypothesis when the input space is bigger than your imagination** — it is a
different kind of testing rather than a better one, and it is the only item here that changes
how tests are written rather than how they are run.
...
> **Refresh (2026-06, verified 2026-06-26):** Vitest 5.0 is in beta (v5.0.0-beta.5, June 15,
> 2026) with **breaking changes**: `@vitest/runner` is no longer published as a separate package
> (inlined); config file lookup no longer traverses ancestor directories; `concurrencyId` and
> `workerId` are now 1-based instead of 0-based. New: `--repeats` CLI flag, coverage threshold
> enhancements. The metrics below reflect 2025 data; current weekly downloads and stars are
> substantially higher. Playwright (1.61.1) and Cypress (15.18.0) continue incremental releases
> with no major version jumps since the survey date.
...
# How pytest plugins work, and why the ecosystem is the product

**Extension added 2026-08-29.** The runner is covered in `pytest.md`; this is the layer on top
of it.

## The extension point is three things, and that is the whole design

**Hooks.** pytest defines named points in its own lifecycle — collection, setup, the call,
teardown, reporting — and a plugin is a module implementing functions with the matching names.
`pytest_collection_modifyitems` reorders or deselects tests; `pytest_runtest_makereport`
decides what a failure looks like. No registration, no base class: the name is the contract.

**Fixtures.** A fixture is a function that produces a value and optionally tears it down, and
tests request one by naming it as a parameter. Plugins ship fixtures the way libraries ship
functions — `mocker` from pytest-mock, `db` from pytest-django — and a fixture can depend on
another fixture, so the layer composes.

**Entry points.** A package declares `pytest11` in its metadata and pytest discovers it on
import. **Installing a plugin activates it.** That is the property that makes the ecosystem
large, and it is also the one that surprises people: `pip install` changes how the suite runs.

## Why this design is the reason pytest won

`unittest` is in the standard library, costs nothing, and is fine. pytest's advantage was never
a better assertion API — it is that the three mechanisms above let other people build the
parts pytest does not have.

**And it is the same shape this range keeps finding.** 1.250 found Flake8 and ESLint holding
off faster Rust rewrites because rules compiled into a binary cannot run a plugin you wrote.
1.251 found mypy holding off pyright and ty because framework plugins are how Django and
SQLAlchemy become checkable at all. Here the extension point is not defending an incumbent
against a rewrite — **it is what the incumbent is.**

That is worth stating precisely, because it sharpens the pattern rather than repeating it: in
linting and type checking the plugin layer is a moat around a core that does the real work. In
testing the core is deliberately small and the layer does the real work.

## What the plugins actually do, by kind

**Capability — the test cannot run without it.** `pytest-asyncio` is the only one here in this
class. pytest's runner calls a test function and checks it did not raise; an `async def`
returns a coroutine that never runs, so without a plugin to drive an event loop the test
silently passes having executed nothing. That failure mode — a green test that ran no code —
is why this is capability rather than convenience.

**Ergonomics over something that already works.** `pytest-cov` wraps `coverage.py`, which is
more installed than the plugin (62.8M against 45.5M weekly) and runs fine as
`coverage run -m pytest`. What the plugin adds is xdist-awareness and not needing a wrapper
command. `pytest-mock` wraps `unittest.mock` from the standard library; what it adds is
fixture-scoped teardown, so a patch is undone when the test ends rather than when a
`with` block closes or a decorator unwinds.

**Execution strategy.** `pytest-xdist` distributes tests across processes. It changes nothing
about how tests are written and everything about whether a suite is bearable, and it is where
test isolation bugs surface — a suite that passes serially and fails under `-n auto` has
shared state it did not know about. That is a real finding rather than a plugin defect.

**A different kind of testing.** `hypothesis` generates inputs and shrinks failures to a
minimal case. It is not a better way to write the tests you have; it is a way to write tests
you would not have thought of, and it is the only item in this layer that changes the author's
job rather than the runner's.

## The cost nobody prices

**Installing a plugin activates it**, and plugins interact. A conftest hook, an autouse
fixture and a plugin's own hook all run for every test, and the order is defined but not
obvious. Two consequences show up in real suites:

- **Debugging gets harder in proportion to plugin count.** `-p no:cacheprovider` and
  `--trace-config` exist because working out which plugin changed the behavior is a real
  activity.
- **A plugin can make a suite pass that should fail.** The `async def` case above is the
  clearest: without pytest-asyncio installed, an async test is collected, called, and reported
  as passing without executing its body. A team that removes the plugin does not get errors —
  it gets a green suite that tests nothing.

That last one is worth a check in CI rather than trust: assert that the number of collected
tests matches expectation, or configure `asyncio_mode = strict` so an undriven coroutine is an
error rather than a pass.

## Versioning

`pytest-asyncio` reached 1.0 in 2026 after years at `0.x`, and its fixture-scoping behavior
changed between minor releases during that period. The rest of the layer is on stable majors.

The rule from 1.250 applies here with a twist: pin plugins, because **a plugin upgrade can
change what your tests mean**, not merely whether they pass. A linter's new rule is a warning;
a type checker's changed inference is a failed build; a test plugin's changed semantics is a
suite that reports the wrong thing.
