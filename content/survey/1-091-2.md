...
description: "Face detection and recognition compared: MediaPipe, Dlib, InsightFace, RetinaFace, MTCNN, OpenCV and the hosted APIs — accuracy, speed, model size and licence."
...
Face detection and recognition compared: MediaPipe, Dlib, InsightFace, RetinaFace, MTCNN, OpenCV and the hosted APIs — accuracy, speed, model size and licence.
...
- **MediaPipe Face (Google)** — Mobile-optimised and fastest on CPU (60-100 FPS) with a dense 468-point 3D mesh under 10 MB. Detection and landmarks only — it does not do recognition.
...
- **InsightFace** — State-of-the-art recognition (99.83% LFW). The survey records it as free for non-commercial use only — a licence question to confirm before it enters a product.
...
- No library leads on every axis, and the split is structural: MediaPipe and RetinaFace detect, Dlib and InsightFace recognise, and picking one library for both usually means accepting second-best at one of them
