...
description: "Face detection and recognition compared: MediaPipe, Dlib, InsightFace, RetinaFace, MTCNN, OpenCV and the hosted APIs — accuracy, speed, model size and license."
keywords: ["face detection", "face recognition", "mediapipe", "dlib", "insightface", "retinaface", "mtcnn", "opencv", "haar cascade", "facial landmarks", "face embedding", "lfw benchmark", "wider face", "face++", "amazon rekognition", "biometrics"]
weight: 1091200
date: 2026-03-06
lastmod: 2026-03-06
---
...
Face detection and recognition compared: MediaPipe, Dlib, InsightFace, RetinaFace, MTCNN, OpenCV and the hosted APIs — accuracy, speed, model size and license.

> **Coverage: S1 of the four-pass method.** S2/S3/S4 have not been completed, so this is a partial survey rather than a finished comparison.

## At a glance


- **MediaPipe Face (Google)** — Mobile-optimized and fastest on CPU (60-100 FPS) with a dense 468-point 3D mesh under 10 MB. Detection and landmarks only — it does not do recognition.
- **Dlib** — The mature all-rounder: 68-point landmarks plus recognition at 99.38% LFW. HOG mode is fast, CNN mode accurate but 1-3 FPS, and the package is heavy (~150 MB).
- **InsightFace** — State-of-the-art recognition (99.83% LFW). The survey records it as free for non-commercial use only — a license question to confirm before it enters a product.
- **RetinaFace** — Highest detection accuracy on WIDER FACE hard (91.4%) — the pick when faces are small, angled or partly hidden. GPU-oriented.
- **MTCNN** — The legacy lightweight option — 2 MB and 5-15 FPS. Superseded on accuracy, still useful where size dominates.
- **OpenCV (Haar and DNN)** — Fastest and smallest via Haar cascades at 70-85% accuracy, plus modern DNN detectors. The embedded and no-dependencies answer.
- **Face++ and Amazon Rekognition** — Hosted commercial services with broad attribute analysis and no model to operate — at per-call cost, with images leaving your infrastructure.


**What the research found**

- No library leads on every axis, and the split is structural: MediaPipe and RetinaFace detect, Dlib and InsightFace recognize, and picking one library for both usually means accepting second-best at one of them
- Model size varies by two orders of magnitude (2 MB to 200 MB), which decides the question outright on embedded and mobile targets
- Licensing is a first-class selection criterion here, not a footnote — the survey records the strongest recognition model as non-commercial, which can rule it out before any benchmark matters
- Detection accuracy figures are only comparable within a benchmark: WIDER FACE 'hard' and LFW measure different things, and 99% on one is not better than 91% on the other
- The hosted APIs trade operational simplicity for per-call cost and for sending biometric images off-premises — a compliance decision as much as a technical one
