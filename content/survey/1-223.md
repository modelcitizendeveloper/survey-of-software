...
description: "Python libraries for implementing CalDAV servers with custom backends. Radicale (21K/month, 4.5K stars) is the clear winner with a documented storage plugin…"
keywords: ["radicale", "xandikos", "wsgidav", "icalendar", "vobject"]
...
Python libraries for implementing CalDAV servers with custom backends. Radicale (21K/month, 4.5K stars) is the clear winner with a documented storage plugin API that supports API-backed backends. Xandikos (1.6K/month) is Git-only, no custom backends. WsgiDAV provides WebDAV but no CalDAV. Building from scratch requires ~2-4 weeks for minimum viable CalDAV.
...
| Library | Use For | License | Latest release |
|---|---|---|---|
| **radicale** | CalDAV server with custom storage backend | GPL-3.0 | 3.8.0 · 2026-09-03 |
| **xandikos** | Lightweight CalDAV server (Git-backed only) | GPL-3.0 | 0.4.6 · 2026-08-31 |
| **wsgidav** | WebDAV server (no CalDAV, but extensible WSGI base) | MIT | 4.3.5 · 2026-06-27 |
| **icalendar** | iCalendar parsing (needed for any CalDAV server) | — | 7.3.0 · 2026-08-19 |
| **vobject** | iCalendar/vCard parsing; used by Radicale internally. Revived under the py-vobject org in 2024 after six dormant years | — | 0.9.9 · 2024-12-16 |
...
- Radicale is the only Python CalDAV server with a plugin API for custom storage
- Storage plugin requires subclassing BaseStorage + BaseCollection (~12 methods)
- radicale-sirius-plugin proves API-backed storage works (fetches from REST API)
- Vikunja already has built-in CalDAV (VTODO), but it's alpha-quality with iOS bugs
- Minimum CalDAV subset for clients: PROPFIND, REPORT (calendar-query), GET, PUT, DELETE
...
### icalendar
**Downloads:** 4.7M/month | **Version:** 7.2.0 | **License:** BSD
...
| Latest Version | 3.7.5 (Jun 2026) | Active, recent release |
...
- Latest release: Jun 2026 (v3.7.5)
...
|---------|----------------|----------------|-------------|---------|---------|
| **Radicale** | Yes (plugin API) | Good | Active (Jun 2026) | GPL-3.0 | **Best choice** |
| Xandikos | No (Git only) | Excellent | Active (May 2026) | GPL-3.0 | Good if Git storage is OK |
...
| Latest Version | 0.4.2 (May 2026) | Still in 0.x (pre-stable) |
...
| done | STATUS:COMPLETED | STATUS:CANCELED |
