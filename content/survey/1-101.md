---
title: "1.101 PDF Generation & Processing Libraries"
description: "PDF in Python: pypdf, ReportLab, fpdf2, WeasyPrint, pdfkit and borb — manipulating existing PDFs, drawing new ones, or rendering HTML and CSS to page."
keywords: ["pdf generation", "pypdf", "reportlab", "fpdf2", "weasyprint", "pdfkit", "borb", "wkhtmltopdf", "html to pdf", "pdf manipulation", "invoice generation", "report generation", "pdf merge", "pdf extraction", "document export"]
...
# 1.101 PDF Generation & Processing Libraries

PDF in Python: pypdf, ReportLab, fpdf2, WeasyPrint, pdfkit and borb — manipulating existing PDFs, drawing new ones, or rendering HTML and CSS to page.
...
|---|---|---|
| **pypdf** | The most downloaded in the category (~9.4M monthly) and the answer for PDFs that already exist — merge, split, rotate, extract. It manipulates rather than authors. | 6.18.0 · 2026-09-07 |
| **ReportLab** | The established generation leader (~4.7M monthly): a programmatic drawing API with full layout control. The choice when the document must look exactly like the specification. | 5.0.1 · 2026-08-20 |
| **fpdf2** | The lightweight, fast-growing alternative (~2M monthly) — much of ReportLab's usefulness with a smaller footprint and gentler API. | 2.8.8 · 2026-08-09 |
| **WeasyPrint** | The HTML/CSS-to-PDF leader, and usually the pragmatic answer: if the layout already exists as a web page, styling it for print beats redrawing it in code. | 70.0 · 2026-09-08 |
| **pdfkit** | A wrapper around wkhtmltopdf — popular and easy, with an external binary as the deployment cost. | 1.0.0 · 2021-11-14 |
| **borb** | The newcomer with a modern API and advanced features; the smallest community of the six, which is the risk. | 3.0.9 · 2026-08-03 |
...
- The category splits three ways and the split, not the library, is the decision: manipulate an existing PDF (pypdf), draw a new one (ReportLab, fpdf2), or render markup (WeasyPrint, pdfkit)
- pypdf leading on downloads is a measure of how much PDF work is editing rather than authoring — it is not a competitor to ReportLab
- The HTML route is the underrated one for reports and invoices: reusing an existing template beats reimplementing the layout in a drawing API, and WeasyPrint needs no external binary the way pdfkit does
- fpdf2's growth against ReportLab is an ergonomics story rather than a capability one
- Deployment shape matters as much as the API — pdfkit's wkhtmltopdf dependency is the sort of thing that surfaces only in a container build
...
# Domain Explainer: PDF Generation & Processing

*For someone who has never made a PDF from code. Every term is defined before it
is used.*
...
A hardware store has a shelf for **cutting things open**, a shelf for **building
things from raw stock**, and a shelf for **assembling a kit somebody else
designed**. All three are woodworking. Buying from the wrong shelf is the most
common mistake, and it is not recoverable by trying harder — a saw does not
become a lathe.

This category is exactly that, and the three shelves are:

1. **Manipulate a PDF that already exists** — split it, merge it, rotate pages,
   extract text, fill a form.
2. **Draw a new PDF from nothing** — place text and shapes at coordinates you
   choose.
3. **Render HTML and CSS to a PDF** — reuse a web page or template you already
   have.

**The split, not the library, is the decision.** Almost every bad outcome in this
category comes from picking a tool from the wrong shelf and then fighting it.

---

## Vocabulary, in Dependency Order

Each term is used by the ones after it.

**PDF (Portable Document Format)** — a file format describing a fixed page: this
text, at this position, in this font, on a page this size. Unlike a web page, it
does not reflow when the window changes. That fixity is the whole point.

**Page box** — the rectangle that defines a page's dimensions. A4, US Letter, or
anything you specify.

**Coordinate space** — where things go on the page, measured in **points** (1/72
of an inch). PDF's origin is the *bottom-left* corner, not the top-left that
screen graphics use. This trips up everyone once.

**Content stream** — the actual drawing instructions inside a PDF: move here,
set this font, show this text. Libraries that "draw" PDFs are writing content
streams for you.

**Font embedding** — including the font file inside the PDF so it renders the
same everywhere. Without it, a reader substitutes something and your layout
shifts.

**Text extraction** — recovering readable text from a PDF. Harder than it sounds:
a PDF stores glyphs at positions, not sentences, so word and line boundaries are
reconstructed by guesswork.

**Form (AcroForm)** — interactive fields inside a PDF that can be filled
programmatically.

**Layout engine** — software that decides where content goes: line breaking,
pagination, tables flowing across pages. This is the expensive part, and shelf 3
exists to borrow one you already have.

**Headless browser** — a browser with no visible window, driven by code. One way
...
### Shelf 1 — Manipulate an existing PDF

You already have a PDF. You want a different PDF out of it.

**The tool: `pypdf`.** Split, merge, rotate, crop, extract text, read and fill
forms, encrypt and decrypt.

**What it cannot do:** create a page with your content on it from scratch. It is
a surgeon, not an author.

This shelf is busier than people expect. `pypdf` leads the whole category on
downloads, and that is a measurement of how much PDF work is *editing* rather
than *authoring*. It is not competing with the tools below; most projects that
need one eventually need both.

### Shelf 2 — Draw a new PDF

You have data. You want a page. You are prepared to say where things go.

**The tools:** `ReportLab`, `fpdf2`, `borb`.

You work in the coordinate space directly, or through a higher-level flowable
system that handles pagination for you. Total control, and total responsibility:
if a table should break across pages, you arrange that.

**When this shelf is right:** precise layout requirements, tight performance
constraints, no HTML template already in existence, or output that is
document-shaped rather than page-shaped (certificates, labels, tickets).

### Shelf 3 — Render HTML and CSS

You already have a template — an invoice, a report, a receipt — as a web page.

**The tools:** `WeasyPrint`, `pdfkit`.

You hand over HTML and CSS and get a PDF back. The layout engine is one somebody
else wrote and you already know how to drive, because it is CSS.

**This is the underrated shelf**, and the survey says so plainly: for reports and
invoices, reusing an existing template beats reimplementing the layout in
drawing calls. If a designer can edit the template afterwards without learning
your programming language, that is worth a great deal.

**The catch is deployment shape.** See below — it is the thing that surfaces in
...
**1. Do you already have the PDF?**
Yes → shelf 1, `pypdf`. Stop reading.

**2. Do you already have an HTML template, or could you write one?**
Yes → shelf 3. You are borrowing a layout engine instead of building one.

**3. Otherwise** → shelf 2. Draw it.

That is the whole decision. What follows is how to choose *within* a shelf.

---

## The Library Facts That Actually Decide Things

*All verified against PyPI on 2026-08-25.*

| Library | Shelf | Version | Last release | License |
|---|---|---|---|---|
| **pypdf** | 1 — manipulate | 6.16.2 | 2026-08-23 | BSD-3-Clause |
| **ReportLab** | 2 — draw | 5.0.1 | 2026-08-20 | BSD |
| **fpdf2** | 2 — draw | 2.8.8 | 2026-08-09 | **LGPL-3.0-only** |
| **borb** | 2 — draw | 3.0.9 | 2026-08-03 | **AGPL-3.0-or-later** |
| **WeasyPrint** | 3 — HTML | 69.0 | 2026-06-02 | BSD |
| **pdfkit** | 3 — HTML | 1.0.0 | **2021-11-14** | MIT |

Two of these rows should change your shortlist before any feature comparison.

### The license column is not decoration

**borb is AGPL-3.0-or-later.** The AGPL is copyleft with a *network clause*:
making the program available to users over a network counts as distribution, so
a hosted service that imports it takes on source-provision obligations. borb
also sells a commercial license, which is the usual signal that the AGPL is
doing deliberate work. If you are building a SaaS product, this is a question for
a lawyer before it is a question about features.

**fpdf2 is LGPL-3.0-only.** Weaker than AGPL — the "lesser" GPL is designed so
that *using* a library does not force your own code open — but it still carries
obligations that BSD and MIT do not, particularly around static linking and
letting users replace the library.

**pypdf, ReportLab and WeasyPrint are permissive** (BSD). If licensing is a
constraint in your organization, that sentence is most of your answer.

### pdfkit has not shipped since 2021

Version 1.0.0, released 2021-11-14. Nearly five years.

That matters more than the date suggests, because of what pdfkit *is*: a thin
wrapper that shells out to **wkhtmltopdf**, a separate binary you install
yourself. So you are depending on an unreleased wrapper *and* an external
executable.

**This is the deployment-shape problem, and it is the one that surfaces late.**
The survey flags it directly: a library that needs a system binary works fine on
a laptop and then fails in a container, a serverless function, or a locked-down
build image — environments where installing arbitrary executables is exactly what
you are not allowed to do. WeasyPrint, which renders in-process, does not have
this failure mode.

Ask "what does this need at runtime that is not a Python package?" early. It is
cheap to answer then and expensive to answer after the first deploy.

### fpdf2 versus ReportLab is about ergonomics, not capability

ReportLab is the long-standing option and can do more. fpdf2's growth against it
is an **ergonomics** story: a smaller, more approachable API for the common case.
Neither is winning on features. If you are drawing something simple, the simpler
API is a real advantage; if you need ReportLab's flowables and platypus layout
system, fpdf2 will not grow into them.

---

## Common Misconceptions

### "PDF generation is a solved problem, any library will do"

Every library here does one shelf well and the others badly or not at all.
"Any library will do" is true only within a shelf.

### "I'll extract the text and get my content back"

Text extraction is reconstruction, not retrieval. A PDF holds glyphs at
coordinates; words, lines, reading order and table structure are all inferred.
Multi-column layouts and tables are where it visibly fails. Budget for
imperfection, or keep the source data.

### "HTML to PDF will look exactly like the browser"

It will look like *that renderer's* interpretation of your CSS. WeasyPrint
implements a large subset of CSS, not all of it, and not the same subset as
Chrome. Test the actual output early rather than assuming parity with what you
see in a browser tab.

### "I'll just use the coordinate API, how hard can layout be?"

Placing one line of text is easy. Line breaking, pagination, keeping a table
header with its rows, and widow/orphan control are a layout engine, and writing
one is a project. That is the whole argument for shelf 3.

---

## What to Watch

- **Your license constraint**, if you have one. It eliminates options faster than
  any feature.
- **What each tool needs at runtime beyond pip.** A system binary is a deployment
  decision disguised as a dependency.
- **Whether you actually need to author, or only to edit.** Shelf 1 is the
  busiest shelf, and people arrive at this category assuming they need shelf 2.

---

## Related Surveys

- **1.100 Text & Document Processing** — the broader category this specializes
- **1.102** and the rest of the 1.10x block — adjacent document-handling tooling
