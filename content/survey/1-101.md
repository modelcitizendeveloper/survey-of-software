...
- **pypdf** — The most downloaded in the category (~9.4M monthly) and the answer for PDFs that already exist — merge, split, rotate, extract. It manipulates rather than authors.
- **ReportLab** — The established generation leader (~4.7M monthly): a programmatic drawing API with full layout control. The choice when the document must look exactly like the specification.
- **fpdf2** — The lightweight, fast-growing alternative (~2M monthly) — much of ReportLab's usefulness with a smaller footprint and gentler API.
- **WeasyPrint** — The HTML/CSS-to-PDF leader, and usually the pragmatic answer: if the layout already exists as a web page, styling it for print beats redrawing it in code.
- **pdfkit** — A wrapper around wkhtmltopdf — popular and easy, with an external binary as the deployment cost.
- **borb** — The newcomer with a modern API and advanced features; the smallest community of the six, which is the risk.
