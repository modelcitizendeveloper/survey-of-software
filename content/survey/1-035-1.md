...
|---|---|---|
| **Jieba** | Dictionary + HMM hybrid | Prototyping, keyword extraction, search systems |
| **PKUSEG** | Neural network (BiLSTM-CRF), domain-specific models | Domain-specific production systems (news, web, medicine) |
| **LAC** | Neural network (BiGRU-CRF), joint processing | Production Chinese-only systems with POS/NER requirements |
| **bert-base-chinese** | Character-level transformer tokenization | Building Chinese-only transformer models |
| **SentencePiece** | Unigram language model, language-independent | Multilingual systems, custom transformer models |
| **spaCy** | Multilingual NLP framework (uses pkuseg backend) | Multilingual pipelines with unified API |
| **HuggingFace Tokenizers** | Pre-trained transformer tokenizers (Qwen, ChatGLM) | Using pre-trained Chinese models |
