...
description: "Intent classification: BART zero-shot, SetFit, fine-tuned DistilBERT, Rasa DIET and GPT-4 — trading labelled examples against latency and accuracy."
...
Intent classification: BART zero-shot, SetFit, fine-tuned DistilBERT, Rasa DIET and GPT-4 — trading labelled examples against latency and accuracy.
...
- **SetFit** — The best accuracy-per-labelled-example in the set — >95% from 8-64 examples, at ~50-100ms. The balanced default when a little labelling is possible.
- **DistilBERT (fine-tuned)** — The choice when latency is the constraint: <50ms at 95%+, from a 66M-parameter model — but it wants 100-1000 labelled examples first.
...
- The real axis in this category is labelled data, not accuracy: zero-shot needs none, SetFit needs tens, DistilBERT needs hundreds, Rasa needs 500+, and each step buys latency
