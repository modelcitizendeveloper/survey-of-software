...
description: "Hardware-backed secure storage on mobile: iOS Keychain Services (SecItem API, kSecAttrAccessible, biometric gating via SecAccessControl) and Secure Enclave (EC…"
keywords: ["verdict", "pypi", "npm", "react-native-keychain", "expo-secure-store", "flutter_secure_storage", "iOS Keychain Services", "Android Keystore", "EncryptedSharedPreferences"]
...
Hardware-backed secure storage on mobile: iOS Keychain Services (SecItem API, kSecAttrAccessible, biometric gating via SecAccessControl) and Secure Enclave (EC P-256 only, CryptoKit, private key never leaves hardware). Android Keystore (TEE/StrongBox hardware backing, KeyPairGenerator, BiometricPrompt+CryptoObject for biometric gating) and EncryptedSharedPreferences (Jetpack Security wrapper). React Native: react-native-keychain v10.0.0 (~350K npm/week, biometric gating, no size limit) is the standard; expo-secure-store v15 (2KB limit, no biometric gate) for simple Expo cases. Flutter: fluttersecurestorage v10 (migrated to Google Tink from deprecated JetSec EncryptedSharedPreferences). Passkeys (FIDO2) complement (not replace) credential storage for authentication use cases.
...
|---|---|---|
| **react-native-keychain** | JavaScript / TypeScript | Secure credential storage with biometric gating in React Native |
| **expo-secure-store** | JavaScript / TypeScript | Simple secure credential storage for Expo managed workflow (2KB limit) |
| **flutter_secure_storage** | Dart | Secure credential storage in Flutter (wraps Keychain / Keystore) |
| **iOS Keychain Services** | Swift / Objective-C | iOS platform API for credential storage with biometric access control |
| **iOS Secure Enclave / CryptoKit** | Swift | Hardware-bound EC P-256 key operations (signing, ECDH); key never leaves SE |
| **Android Keystore** | Kotlin / Java | Android platform API for hardware-backed key generation and key operations |
| **EncryptedSharedPreferences** | Kotlin / Java | Jetpack Security wrapper: encrypted key-value store backed by Android Keystore |


**What the research found**

- Platform APIs are permanent — iOS Keychain and Android Keystore are OS fundamentals with no viability risk.
- Cross-platform wrappers are thin — react-native-keychain and flutter_secure_storage do 95% of what you need with < 10 lines of code. Their lock-in risk is minimal.
- Biometric gating requires react-native-keychain — expo-secure-store does not support it. This is the primary decision factor in React Native.
- For key operations, go native — Secure Enclave signing and Android Keystore attestation require platform APIs. Cross-platform libs expose the common case only.
- Passkeys complement, not replace — Passkeys replace passwords for authentication. Secure storage is still needed for tokens, session credentials, and encryption keys.
...
**GitHub**: oblador/react-native-keychain | ~3.4K stars | **npm**: `react-native-keychain` | ~350K downloads/week
**Version**: 10.0.0 | **Language**: JS + native (Swift/Kotlin)
...
**Version**: 15.0.8 | **Size limit**: 2KB per value
...
**pub.dev**: `flutter_secure_storage` | 160/160 pub points | **version**: 10.0.0 (2025)
**Downloads**: High (pub.dev). v9+ migrated Android backend from JetSec to Google Tink.
