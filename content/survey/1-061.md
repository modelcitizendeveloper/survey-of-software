...
description: "Python hashing: xxhash, BLAKE3, mmh3, hashlib and pyhash — fast non-cryptographic checksums versus cryptographic digests, and why mixing them up is the risk."
keywords: ["hashing", "xxhash", "blake3", "mmh3", "murmurhash", "hashlib", "pyhash", "non-cryptographic hash", "cryptographic hash", "checksum", "sha256", "content addressing", "deduplication", "consistent hashing", "sharding", "hash table"]
...
Python hashing: xxhash, BLAKE3, mmh3, hashlib and pyhash — fast non-cryptographic checksums versus cryptographic digests, and why mixing them up is the risk.
...
|---|---|---|
| **xxhash** | The non-cryptographic leader by adoption (~25M monthly downloads) — extremely fast checksums and cache keys. The default when speed is the goal and no adversary is in the threat model. | 4.0.1 · 2026-08-17 |
| **blake3** | The modern cryptographic choice — parallelisable and far faster than SHA-2 while remaining a real cryptographic hash. The pick when you need both properties at once. | 1.0.9 · 2026-06-22 |
| **mmh3 (MurmurHash3)** | The database-oriented option (~12M monthly) — the hash behind a lot of partitioning, Bloom filters and sketching. Chosen for distribution quality rather than raw speed. | 5.3.0 · 2026-08-26 |
| **hashlib (standard library)** | The universal baseline: SHA-2 family, no dependency, present everywhere. Correct until profiling or a parallelism requirement says otherwise. | — |
| **pyhash** | A multi-algorithm wrapper — useful for benchmarking or matching a hash chosen elsewhere, rather than as a default. | 0.9.3 · 2019-03-07 |
...
- The category's real failure mode is a category error: using a fast non-cryptographic hash where an adversary can choose the input, or paying cryptographic cost for a cache key that has no attacker
- xxhash owns the non-cryptographic default and hashlib the cryptographic baseline; almost every project needs exactly one of each and no more
- BLAKE3 is the notable change — it removes the historic assumption that cryptographic hashing must be the slow option
- mmh3's persistence is about distribution quality, not speed: partitioning and Bloom filters care how evenly a hash spreads, and that is a different property from throughput
