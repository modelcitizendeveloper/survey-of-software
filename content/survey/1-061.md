...
- **xxhash** — The non-cryptographic leader by adoption (~25M monthly downloads) — extremely fast checksums and cache keys. The default when speed is the goal and no adversary is in the threat model.
- **blake3** — The modern cryptographic choice — parallelisable and far faster than SHA-2 while remaining a real cryptographic hash. The pick when you need both properties at once.
- **mmh3 (MurmurHash3)** — The database-oriented option (~12M monthly) — the hash behind a lot of partitioning, Bloom filters and sketching. Chosen for distribution quality rather than raw speed.
...
- **pyhash** — A multi-algorithm wrapper — useful for benchmarking or matching a hash chosen elsewhere, rather than as a default.
