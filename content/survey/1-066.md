...
description: "Python anonymous transport via Tor is mature and well-supported: stem (Tor Project-maintained) provides full circuit control and onion service creation, while…"
keywords: ["verdict", "pypi", "stem", "torpy", "Nym websocket API", "Katzenpost"]
...
Python anonymous transport via Tor is mature and well-supported: stem (Tor Project-maintained) provides full circuit control and onion service creation, while requests + SOCKS5 handles anonymous HTTP. torpy offers pure-Python Tor as a fallback when the daemon cannot be installed. For global-passive-adversary threat models, Nym (websocket API to local daemon) provides cover traffic that defeats timing correlation. I2P is accessible via a ~50-line SAM v3 asyncio client; its Python library (i2plib) is inactive. Katzenpost provides post-quantum mixnet capability but has no production Python SDK.
...
| Library | Language | Use Case | Latest release |
|---|---|---|---|
| **stem** | Python | Tor circuit control, onion service creation | 1.8.2 · 2023-06-06 |
| **requests + SOCKS5 via Tor** | Python | Anonymous HTTP/HTTPS through Tor daemon | 2.34.2 · 2026-05-14 |
| **torpy** | Python | Pure Python Tor (no daemon required) | 1.1.6 · 2021-04-15 |
| **i2plib / DIY SAM client** | Python | I2P network access via SAM bridge | 0.0.14 · 2019-02-05 |
| **Nym websocket API** | Python | GPA-resistant anonymous messaging via Nym mixnet | 17.1 · 2026-08-26 |
| **Katzenpost** | Go | Post-quantum decryption mixnet | — |
...
- **The durable answer is not a library at all — it is SOCKS5 through a Tor daemon** — requests plus SOCKS5 pointed at a running daemon is the survey's only ⭐⭐⭐⭐⭐ Permanent slot. The protocol is stable and Tor has organizational backing; a Python library in the path is the part that rots.
- **stem is the Tor Project's own control library and has no alternative for onion services** — Circuit control and ephemeral onion services both route through it. The survey records no competitor for the second.
- **torpy is a last resort, and the reason is audit rather than function** — It is pure Python with no daemon, which is genuinely useful in constrained environments — and the survey names it explicitly as something not to build production security on.
- **I2P has a stable protocol and a dead library** — i2plib is inactive and named under 'what not to build on'. Reaching I2P from Python means writing a SAM v3 client yourself.
- **Nym is the thing to watch, and the blocker is an official Python SDK** — It is the answer for global-passive-adversary threat models once that lands. Until then it is a websocket API the survey warns against using unpinned.
