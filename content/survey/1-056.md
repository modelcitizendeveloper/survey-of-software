...
| **orjson** | Rust; returns bytes, not str; native datetime/UUID/dataclass/numpy | Serialisation-heavy services that can take bytes back |
...
- **The 6x is a serialising number — parsing is about 2x** — orjson writes JSON 5.4x faster than the standard library on 512 KB of logs and 10x on numeric data, and reads it back 1.8x and 2.0x faster. Measured on 3.11, 3.12 and 3.14 alike (1.6-2.1x loads, 5-13x dumps), so it is not a new-interpreter effect. Most programs parse more than they serialise, and most quote the multiple for the wrong ritual.
...
- **python-rapidjson is slower than the standard library on every sample** — Loads at 0.80-0.83x stdlib across logs, nested config and numeric data; dumps 0.9-1.1x. The 2025 edition said 'sometimes beaten by stdlib'; measured, it is every time. Choose it for RapidJSON compatibility or not at all.
...
serialise them natively; the standard library and ujson need a `default=` hook.
...
The second wave came from outside C. orjson (Rust) took serialisation to 5–10× by never
...
documents and the list of what the previous edition got wrong. S3 walks personas; S4 is the
...
*Rewritten 2026-08-19 after every figure was measured for the JSONic rituals floor model
(`/workshop/jsonic-rituals/`). Download counts are pypistats.org, last 30 days, read
2026-08-19. Throughput is what a Python caller sees on one machine (CPython 3.14.7,
aarch64, single thread), cross-checked on 3.11 and 3.12 — the full sweep and the method are
in S2 §0. The 2025 edition's claims that did not survive are listed in S2 §0.4.*
...
programs parse more than they serialise, and most citations quote the multiple for the
wrong ritual.
...
"orjson 6× faster than stdlib" is a dumps figure applied to parsing, where it is ~2×;
"msgspec 2× faster than orjson with schemas" is 1.3× on loads; "6–9× less memory" is
2.2–2.6×, typed against untyped, and untyped libraries hold the same memory as the standard
library; "ujson 3×" is 1.1–1.7×; "rapidjson sometimes beaten by stdlib" is every time; S2's
"pydantic 10× faster than alternatives" describes pydantic v2 against v1, not against this
category. Every download count is current. The record is S2 §0.4.
...
1. **The 6× is a dumps number.** orjson serialises 5–13× faster than the standard library
   ...
   more than they serialise; most citations quote the multiple for the wrong ritual.
...
| Recorded | Actual |
|---|---|
| "orjson 6× faster than stdlib json, consistently fastest across all benchmarks" | 5–13× on dumps, 1.6–2.7× on loads; on loads msgspec typed is faster |
| "msgspec 2× faster than orjson with schemas" | 1.3× on loads; orjson faster on dumps |
| "msgspec 6–9× less memory" | 2.2–2.6× resident, Structs vs dicts; untyped libraries hold the same memory |
| "ujson 3× faster than stdlib" | 1.1–1.7× loads, 1.2–2.7× dumps |
| "rapidjson sometimes beaten by stdlib" | slower than stdlib on every sample |
| "pydantic 10× faster than alternatives" (S2 §1) | 0.4–0.6× stdlib as a parser; the 10× is pydantic v2 over v1 |
| "orjson 6,904+ stars" | not re-checked; 238M downloads/mo is the load-bearing figure |

The 2024-dated benchmark prose in §2 below is unsourced and retained for the record only;
where it disagrees with §0, §0 wins.
...
- **rapidjson**: Slower than stdlib json on every sample measured in 2026 (§0) — "sometimes" was generous

#### Medium Payloads (567KB - 2.3MB)
- **msgspec with schema**: Fastest parser — 1.3x orjson on loads, measured (§0); the "2x" did not survive
...
> **Note (2026-08-19):** the multiples quoted below come from the 2025 edition. Measured for the floor model: orjson is 5–13× the standard library on dumps and 1.6–2.7× on loads; msgspec typed is the fastest parser at 1.3× orjson; ujson 1.1–1.7×; python-rapidjson slower than stdlib. Persona advice stands where it depends on the ritual, and S2 §0 has the numbers.
