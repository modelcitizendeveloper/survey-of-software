...
description: "Python JSON libraries compared: orjson, msgspec, ujson, python-rapidjson and stdlib json — the 6x speed gap, and what schema validation buys beyond it."
keywords: ["json", "orjson", "msgspec", "ujson", "python-rapidjson", "rapidjson", "json serialization", "json parsing", "schema validation", "fastapi json", "json performance", "msgpack", "dataclasses", "serialization"]
weight: 1056000
date: 2026-03-06
lastmod: 2026-03-06
---
...
Python JSON libraries compared: orjson, msgspec, ujson, python-rapidjson and stdlib json — the 6x speed gap, and what schema validation buys beyond it.

## At a glance


- **orjson** — The default when speed is the goal — around 6x stdlib and consistently fastest in the survey's benchmarks, with native handling of dataclasses, datetime, numpy and UUID. Returns bytes rather than str, and builds via Rust.
- **msgspec** — Fastest of all *with* schemas — reported ~2x orjson and 6-9x less memory — and it does MessagePack too. The cost is defining the schema, which is also the point.
- **ujson** — The drop-in: ~3x stdlib, a stable C implementation, no API change. The choice when you want a free speedup and nothing else.
- **python-rapidjson** — Established and configurable, but the survey found its performance inconsistent — sometimes below stdlib. Selected for RapidJSON ecosystem compatibility rather than speed.
- **Standard library json** — The baseline, and not actually slow for most workloads. Predictable, zero-dependency, and the right answer until profiling says otherwise.


**What the research found**

- The headline gap is real but narrow in scope: orjson's ~6x only matters where serialization is a measurable share of the workload, which is fewer services than the benchmarks suggest
- msgspec changes the question from 'how fast can you parse' to 'why parse into untyped dicts at all' — schema-aware decoding wins on both speed and memory, and validates for free
- orjson's bytes-not-str return is the one API break in the category and the reason 'drop-in' belongs to ujson instead
- python-rapidjson is the survey's caution: an established name whose measured performance did not match its reputation
