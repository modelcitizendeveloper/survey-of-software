...
sidebar_label: "1.080.1 Qr Code Generation"
...
**Example Application**: Using **segno** provides compliance safety margin if customers later require ISO certification.
...
- **Small application estimate**: ~4 hours (few QR generation points)
...
**Example Application**: Using **segno** (zero dependencies) = maximum deployment simplicity.
...
2. **Internal Fork Preparation** (Low Cost):
   - Document library usage patterns in application
   - Maintain test suite validating QR outputs
   - Keep migration path to alternative library documented
