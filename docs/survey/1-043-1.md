...
sidebar_label: "1.043.1 Task Queue Libraries"
...
**Problem**: Document generation and PDF manipulation block user requests, causing timeouts
...
**Problem**: Complex analytics queries across multiple databases can take minutes to complete
...
### Bulk Asset Generation Operations
**Problem**: Bulk generation of documents or assets for template libraries creates server bottlenecks
...
def generate_document_pdf(template_id, options):
    # Heavy PDF processing work
    return pdf_processor.generate(template_id, options)

# Queue the job instead of blocking
job = queue.enqueue(generate_document_pdf, template_id, options)
...
### Cost-Benefit Analysis (Based on Typical SaaS Scale)
